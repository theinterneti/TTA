...
def fix_b904_in_file(file_path: Path) -> int:
    ...
    try:
        ...
        patterns = [
            # Pattern 1: raise SomeError(f"message: {e}") -> add from e
            (r'(\s+)raise\s+(\w+Error)\(f?"[^"]*\{e\}[^"]*"\)\s*$', r'\1raise \2(f"...: {e}") from e'),
            
            # Pattern 2: raise SomeError("message") -> add from e (when e is caught)
            (r'(\s+)raise\s+(\w+Error)\([^)]+\)\s*$', r'\1raise \2(...) from e'),
            
            # Pattern 3: raise Exception(f"message: {e}") -> add from e  
            (r'(\s+)raise\s+Exception\(f?"[^"]*\{e\}[^"]*"\)\s*$', r'\1raise Exception(f"...: {e}") from e'),
        ]
