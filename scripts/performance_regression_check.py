...
class PerformanceRegressionDetector:
    ...
    def load_baseline_metrics(self, baseline_file: Optional[Path] = None) -> Dict[str, float]:
    ...
    def calculate_aggregated_metrics(self, metrics: List[PerformanceMetric]) -> Dict[str, float]:
        ...
        for category, values in by_category.items():
            if values:
                ...
                aggregated[f"p95_{category}_execution_time"] = (
                    statistics.quantiles(values, n=20)[18] if len(values) > 1 else values[0]
                )
    ...
    def detect_regressions(self, current_metrics: Dict[str, float], 
                          baseline_metrics: Dict[str, float]) -> List[RegressionResult]:
    ...
    def generate_report(self, results: List[RegressionResult]) -> str:
        ...
        if regressions:
            ...
            for result in sorted(regressions, key=lambda x: abs(x.change_percent), reverse=True):
                status_emoji = "🔴" if result.severity == "critical" else "🟡" if result.severity == "major" else "🟠"
                report.append(f"{status_emoji} **{result.metric_name}** ({result.severity.upper()})")
        ...
        for result in sorted(results, key=lambda x: x.metric_name):
            ...
            report.append(f"| {result.metric_name} | {result.current_value:.3f} | "
...
def main():
    parser = argparse.ArgumentParser(description="Detect performance regressions in TTA tests")
    parser.add_argument("--test-results", type=Path, required=True,
                       help="Directory containing test results")
    parser.add_argument("--baseline-file", type=Path,
                       help="JSON file containing baseline metrics")
    parser.add_argument("--baseline-branch", type=str, default="main",
    ...
    print(f"Loaded {len(current_metrics_raw)} raw metrics, aggregated to {len(current_metrics)} metrics")
    ...
    major_regressions = [r for r in results if r.is_regression and r.severity == "major"]
