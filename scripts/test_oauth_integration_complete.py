    ...
    try:
        ...
        if success:
            ...
            if clinical_user:
                ...
                logger.info(f"   Authentication method: JWT (HIPAA-compliant)")
                logger.info(f"   Session timeout: 30 minutes")
        ...
        if success:
            ...
            if player_user:
                ...
                logger.info(f"   Authentication method: JWT (with OAuth available)")
                logger.info(f"   Session timeout: Extended for casual use")
        ...
        for provider in ["google", "microsoft", "facebook"]:
            try:
                auth_data = auth_service.get_oauth_authorization_url(provider, "patient")
...
async def test_frontend_integration():
    ...
    try:
        ...
        patient_login_path = Path("web-interfaces/patient-interface/src/pages/auth/LoginPage.tsx")
        if patient_login_path.exists():
            with open(patient_login_path, 'r') as f:
                content = f.read()
        ...
        if patient_app_path.exists():
            with open(patient_app_path, 'r') as f:
                content = f.read()
                if "/auth/callback/:provider" in content:
                    logger.info("✅ Patient interface OAuth callback routing configured")
                else:
                    logger.warning("⚠️ Patient interface OAuth callback routing not found")
        ...
        if shared_index_path.exists():
            with open(shared_index_path, 'r') as f:
                content = f.read()
