    ...
    try:
        ...
        if verify_only:
            ...
            if verification_results["schema_valid"]:
                ...
                logger.info(f"Found {len(verification_results['constraints'])} constraints")
            ...
            if success:
                ...
                if verification_results["schema_valid"]:
                    ...
                    logger.info(f"Created {len(verification_results['constraints'])} constraints")
                    logger.info(f"Created {len(verification_results['indexes'])} indexes")
                else:
...
def main():
    ...
    if not args.password:
        logger.error("Neo4j password is required. Set --password or NEO4J_PASSWORD environment variable.")
        sys.exit(1)
