...
from src.infrastructure.security_framework import SecurityFramework, SecurityConfiguration
...
async def test_security_framework_initialization():
    ...
    try:
        ...
        encrypted_result = await security_framework.encrypt_data(test_data, "patient_data")
...
async def test_security_testing_framework(security_framework):
    ...
    try:
        ...
        compliance_report = await testing_framework.generate_compliance_report(test_suite)
        ...
        logger.info(f"   Compliance Score: {compliance_report['overall_compliance_score']:.1f}%")
        ...
        for req, status in compliance_report["compliance_requirements"].items():
            ...
            req_score = (status["passed"] / total_req_tests) * 100 if total_req_tests > 0 else 0
            req_status = "✅" if status["failed"] == 0 else "⚠️" if req_score >= 70 else "❌"
            logger.info(f"   {req_status} {req}: {status['passed']}/{total_req_tests} tests passed ({req_score:.1f}%)")
