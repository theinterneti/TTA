...
class ExceptionChainingAnalyzer(ast.NodeVisitor):
    ...
    def visit_ExceptHandler(self, node: ast.ExceptHandler):
        ...
        # Save current state
        self.exception_stack.append((self.in_except_handler, self.current_exception_var))
...
def fix_file(filepath: Path, dry_run: bool = False) -> Tuple[bool, int, str, List[Dict]]:
    """
    ...
    if all_fixes and not args.dry_run:
        ...
        with open(report_path, 'w') as f:
            ...
            for fix in all_fixes:
                f.write(f"  {fix['file']}:{fix['line']} - Added 'from {fix['exception_var']}'\n")
