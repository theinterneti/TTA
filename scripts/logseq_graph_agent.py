...
Usage:
    python scripts/logseq_graph_agent.py --sync-coverage
    python scripts/logseq_graph_agent.py --process-files
    python scripts/logseq_graph_agent.py --sync-journals
    python scripts/logseq_graph_agent.py --all
"""
...
def sync_coverage(data: dict[str, Any]):
    ...
    for group, components in data.items():
        for name, info in components.items():
            ...
            namespace = (
                get_namespace(REPO_ROOT / component_path)
                if component_path
                else "TTA.dev"
            )
    ...
    try:
        ...
        if ext == ".py":
            ...
            if len(lines) > insert_idx and (
                ...
                # Inside docstring
                # Better: Insert immediately after opening quotes
                quote_char = lines[insert_idx].strip()[:3]
                lines.insert(insert_idx + 1, f"Logseq: {citation}")
            else:
                lines.insert(insert_idx, f"# Logseq: {citation}")
...
def extract_blocks(
    file_path: Path, content: str, namespace: str, rel_path: Path = None
):
...
def process_files(files=None):
    ...
    for file_path in file_iterator:
        ...
        if parts[0] in ["src", "recovered-tta-storytelling"]:
            parts.pop(0)
...
def main():
    ...
    parser.add_argument(
        "--sync-coverage", action="store_true", help="Sync coverage data"
    )
    parser.add_argument(
        "--process-files", action="store_true", help="Process code/doc files"
    )
