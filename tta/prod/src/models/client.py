...
class UnifiedModelClient:
            ...
            try:
                ...
                if self.cost_tracker:
                    ...
                    cost = self.cost_tracker.estimate_cost(
                        adjusted_model, input_tokens, output_tokens
                    )
                    self.cost_tracker.add_usage(
                        adjusted_model, input_tokens, output_tokens, cost
                    )

                logger.info(
                    f"Successfully generated text using {provider_type} with model {adjusted_model}"
                )
                return result

            except Exception as e:
                logger.warning(
                    f"Provider {provider_type} failed (attempt {attempt + 1}): {e}"
                )
        ...
        **kwargs,
        ...
        for attempt, provider_type in enumerate(providers_to_try):
            try:
                ...
                logger.info(
                    f"Successfully streamed text using {provider_type} with model {adjusted_model}"
                )
                return
    ...
    def _adjust_model_for_provider(
    ...
    def get_cost_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "remaining_budget": model_config.max_daily_cost
