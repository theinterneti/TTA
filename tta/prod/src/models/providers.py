...
from .config import ProviderType, model_config
...
class LocalProvider(ModelProvider):
    ...
    async def _ollama_generate(
        ...
        **kwargs,
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.endpoint}/api/generate", json=payload, timeout=120.0
            )
            response.raise_for_status()
    ...
    async def stream_generate(
        ...
        **kwargs,
        ...
        if self.model_type == "ollama":
            async for chunk in self._ollama_stream_generate(
                prompt, model, temperature, max_tokens, **kwargs
            ):
                yield chunk
        else:
            raise NotImplementedError(
                f"Local model type {self.model_type} not implemented"
            )
    ...
    async def _ollama_stream_generate(
        ...
        **kwargs,
        ...
        async with httpx.AsyncClient() as client:
            async with client.stream(
                "POST", f"{self.endpoint}/api/generate", json=payload, timeout=120.0
            ) as response:
                response.raise_for_status()
    ...
    async def is_available(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                if self.model_type == "ollama":
                    response = await client.get(
                        f"{self.endpoint}/api/tags", timeout=5.0
                    )
                    return response.status_code == 200
...
class ModelProviderFactory:
    ...
    async def get_available_provider(
        cls, preferred_providers: list[ProviderType]
    ) -> ModelProvider | None:
