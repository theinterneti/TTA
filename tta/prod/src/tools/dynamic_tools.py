...
class DynamicTool(BaseTool):
    ...
    def _compile_function(self) -> None:
        """
        Compile the function code and set the action_fn. Also register callable
        in the central CallableRegistry with a consistent name/version mapping.
        ...
        """
        try:
            # Execute code with a unified global/local env so imports are visible at runtime
            env: dict = {}
            exec(self.function_code, env, env)
            ...
            if function_name in env:
                ...
                try:
                    ...
                    _creg.register_callable(reg_name, version, self.action_fn)
