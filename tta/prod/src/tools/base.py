    ...
    def execute(self, **kwargs) -> Any:
        ...
        try:
            ...
            if allowed_list:
                ...
                ToolPolicy(allowed_callables=allowed_list).validate_callable_allowed(dotted)
        except Exception:
            pass
        ...
        try:
            ...
            from src.agent_orchestration.tools.models import ToolSpec, ToolPolicy
            ...
            from src.agent_orchestration.tools.metrics import get_tool_metrics  # noqa: F401 (side effects)
        ...
        # Build registry
        try:
            redis_url = os.environ.get("TTA_REDIS_URL", os.environ.get("TEST_REDIS_URI", "redis://localhost:6379/0"))
        ...
        name = self.name if isinstance(self.name, str) and len(self.name) >= 3 else f"dyn.{self.name}"
        caps = [c for c, v in (("kg_read", self.kg_read), ("kg_write", self.kg_write)) if v]
        safety = []
        if self.kg_read or self.kg_write:
            safety.append("network")
        spec = ToolSpec(name=name, version=version, description=self.description, capabilities=caps, safety_flags=safety)
        ...
        if asyncio.iscoroutinefunction(self.action_fn):
            try:
                ...
                return asyncio.run(svc.invoke_tool_by_spec(spec, self.action_fn, **kwargs))  # type: ignore
