...
class DynamicGraphManager:
    ...
    async def process_text(
        ...
        for entity_type in entity_types:
            try:
                ...
                entities = await self.object_extractor.extract_objects(
                    text=text, object_type=entity_type, schema=schema
                )
        ...
        for relationship_type in relationship_types:
            try:
                ...
                if relationship_type not in self.schema_mapper._relationship_mappers:
                    logger.warning(
                        f"Relationship type {relationship_type} not registered"
                    )
                    continue
                ...
                if (
                    source_type not in entities_by_type
                    or not entities_by_type[source_type]
                ):
                    logger.warning(
                        f"No entities of source type {source_type} for relationship {relationship_type}"
                    )
                    relationships_by_type[relationship_type] = []
                    continue

                if (
                    target_type not in entities_by_type
                    or not entities_by_type[target_type]
                ):
                    logger.warning(
                        f"No entities of target type {target_type} for relationship {relationship_type}"
                    )
                    relationships_by_type[relationship_type] = []
                    continue
                ...
                source_entities = [
                    e["original"]
                    for e in [
                        self._entity_cache[n["id"]]
                        for n in entities_by_type[source_type]
                    ]
                ]
                target_entities = [
                    e["original"]
                    for e in [
                        self._entity_cache[n["id"]]
                        for n in entities_by_type[target_type]
                    ]
                ]
                ...
                for rel in relationships:
                    try:
                        ...
                        if not source_id or not target_id:
                            logger.warning(
                                f"Relationship missing source_id or target_id: {rel}"
                            )
                            continue
                ...
                logger.info(
                    f"Extracted {len(rel_objects)} {relationship_type} relationships"
                )
            except Exception as e:
        ...
        return {"entities": entities_by_type, "relationships": relationships_by_type}
    ...
    async def _create_relationship(
        self, relationship_type: str, relationship: dict[str, Any]
    ) -> None:
        """
    ...
    def _get_relationship_schema(self, relationship_type: str) -> dict[str, Any] | None:
        """
        ...
        if relationship_type == "HAS_MEMORY":
            return {
                "type": "object",
                "properties": {
    ...
    async def analyze_text_for_graph_updates(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                entity_types = [
                    item["type"] for item in analysis.get("entity_types", [])
                ]
                relationship_types = [
                    item["type"] for item in analysis.get("relationship_types", [])
                ]
