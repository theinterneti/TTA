"""
Agent to MCP Adapter (PRODUCTION READY)

This module provides an adapter that allows TTA agents to be exposed as MCP servers.
It converts agent methods and capabilities into MCP tools and resources.

NOTE: This adapter is designed for production or prototype use.
It provides a robust way to expose TTA agents as MCP servers.
"""
...
class AgentMCPAdapter:
    """
    Adapter that converts a TTA agent into an MCP server.

    This adapter takes a TTA agent and exposes its methods and capabilities
    ...
    def _register_agent_resources(self):
        ...
        # If the agent has a neo4j_manager, register knowledge graph resources
        if hasattr(self.agent, "neo4j_manager") and self.agent.neo4j_manager:
            ...
            def get_knowledge(query: str) -> str:
                ...
                try:
                    ...
                    results = self.agent.neo4j_manager.query(query)
