    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.post(
                f"{BASE_URL}/api/v1/auth/login", json=auth_data
            ) as response:
                if response.status == 200:
                    ...
                    async with session.post(
                        ...
                        if reg_response.status in [200, 201]:
                            ...
                            print(
                                f"❌ Authentication failed: {await reg_response.text()}"
                            )
                            return
        ...
        print(
            f"   Physical description: {character_data['appearance']['physical_description'][:50]}..."
        )
        print(f"   Backstory: {character_data['background']['backstory'][:50]}...")
        print(
            f"   Primary concerns: {len(character_data['therapeutic_profile']['primary_concerns'])}"
        )
        print(
            f"   Therapeutic goals: {len(character_data['therapeutic_profile']['therapeutic_goals'])}"
        )
        print()

        try:
            async with session.post(
                f"{BASE_URL}/api/v1/characters/", headers=headers, json=character_data
            ) as response:
                ...
                if response.status == 201:
                    ...
                    async with session.get(
                        f"{BASE_URL}/api/v1/characters/{character_id}", headers=headers
                    ) as get_response:
                        if get_response.status == 200:
                            ...
                            print(
                                f"   Personality traits: {len(retrieved.get('background', {}).get('personality_traits', []))}"
                            )
                            print(
                                f"   Life goals: {len(retrieved.get('background', {}).get('life_goals', []))}"
                            )
                            print(
                                f"   Therapeutic goals: {len(retrieved.get('therapeutic_profile', {}).get('therapeutic_goals', []))}"
                            )
                            print()
                            ...
                            async with session.get(
                                f"{BASE_URL}/api/v1/characters/", headers=headers
                            ) as list_response:
                                if list_response.status == 200:
                                    characters = await list_response.json()
                                    print("✅ Characters listed successfully!")
                                    print(f"   Total characters: {len(characters)}")
                                    for char in characters:
                                        print(
                                            f"   - {char.get('name')} (ID: {char.get('character_id')})"
                                        )
                                    print()
                                else:
                                    print(
                                        f"❌ Failed to list characters: {await list_response.text()}"
                                    )
                        else:
                            print(
                                f"❌ Failed to retrieve character: {await get_response.text()}"
                            )
                    ...
                    print(
                        f"❌ Character creation failed with status {response.status}:"
                    )
                    print(f"   {response_text}")
