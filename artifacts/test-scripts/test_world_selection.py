    ...
    async def test_world_details_api(self) -> bool:
        ...
        for world in self.worlds:
            ...
            try:
                ...
                async with httpx.AsyncClient() as client:
                    response = await client.get(
                        f"{self.base_url}/api/v1/worlds/{world_id}",
                        headers=headers
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        required_fields = [
                            "world_id", "name", "description", "long_description",
                            "therapeutic_themes", "key_characters", "main_storylines"
                        ]
                        ...
                        if not missing_fields:
                            ...
                            print(f"      Storylines: {len(details['main_storylines'])}")
                            print(f"      Techniques: {len(details['therapeutic_techniques_used'])}")
                            success_count += 1
                        else:
                            print(f"   ❌ {world_name}: Missing fields: {missing_fields}")
                    else:
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        world_name = next(w["name"] for w in self.worlds if w["world_id"] == test_case["world_id"])
                        
                        if keyword_matches >= 1:
                            print(f"   ✅ {world_name}: Contextual AI response")
                            print(f"      Keywords matched: {keyword_matches}/{len(test_case['expected_keywords'])}")
                            success_count += 1
                        else:
                            print(f"   ⚠️ {world_name}: Generic response (no context keywords)")
                    else:
                        print(f"   ❌ {test_case['world_id']}: Conversation API error {response.status_code}")
