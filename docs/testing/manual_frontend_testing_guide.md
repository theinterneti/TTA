# TTA Core Gameplay Loop - Manual Frontend Testing Guide

> ⚠️ **This document has moved!**
>
> This content is now maintained in the TTA-notes knowledge base:
> - **New location:** [[TTA/Workflows/TTA Core Gameplay Loop - Manual Frontend Testing Guide]]
> - **Path:** `.augment/kb/Workflows/TTA Core Gameplay Loop - Manual Frontend Testing Guide.md`
>
> If using Logseq with TTA-notes, you can access it directly in the graph.
> Otherwise, see: `.augment/kb/Workflows/TTA Core Gameplay Loop - Manual Frontend Testing Guide.md`

**Migration date:** 2025-11-01


---
**Logseq:** [[TTA.dev/Docs/Testing/Manual_frontend_testing_guide]]
