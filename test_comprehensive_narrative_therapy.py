...
async def test_comprehensive_narrative_therapy():
    ...
    framework_test_cases = [
        ...
        ("I want to tell you my story about overcoming depression", TherapeuticFramework.NARRATIVE),
    ...
    for i, (test_input, expected_framework) in enumerate(framework_test_cases, 1):
        ...
        print(f"      Expected: {expected_framework.value}, {'✓' if is_correct else '✗'}")
    ...
    responses = []
    for i in range(10):
        response = chat_interface._get_framework_response_template(TherapeuticFramework.NARRATIVE)
        responses.append(response)

    unique_responses = len(set(responses))
    diversity_score = unique_responses / len(responses) * 100

    print(f"Generated {len(responses)} responses, {unique_responses} unique ({diversity_score:.1f}% diversity)")
    ...
    for i, test_case in enumerate(story_test_cases, 1):
        ...
        type_correct = story_type == test_case["expected_type"] if test_case["should_detect"] else True

        print(f"  {i}. '{test_case['content'][:50]}...'")
        print(f"     Detected: {detected} (expected: {test_case['should_detect']}) {'✓' if detection_correct else '✗'}")
        if test_case["should_detect"]:
            print(f"     Type: {story_type} (expected: {test_case['expected_type']}) {'✓' if type_correct else '✗'}")
    ...
    try:
        enhanced_response = await chat_interface._generate_narrative_response(narrative_message, context, None)
    ...
    # Success criteria
    overall_success = (
        framework_success_rate >= 90 and
        diversity_score >= 50 and
        story_success_rate >= 80 and
        interactive_features_working and
        continuity_working
    )
