...
async def create_test_user_and_login():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            async with session.post(f"{API_BASE_URL}/api/v1/auth/register", json=register_data) as response:
...
async def test_nexus_state_endpoint(token):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(f"{API_BASE_URL}/api/v1/nexus/state", headers=headers) as response:
                if response.status == 200:
                    ...
                    print(f"   - Narrative strength: {data.get('narrative_strength', 0)}")
...
async def test_story_spheres_endpoint(token):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(f"{API_BASE_URL}/api/v1/nexus/spheres", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    ...
                    async with session.get(f"{API_BASE_URL}/api/v1/nexus/spheres?genre=fantasy", headers=headers) as filter_response:
                        if filter_response.status == 200:
                            filter_data = await filter_response.json()
                            fantasy_spheres = filter_data.get('spheres', [])
                            print(f"✅ Genre filter working: {len(fantasy_spheres)} fantasy spheres")
                        else:
...
async def test_world_creation_endpoint(token):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(f"{API_BASE_URL}/api/v1/nexus/worlds", headers=headers, json=world_data) as response:
...
async def test_world_details_endpoint(token, world_id):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(f"{API_BASE_URL}/api/v1/nexus/worlds/{world_id}", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
...
async def test_world_search_endpoint(token):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            # Test basic search
            async with session.get(f"{API_BASE_URL}/api/v1/nexus/worlds/search", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    results = data.get('results', [])
                    total_count = data.get('total_count', 0)
                    print(f"✅ Basic search successful: {len(results)} results, {total_count} total")
                    ...
                    async with session.get(f"{API_BASE_URL}/api/v1/nexus/worlds/search?{params_str}", headers=headers) as filter_response:
                        if filter_response.status == 200:
                            filter_data = await filter_response.json()
                            filter_results = filter_data.get('results', [])
                            print(f"✅ Filtered search successful: {len(filter_results)} fantasy worlds")
                        else:
...
async def test_world_entry_endpoint(token, world_id):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(f"{API_BASE_URL}/api/v1/nexus/worlds/{world_id}/enter", headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
...
async def test_error_handling(token):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            async with session.get(f"{API_BASE_URL}/api/v1/nexus/worlds/invalid_world_id", headers=headers) as response:
            ...
            async with session.post(f"{API_BASE_URL}/api/v1/nexus/worlds", headers={**headers, "Content-Type": "application/json"}, json=invalid_data) as response:
...
async def run_api_endpoint_tests():
    ...
    if results["world_creation"]:
        results["world_details"] = await test_world_details_endpoint(token, results["world_creation"])
        results["world_entry"] = await test_world_entry_endpoint(token, results["world_creation"])
    ...
    if passed_tests == total_tests:
        print("🎉 All API endpoint tests passed! Nexus Codex Phase 1 is fully functional!")
    else:
