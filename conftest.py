...
def _skip_neo4j_marker(request):
    ...
    if marker is not None:
        if not request.config.getoption("--neo4j") and os.environ.get(
            "RUN_NEO4J_TESTS"
        ) not in {"1", "true", "True"}:
            pytest.skip(
                "Skipping Neo4j tests (enable with --neo4j or RUN_NEO4J_TESTS=1)"
            )
...
def _skip_redis_marker(request):
    ...
    if marker is not None:
        if not request.config.getoption("--redis") and os.environ.get(
            "RUN_REDIS_TESTS"
        ) not in {"1", "true", "True"}:
            pytest.skip(
                "Skipping Redis tests (enable with --redis or RUN_REDIS_TESTS=1)"
            )
