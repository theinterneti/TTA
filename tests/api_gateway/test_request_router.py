...
from src.api_gateway.core import RequestRouter, GatewayCore
from src.api_gateway.models import RouteRule, RequestMethod, RouteType, ServiceInfo, ServiceType, ServiceEndpoint
...
class TestRequestRouter:
    ...
    @pytest.mark.asyncio
    async def test_initialize_with_default_routes(self, mock_gateway_core, mock_discovery_manager):
        ...
        added_rules = [call.args[0] for call in mock_gateway_core.add_route_rule.call_args_list]
    ...
    def test_create_default_routes(self, mock_gateway_core, mock_discovery_manager):
        ...
        auth_login_route = next((r for r in default_routes if r.name == "auth-login"), None)
        ...
        # Check player routes
        players_route = next((r for r in default_routes if r.name == "players-api"), None)
        ...
        sessions_route = next((r for r in default_routes if r.name == "sessions-api"), None)
        ...
        chat_ws_route = next((r for r in default_routes if r.name == "chat-websocket"), None)
    ...
    @pytest.mark.asyncio
    async def test_remove_dynamic_route_success(self, request_router, mock_gateway_core):
    ...
    @pytest.mark.asyncio
    async def test_remove_dynamic_route_not_found(self, request_router, mock_gateway_core):
    ...
    def test_get_route_rules(self, request_router, mock_gateway_core):
        ...
        mock_rules = [
            RouteRule(name="rule1", path_pattern="/api/v1/test1", target_service="service1"),
            RouteRule(name="rule2", path_pattern="/api/v1/test2", target_service="service2")
        ]
    ...
    @pytest.mark.asyncio
    async def test_update_routes_from_services(self, request_router, mock_discovery_manager, mock_gateway_core):
    ...
    @pytest.mark.asyncio
    async def test_update_routes_from_services_no_duplicates(self, request_router, mock_discovery_manager, mock_gateway_core):
        ...
        mock_discovery_manager.get_all_healthy_services.return_value = [existing_service, new_service]
    ...
    @pytest.mark.asyncio
    async def test_update_routes_from_services_error_handling(self, request_router, mock_discovery_manager, mock_gateway_core):
        """Test error handling in route updates."""
        mock_discovery_manager.get_all_healthy_services.side_effect = Exception("Service discovery failed")
...
class TestRouteRuleProperties:
    ...
    def test_authentication_route_properties(self):
        ...
        auth_routes = [r for r in routes if r.target_service == "authentication-service"]
        
        for route in auth_routes:
            assert route.therapeutic_priority is False  # Auth routes are not therapeutic
