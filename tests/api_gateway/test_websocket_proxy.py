...
from src.api_gateway.websocket.connection_manager import WebSocketConnectionManager, ConnectionType
...
from src.api_gateway.models import (
    AuthContext, UserPermissions, UserRole, ServiceInfo, ServiceType, ServiceEndpoint
)
...
def websocket_proxy(connection_manager, mock_service_router, mock_discovery_manager):
    ...
    return WebSocketProxy(connection_manager, mock_service_router, mock_discovery_manager)
...
class TestWebSocketProxy:
    ...
    @pytest.mark.asyncio
    async def test_select_backend_service_success(self, websocket_proxy, mock_service_info, mock_auth_context):
        """Test successful backend service selection."""
        websocket_proxy.service_router.select_service.return_value = mock_service_info
        
        service = await websocket_proxy._select_backend_service("test-service", mock_auth_context)
    ...
    @pytest.mark.asyncio
    async def test_select_backend_service_crisis_mode(self, websocket_proxy, mock_service_info, mock_auth_context):
    ...
    @pytest.mark.asyncio
    async def test_select_backend_service_no_auth(self, websocket_proxy, mock_service_info):
    ...
    @pytest.mark.asyncio
    async def test_select_backend_service_error(self, websocket_proxy, mock_auth_context):
        """Test backend service selection error handling."""
        websocket_proxy.service_router.select_service.side_effect = Exception("Service error")
    ...
    async def test_connect_to_backend_success(self, mock_session_class, websocket_proxy, mock_service_info, mock_auth_context):
        ...
        # Check URL
        assert call_args[0][0] == f"ws://{mock_service_info.endpoint.host}:{mock_service_info.endpoint.port}/ws"
    ...
    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession')
    async def test_connect_to_backend_error(self, mock_session_class, websocket_proxy, mock_service_info, mock_auth_context):
    ...
    @pytest.mark.asyncio
    async def test_handle_gateway_message_ping(self, websocket_proxy, connection_manager, mock_websocket):
        """Test handling ping message."""
        # Create connection
        connection_id = await connection_manager.connect(mock_websocket, ConnectionType.CHAT)
    ...
    @pytest.mark.asyncio
    async def test_handle_gateway_message_join_session(self, websocket_proxy, connection_manager, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_handle_gateway_message_leave_session(self, websocket_proxy, connection_manager, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_handle_gateway_message_therapeutic_event(self, websocket_proxy, connection_manager, mock_websocket):
        """Test handling therapeutic event message."""
        # Create connection
        connection_id = await connection_manager.connect(mock_websocket, ConnectionType.THERAPEUTIC_SESSION)
        connection = connection_manager.get_connection(connection_id)
    ...
    @pytest.mark.asyncio
    async def test_handle_gateway_message_crisis_alert(self, websocket_proxy, connection_manager, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_handle_gateway_message_unknown_type(self, websocket_proxy, connection_manager, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_handle_gateway_message_error(self, websocket_proxy, connection_manager, mock_websocket):
...
class TestWebSocketProxyIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_proxy_flow_mock(self, websocket_proxy, connection_manager, mock_websocket, 
                                       mock_service_info, mock_auth_context):
