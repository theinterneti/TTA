...
from src.api_gateway.models import ServiceEndpoint, ServiceInfo, ServiceType
...
class TestWeightedRoundRobinLoadBalancer:
    ...
    def test_therapeutic_priority_boost(self, sample_services):
        ...
        for _ in range(200):
            selected = balancer.select_service(
                healthy_services, therapeutic_priority=True
            )
            if selected:
                selections[selected.name] = selections.get(selected.name, 0) + 1
    ...
    def test_crisis_mode_boost(self, sample_services):
        ...
        for _ in range(200):
            selected = balancer.select_service(
                healthy_services, therapeutic_priority=True, crisis_mode=True
            )
            if selected:
                selections[selected.name] = selections.get(selected.name, 0) + 1
...
class TestLeastConnectionsLoadBalancer:
    ...
    def test_least_connections_selection(self, sample_services):
        ...
        balancer.service_metrics[str(healthy_services[0].id)] = ServiceMetrics(
            service_id=str(healthy_services[0].id), active_connections=5
        )
        balancer.service_metrics[str(healthy_services[1].id)] = ServiceMetrics(
            service_id=str(healthy_services[1].id), active_connections=2
        )
        balancer.service_metrics[str(healthy_services[2].id)] = ServiceMetrics(
            service_id=str(healthy_services[2].id), active_connections=8
        )
    ...
    def test_therapeutic_priority_connection_reduction(self, sample_services):
        ...
        balancer.service_metrics[str(healthy_services[0].id)] = ServiceMetrics(
            service_id=str(healthy_services[0].id), active_connections=3
        )
        balancer.service_metrics[str(healthy_services[1].id)] = ServiceMetrics(
        ...
        selected = balancer.select_service(healthy_services, therapeutic_priority=True)
...
class TestHealthBasedLoadBalancer:
    ...
    def test_health_based_selection(self, sample_services):
        ...
        assert selections[best_service_id] > selections.get(
            str(healthy_services[1].id), 0
        )
...
class TestTherapeuticPriorityLoadBalancer:
    ...
    def test_crisis_mode_therapeutic_only(self, sample_services):
        ...
        # Set up crisis loads
        therapeutic_service = next(
            s for s in healthy_services if s.therapeutic_priority
        )
        balancer.service_metrics[str(therapeutic_service.id)] = ServiceMetrics(
            service_id=str(therapeutic_service.id), crisis_load=1
        )

        selected = balancer.select_service(healthy_services, crisis_mode=True)
    ...
    def test_therapeutic_request_prefers_therapeutic_services(self, sample_services):
        ...
        for _ in range(50):
            ...
            if selected:
                selections[selected.therapeutic_priority] = (
                    selections.get(selected.therapeutic_priority, 0) + 1
                )
