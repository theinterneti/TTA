...
from src.api_gateway.models import AuthContext, UserPermissions, UserRole, ServiceInfo, ServiceType, ServiceEndpoint
...
class TestWebSocketConnectionManager:
    ...
    @pytest.mark.asyncio
    async def test_connect_success(self, connection_manager, mock_websocket, mock_auth_context):
    ...
    @pytest.mark.asyncio
    async def test_connect_therapeutic_session(self, connection_manager, mock_websocket, mock_auth_context):
    ...
    @pytest.mark.asyncio
    async def test_connect_crisis_mode(self, connection_manager, mock_websocket, mock_auth_context):
    ...
    @pytest.mark.asyncio
    async def test_connect_max_connections_exceeded(self, connection_manager, mock_websocket):
    ...
    @pytest.mark.asyncio
    async def test_connect_max_per_user_exceeded(self, connection_manager, mock_websocket, mock_auth_context):
        """Test connection rejection when max per user exceeded."""
        # Set low limit for testing
        connection_manager.max_connections_per_user = 1
        
        # First connection should succeed
        await connection_manager.connect(mock_websocket, ConnectionType.CHAT, mock_auth_context)
        ...
        with pytest.raises(Exception, match="Maximum connections per user exceeded"):
            await connection_manager.connect(mock_websocket2, ConnectionType.CHAT, mock_auth_context)
        
        mock_websocket2.close.assert_called_once()
    
    @pytest.mark.asyncio
    async def test_disconnect(self, connection_manager, mock_websocket, mock_auth_context):
    ...
    async def test_get_user_connections(self, connection_manager, mock_auth_context):
        ...
        conn_id1 = await connection_manager.connect(mock_ws1, ConnectionType.CHAT, mock_auth_context)
        conn_id2 = await connection_manager.connect(mock_ws2, ConnectionType.NARRATIVE, mock_auth_context)
        
        user_connections = connection_manager.get_user_connections(str(mock_auth_context.user_id))
    ...
    async def test_broadcast_to_user(self, connection_manager, mock_auth_context):
        ...
        await connection_manager.connect(mock_ws1, ConnectionType.CHAT, mock_auth_context)
        await connection_manager.connect(mock_ws2, ConnectionType.NARRATIVE, mock_auth_context)
        
        message = {"type": "broadcast", "data": "hello all"}
        sent_count = await connection_manager.broadcast_to_user(str(mock_auth_context.user_id), message)
    ...
    async def test_broadcast_to_session(self, connection_manager, mock_auth_context):
        ...
        await connection_manager.connect(
            mock_ws1, ConnectionType.THERAPEUTIC_SESSION, 
            mock_auth_context, therapeutic_session_id=session_id
        )
        await connection_manager.connect(
            mock_ws2, ConnectionType.THERAPEUTIC_SESSION, 
            mock_auth_context, therapeutic_session_id=session_id
        )
