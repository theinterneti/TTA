...
def test_player_experience_lifecycle_via_orchestrator() -> None:
    ...
    with patch('src.orchestration.orchestrator.TTAOrchestrator._import_repository_components') as mock_repo:
        with patch('src.orchestration.orchestrator.TTAOrchestrator._validate_repositories') as mock_validate:
            with patch('src.orchestration.orchestrator.TTAOrchestrator._import_core_components') as mock_core:
                ...
                assert orch.get_component_status('player_experience') in {ComponentStatus.RUNNING, ComponentStatus.ERROR, ComponentStatus.STOPPED}
