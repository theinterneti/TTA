...
from src.player_experience.api.auth import SECRET_KEY, ALGORITHM, create_tokens_for_player
from src.player_experience.models.player import PlayerProfile, TherapeuticPreferences, PrivacySettings
from src.player_experience.models.enums import IntensityLevel, TherapeuticApproach
from src.player_experience.managers.player_profile_manager import PlayerProfileManagerError, DataAccessRestrictedError
...
class TestPlayerManagementAPI:
    ...
    def test_create_player_success(self, client, mock_player_manager, sample_player_profile):
        ...
        with patch('src.player_experience.database.player_profile_repository.PlayerProfileRepository'), \
             patch('src.player_experience.api.routers.players.get_player_manager', return_value=mock_player_manager):
    ...
    def test_create_player_username_exists(self, client, mock_player_manager):
        ...
        mock_player_manager.create_player_profile.side_effect = PlayerProfileManagerError("Username 'testuser' already exists")
    ...
    def test_get_player_unauthorized(self, client, mock_player_manager, sample_player_profile):
    ...
    def test_export_player_data_success(self, client, mock_player_manager, sample_player_profile, auth_headers):
        ...
        assert data["export_data"]["player_profile"]["player_id"] == sample_player_profile.player_id
    ...
    def test_invalid_token_authentication(self, client, mock_player_manager, sample_player_profile, invalid_auth_headers):
        """Test API access with invalid authentication token."""
        with patch('src.player_experience.api.routers.players.get_player_manager', return_value=mock_player_manager):
            response = client.get(f"/api/v1/players/{sample_player_profile.player_id}", headers=invalid_auth_headers)
        
        assert response.status_code == 401
        data = response.json()
        assert "Authentication Failed" in data["error"]
    
    def test_missing_authorization_header(self, client, mock_player_manager, sample_player_profile):
    ...
    def test_malformed_authorization_header(self, client, mock_player_manager, sample_player_profile):
        """Test API access with malformed authorization header."""
        headers = {"Authorization": "InvalidFormat token"}
        
        response = client.get(f"/api/v1/players/{sample_player_profile.player_id}", headers=headers)
    ...
    def test_api_documentation_generation(self, client):
        ...
        assert "/api/v1/players/{player_id}/therapeutic-preferences" in openapi_spec["paths"]
