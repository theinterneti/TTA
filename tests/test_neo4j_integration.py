...
from src.components.gameplay_loop.database.neo4j_manager import (
    Neo4jConnectionManager, Neo4jGameplayManager, NarrativeGraphManager, 
    ProgressGraphManager, RelationshipManager
)
from src.components.gameplay_loop.database.migrations import MigrationManager, setup_database
...
from src.components.gameplay_loop.models.core import (
    GameplaySession, NarrativeScene, UserChoice, GameplayState, SceneType, ChoiceType
)
from src.components.gameplay_loop.models.progress import (
    ProgressMetric, SkillDevelopment, ProgressType, SkillLevel
)
...
class TestSessionOperations:
    ...
    async def test_create_session_node(self, gameplay_manager, sample_session):
        ...
        retrieved_session = await gameplay_manager.get_session_by_id(sample_session.session_id)
...
class TestNarrativeOperations:
    ...
    async def test_create_scene_node(self, gameplay_manager, sample_session, sample_scene):
        """Test creating a scene node."""
        # Create session first
        await gameplay_manager.create_session_node(sample_session)
        
        # Create scene
        success = await gameplay_manager.narrative_manager.create_scene_node(sample_scene)
        assert success
    
    async def test_create_choice_node(self, gameplay_manager, sample_session, sample_scene):
        ...
        choices = await gameplay_manager.narrative_manager.get_scene_choices(sample_scene.scene_id)
    ...
    async def test_get_narrative_path(self, gameplay_manager, sample_session):
        ...
        # Get narrative path
        path = await gameplay_manager.narrative_manager.get_narrative_path(sample_session.session_id)
        assert len(path) >= 1
...
class TestProgressOperations:
    ...
    async def test_get_user_progress_metrics(self, gameplay_manager):
        ...
        # Get metrics
        metrics = await gameplay_manager.progress_manager.get_user_progress_metrics("test_user_456")
...
class TestRelationshipOperations:
    ...
    async def test_create_relationship(self, gameplay_manager, connection_manager):
        ...
        query1 = "CREATE (u:User {user_id: 'rel_test_user', id: 'rel_test_user'}) RETURN u"
        ...
        success = await gameplay_manager.relationship_manager.create_relationship(
            "rel_test_user", NodeType.USER,
            "rel_test_session", NodeType.SESSION,
...
class TestQueryPerformance:
    ...
    async def test_session_query_performance(self, gameplay_manager, connection_manager):
