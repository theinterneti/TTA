...
from src.agent_orchestration.capabilities.auto_discovery import (
    AutoDiscoveryManager, DiscoveryConfig, DiscoveryStrategy
)
from src.agent_orchestration.api.diagnostics import DiagnosticsAPI
from src.agent_orchestration.models import AgentId, AgentType, AgentCapability, CapabilityType
...
class TestCapabilitySystemIntegration:
    ...
    async def test_capability_matching_integration(
        ...
        world_builder_agents = await redis_registry.find_agents_by_type(AgentType.WORLD_BUILDER)
    ...
    async def test_capability_system_cleanup(
        ...
        cleanup_agents_before = {
            agent_id: info for agent_id, info in all_agents_before.items()
    ...
    async def test_backward_compatibility_validation(
        ...
        manual_agent_id = AgentId(
            agent_type=AgentType.WORLD_BUILDER,
            instance="manual_registration_001"
        )
