...
class TestWebSocketRealAgentIntegration:
    ...
    async def test_complete_workflow_with_websocket_events(
        ...
        mock_websocket.receive_text.side_effect = [
            json.dumps({"type": "subscribe", "event_types": ["agent_status", "workflow_progress"]}),
        ...
        agent_events = [e for e in received_events if e.get("event_type") == "agent_status"]
        assert len(agent_events) > 0
        
        # Check for workflow progress events
        workflow_events = [e for e in received_events if e.get("event_type") == "workflow_progress"]
        assert len(workflow_events) > 0
    
    async def test_progressive_feedback_integration(
        self,
        enhanced_agent_proxies,
        websocket_manager,
        mock_websocket,
        event_publisher
    ):
        ...
        feedback_events = [e for e in received_events if e.get("event_type") == "progressive_feedback"]
    ...
    async def test_error_reporting_integration(
        ...
        error_events = [e for e in received_events if "error" in e.get("event_type", "").lower()]
    ...
    async def test_connection_recovery_with_subscriptions(
        ...
        user_subscriptions = await websocket_manager.get_user_subscriptions("test_user_recovery")
    ...
    async def test_real_time_workflow_progress_tracking(
        ...
        mock_websocket.send_json.side_effect = capture_event
        mock_websocket.receive_text.side_effect = [
            json.dumps({"type": "subscribe", "event_types": ["workflow_progress", "agent_status"]}),
            asyncio.CancelledError()
        ]
        ...
        assert len(progress_events) >= 3  # Should have start, intermediate, and completion events
    ...
    async def test_agent_operation_progress_tracking(
        ...
        mock_websocket.send_json.side_effect = capture_event
        mock_websocket.receive_text.side_effect = [
            json.dumps({"type": "subscribe", "event_types": ["agent_status", "progressive_feedback"]}),
            asyncio.CancelledError()
        ]
        ...
        nga_result = await nga_proxy.process({
            ...
            "context": {"session_id": "test_session", "world_state": wba_result.get("world_state", {})}
        })
    ...
    async def test_progressive_feedback_with_intermediate_results(
        ...
        await feedback_manager.complete_agent_operation(
            operation_id=operation_id,
            final_result={"complete_story": "Once upon a time, in a land far away, there lived a brave adventurer who discovered amazing treasures."},
            success=True
        )
        ...
        events_with_results = [
            ...
            if e.get("intermediate_result") is not None
        ]
