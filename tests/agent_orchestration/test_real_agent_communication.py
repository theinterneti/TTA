...
from src.agent_orchestration import (
    ...
    AgentId, AgentType, MessageCoordinator
)
...
from src.agent_orchestration.protocol_bridge import MessageRouter, ProtocolTranslator
...
class TestRealAgentCommunication:
    ...
    async def test_complete_workflow_chain(self, real_ipa_proxy, real_wba_proxy, real_nga_proxy):
    ...
    async def test_concurrent_agent_communication(self, real_ipa_proxy, real_wba_proxy, real_nga_proxy):
...
class TestCompleteWorkflowChains:
        ...
        return {
            "therapeutic_scenarios": [
                ...
                },
                {
                    ...
                    "world_context": {"location": "training_grounds", "mood": "uncertain"}
                },
                {
    ...
    async def test_therapeutic_workflow_chain(self, real_ipa_proxy, real_wba_proxy, real_nga_proxy, workflow_test_data):
        ...
        for scenario in workflow_test_data["therapeutic_scenarios"]:
            ...
            therapeutic_prompt = f"Provide supportive guidance for: {scenario['player_input']}"
            nga_result = await real_nga_proxy.process({
            ...
            supportive_indicators = ["support", "help", "understand", "together", "can", "able"]
            has_supportive_language = any(indicator in story for indicator in supportive_indicators)
            ...
            if nga_result.get("source") == "real_nga":
                assert has_supportive_language, f"Missing supportive language in {scenario['name']}"

        return results

    async def test_narrative_continuity_chain(self, real_ipa_proxy, real_wba_proxy, real_nga_proxy):
        ...
        for i, player_input in enumerate(narrative_sequence):
            ...
            world_updates = {
                "sequence_step": i + 1,
                "current_action": ipa_result.get("routing", {}).get("intent", "unknown"),
                "narrative_history": [r["narrative"] for r in workflow_results[-3:]]  # Last 3 narratives
            }
    ...
    async def test_error_recovery_chain(self, real_ipa_proxy, real_wba_proxy, real_nga_proxy):
        ...
        for result in recovery_results:
            if result["recovered"]:
                ...
                assert result["result"]["source"] in ["real_ipa", "real_wba", "real_nga", "mock_fallback"]
