...
from tests.agent_orchestration.test_multi_agent_workflow_integration import IntegrationTestHelper, PerformanceMetrics
...
class TestConcurrentWorkflowExecution:
    ...
    async def test_concurrent_workflow_isolation(
        ...
        test_env = await integration_helper.setup_test_environment("concurrent_isolation")
        ...
        try:
            ...
            for i in range(concurrent_count):
                ...
                request = OrchestrationRequest(
                    ...
                    input={**sample_player_inputs[i % len(sample_player_inputs)], "request_id": i},
            ...
            start_time = time.time()
            tasks = [execute_single_workflow(req, req_id) for req, req_id in concurrent_requests]
            ...
            assert len(set(request_ids)) == len(request_ids), "Request IDs not unique - isolation failed"
            
            session_ids = [result['session_id'] for result in successful_results if result['session_id']]
            assert len(set(session_ids)) == len(session_ids), "Session IDs not unique - isolation failed"
            ...
            assert concurrency_factor > 1.5, f"Low concurrency factor {concurrency_factor:.2f} - may be running sequentially"
            
            print(f"✓ Concurrent execution: {len(successful_results)}/{len(results)} successful")
    ...
    async def test_workflow_resource_contention(
        ...
        test_env = await integration_helper.setup_test_environment("resource_contention")
        ...
        try:
            ...
            class ResourceLimitedAgent(WorldBuilderAgentProxy):
                ...
                async def process(self, input_payload: dict) -> dict:
                    ...
                    if agent_key not in resource_usage:
                        resource_usage[agent_key] = {'max_concurrent': 0, 'total_requests': 0}
            ...
            with patch('src.agent_orchestration.agents.AgentRegistry.get_agent') as mock_get_agent:
                ...
                async def execute_with_contention(request, request_id):
                    start_time = time.time()
                    response, run_id, error = workflow_manager.execute_workflow("contention_test", request)
                    execution_time = time.time() - start_time
                ...
                tasks = [execute_with_contention(req, req_id) for req, req_id in concurrent_requests]
                results = await asyncio.gather(*tasks, return_exceptions=True)
                
                # Analyze resource contention results
                successful_results = [r for r in results if not isinstance(r, Exception) and r['error'] is None]
                
                # Verify resource limits were respected
                wba_key = f"WBA:limited"
                if wba_key in resource_usage:
                    ...
                    print(f"✓ Resource usage - Max concurrent: {max_concurrent}, Total requests: {total_requests}")
                    
                    # Verify that resource contention was handled (some requests should have been queued)
                    assert total_requests >= len(concurrent_requests), "Not all requests were processed"
                    
                    # Verify execution times show contention effects
                    execution_times = [r['execution_time'] for r in successful_results]
                    if len(execution_times) > 1:
                        time_variance = statistics.stdev(execution_times)
                        assert time_variance > 0.1, "Execution times too uniform - contention not evident"
                
                print(f"✓ Resource contention test: {len(successful_results)}/{len(results)} successful")
...
class TestPerformanceBenchmarks:
    ...
    async def test_workflow_performance_benchmarks(
        ...
        test_env = await integration_helper.setup_test_environment("performance_benchmark")
        ...
        try:
            ...
            for scenario_name, scenario in performance_test_scenarios.items():
                ...
                async def benchmark_workflow(workflow_id):
                    ...
                    request = OrchestrationRequest(
                        entrypoint=AgentType.IPA,
                        input=sample_player_inputs[workflow_id % len(sample_player_inputs)]
                    )

                    workflow_start = time.time()
                    response, run_id, error = workflow_manager.execute_workflow("benchmark_test", request)
                ...
                success_rate = completed_workflows / workflow_id if workflow_id > 0 else 0
                ...
                print(f"✓ {scenario_name}: {completed_workflows} workflows, {success_rate:.2%} success rate")
                print(f"✓ Average time: {avg_time:.2f}s, Throughput: {throughput:.2f} workflows/sec")

            # Generate benchmark report
            print("\n=== PERFORMANCE BENCHMARK REPORT ===")
            for scenario_name, results in benchmark_results.items():
                print(f"\n{scenario_name.upper()}:")
                print(f"  Completed: {results['completed_workflows']}/{results['total_attempts']}")
    ...
    async def test_memory_and_cpu_usage_monitoring(
        ...
        test_env = await integration_helper.setup_test_environment("resource_monitoring")
        ...
        try:
            ...
            async def load_test_workflow(workflow_id):
                request = OrchestrationRequest(
                    entrypoint=AgentType.IPA,
                    input={**sample_player_inputs[workflow_id % len(sample_player_inputs)], "load_test_id": workflow_id}
                )

                response, run_id, error = workflow_manager.execute_workflow("resource_test", request)
                return error is None
            ...
            if resource_samples:
                ...
                cpu_values = [sample['cpu_percent'] for sample in resource_samples if sample['cpu_percent'] > 0]
                ...
                assert memory_increase < 500, f"Memory increase {memory_increase:.1f}MB too high - possible memory leak"
                assert max_memory < 2000, f"Peak memory {max_memory:.1f}MB too high"
                assert avg_cpu < 80, f"Average CPU {avg_cpu:.1f}% too high"

                print(f"✓ Resource monitoring: {successful_workflows}/{workflow_id} workflows successful")
                print(f"✓ Memory: {initial_memory:.1f}MB → {max_memory:.1f}MB (Δ{memory_increase:.1f}MB)")
                ...
                if memory_cleanup > 50:  # Significant memory was freed
                    print(f"✓ Memory cleanup: {memory_cleanup:.1f}MB freed after load test")
        ...
        try:
            ...
            def scale_agent_pool(agent_type, target_load):
                ...
                if target_load > current_active * 2 and current_active < pool['max']:
                    new_instances = min(pool['max'] - current_active, max(1, target_load // 2))
                ...
                # Scale down if load is low (simplified logic)
                    ...
                    instances_to_remove = min(current_active - 1, current_active - target_load)
            ...
            load_patterns = [
                ...
                {'duration': 10, 'concurrent_requests': 8, 'description': 'medium_load'},
            ...
            for pattern in load_patterns:
                ...
                async def execute_scaling_workflow(wf_id):
                    ...
                    request = OrchestrationRequest(
                        entrypoint=AgentType.IPA,
                        input={**sample_player_inputs[wf_id % len(sample_player_inputs)], "scaling_test_id": wf_id}
                    )
                    ...
                    for agent_type, pool in agent_pools.items():
                        if pool['instances']:
                            ...
                            selected_agent = pool['instances'][wf_id % len(pool['instances'])]
                ...
                while time.time() - start_time < duration:
                    ...
                    while len(active_workflows) < concurrent_requests:
                        task = asyncio.create_task(execute_scaling_workflow(workflow_id))
                ...
                print(f"✓ {pattern['description']}: {completed_workflows} workflows completed")
            ...
            assert len(scale_up_events) > 0, "No scale-up events detected - scaling may not be working"

            # Verify agents scaled appropriately for high load
            high_load_events = [e for e in scale_up_events if e['trigger_load'] >= 10]
            assert len(high_load_events) > 0, "No scaling detected for high load scenarios"
            ...
            for agent_type, pool in agent_pools.items():
                assert pool['active'] <= pool['max'], f"{agent_type.value} exceeded maximum pool size"
