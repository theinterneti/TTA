# Logseq: [[TTA.dev/Tests/Agent_orchestration/Test_safety_diagnostics_endpoints]]
import json
import os
...
async def test_safety_endpoints_when_diagnostics_disabled(redis_client):
    ...
    assert g.json().get("enabled") in (
        True,
        False,
    )  # service may still be enabled; diagnostics disabled refers to admin
