...
from src.agent_orchestration import (
    AgentId, AgentType, MessageType, MessagePriority,
    AgentMessage, OrchestrationRequest, OrchestrationResponse,
    WorkflowManager, WorkflowDefinition, WorkflowType, AgentStep,
    Agent, AgentRegistry,
    InputProcessorAgentProxy, WorldBuilderAgentProxy, NarrativeGeneratorAgentProxy,
    MessageCoordinator, AgentContext, SessionContext,
)
...
def sample_player_inputs():
    ...
    return [
        ...
        },
        {
            ...
            "context": {"location": "forest_clearing", "direction": "north", "mood": "adventurous"}
        },
        {
            "text": "talk to the wise elder about my fears",
            "expected_intent": "talk_to",
            "context": {"location": "village_center", "npc": "wise_elder", "mood": "anxious", "therapeutic_topic": "anxiety"}
        },
        {
            "text": "examine the ancient journal on the table",
            "expected_intent": "examine",
            "context": {"location": "library", "object": "ancient_journal", "mood": "contemplative"}
        },
        {
            "text": "I feel overwhelmed by all these choices",
            "expected_intent": "express_emotion",
            "context": {"location": "crossroads", "emotion": "overwhelmed", "therapeutic_topic": "decision_making"}
        }
    ]
...
def expected_narrative_outputs():
    ...
    return {
        ...
        "talk_to_elder_anxiety": {
            ...
            "therapeutic_elements": ["cognitive_reframing", "self_compassion", "perspective_taking"],
...
class WorkflowStateVerifier:
    ...
    @staticmethod
    def verify_message_routing(messages: List[AgentMessage], expected_sequence: List[AgentType]) -> bool:
        """Verify that messages follow the expected agent sequence."""
        if len(messages) != len(expected_sequence):
            return False
        
        for i, (message, expected_agent) in enumerate(zip(messages, expected_sequence)):
    ...
    @staticmethod
    def verify_state_persistence(initial_state: Dict[str, Any], final_state: Dict[str, Any]) -> Dict[str, bool]:
        """Verify that critical state elements are preserved across workflow execution."""
        checks = {
            "session_id_preserved": initial_state.get("session_id") == final_state.get("session_id"),
            "player_id_preserved": initial_state.get("player_id") == final_state.get("player_id"),
            "therapeutic_context_maintained": "therapeutic_profile" in final_state,
            "game_state_updated": final_state.get("game_state", {}) != initial_state.get("game_state", {}),
        }
        return checks
    ...
    def verify_response_aggregation(responses: List[Dict[str, Any]]) -> Dict[str, bool]:
        ...
        checks = {
            ...
            "responses_have_content": all("response" in r or "output" in r for r in responses),
            "therapeutic_validation_present": any("therapeutic_validation" in r for r in responses),
...
class Neo4jStateVerifier:
    ...
    async def verify_workflow_state_transitions(self, workflow_id: str) -> Dict[str, Any]:
        ...
        with self.driver.session() as session:
            ...
            return {
                ...
                "has_completion_time": workflow_data["e"].get("completed_at") is not None,
...
class RedisStateVerifier:
    ...
    async def verify_workflow_coordination_state(self, workflow_id: str) -> Dict[str, Any]:
    ...
    def _calculate_oldest_message_age(self, messages: List[Dict]) -> Optional[float]:
...
class IntegrationTestHelper:
    ...
    async def cleanup_test_data(self, session_id: str, workflow_id: str, player_id: str):
    ...
    async def wait_for_workflow_completion(self, workflow_id: str, timeout: float = 30.0) -> bool:
        """Wait for workflow to complete with timeout."""
        start_time = time.time()

        while time.time() - start_time < timeout:
            state = await self.redis_verifier.verify_workflow_coordination_state(workflow_id)
    ...
    async def simulate_agent_failure(self, agent_id: str, failure_duration: float = 5.0):
    ...
    return {
        "anxiety_management": {
            ...
            "expected_wba_output": {
                ...
                "therapeutic_modifications": ["reduced_threat_level", "added_comfort_elements"],
            ...
            "expected_nga_output": {
                ...
                "therapeutic_techniques": ["breathing_exercise", "reframing", "curiosity_cultivation"],
        ...
        "self_confidence_building": {
            ...
            "expected_wba_output": {
                "scene_modifications": ["highlight_player_strengths", "show_villager_appreciation"],
            ...
            "expected_nga_output": {
                ...
                "therapeutic_techniques": ["strength_identification", "past_success_recall", "courage_reframing"],
        ...
        "grief_processing": {
            ...
            "expected_wba_output": {
                "scene_adjustments": ["gentle_atmosphere", "memorial_elements", "peaceful_setting"],
            ...
            "expected_nga_output": {
                ...
                "therapeutic_techniques": ["grief_normalization", "memory_honoring", "gentle_invitation"],
...
def therapeutic_validation_data():
    ...
    return {
        ...
        "concerning_content": {
            ...
            "therapeutic_concerns": ["hopelessness", "self_harm_ideation", "catastrophizing"],
        ...
        "blocked_content": {
            ...
            "safety_violations": ["violence", "explicit_content", "inappropriate_themes"],
