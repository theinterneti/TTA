...
async def test_websocket_connection_manager_creation():
    ...
    manager = WebSocketConnectionManager(
        config=config, agent_registry=None, redis_client=None
    )
...
async def test_websocket_connection_health_status():
    ...
    mock_websocket = Mock()
    connection = WebSocketConnection(
        websocket=mock_websocket, connection_id="test-conn", user_id="test-user"
    )
...
async def test_event_authorization():
    ...
    assert manager._is_authorized_for_event_type(
