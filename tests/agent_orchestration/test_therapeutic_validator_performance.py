...
class TestTherapeuticValidatorPerformance:
    ...
    def test_batch_validation_performance(self):
        ...
        assert avg_time_per_validation < 0.025, f"Batch validation too slow: {avg_time_per_validation:.3f}s per validation"
        assert validations_per_second > 40, f"Throughput too low: {validations_per_second:.1f} validations/sec"
    ...
    def test_concurrent_validation_performance(self):
        ...
        assert validations_per_second > 30, f"Concurrent throughput too low: {validations_per_second:.1f} validations/sec"
    ...
    def test_large_content_performance(self):
        ...
        assert max(times) < 0.500, f"Large content validation too slow: {max(times):.3f}s"
...
class TestTherapeuticValidatorAccuracy:
    ...
    def test_crisis_detection_accuracy(self):
        ...
        for text, expected_types in true_positive_cases:
            result = self.validator.validate_text(text)
            if result.crisis_detected and any(ct in result.crisis_types for ct in expected_types):
                true_positives += 1
            else:
                print(f"False negative: '{text}' - Expected {[ct.value for ct in expected_types]}, got {[ct.value for ct in result.crisis_types]}")
    ...
    def test_therapeutic_appropriateness_accuracy(self):
        ...
        for text, (min_expected, max_expected) in test_cases:
            ...
            if min_expected <= appropriateness <= max_expected:
                ...
                print(f"Inappropriate score: '{text}' -> {appropriateness:.2f} (expected {min_expected:.2f}-{max_expected:.2f})")
        ...
        assert accuracy >= 0.85, f"Therapeutic appropriateness accuracy too low: {accuracy:.2%}"
    ...
    def test_alternative_generation_quality(self):
        ...
        for text, expected_keywords in crisis_cases:
            ...
            if result.alternative_content:
                ...
                keyword_matches = sum(1 for keyword in expected_keywords if keyword in alternative)
                keyword_score = keyword_matches / len(expected_keywords)
                
                # Check for supportive tone (basic heuristic)
                supportive_words = ["support", "help", "care", "understand", "here", "professional"]
                supportive_count = sum(1 for word in supportive_words if word in alternative)
                supportive_score = min(1.0, supportive_count / 2)  # At least 2 supportive words
                ...
                print(f"    Keywords: {keyword_score:.2f}, Supportive: {supportive_score:.2f}, Length: {length_score:.2f}")
            else:
        ...
        print(f"  Alternatives generated: {sum(1 for score in quality_scores if score > 0)}/{len(crisis_cases)}")
        
        # Should generate high-quality alternatives
        assert avg_quality >= 0.75, f"Alternative generation quality too low: {avg_quality:.2f}"
