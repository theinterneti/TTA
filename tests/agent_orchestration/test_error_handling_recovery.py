...
from tests.agent_orchestration.test_multi_agent_workflow_integration import IntegrationTestHelper
    ...
    async def test_agent_unavailability_failover(
        ...
        try:
            ...
            workflow_def = WorkflowDefinition(
                ...
                agent_sequence=[
                    AgentStep(agent=AgentType.IPA, name="input_processing", timeout_seconds=5),
                    AgentStep(agent=AgentType.WBA, name="world_building", timeout_seconds=5),
                    AgentStep(agent=AgentType.NGA, name="narrative_generation", timeout_seconds=5),
                ],
            ...
            await integration_helper.simulate_agent_failure(wba_agent_id, failure_duration=3.0)
            ...
            response, run_id, error = workflow_manager.execute_workflow("failover_test", request)
            execution_time = time.time() - start_time
            
            # Verify failover behavior
            if error is not None:
                # If workflow failed, verify it was due to agent unavailability
                assert "unavailable" in str(error).lower() or "timeout" in str(error).lower(), \
                ...
                assert "fallback" in str(metadata).lower() or "retry" in str(metadata).lower(), \
        ...
        try:
            ...
            with patch('src.agent_orchestration.agents.AgentRegistry.get_agent') as mock_get_agent:
                ...
                for i in range(6):
                    ...
                    response, run_id, error = workflow_manager.execute_workflow("circuit_test", request)
                    ...
                    if i >= 4:  # After 5 failures, circuit should be open
                        ...
                        assert "circuit" in str(error).lower() or "unavailable" in str(error).lower(), \
...
class TestNetworkFailureScenarios:
    ...
    async def test_redis_connection_failure(
        ...
        try:
            ...
            with patch('src.agent_orchestration.coordinators.redis_message_coordinator.RedisMessageCoordinator.send_message') as mock_send:
                ...
                response, run_id, error = workflow_manager.execute_workflow("redis_failure_test", request)
                
                # Verify graceful handling of Redis failure
                if error is not None:
                    assert "connection" in str(error).lower() or "redis" in str(error).lower(), \
                        f"Error should indicate connection issue: {error}"
                else:
                    # If workflow succeeded, verify fallback was used
                    assert response is not None, "Response should exist if workflow succeeded"
                    metadata = response.workflow_metadata
                    assert "fallback" in str(metadata).lower() or "degraded" in str(metadata).lower(), \
    ...
    async def test_neo4j_write_failure_recovery(
        ...
        try:
            ...
            with patch('src.components.neo4j_manager.Neo4jManager.execute_write') as mock_write:
                ...
                response, run_id, error = workflow_manager.execute_workflow("neo4j_failure_test", request)
                ...
                if error is None:
                    ...
                    assert "neo4j" in str(error).lower() or "database" in str(error).lower(), \
    ...
    async def test_invalid_message_format_handling(
        ...
        try:
            ...
            for _ in range(len(malformed_messages)):
                try:
                    received = await coordinator.receive_message(agent_id, timeout_seconds=1.0)
                    if received is not None:
                        valid_messages_received += 1
                except Exception as e:
                    errors_handled += 1
                    # Verify error is handled appropriately
                    assert "format" in str(e).lower() or "validation" in str(e).lower() or "json" in str(e).lower(), \
                        f"Unexpected error type for malformed message: {e}"
            
            # Verify system handled malformed messages without crashing
            assert errors_handled > 0, "Should have encountered errors with malformed messages"
            
            # Verify queue is cleaned up (malformed messages removed or handled)
            final_queue_length = await redis_client.llen(queue_key)
            assert final_queue_length == 0, "Queue should be cleaned of malformed messages"
        ...
        try:
            ...
            workflow_def = WorkflowDefinition(
                ...
                agent_sequence=[
                    ...
                    AgentStep(agent=AgentType.WBA, name="slow_wba", timeout_seconds=3),  # Will timeout
            ...
            with patch('src.agent_orchestration.agents.AgentRegistry.get_agent') as mock_get_agent:
                ...
                start_time = time.time()
                response, run_id, error = workflow_manager.execute_workflow("timeout_test", request)
                execution_time = time.time() - start_time

                # Verify timeout behavior
                if error is not None:
                    # Should timeout and fail
                    assert "timeout" in str(error).lower(), f"Expected timeout error, got: {error}"
                    assert execution_time < 10.0, "Should have timed out before slow agent completed"
                    assert execution_time >= 3.0, "Should have waited for timeout period"
                else:
                    # If succeeded, should have used recovery mechanism
                    assert response is not None, "Response should exist if workflow succeeded"
                    metadata = response.workflow_metadata
                    assert "timeout" in str(metadata).lower() or "recovery" in str(metadata).lower(), \
