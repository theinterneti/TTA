...
def test_mock_workflow_execution():
    ...
    response, run_id, error = workflow_manager.execute_workflow("test_workflow", request)
...
def test_test_data_fixtures():
    ...
    mock_sample_inputs = MockFixture(lambda: [
        {
            "text": "look around",
    ...
    assert "expected_intent" in inputs[0], "Sample input should have 'expected_intent' field"
...
def test_performance_metrics_utility():
    ...
    assert workflow_stats["max_time"] >= workflow_stats["min_time"], "Invalid min/max times"
...
def test_workflow_state_verifier():
    ...
    assert persistence_checks["therapeutic_context_maintained"], "Therapeutic context not maintained"
    ...
    assert aggregation_checks["therapeutic_validation_present"], "Therapeutic validation missing"
...
if __name__ == "__main__":
    ...
    print("  uv run pytest tests/agent_orchestration/test_*integration*.py --redis --neo4j")
    print("\nTo run individual test categories:")
    print("  uv run pytest tests/agent_orchestration/test_end_to_end_workflows.py --redis --neo4j")
    print("  uv run pytest tests/agent_orchestration/test_error_handling_recovery.py --redis")
    print("  uv run pytest tests/agent_orchestration/test_state_persistence_aggregation.py --neo4j --redis")
