...
class TestRealisticCrisisScenarios:
    ...
    def test_complex_mixed_scenarios(self):
        ...
        complex_scenarios = [
            ...
            ("Suicidal + self-harm", "I want to kill myself and I've been cutting", 
             [CrisisType.SUICIDAL_IDEATION, CrisisType.SELF_HARM]),
            ("Depression + suicidal", "I feel hopeless and want to die", 
             [CrisisType.SEVERE_DEPRESSION, CrisisType.SUICIDAL_IDEATION]),
            ("All three", "I'm worthless, I hurt myself, and I want to die", 
             [CrisisType.SEVERE_DEPRESSION, CrisisType.SELF_HARM, CrisisType.SUICIDAL_IDEATION]),
            
            # Contextual scenarios
            ("Recent loss", "Since my mom died, I don't want to be here anymore", 
             [CrisisType.SUICIDAL_IDEATION]),
            ("Relationship crisis", "After the breakup, I feel like ending it all", 
             [CrisisType.SUICIDAL_IDEATION]),
            ("Academic pressure", "If I fail this exam, I might as well kill myself", 
             [CrisisType.SUICIDAL_IDEATION]),
        ]
        ...
        for description, text, expected_crisis_types in complex_scenarios:
            ...
            assert result.crisis_detected, f"Crisis not detected for {description}: {text}"
    ...
    def test_sensitivity_and_specificity(self):
        ...
        print(f"Crisis Detection Performance:")
    ...
    def test_intervention_appropriateness(self):
        ...
        crisis_scenarios = [
            ("I want to kill myself", CrisisType.SUICIDAL_IDEATION, ["crisis helpline", "988"]),
            ("I want to hurt myself", CrisisType.SELF_HARM, ["stay safe", "grounding"]),
            ("I feel hopeless", CrisisType.SEVERE_DEPRESSION, ["difficult time", "support"]),
        ]
        ...
        for text, expected_crisis, expected_keywords in crisis_scenarios:
            ...
            assert expected_crisis in result.crisis_types, f"Wrong crisis type for: {text}"
            assert result.alternative_content is not None, f"No alternative provided for: {text}"
            
            # Check that alternative contains appropriate keywords
            alternative_lower = result.alternative_content.lower()
            keyword_found = any(keyword in alternative_lower for keyword in expected_keywords)
            assert keyword_found, \
                f"Alternative doesn't contain expected keywords {expected_keywords} for: {text}"
    
    def _test_crisis_scenarios(self, scenarios: List[Tuple[str, str, bool]], crisis_type: CrisisType):
...
class TestCrisisInterventionMechanisms:
    ...
    def test_escalation_thresholds(self):
        ...
        high_confidence_result = self.validator.validate_text("I want to kill myself right now")
    ...
    def test_therapeutic_appropriateness_scoring(self):
        ...
        safe_result = self.validator.validate_text("I would like to talk about my feelings")
        ...
        for text in crisis_texts:
            ...
            if result.alternative_content:
                ...
                supportive_indicators = [
                    "support", "help", "care", "here", "understand", 
                    "concerned", "value", "professional", "resources"
                ]
                has_supportive = any(indicator in alternative for indicator in supportive_indicators)
                assert has_supportive, f"Alternative lacks supportive language: {alternative}"
