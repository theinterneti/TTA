...
from src.agent_orchestration.realtime.agent_event_integration import AgentWorkflowCoordinator
...
class TestWorkflowChainValidation:
    ...
    async def test_complete_workflow_chain_execution(
        ...
        job_related = any("job" in str(entity).lower() or "work" in str(entity).lower() 
        ...
        supportive_keywords = ["manage", "help", "support", "cope", "balance", "strategies"]
        contains_support = any(keyword in final_story.lower() for keyword in supportive_keywords)
    ...
    async def test_workflow_data_flow_integrity(
        ...
        entity_incorporation = any(
            entity in world_state_text for entity in entity_strings
        ...
        world_state_incorporation = any(
            element in narrative_lower for element in world_elements
            if len(element) > 3
        )
        ...
        print(f"Data flow validation completed:")
        ...
        print(f"  Intent-therapeutic alignment (IPA→NGA): {intent_therapeutic_alignment}")
        ...
        for i, problematic_input in enumerate(problematic_inputs):
            ...
            try:
                ...
                assert any(keyword in error_message for keyword in 
                          ["validation", "input", "format", "length"])
        
        print(f"Error propagation test completed for {len(problematic_inputs)} problematic inputs")
    ...
    async def test_workflow_therapeutic_consistency(
        ...
        therapeutic_inputs = [
            {
                ...
                "expected_therapeutic_elements": ["support", "hope", "professional", "help"]
            },
            {
                "input": "I have social anxiety and avoid people.",
                "expected_therapeutic_elements": ["gradual", "practice", "support", "confidence"]
            },
            {
                "input": "I'm struggling with grief after losing someone.",
                "expected_therapeutic_elements": ["process", "time", "support", "healing"]
            }
        ]
        ...
        for i, scenario in enumerate(therapeutic_inputs):
            ...
            assert safety_assessment.get("risk_level") in ["LOW", "MEDIUM"]  # Should not be high risk
            ...
            therapeutic_count = sum(
                1 for element in scenario["expected_therapeutic_elements"]
                if element in story
            )
        ...
        print(f"Therapeutic consistency validated across {len(therapeutic_inputs)} scenarios")
