...
from src.agent_orchestration.realtime.models import AgentStatusEvent, AgentStatus
...
class TestWebSocketPerformance:
    ...
    async def test_concurrent_connection_capacity(self, high_capacity_websocket_manager):
        ...
        assert stats["active_connections"] >= successful_connections * 0.8  # Most connections active
    ...
    async def test_high_event_throughput(self, high_capacity_websocket_manager, event_publisher):
        ...
        assert event_publisher.events_published >= total_events * 0.9  # At least 90% published
    ...
    async def test_connection_churn_performance(self, high_capacity_websocket_manager):
        ...
        for cycle in range(churn_cycles):
            ...
            for i in range(connections_per_cycle):
                ...
                mock_ws.receive_text = AsyncMock(side_effect=[
                    json.dumps({"type": "auth", "user_id": f"churn_user_{connection_id}"}),
                    json.dumps({"type": "subscribe", "event_types": ["agent_status"]}),
        ...
        # Assertions
        assert connections_per_second > 50  # Should handle at least 50 connections/second
        ...
        print(f"Connection Churn Test Results:")
    ...
    async def test_large_message_handling(self, high_capacity_websocket_manager):
        ...
        for i in range(messages_per_connection):
            ...
            sent_count = await high_capacity_websocket_manager.broadcast_event(large_event)
        ...
        # Calculate metrics
        duration = end_time - start_time
        throughput_mbps = (total_bytes_sent / (1024 * 1024)) / duration if duration > 0 else 0
        
        # Assertions
        assert throughput_mbps > 1.0  # Should handle at least 1 MB/s
        assert messages_received >= (connection_count * messages_per_connection * 0.8)  # 80% delivery
        
        print(f"Large Message Test Results:")
    ...
    async def test_memory_usage_under_load(self, high_capacity_websocket_manager, event_publisher):
