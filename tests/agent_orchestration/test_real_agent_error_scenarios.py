...
from src.agent_orchestration.adapters import AgentCommunicationError, RetryConfig
...
class TestRealAgentErrorScenarios:
    ...
    async def test_retry_mechanism_exhaustion(self, no_fallback_ipa):
        ...
        with patch.object(no_fallback_ipa, 'ipa_adapter') as mock_adapter:
            mock_adapter.process_input.side_effect = AgentCommunicationError("Simulated failure")
    ...
    async def test_malformed_input_handling(self, error_prone_coordinator):
        ...
        proxies = [
            InputProcessorAgentProxy(coordinator=error_prone_coordinator, enable_real_agent=True, fallback_to_mock=True),
            WorldBuilderAgentProxy(coordinator=error_prone_coordinator, enable_real_agent=True, fallback_to_mock=True),
            NarrativeGeneratorAgentProxy(coordinator=error_prone_coordinator, enable_real_agent=True, fallback_to_mock=True)
        ]
            ...
            for malformed_input in malformed_inputs:
                try:
                    if isinstance(proxy, InputProcessorAgentProxy):
                        if malformed_input is None or "text" not in malformed_input or not malformed_input.get("text"):
                            # Should raise ValueError for missing text
                            with pytest.raises(ValueError):
                                await proxy.process(malformed_input)
                            error_results.append({"proxy": "IPA", "input": malformed_input, "handled": True})
                        else:
                            result = await proxy.process(malformed_input)
                            error_results.append({"proxy": "IPA", "input": malformed_input, "result": result})
                    
                    elif isinstance(proxy, WorldBuilderAgentProxy):
                        if malformed_input is None or "world_id" not in malformed_input or not malformed_input.get("world_id"):
                            # Should raise ValueError for missing world_id
                            with pytest.raises(ValueError):
                                await proxy.process(malformed_input)
                            error_results.append({"proxy": "WBA", "input": malformed_input, "handled": True})
                        else:
                            result = await proxy.process(malformed_input)
                            error_results.append({"proxy": "WBA", "input": malformed_input, "result": result})
                    
                    elif isinstance(proxy, NarrativeGeneratorAgentProxy):
                        if malformed_input is None or "prompt" not in malformed_input or not malformed_input.get("prompt"):
                            # Should raise ValueError for missing prompt
                            with pytest.raises(ValueError):
                                await proxy.process(malformed_input)
                            error_results.append({"proxy": "NGA", "input": malformed_input, "handled": True})
                        else:
                            result = await proxy.process(malformed_input)
                            error_results.append({"proxy": "NGA", "input": malformed_input, "result": result})
        ...
        handled_count = sum(1 for result in error_results if result.get("handled", False))
        ...
        async def process_with_potential_failure(proxy_id: int):
            ...
            if random.random() < failure_rate:
                ...
                with patch.object(proxy, 'ipa_adapter') as mock_adapter:
                    mock_adapter.process_input.side_effect = AgentCommunicationError("Simulated concurrent failure")
                    
                    try:
                        result = await proxy.process({"text": f"concurrent test {proxy_id}"})
    ...
    async def test_resource_exhaustion_handling(self, error_prone_coordinator):
        ...
        async def rapid_processing(proxy_id: int):
            ...
            for j in range(5):  # 5 requests per proxy
                try:
                    result = await proxy.process({"text": f"resource test {proxy_id}-{j}"})
        ...
        for proxy_results in all_results:
            if isinstance(proxy_results, list):
                ...
                flattened_results.append({"success": False, "error": str(proxy_results)})
        
        # Analyze resource handling
        successful_count = sum(1 for r in flattened_results if r.get("success", False))
        fallback_count = sum(1 for r in flattened_results if r.get("source") == "mock_fallback")
        ...
        assert success_rate > 0.5, f"Success rate too low under resource pressure: {success_rate}"
        
        # Should complete within reasonable time
        total_time = end_time - start_time
        assert total_time < 60.0, f"Resource exhaustion test took too long: {total_time}s"
    ...
    async def test_network_partition_simulation(self, error_prone_coordinator):
        ...
        with patch.object(partition_proxy, 'ipa_adapter') as mock_adapter:
            ...
            mock_adapter.process_input.side_effect = AgentCommunicationError("Network partition")
