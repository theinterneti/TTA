...
class TestCompleteWorkflowIntegration:
    ...
    async def test_basic_workflow_execution(
        ...
        try:
            ...
            response, run_id, error = workflow_manager.execute_workflow("test_basic", request)
            ...
            assert hasattr(response, 'workflow_metadata'), "Response missing workflow_metadata"
            assert response.workflow_metadata.get('steps_executed') == 3, "Expected 3 steps executed"
            ...
            # Check Redis state
            redis_state = await integration_helper.redis_verifier.verify_workflow_coordination_state(run_id)
            assert redis_state['workflow_state_exists'], "Workflow state not persisted in Redis"
            
            # Check Neo4j state (if available)
            try:
                neo4j_state = await integration_helper.neo4j_verifier.verify_workflow_state_transitions(run_id)
    ...
    async def test_therapeutic_workflow_validation(
        ...
        test_env = await integration_helper.setup_test_environment("therapeutic_workflow")
        ...
        try:
            ...
            response, run_id, error = workflow_manager.execute_workflow("therapeutic_test", request)
    ...
    async def test_workflow_with_state_persistence(
        ...
        try:
            workflow_manager.register_workflow("state_test", multi_agent_workflow_definition)
            ...
            for i, player_input in enumerate(sample_player_inputs[:3]):
                ...
                response, run_id, error = workflow_manager.execute_workflow("state_test", request)
                ...
                # Check Redis state
                redis_state = await integration_helper.redis_verifier.verify_workflow_coordination_state(run_id)
                assert redis_state['workflow_state_exists'], f"State not persisted after step {i}"
                
                # Verify session state in Neo4j
                session_state = await integration_helper.neo4j_verifier.verify_session_state(
                    test_env['session_id']
                )
                assert session_state['session_exists'], f"Session not persisted after step {i}"
            
            # Verify final state consistency
            final_redis_state = await integration_helper.redis_verifier.verify_workflow_coordination_state(run_id)
            final_neo4j_state = await integration_helper.neo4j_verifier.verify_session_state(
                test_env['session_id']
            )
            
            # State consistency checks
            assert final_redis_state['workflow_state_exists'], "Final Redis state missing"
            assert final_neo4j_state['session_exists'], "Final Neo4j state missing"
            assert final_neo4j_state['therapeutic_profile_persisted'], "Therapeutic profile not persisted"
...
class TestMessageRoutingIntegration:
    ...
    async def test_message_routing_sequence(
        ...
        try:
            ...
            with patch('src.agent_orchestration.coordinators.redis_message_coordinator.RedisMessageCoordinator.send_message', 
                      side_effect=mock_send_message):
                
                workflow_manager = WorkflowManager()
                workflow_manager.register_workflow("routing_test", multi_agent_workflow_definition)
                ...
                response, run_id, error = workflow_manager.execute_workflow("routing_test", request)
                ...
                for msg in messages_sent:
                    if hasattr(msg, 'recipient_id') and hasattr(msg.recipient_id, 'type'):
                        routed_agents.append(msg.recipient_id.type)
                
                # Verify routing follows expected sequence
                verifier = WorkflowStateVerifier()
                routing_correct = verifier.verify_message_routing(messages_sent, expected_sequence)
                assert routing_correct, f"Message routing incorrect. Expected: {expected_sequence}, Got: {routed_agents}"
        ...
        try:
            ...
            class MockAgent:
                ...
                async def process(self, input_payload):
                    ...
                    transformed = {
                        ...
                        'priority': 'high' if 'grief' in str(input_payload) else 'normal'
                    }
            ...
            for msg in transformed_messages:
                if 'grief' in str(msg['original_input']):
                    assert msg['priority'] == 'high', "Therapeutic content should have high priority"
            
            # Verify message chain integrity
            assert transformed_messages[1]['original_input'] == transformed_messages[0], "Message chain broken"
            assert transformed_messages[2]['original_input'] == transformed_messages[1], "Message chain broken"
    ...
    async def test_message_delivery_confirmation(
        ...
        test_env = await integration_helper.setup_test_environment("delivery_confirmation")

        try:
            from src.agent_orchestration.coordinators.redis_message_coordinator import RedisMessageCoordinator
            from src.agent_orchestration import AgentMessage, MessageType, MessagePriority, AgentId
            ...
            queue_state = await integration_helper.redis_verifier.verify_message_queue_state(
            ...
            received_message = await coordinator.receive_message(wba_id, timeout_seconds=5.0)
            assert received_message is not None, "Message not received"
            assert received_message.correlation_id == "test_correlation_123", "Correlation ID mismatch"

            # Verify queue is empty after receipt
            post_receipt_state = await integration_helper.redis_verifier.verify_message_queue_state(
                f"{wba_id.type.value}:{wba_id.instance}"
            )
            assert post_receipt_state['queue_length'] == 0, "Queue not cleared after receipt"
