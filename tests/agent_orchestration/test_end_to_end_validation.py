...
from src.agent_orchestration.performance.response_time_monitor import (
    ResponseTimeMonitor, get_response_time_monitor
)
...
class TestEndToEndValidation:
    ...
    async def performance_monitor(self):
        ...
        # Register agents
        coordinator.register_agent("e2e_ipa", AgentType.INPUT_PROCESSOR, max_concurrent=5)
        coordinator.register_agent("e2e_wba", AgentType.WORLD_BUILDER, max_concurrent=3)
        coordinator.register_agent("e2e_nga", AgentType.NARRATIVE_GENERATOR, max_concurrent=3)
    ...
    async def test_crisis_intervention_workflow(
        ...
        print(f"Crisis intervention type: {crisis_response.get('intervention_type', 'unknown')}")
    ...
    async def test_session_context_persistence(
        ...
        psychology_context = any(word in story.lower() for word in ["psychology", "student", "sarah", "college"])
        
        # At minimum, the session should be consistent
        assert second_result["session_id"] == session_id
        
        print(f"Session context persistence validated across {len([first_result, second_result])} interactions")
    ...
    async def test_concurrent_workflow_handling(
        ...
        for i in range(num_concurrent):
            user_input = f"Hello, I'm user {i+1} and I need help with stress management."
        ...
        print(f"Concurrent workflow test: {num_concurrent} workflows completed in {total_time:.2f}s")
    ...
    async def test_error_handling_and_recovery(
        ...
        print(f"Error handling test completed for {len(malformed_inputs)} malformed inputs")
    ...
    async def test_real_time_event_integration(
        ...
        print(f"Agent events: {len(agent_events)}, Workflow events: {len(workflow_events)}")
    ...
    async def test_therapeutic_content_validation(
        ...
        print(f"Therapeutic content validation completed for {len(therapeutic_scenarios)} scenarios")
