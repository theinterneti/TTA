...
class TestRealAgentPerformance:
    ...
    async def measure_single_agent_latency(self, proxy, test_input: Dict[str, Any], num_requests: int = 100) -> PerformanceMetrics:
    ...
    async def test_ipa_latency_benchmark(self, performance_proxies):
        ...
        metrics = await self.measure_single_agent_latency(ipa_proxy, test_input, num_requests=50)
        
        # Performance assertions
        assert metrics.error_rate < 0.1, f"Error rate too high: {metrics.error_rate}"
        assert metrics.average_latency < 5000, f"Average latency too high: {metrics.average_latency}ms"
        assert metrics.throughput_rps > 1, f"Throughput too low: {metrics.throughput_rps} RPS"
    ...
    async def test_wba_latency_benchmark(self, performance_proxies):
        ...
        metrics = await self.measure_single_agent_latency(wba_proxy, test_input, num_requests=50)
        
        # Performance assertions
        assert metrics.error_rate < 0.1, f"Error rate too high: {metrics.error_rate}"
        assert metrics.average_latency < 8000, f"Average latency too high: {metrics.average_latency}ms"
        assert metrics.throughput_rps > 0.5, f"Throughput too low: {metrics.throughput_rps} RPS"
    ...
    async def test_nga_latency_benchmark(self, performance_proxies):
        ...
        test_input = {
            ...
            "context": {"session_id": "perf_test", "world_state": {"location": "temple"}}
        }
        
        metrics = await self.measure_single_agent_latency(nga_proxy, test_input, num_requests=30)
        ...
        assert metrics.average_latency < 15000, f"Average latency too high: {metrics.average_latency}ms"
        assert metrics.throughput_rps > 0.2, f"Throughput too low: {metrics.throughput_rps} RPS"
        ...
        for concurrency in concurrency_levels:
            async def concurrent_workflow(worker_id: int):
                ...
                nga_result = await performance_proxies["nga"].process({
                    ...
                    "context": {"session_id": session_id, "world_state": wba_result["world_state"]}
                })
            ...
            throughput_results[concurrency] = {
                ...
                "total_time": total_time,
                "workflows_per_second": len(successful_results) / total_time if total_time > 0 else 0,
                "average_workflow_time": statistics.mean([r["total_time"] for r in successful_results]) if successful_results else 0,
                "average_ipa_time": statistics.mean([r["ipa_time"] for r in successful_results]) if successful_results else 0,
                "average_wba_time": statistics.mean([r["wba_time"] for r in successful_results]) if successful_results else 0,
                "average_nga_time": statistics.mean([r["nga_time"] for r in successful_results]) if successful_results else 0
            }
        
        # Verify throughput scaling
        for concurrency, result in throughput_results.items():
            assert result["workflows_per_second"] > 0, f"No throughput at concurrency {concurrency}"
            assert result["successful_workflows"] == concurrency, f"Not all workflows succeeded at concurrency {concurrency}"
    ...
    async def test_sustained_load_benchmark(self, performance_proxies):
        ...
        assert actual_rps >= target_rps * 0.8, f"Actual RPS too low: {actual_rps} (target: {target_rps})"
        
        if latencies:
            avg_latency = statistics.mean(latencies)
            assert avg_latency < 10000, f"Average latency too high under sustained load: {avg_latency}ms"
    ...
    async def test_memory_usage_benchmark(self, performance_proxies):
        ...
        return {
            ...
            "memory_per_operation_kb": (total_memory_increase * 1024) / num_operations if num_operations > 0 else 0
        }
