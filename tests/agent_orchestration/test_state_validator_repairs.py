...
from src.agent_orchestration.models import AgentType, AgentId, AgentMessage, MessageType
...
async def test_state_validator_repairs_stale_reservations(redis_client):
    ...
    m = AgentMessage(message_id="msg-000001", sender=sender, recipient=recipient, message_type=MessageType.EVENT)
    ...
    await redis_client.zadd(f"ao:reserved_deadlines:{recipient.type.value}:{recipient.instance}", {rm.token: 1})
    ...
    ready_count = await redis_client.zcard(f"ao:sched:{recipient.type.value}:{recipient.instance}:prio:5")
