...
from src.agent_orchestration.tools.models import ToolSpec, ToolPolicy, ToolParameter
...
async def test_kg_enabled_tools_policy_network(redis_client, monkeypatch):
    ...
    spec = ToolSpec(name="kg.read", version="1.0.0", description="KG read", safety_flags=["network"])  # KG flag
    await reg.register_tool(spec)

    # Disallow network -> should block
    cfg = ToolPolicyConfig(allow_network_tools=False, callable_allowlist=[
    ...
    cfg2 = ToolPolicyConfig(allow_network_tools=True, callable_allowlist=cfg.callable_allowlist)
...
async def test_policy_enforcement_matrix(redis_client):
    ...
    cases = [
        ...
        (dict(allow_network_tools=False, allow_filesystem_tools=False), ["network", "filesystem"], True),
    ]

    for cfg_kwargs, flags, expect_block in cases:
        spec = ToolSpec(name=f"t.{'-'.join(flags)}", version="1.0.0", description="combo", safety_flags=flags)
        await reg.register_tool(spec)
        cfg = ToolPolicyConfig(callable_allowlist=[
