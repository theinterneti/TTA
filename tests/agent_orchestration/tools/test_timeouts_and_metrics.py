...
async def test_async_timeout_and_metrics(redis_client):
    ...
    cfg = ToolPolicyConfig(default_timeout_ms=50, callable_allowlist=[
    ...
    svc = ToolInvocationService(registry=reg, coordinator=ToolCoordinator(registry=reg, policy=policy), policy=policy)
