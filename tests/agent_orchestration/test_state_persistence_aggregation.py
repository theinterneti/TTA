...
class TestStatePersistenceIntegration:
    ...
    async def test_session_state_persistence_across_handoffs(
        self, integration_helper, therapeutic_session_context, sample_player_inputs
    ):
        ...
        try:
            ...
            with integration_helper.neo4j.session() as session:
                session.run(
                    ...
                    therapeutic_profile=json.dumps(
                        therapeutic_session_context.therapeutic_profile
                    ),
                    game_state=json.dumps(therapeutic_session_context.game_state),
                    location_id=therapeutic_session_context.game_state[
                        "current_location_id"
                    ],
                )
            ...
            for i, player_input in enumerate(sample_player_inputs[:3]):
                ...
                interaction_state = (
                    await integration_helper.neo4j_verifier.verify_agent_interactions(
                        test_env["session_id"]
                    )
                )
    ...
    async def test_workflow_state_transitions_persistence(
        self, integration_helper, multi_agent_workflow_definition, sample_player_inputs
    ):
        ...
        try:
            ...
            workflow_manager.register_workflow(
                "transitions_test", multi_agent_workflow_definition
            )
            ...
            workflow_state = await integration_helper.neo4j_verifier.verify_workflow_state_transitions(
                run_id
            )
            ...
            assert workflow_state["status"] in [
                "completed",
                "success",
            ], f"Unexpected workflow status: {workflow_state['status']}"
        ...
        try:
            ...
            for i, player_input in enumerate(sample_player_inputs[:3]):
                ...
                session_context = SessionContext(
                    ...
                    game_state={
                        **therapeutic_session_context.game_state,
                        "request_id": i,
                    },
                )
            ...
            for result in successful_results:
                await integration_helper.cleanup_test_data(
                    result["session_id"], result["run_id"], test_env["player_id"]
                )
