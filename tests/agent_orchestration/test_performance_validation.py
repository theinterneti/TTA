...
from src.agent_orchestration.performance.response_time_monitor import (
    ResponseTimeMonitor, get_response_time_monitor, OperationType
)
...
class TestPerformanceValidation:
    ...
    async def performance_infrastructure(self, redis_client):
        ...
        coordinator = IntelligentAgentCoordinator(
            response_time_monitor=monitor,
            target_response_time=2.0
        )
        
        # Register agents for optimization
        coordinator.register_agent("perf_ipa", AgentType.INPUT_PROCESSOR, max_concurrent=10)
        coordinator.register_agent("perf_wba", AgentType.WORLD_BUILDER, max_concurrent=8)
        coordinator.register_agent("perf_nga", AgentType.NARRATIVE_GENERATOR, max_concurrent=8)
    ...
    async def test_two_second_response_time_validation(
        ...
        service, monitor, coordinator, analytics, alerting = optimized_orchestration_service
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            print(f"{scenario['complexity'].capitalize()} query: {response_time:.2f}s (target: {scenario['target_time']}s)")
        ...
        assert max_response_time < 2.0, f"Maximum response time {max_response_time:.2f}s exceeds 2.0s SLA"
        assert p95_response_time < 2.0, f"P95 response time {p95_response_time:.2f}s exceeds 2.0s SLA"
        ...
        if OperationType.WORKFLOW_EXECUTION in stats:
            ...
            assert workflow_stats.meets_sla, "Workflow execution does not meet 2-second SLA"
            assert workflow_stats.success_rate >= 1.0, "Not all workflows completed successfully"
    ...
    async def test_performance_optimization_effectiveness(
        ...
        max_response_time = max(response_times)
        assert max_response_time < 2.5, f"Maximum response time {max_response_time:.2f}s exceeds acceptable limit"
        ...
        print(f"Optimization effectiveness test:")
    ...
    async def test_performance_under_load(
        ...
        assert max_individual_time < 3.0, f"Maximum individual response time {max_individual_time:.2f}s under load exceeds limit"
        assert avg_individual_time < 2.5, f"Average individual response time {avg_individual_time:.2f}s under load exceeds limit"
        ...
        assert concurrency_efficiency > 2.0, f"Concurrency efficiency {concurrency_efficiency:.1f}x is too low"
    ...
    async def test_performance_analytics_validation(
        ...
        test_scenarios = [
            ...
            ("Complex query", "I'm dealing with multiple mental health challenges including anxiety, depression, and relationship issues"),
        ...
        for scenario_name, test_input in test_scenarios:
            for i in range(3):  # 3 iterations per scenario
                session_id = f"analytics_session_{scenario_name.replace(' ', '_')}_{i+1}"
        ...
        assert overall_health in ["excellent", "good", "fair", "poor", "critical", "no_data"]
        
        # Should not have critical bottlenecks for test workload
        critical_bottlenecks = [
            b for b in analysis_results["bottlenecks"]
            if b.get("severity", 0) > 0.8
        ]
        assert len(critical_bottlenecks) == 0, f"Found critical bottlenecks: {critical_bottlenecks}"
        ...
        print(f"Performance analytics validation:")
    ...
    async def test_performance_alerting_validation(
        ...
        performance_alerts = [a for a in received_alerts if a.alert_type.value == "response_time_violation"]
        assert len(performance_alerts) == 0, f"Unexpected performance alerts: {performance_alerts}"
        ...
        print(f"Performance alerting validation:")
    ...
    async def test_end_to_end_performance_validation(
        ...
        assert total_response_time < 2.0, f"End-to-end response time {total_response_time:.2f}s exceeds 2.0s SLA"
        ...
        assert performance_summary["overall_performance"] in ["excellent", "good", "acceptable"]
        assert optimization_stats["system_load"] < 0.8  # System should not be overloaded
        
        print(f"End-to-end performance validation:")
        ...
        print(f"  Therapeutic content: {len(result.get('nga_result', {}).get('therapeutic_elements', []))} elements")
