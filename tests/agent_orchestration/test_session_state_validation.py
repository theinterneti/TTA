...
class TestSessionStateValidation:
    ...
    async def test_session_context_persistence(
        ...
        first_input = "Hi, I'm Alex, a 25-year-old software developer working at a startup."
        ...
        second_input = "I'm feeling burned out from working long hours. What should I do?"
        ...
        character_info_maintained = (
            "character_info" in second_world_state or
            "context" in second_world_state or
            any("alex" in str(v).lower() for v in second_world_state.values()) or
            any("developer" in str(v).lower() for v in second_world_state.values())
        )
        ...
        context_references = any(word in third_story for word in 
                               ["burnout", "work", "hours", "balance", "developer", "startup"])
        
        print(f"Session context persistence test:")
    ...
    async def test_multi_session_isolation(
        ...
        session_2_input = "I'm John, an engineer who enjoys hiking and outdoor activities."
        ...
        session_1_correct = any(word in session_1_story for word in ["maria", "teacher", "fantasy", "book"])
        session_1_isolated = not any(word in session_1_story for word in ["john", "engineer", "hiking"])
        
        # Session 2 should reference John/engineering/hiking, not Maria/teaching/fantasy
        session_2_correct = any(word in session_2_story for word in ["john", "engineer", "hiking", "trail"])
        session_2_isolated = not any(word in session_2_story for word in ["maria", "teacher", "fantasy"])
    ...
    async def test_world_state_persistence(
        ...
        print(f"World state persistence test:")
        ...
        print(f"  World state size progression: {world_elements_count_1} → {world_elements_count_2}")
    ...
    async def test_session_recovery_after_interruption(
        ...
        context_recovered = any(word in continue_story for word in 
                              ["sarah", "psychology", "student", "thesis", "research"])
        
        # Should provide relevant information about anxiety disorders
        relevant_response = any(word in continue_story for word in 
                              ["anxiety", "disorder", "types", "research"])
        
        print(f"Session recovery test:")
        print(f"  Context recovered: {context_recovered}")
        print(f"  Relevant response: {relevant_response}")
    
    async def test_state_consistency_across_agents(
        self,
        orchestration_service
    ):
        ...
        user_input = "I'm dealing with work stress and need help developing coping strategies."
        ...
        work_stress_identified = (
            any("work" in str(entity).lower() or "stress" in str(entity).lower() 
                for entity in ipa_entities) or
            any(word in ipa_intent.lower() for word in ["work", "stress", "coping"])
        )
        ...
        work_stress_in_world = any(word in world_state_text for word in 
                                 ["work", "stress", "coping", "job"])
        ...
        therapeutic_response = (
            any(word in narrative.lower() for word in 
                ["stress", "coping", "manage", "balance", "strategies"]) and
            len(therapeutic_elements) > 0
        )
        
        print(f"State consistency test:")
    ...
    async def test_long_session_memory_management(
        self,
        orchestration_service
    ):
        ...
        remembers_identity = any(word in final_story for word in 
                               ["emma", "graphic", "designer"])
        
        # Should remember the project context
        remembers_project = any(word in final_story for word in 
                              ["project", "brand", "client", "startup"])
        
        # Should address the current concern (creativity under pressure)
        addresses_concern = any(word in final_story for word in 
                              ["creativity", "pressure", "deadline", "quality"])
        ...
        print(f"Long session memory management test:")
