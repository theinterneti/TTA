...
class TestPlayerExperienceOrchestrationIntegration:
    ...
    def test_component_configuration_access(self, player_experience_component):
        ...
        assert player_experience_component.config.get("player_experience.api.port", 8080) == 8080
        assert player_experience_component.config.get("player_experience.web.port", 3000) == 3000
        assert player_experience_component.config.get("player_experience.therapeutic.default_intensity") == "medium"
    
    @patch('src.components.player_experience_component.PlayerExperienceComponent._run_docker_compose')
    @patch('src.components.player_experience_component.PlayerExperienceComponent._is_api_running')
    def test_component_start_success(self, mock_is_api_running, mock_run_docker_compose, player_experience_component):
    ...
    @patch('requests.get')
    def test_component_start_failure(self, mock_requests_get, mock_subprocess_run, player_experience_component):
    ...
    @patch('src.components.player_experience_component.PlayerExperienceComponent._run_docker_compose')
    @patch('src.components.player_experience_component.PlayerExperienceComponent._is_api_running')
    def test_component_stop_success(self, mock_is_api_running, mock_run_docker_compose, player_experience_component):
    ...
    @patch('requests.get')
    def test_health_status_monitoring(self, mock_requests_get, player_experience_component):
    ...
    @patch('subprocess.run')
    def test_docker_compose_command_execution(self, mock_subprocess_run, player_experience_component):
    ...
    @patch('subprocess.run')
    def test_container_status_checking(self, mock_subprocess_run, player_experience_component):
        """Test container status checking functionality."""
        # Mock Docker ps command output
        mock_result = Mock()
        mock_result.returncode = 0
        mock_result.stdout = "player-experience-api,Up 5 minutes,player-experience:latest"
        mock_result.stderr = ""
        mock_subprocess_run.return_value = mock_result
...
class TestPlayerExperienceDeploymentValidation:
    ...
    @patch('redis.Redis')
    @patch('requests.get')
    def test_dependency_health_checking(self, mock_requests_get, mock_redis, mock_config):
    ...
    @patch('redis.Redis')
    @patch('requests.get')
    def test_dependency_health_checking_failure(self, mock_requests_get, mock_redis, mock_config):
        ...
        mock_requests_get.side_effect = requests.exceptions.ConnectionError("Neo4j connection failed")
...
"""
    ...
    @patch('src.orchestration.orchestrator.TTAOrchestrator._validate_repositories')
    @patch('src.orchestration.orchestrator.TTAOrchestrator._import_repository_components')
    def test_full_orchestration_lifecycle(self, mock_import_repo, mock_validate, test_config_path):
    ...
    @patch('src.components.player_experience_component.PlayerExperienceComponent._run_docker_compose')
    @patch('src.components.player_experience_component.PlayerExperienceComponent._is_api_running')
    def test_orchestrated_component_startup(self, mock_is_api_running, mock_run_docker_compose,
                                          mock_import_repo, mock_validate, test_config_path):
