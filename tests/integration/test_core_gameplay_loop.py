...
from src.components.gameplay_loop.models.core import EmotionalState, ChoiceType
...
class TestCoreGameplayLoopIntegration:
    ...
    async def test_complete_gameplay_session_flow(self, gameplay_controller):
        ...
        while choice_count < max_choices and session.available_choices:
            ...
            assert processing_time < 2.0, f"Processing took {processing_time:.2f}s (target: <2.0s)"
            ...
            assert consequences.therapeutic_value_realized > 0, "Should have therapeutic value"
            assert len(consequences.therapeutic_insights) > 0, "Should have therapeutic insights"
            assert len(consequences.learning_opportunities) > 0, "Should have learning opportunities"
    ...
    async def test_therapeutic_progression_validation(self, gameplay_controller):
        ...
        assert all(value > 0 for value in therapeutic_values), "All choices should have therapeutic value"
        ...
        if len(therapeutic_values) > 1:
            ...
            assert avg_therapeutic_value > 0.3, "Should maintain meaningful therapeutic engagement"
    ...
    async def test_emotional_state_adaptation(self, gameplay_controller):
        ...
        emotional_states = [EmotionalState.CALM, EmotionalState.ANXIOUS, EmotionalState.OVERWHELMED]
        ...
        for emotional_state in emotional_states:
            ...
            if session.available_choices:
                ...
                if emotional_state == EmotionalState.ANXIOUS:
                    ...
                    assert any(word in insights_text for word in ["calm", "safe", "gentle", "peace"])
                elif emotional_state == EmotionalState.OVERWHELMED:
                    # Should contain simplifying or supportive language
                    assert any(word in insights_text for word in ["simple", "step", "support", "manageable"])
