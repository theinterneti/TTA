...
class ServicesAPITestClient:
    ...
    def create_auth_token(self, user_id: str = "test_admin", username: str = "admin") -> str:
...
class TestServiceManagementEndpoints:
    ...
    def test_reconnect_all_services_with_auth(self, api_client):
        ...
        with patch('src.player_experience.api.services.connection_manager.ServiceConnectionManager') as mock_manager_class:
            ...
            mock_manager.health_check.return_value = {
                ...
                "using_mocks": True,
                "services": {"neo4j": {"status": "connected"}, "redis": {"status": "connected"}}
            }
            mock_manager_class.return_value = mock_manager
            
            response = api_client.client.post("/api/v1/services/reconnect", headers=headers)
            assert response.status_code == 200
    ...
    def test_reconnect_individual_service_with_auth(self, api_client):
        ...
        with patch('src.player_experience.api.services.connection_manager.get_service_manager') as mock_get_manager:
            mock_manager = AsyncMock()
            ...
            response = api_client.client.post("/api/v1/services/neo4j/reconnect", headers=headers)
            assert response.status_code == 200
    ...
    def test_reconnect_invalid_service(self, api_client):
        ...
        response = api_client.client.post("/api/v1/services/invalid/reconnect", headers=headers)
    ...
    def test_reconnect_service_failure(self, api_client):
        ...
        with patch('src.player_experience.api.services.connection_manager.get_service_manager') as mock_get_manager:
            ...
            response = api_client.client.post("/api/v1/services/redis/reconnect", headers=headers)
            assert response.status_code == 200
...
class TestServiceEndpointIntegration:
    ...
    def test_service_health_data_consistency(self, api_client):
        ...
        for response in responses[1:]:
            ...
            assert set(response["services"].keys()) == set(first_response["services"].keys())
...
class TestServiceEndpointSecurity:
    ...
    def test_service_management_requires_valid_token(self, api_client):
        ...
        response = api_client.client.post("/api/v1/services/services/reconnect", headers=invalid_headers)
        assert response.status_code == 401
    ...
    def test_service_config_requires_valid_token(self, api_client):
        ...
        response = api_client.client.get("/api/v1/services/config", headers=malformed_headers)
        assert response.status_code == 401
