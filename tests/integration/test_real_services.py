...
from src.player_experience.api.config import (
    ProductionSettings,
    TestingSettings,
)
...
class TestNeo4jConnectionManager:
    ...
    async def test_connection_success_scenario(self, test_settings):
        ...
        with patch(
            "src.player_experience.api.services.connection_manager.AsyncGraphDatabase"
        ) as mock_graph:
...
class TestRedisConnectionManager:
    ...
    async def test_connection_success_scenario(self, test_settings):
        ...
        with patch(
            "src.player_experience.api.services.connection_manager.redis"
        ) as mock_redis:
...
class TestServiceConnectionManager:
    ...
    @pytest.mark.asyncio
    async def test_service_manager_initialization_with_real_services(
        self, test_settings
    ):
    ...
    async def test_service_initialization_success(self, test_settings):
        ...
        with (
            patch.object(manager.neo4j, "connect", return_value=True)
            if hasattr(manager.neo4j, "connect")
            else patch("builtins.hasattr", return_value=False)
        ):
            with (
                patch.object(manager.redis, "connect", return_value=True)
                if hasattr(manager.redis, "connect")
                else patch("builtins.hasattr", return_value=False)
            ):
