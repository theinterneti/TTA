...
Examples:
  ...
  python run_comprehensive_tests.py --all --max-concurrent 10
    ...
    test_group.add_argument(
        "--categories",
        nargs="+",
        choices=["standard", "adversarial", "load_stress", "data_pipeline", "dashboard"],
    ...
    parser.add_argument(
        "--metrics",
        action="store_true",
        help="Collect system metrics during testing"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
async def main():
    ...
    try:
        ...
        if args.all:
            categories = ["standard", "adversarial", "load_stress", "data_pipeline", "dashboard"]
        else:
