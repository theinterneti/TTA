...
from ..comprehensive_test_battery import TestResult, TestCategory, TestStatus
...
class TestReportGenerator:
    ...
    def __init__(self, results_directory: str = "./testing/results/comprehensive_battery"):
    ...
    async def generate_comprehensive_report(
        ...
        try:
            ...
            # Executive summary
            summary_path = await self._generate_executive_summary(report_data, timestamp)
    ...
    def _group_results_by_category(self, results: List[TestResult]) -> Dict[str, Dict[str, Any]]:
        ...
        for category in TestCategory:
            ...
            if category_results:
                ...
                avg_duration = sum(r.duration_seconds for r in category_results) / len(category_results)
    ...
    def _analyze_performance_metrics(
        ...
        if metrics_data:
            ...
            cpu_usage = metrics_data.get("system_resource_summary", {}).get("cpu_usage", {})
            memory_usage = metrics_data.get("system_resource_summary", {}).get("memory_usage", {})
            
            if cpu_usage.get("max", 0) > 80:
                analysis["performance_issues"].append("High CPU usage detected (>80%)")
            
            if memory_usage.get("max", 0) > 80:
                analysis["performance_issues"].append("High memory usage detected (>80%)")
    ...
    def _identify_issues(self, results: List[TestResult]) -> List[Dict[str, Any]]:
        ...
        for result in failed_results:
            issues.append({
                "type": "test_failure",
                "severity": "high" if result.category in [TestCategory.ADVERSARIAL, TestCategory.DATA_PIPELINE] else "medium",
                "test_name": result.test_name,
                "category": result.category.value,
    ...
    def _generate_recommendations(
        ...
        adversarial_failures = [r for r in results if r.category == TestCategory.ADVERSARIAL and not r.passed]
        ...
        pipeline_failures = [r for r in results if r.category == TestCategory.DATA_PIPELINE and not r.passed]
        ...
        total_success_rate = (len([r for r in results if r.passed]) / len(results)) * 100 if results else 0
    ...
    def _serialize_config(self, config: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "load_test_duration_minutes": getattr(config, 'load_test_duration_minutes', None),
    ...
    async def _generate_csv_report(self, results: List[TestResult], timestamp: str) -> str:
        ...
        with open(csv_path, 'w', newline='') as f:
            ...
            # Write header
            writer.writerow([
                'Test Name', 'Category', 'Status', 'Passed', 'Duration (s)', 
                'Error Message', 'Start Time', 'End Time'
            ])
    ...
    async def _generate_executive_summary(self, report_data: Dict[str, Any], timestamp: str) -> str:
