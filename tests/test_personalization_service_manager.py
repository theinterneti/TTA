...
from src.player_experience.managers.personalization_service_manager import (
    PersonalizationServiceManager, PlayerFeedback, AdaptationResult, 
    CrisisSupportResource, CrisisDetectionSystem
)
from src.player_experience.models.therapeutic_settings import (
    EnhancedTherapeuticSettings, TherapeuticBoundary
)
from src.player_experience.models.enums import IntensityLevel, TherapeuticApproach, CrisisType
from src.player_experience.models.session import Recommendation
...
class TestCrisisDetectionSystem(unittest.TestCase):
    ...
    def test_crisis_detection_suicidal_ideation(self):
        ...
        crisis_detected, crisis_types, confidence = self.crisis_detector.detect_crisis_indicators(text, context)
    ...
    def test_get_crisis_resources_emergency_only(self):
        ...
        emergency_resources = self.crisis_detector.get_crisis_resources(crisis_types, emergency_only=True)
        all_resources = self.crisis_detector.get_crisis_resources(crisis_types, emergency_only=False)
...
class TestPersonalizationServiceManager(unittest.TestCase):
    ...
    def test_update_therapeutic_settings_with_conflicts(self):
        ...
        conflicted_settings = EnhancedTherapeuticSettings(
            ...
            preferred_approaches=[TherapeuticApproach.MINDFULNESS],  # Potential mismatch
        ...
        self.assertTrue(success)
        self.assertGreaterEqual(len(conflicts), 0)  # May have remaining non-critical conflicts
    ...
    def test_get_adaptive_recommendations_with_context(self):
        ...
        stress_recommendations = [r for r in recommendations if "stress" in r.description.lower()]
    ...
    def test_process_crisis_feedback(self):
        ...
        self.assertIsInstance(result, AdaptationResult)
        self.assertEqual(result.player_id, self.test_player_id)
        self.assertFalse(result.requires_player_approval)  # Crisis adaptations are automatic
    ...
    def test_detect_crisis_situation(self):
        ...
        crisis_detected, crisis_types, emergency_resources = self.manager.detect_crisis_situation(
            self.test_player_id, text, context
        )
        ...
        crisis_feedback = [f for f in player_feedback if f.feedback_type == "crisis_indicator"]
    ...
    def test_adaptation_history_storage(self):
        ...
        player_adaptations = self.manager.adaptation_history.get(self.test_player_id, [])
    ...
    def test_recommendations_with_feedback_history(self):
        ...
        adjustment_recommendations = [r for r in recommendations if "adjust" in r.title.lower()]
