...
from src.player_experience.models.session import (
    SessionContext, TherapeuticSettings, ProgressMarker
)
...
class TestSessionIntegrationManagerCore(unittest.TestCase):
    ...
    def test_session_state_preservation(self):
        ...
        async def run_test():
            ...
            self.mock_narrative_engine.get_session_state.assert_called_once_with(session.session_id)
            
            # Verify session variables contain narrative state
            self.assertIn('narrative_state', session.session_variables)
            self.assertEqual(session.session_variables['narrative_state']['narrative_position'], 10)
    ...
    def test_session_interaction_updates(self):
        ...
        async def run_test():
            ...
            self.assertIn('mindfulness_exercise', session.therapeutic_interventions_used)
    ...
    def test_therapeutic_settings_update(self):
        ...
        async def run_test():
            ...
            self.assertEqual(session.therapeutic_settings.intervention_frequency, "frequent")
