...
class TestEnhancedTherapeuticSettings(unittest.TestCase):
    ...
    def test_add_preferred_approach(self):
        ...
        self.assertIn(TherapeuticApproach.NARRATIVE_THERAPY, settings.preferred_approaches)
    ...
    def test_remove_preferred_approach(self):
        ...
        removed = settings.remove_preferred_approach(TherapeuticApproach.NARRATIVE_THERAPY)
        self.assertFalse(removed)
    ...
    def test_create_new_version(self):
        ...
        new_settings = settings.create_new_version(
            change_summary="Updated intensity level",
            created_by=self.test_player_id
        )
        
        self.assertEqual(new_settings.version.version_number, original_version + 1)
        self.assertEqual(new_settings.version.previous_version_id, settings.version.version_id)
        self.assertEqual(new_settings.version.created_by, self.test_player_id)
        self.assertEqual(new_settings.settings_id, settings.settings_id)  # Same settings ID
...
class TestTherapeuticPreferencesValidator(unittest.TestCase):
    ...
    def test_intensity_approach_mismatch(self):
        ...
        settings = EnhancedTherapeuticSettings(
            ...
            preferred_approaches=[TherapeuticApproach.MINDFULNESS]  # Gentle approach with high intensity
        )
        ...
        mismatch_conflicts = [c for c in conflicts if c.conflict_type == SettingsConflictType.INTENSITY_APPROACH_MISMATCH]
    ...
    def test_contradictory_preferences(self):
        ...
        contradictory_conflicts = [c for c in conflicts if c.conflict_type == SettingsConflictType.CONTRADICTORY_PREFERENCES]
    ...
    def test_unsafe_combinations(self):
        ...
        unsafe_conflicts = [c for c in conflicts if c.conflict_type == SettingsConflictType.UNSAFE_COMBINATION]
    ...
    def test_resolve_conflict(self):
        ...
        unsafe_conflict = next(c for c in conflicts if c.conflict_type == SettingsConflictType.UNSAFE_COMBINATION)
        
        # Resolve the conflict
        resolution_option = unsafe_conflict.resolution_options[0]
        resolved_settings = self.validator.resolve_conflict(settings, unsafe_conflict, resolution_option)
        
        # Check that the conflict is resolved
        self.assertTrue(resolved_settings.crisis_monitoring_enabled)
        self.assertGreater(resolved_settings.version.version_number, settings.version.version_number)
...
class TestSettingsMigrationManager(unittest.TestCase):
    ...
    def test_migrate_to_v1_from_basic_settings(self):
        ...
        migrated_settings = self.migration_manager.migrate_settings(old_settings, target_version=1)
        ...
        self.assertEqual(migrated_settings.intensity_level, IntensityLevel.HIGH)  # 0.7 -> HIGH
        self.assertEqual(migrated_settings.preferred_approaches, [TherapeuticApproach.CBT, TherapeuticApproach.MINDFULNESS])
        ...
        self.assertEqual(migrated_settings.trigger_warnings, ["loud_noises", "conflict"])
    ...
    def test_migrate_float_intensity_conversion(self):
        ...
        migrated_low = self.migration_manager.migrate_settings(old_settings_low, target_version=1)
        ...
        migrated_medium = self.migration_manager.migrate_settings(old_settings_medium, target_version=1)
        ...
        migrated_high = self.migration_manager.migrate_settings(old_settings_high, target_version=1)
    ...
    def test_migrate_invalid_approaches_filtering(self):
        ...
        self.assertIn(TherapeuticApproach.MINDFULNESS, migrated_settings.preferred_approaches)
