...
@pytest.mark.asyncio
async def test_comprehensive_framework_integration(mock_service_manager, comprehensive_test_config):
    ...
    neo4j_driver = await mock_service_manager.get_neo4j_driver(
        ...
        auth=(comprehensive_test_config.neo4j_user, comprehensive_test_config.neo4j_password)
    )
    
    async with neo4j_driver.session() as session:
        result = await session.run("CREATE (n:TestNode {name: $name}) RETURN n", name="integration_test")
        record = await result.single()
        assert record is not None
    
    # Test Redis integration
    redis_client = await mock_service_manager.get_redis_client(comprehensive_test_config.redis_url)
...
@pytest.mark.asyncio
async def test_mixed_service_environment(mock_service_manager, comprehensive_test_config):
    ...
    await redis_client.hset(f"session:{user_id}", mapping={
    ...
    assert stored_session[b"user_id"] == user_id.encode() or stored_session["user_id"] == user_id
...
async def test_comprehensive_test_data_generation():
    ...
    try:
        from tests.comprehensive_battery.utils.test_data_generator import TestDataGenerator
...
async def test_mock_only_functionality(mock_service_manager):
    ...
    try:
        ...
        redis_client = await mock_service_manager.get_redis_client("redis://localhost:6379")
