...
from src.player_experience.managers.therapeutic_profile_integration import (
    TherapeuticProfileIntegrationService, PersonalizationContext,
    TherapeuticAdaptation, PersonalizationRecommendation
)
...
class TestTherapeuticProfileIntegrationService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_background = CharacterBackground(
            ...
            relationships={"family": "close-knit family", "professional": "supportive colleagues"}
        )
    ...
    def test_create_therapeutic_profile_from_character(self):
        ...
        enhanced_profile = self.service.create_therapeutic_profile_from_character(self.test_character)
        
        # Should have original concerns plus extracted ones
        self.assertIn("work stress", enhanced_profile.primary_concerns)
        self.assertIn("anxiety", enhanced_profile.primary_concerns)  # From anxious trait
        self.assertIn("perfectionism", enhanced_profile.primary_concerns)  # From perfectionist trait
        ...
        self.assertIn("learning and growth", enhanced_profile.comfort_zones)  # From education value
        
        # Should have challenge areas
        self.assertIn("perfectionism", enhanced_profile.challenge_areas)
        self.assertIn("stress management", enhanced_profile.challenge_areas)  # From anxious trait
        
        # Should have coping strategies
        self.assertIn("social support", enhanced_profile.coping_strategies)  # From empathetic trait
        self.assertIn("planning", enhanced_profile.coping_strategies)  # From organized trait
        ...
        goal_descriptions = [goal.description for goal in enhanced_profile.therapeutic_goals]
    ...
    def test_create_personalization_context(self):
        ...
        self.assertEqual(context.personality_traits, self.test_character.background.personality_traits)
        self.assertEqual(context.core_values, self.test_character.background.core_values)
        self.assertEqual(context.preferred_intensity, self.test_character.therapeutic_profile.preferred_intensity)
        self.assertEqual(context.readiness_level, self.test_character.therapeutic_profile.readiness_level)
        
        # Should extract life experiences
        self.assertGreater(len(context.life_experiences), 0)
        self.assertTrue(any("educational" in exp.lower() for exp in context.life_experiences))
        
        # Should be stored in service
        self.assertIn(self.test_character.character_id, self.service.personalization_contexts)
    
    def test_generate_personalization_recommendations(self):
        """Test generating personalization recommendations."""
        recommendations = self.service.generate_personalization_recommendations(self.test_character)
        ...
        cbt_recs = [rec for rec in recommendations if "CBT" in rec.title or "Cognitive" in rec.title]
    ...
    def test_adapt_therapeutic_content(self):
        ...
        original_content = "Let's work on managing your stress levels, [CHARACTER_NAME]."
    ...
    def test_adapt_content_for_anxious_character(self):
        ...
        anxious_background = CharacterBackground(
            name="Bob Smith",
            personality_traits=["anxious", "worried", "sensitive"]
        )
    ...
    def test_adapt_content_for_high_intensity_character(self):
        ...
        high_intensity_profile = TherapeuticProfile(preferred_intensity=IntensityLevel.HIGH)
        ...
        adaptation = self.service.adapt_therapeutic_content(high_intensity_character, content)
    ...
    def test_get_character_adaptations(self):
        ...
        adaptations = self.service.get_character_adaptations(self.test_character.character_id)
    ...
    def test_get_character_recommendations(self):
        ...
        recommendations = self.service.get_character_recommendations(self.test_character.character_id)
    ...
    def test_update_adaptation_effectiveness(self):
        ...
        # Create adaptation
        adaptation = self.service.adapt_therapeutic_content(self.test_character, "Test content")
        original_score = adaptation.effectiveness_score
        
        # Update effectiveness
        new_score = 0.9
        result = self.service.update_adaptation_effectiveness(adaptation.adaptation_id, new_score)
        ...
        # Test score clamping
        self.service.update_adaptation_effectiveness(adaptation.adaptation_id, 1.5)  # Too high
        self.assertEqual(adaptation.effectiveness_score, 1.0)
        
        self.service.update_adaptation_effectiveness(adaptation.adaptation_id, -0.5)  # Too low
    ...
    def test_extract_therapeutic_insights(self):
        ...
        self.assertIn("learning and growth", insights["comfort_zones"])  # From education value
    ...
    def test_generate_therapeutic_goals(self):
        ...
        anxiety_goals = [goal for goal in goals if "anxiety" in goal.description.lower()]
    ...
    def test_calculate_readiness_adjustment(self):
        ...
        adjustment = self.service._calculate_readiness_adjustment(challenging_background)
    ...
    def test_recommend_therapeutic_approaches(self):
        ...
        recommendations = self.service._recommend_therapeutic_approaches(self.test_character)
        ...
        creative_recs = self.service._recommend_therapeutic_approaches(creative_character)
    ...
    def test_recommend_intensity_adjustments(self):
        ...
        recommendations = self.service._recommend_intensity_adjustments(low_readiness_character)
        low_intensity_recs = [rec for rec in recommendations if "Low Intensity" in rec.title]
        ...
        recommendations = self.service._recommend_intensity_adjustments(high_readiness_character)
        high_intensity_recs = [rec for rec in recommendations if "Higher Intensity" in rec.title]
