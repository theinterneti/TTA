...
from src.components.gameplay_loop.narrative.emotional_safety_system import (
    EmotionalSafetySystem, EmotionalStateSnapshot, EmotionalTrigger, SafetyIntervention,
    EmotionalState, DistressLevel, InterventionType, TriggerCategory
)
...
class TestEmotionalSafetySystem:
    ...
    @pytest.mark.asyncio
    async def test_monitor_emotional_state_normal(self, emotional_safety_system, session_state):
        ...
        snapshot = await emotional_safety_system.monitor_emotional_state(session_state, interaction_data)
        ...
        assert len(emotional_safety_system.emotional_snapshots[session_state.user_id]) == 1
    
    @pytest.mark.asyncio
    async def test_monitor_emotional_state_distressed(self, emotional_safety_system, distressed_session_state, event_bus):
        ...
        snapshot = await emotional_safety_system.monitor_emotional_state(distressed_session_state, interaction_data)
        
        # Check high distress level
        assert snapshot.distress_level >= DistressLevel.HIGH
        
        # Check that intervention was triggered
        assert "immediate_support_provided" in distressed_session_state.context or \
               "crisis_intervention_active" in distressed_session_state.context
    ...
    def test_detect_trigger_indicators(self, emotional_safety_system):
        ...
        interaction_data = {"content": "The violence in that scene reminded me of my trauma"}
        indicators = emotional_safety_system._detect_trigger_indicators(interaction_data)
        assert "potential_trauma_trigger" in indicators
        
        # Test anxiety-related content
        interaction_data = {"content": "I'm feeling really scared and panicked about this"}
        indicators = emotional_safety_system._detect_trigger_indicators(interaction_data)
    ...
    def test_identify_protective_factors(self, emotional_safety_system, session_state):
        ...
        session_state.context = {"coping_skills_used": True, "support_system_active": True}
    ...
    async def test_detect_triggers_in_content(self, emotional_safety_system):
        ...
        trauma_triggers = [t for t in triggers if t.category == TriggerCategory.TRAUMA_RELATED]
        ...
        anxiety_triggers = [t for t in triggers if t.category == TriggerCategory.ANXIETY_INDUCING]
        ...
        depression_triggers = [t for t in triggers if t.category == TriggerCategory.DEPRESSION_TRIGGERING]
    ...
    async def test_select_interventions(self, emotional_safety_system):
        ...
        if len(interventions) > 1:
            assert interventions[0].estimated_effectiveness >= interventions[1].estimated_effectiveness
    
    @pytest.mark.asyncio
    async def test_crisis_protocol_activation(self, emotional_safety_system, distressed_session_state):
        """Test crisis protocol activation."""
        snapshot = EmotionalStateSnapshot(
            user_id=distressed_session_state.user_id,
            session_id=distressed_session_state.session_id,
            primary_emotions={EmotionalState.DEPRESSED: 0.9, EmotionalState.OVERWHELMED: 1.0},
            distress_level=DistressLevel.CRITICAL
        )
        
        await emotional_safety_system._activate_crisis_protocol(distressed_session_state, snapshot)
    ...
    @pytest.mark.asyncio
    async def test_provide_immediate_support(self, emotional_safety_system, session_state):
        ...
        interventions = await emotional_safety_system._select_interventions(snapshot)
        await emotional_safety_system._provide_immediate_support(session_state, snapshot, interventions)
    ...
    async def test_offer_coping_support(self, emotional_safety_system, session_state):
        ...
        interventions = await emotional_safety_system._select_interventions(snapshot)
        await emotional_safety_system._offer_coping_support(session_state, snapshot, interventions)
    ...
    @pytest.mark.asyncio
    async def test_emotional_regulation_support(self, emotional_safety_system, session_state):
        """Test emotional regulation support provision."""
        support_response = await emotional_safety_system.provide_emotional_regulation_support(
