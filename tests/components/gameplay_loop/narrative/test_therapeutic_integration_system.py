...
from src.components.gameplay_loop.narrative.therapeutic_integration_system import (
    TherapeuticIntegrationSystem, TherapeuticConcept, TherapeuticProgress, ResistancePattern,
    TherapeuticIntegration, TherapeuticApproach, IntegrationStrategy, ProgressMilestone, ResistanceType
)
...
class TestTherapeuticIntegrationSystem:
    ...
    def mindfulness_session_state(self):
        ...
        state.context = {
            ...
            "therapeutic_preferences": {
                "learning_style": "mindful"
            }
        }
        return state
    
    @pytest.mark.asyncio
    async def test_integrate_therapeutic_concept_experiential(self, integration_system, session_state, event_bus):
        ...
        assert any("experience" in opp.lower() for opp in integration.practice_opportunities)
    ...
    @pytest.mark.asyncio
    async def test_integrate_therapeutic_concept_metaphorical(self, integration_system, session_state, event_bus):
        ...
        assert any(word in integration.narrative_embedding.lower() 
                  for word in ["detective", "scientist", "judge"])
        
        # Check practice opportunities include metaphorical exploration
        assert any("metaphorical" in opp.lower() for opp in integration.practice_opportunities)
    
    @pytest.mark.asyncio
    async def test_track_therapeutic_progress_new_concept(self, integration_system, session_state, event_bus):
    ...
    @pytest.mark.asyncio
    async def test_track_therapeutic_progress_milestone_achievement(self, integration_system, session_state, event_bus):
        ...
        assert ProgressMilestone.SKILL_ACQUISITION in updated_progress.milestones_achieved
    ...
    @pytest.mark.asyncio
    async def test_detect_therapeutic_resistance_low_engagement(self, integration_system, session_state, event_bus):
    ...
    @pytest.mark.asyncio
    async def test_detect_therapeutic_resistance_insufficient_indicators(self, integration_system, session_state):
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_concept_from_template(self, integration_system, cbt_session_state):
        """Test concept creation from template."""
        concept_id = "thought_challenging"
        
        concept = await integration_system._get_or_create_concept(concept_id, cbt_session_state)
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_concept_generic(self, integration_system, session_state):
        """Test generic concept creation when no template matches."""
        concept_id = "custom_therapeutic_concept"
        
        concept = await integration_system._get_or_create_concept(concept_id, session_state)
    ...
    def test_get_therapeutic_progress_summary(self, integration_system, session_state):
        ...
        progress_list = []
        for i, concept in enumerate(["anxiety_management", "emotional_regulation", "thought_challenging"]):
            progress = TherapeuticProgress(
                ...
                total_attempts=i + 1,
                milestones_achieved=[ProgressMilestone.SKILL_ACQUISITION] if i > 0 else [],
    ...
    def test_get_therapeutic_progress_summary_no_data(self, integration_system):
        ...
        summary = integration_system.get_therapeutic_progress_summary("nonexistent_user")
