...
from src.components.gameplay_loop.narrative.consequence_system import (
    ConsequenceSystem, ConsequenceGenerationContext, ConsequenceFraming,
    ConsequenceSeverity, LearningOpportunityType, ConsequencePattern,
    CausalityExplanation
)
from src.components.gameplay_loop.models.core import (
    UserChoice, ChoiceType, ConsequenceType, TherapeuticOutcome
)
...
class TestConsequenceSystem:
    ...
    def session_state(self):
        ...
        state.progress_metrics = {"anxiety_management": 0.3, "communication_skills": 0.5}
    ...
    @pytest.mark.asyncio
    async def test_generate_consequences_positive_choice(self, consequence_system, session_state, positive_choice):
        ...
        assert consequence_set.consequence_type in [ConsequenceType.IMMEDIATE, ConsequenceType.THERAPEUTIC]
    ...
    @pytest.mark.asyncio
    async def test_generate_consequences_challenging_choice(self, consequence_system, session_state, challenging_choice):
        ...
        for consequence in consequence_set.consequences:
            ...
            assert any(word in frame.lower() for word in ["learn", "opportunity", "experience", "growth"])
    
    @pytest.mark.asyncio
    async def test_therapeutic_outcome_generation(self, consequence_system, session_state, positive_choice):
    ...
    @pytest.mark.asyncio
    async def test_learning_opportunity_creation(self, consequence_system, session_state, positive_choice):
        ...
        for opportunity in consequence_set.learning_opportunities:
            ...
            assert any(word in opportunity.lower() for word in ["practice", "opportunity", "chance", "learn"])
    
    @pytest.mark.asyncio
    async def test_consequence_framing_approaches(self, consequence_system, session_state):
        ...
        for consequence in consequence_set.consequences:
            frame = consequence["therapeutic_frame"]
            assert any(word in frame.lower() for word in ["positive", "well done", "skills", "growing"])
    
    @pytest.mark.asyncio
    async def test_reflection_prompt_generation(self, consequence_system, session_state, positive_choice):
    ...
    @pytest.mark.asyncio
    async def test_choice_pattern_tracking(self, consequence_system, session_state, positive_choice):
    ...
    @pytest.mark.asyncio
    async def test_causality_explanation(self, consequence_system, session_state, positive_choice):
    ...
    async def test_template_selection(self, consequence_system, session_state):
        ...
        comm_related = any(
            "communication" in outcome.outcome_type.lower() or "expression" in outcome.outcome_type.lower()
            for outcome in consequence_set.therapeutic_outcomes
        )
        assert comm_related
    
    @pytest.mark.asyncio
    async def test_therapeutic_framework_selection(self, consequence_system, session_state):
        ...
        anxiety_related = any(
            "anxiety" in opportunity.lower() or "stress" in opportunity.lower() or 
            "thoughts" in opportunity.lower() or "feelings" in opportunity.lower()
            for opportunity in consequence_set.learning_opportunities
        )
        assert anxiety_related
    
    @pytest.mark.asyncio
    async def test_consequence_severity_determination(self, consequence_system, session_state):
        ...
        for consequence in consequence_set.consequences:
            ...
            assert severity in [ConsequenceSeverity.MODERATE.value, ConsequenceSeverity.SIGNIFICANT.value]
    ...
    @pytest.mark.asyncio
    async def test_event_publishing(self, consequence_system, session_state, positive_choice, event_bus):
