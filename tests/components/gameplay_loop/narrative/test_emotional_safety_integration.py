...
from src.components.gameplay_loop.narrative.emotional_safety_system import (
    EmotionalSafetySystem, EmotionalState, DistressLevel
)
from src.components.gameplay_loop.models.core import UserChoice, ChoiceType, NarrativeScene
...
class TestEmotionalSafetyIntegration:
    ...
    def session_state(self):
        ...
        state.progress_metrics = {"anxiety_management": 0.4, "emotional_regulation": 0.3}
        state.context = {}
    ...
    @pytest.mark.asyncio
    async def test_emotional_safety_monitoring_during_scene_entry(self, therapeutic_integrator, session_state, distressing_scene):
        ...
        await therapeutic_integrator.monitor_emotional_safety(session_state, scene_interaction_data)
    ...
    @pytest.mark.asyncio
    async def test_emotional_safety_monitoring_during_choice_processing(self, therapeutic_integrator, session_state, distressing_choice):
        ...
        await therapeutic_integrator.monitor_emotional_safety(session_state, choice_interaction_data)
        
        # Check that emotional state was monitored
        user_id = session_state.user_id
        emotional_snapshots = therapeutic_integrator.emotional_safety_system.emotional_snapshots
    ...
    @pytest.mark.asyncio
    async def test_emotional_support_provision(self, therapeutic_integrator, session_state):
    ...
    @pytest.mark.asyncio
    async def test_high_distress_intervention_triggering(self, therapeutic_integrator, session_state):
        ...
        await therapeutic_integrator.monitor_emotional_safety(session_state, interaction_data)
        
        # Check that intervention was triggered
        assert (session_state.context.get("immediate_support_provided", False) or
                session_state.context.get("crisis_intervention_active", False) or
                session_state.context.get("coping_support_offered", False))
        
        # Check metrics
        assert therapeutic_integrator.emotional_interventions > 0
    
    @pytest.mark.asyncio
    async def test_crisis_protocol_activation(self, therapeutic_integrator, session_state):
    ...
    def test_emotional_safety_status_reporting(self, therapeutic_integrator, session_state):
    ...
    @pytest.mark.asyncio
    async def test_trigger_detection_and_intervention_suggestion(self, therapeutic_integrator, session_state):
    ...
    @pytest.mark.asyncio
    async def test_therapeutic_context_integration(self, therapeutic_integrator, session_state):
    ...
    @pytest.mark.asyncio
    async def test_safety_monitor_integration(self, therapeutic_integrator, session_state):
        """Test integration with safety monitoring."""
        # Create safety monitor
        from src.components.gameplay_loop.narrative.therapeutic_integrator import SafetyMonitor
        
        safety_monitor = SafetyMonitor(
            monitor_id=session_state.session_id,
            safety_level="standard"
        )
        therapeutic_integrator.safety_monitors[session_state.session_id] = safety_monitor
        ...
        updated_monitor = therapeutic_integrator.safety_monitors[session_state.session_id]
        assert "emotional_distress" in updated_monitor.safety_level
        assert len(updated_monitor.risk_factors) > 0 or len(updated_monitor.protective_factors) > 0
    ...
    @pytest.mark.asyncio
    async def test_emotional_pattern_detection(self, therapeutic_integrator, session_state):
        ...
        if "emotional_pattern_alert" in session_state.context:
            assert session_state.context["emotional_pattern_alert"] == "increasing_distress"
    ...
    def test_emotional_safety_system_initialization(self, therapeutic_integrator):
        ...
        assert isinstance(therapeutic_integrator.emotional_safety_system, EmotionalSafetySystem)
        
        # Check that system is properly configured
        emotional_system = therapeutic_integrator.emotional_safety_system
        assert emotional_system.monitoring_enabled == True
