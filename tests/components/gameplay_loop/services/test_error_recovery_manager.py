...
from src.components.gameplay_loop.services.error_recovery_manager import (
    ErrorRecoveryManager, ErrorContext, RecoveryResult, SystemBackup,
...
class TestErrorRecoveryManager:
    ...
    @pytest.mark.asyncio
    async def test_handle_error_basic(self, error_recovery_manager, sample_exception, event_bus):
        """Test basic error handling."""
        recovery_result = await error_recovery_manager.handle_error(
            sample_exception, "test_component", "test_function",
    ...
    @pytest.mark.asyncio
    async def test_handle_therapeutic_error(self, error_recovery_manager, therapeutic_context, event_bus):
        """Test handling of therapeutic errors."""
        therapeutic_exception = Exception("Therapeutic safety violation")
        
        recovery_result = await error_recovery_manager.handle_error(
            therapeutic_exception, "therapeutic_integrator", "process_choice",
            "user_123", "session_456", therapeutic_context
        )
    ...
    async def test_handle_crisis_situation(self, error_recovery_manager, event_bus):
        ...
        recovery_result = await error_recovery_manager.handle_error(
            crisis_exception, "emotional_safety", "assess_distress",
            "user_123", "session_456", crisis_context
        )
        
        # Check that crisis was properly identified
        error_context = error_recovery_manager.error_history[0]
        assert error_context.severity == ErrorSeverity.THERAPEUTIC_CRITICAL
        assert error_context.requires_therapeutic_intervention == True
        assert error_context.affects_therapeutic_safety == True
    ...
    async def test_error_categorization(self, error_recovery_manager):
        ...
        test_cases = [
            ("therapeutic_integrator", "process_choice", ErrorCategory.THERAPEUTIC_ERROR),
            ...
            ("integration_service", "service unavailable", ErrorCategory.INTEGRATION_ERROR),
            ("performance_monitor", "timeout occurred", ErrorCategory.PERFORMANCE_ERROR),
            ("unknown_component", "unknown error", ErrorCategory.SYSTEM_ERROR)
        ]
        
        for component, message, expected_category in test_cases:
            exception = Exception(message)
            await error_recovery_manager.handle_error(exception, component, "test_function")
    ...
    async def test_error_severity_assessment(self, error_recovery_manager):
        ...
        for message, expected_severity in severity_test_cases:
            exception = Exception(message)
            await error_recovery_manager.handle_error(exception, "test_component", "test_function")
    ...
    async def test_restore_from_backup(self, error_recovery_manager):
        ...
        # Restore from backup
        recovery_result = await error_recovery_manager.restore_from_backup(backup.backup_id)
        ...
        assert recovery_result.data_recovered == True
        assert recovery_result.user_message != ""
        assert recovery_result.therapeutic_message != ""
    ...
    async def test_restore_from_invalid_backup(self, error_recovery_manager):
        ...
        recovery_result = await error_recovery_manager.restore_from_backup("nonexistent_backup")
        
        # Check failure handling
        assert recovery_result.success == False
        assert recovery_result.escalation_needed == True
    ...
    async def test_fallback_mechanisms_loading(self, error_recovery_manager):
        ...
        key_components = [
            "narrative_engine", "choice_processor", "therapeutic_integrator",
            "character_development", "session_manager", "collaborative_system"
        ]
    ...
    async def test_therapeutic_interventions_loading(self, error_recovery_manager):
        ...
        key_interventions = [
            "session_interruption", "data_recovery", "system_degradation", "collaborative_failure"
        ]
