...
from src.components.gameplay_loop.controllers.gameplay_loop_controller import (
    GameplayLoopController, SessionConfiguration, SessionBreakPoint, SessionSummary,
    SessionPhase, BreakPointType, SessionPacing
)
...
class TestGameplayLoopController:
    ...
    @pytest.fixture
    def controller(self, narrative_engine, choice_processor, session_manager, event_bus):
        """Create gameplay loop controller instance."""
        return GameplayLoopController(narrative_engine, choice_processor, session_manager, event_bus)
    ...
    @pytest.mark.asyncio
    async def test_start_session_new_user(self, controller, session_config, session_manager, narrative_engine, event_bus):
        """Test starting a new session for a user."""
        # Mock no existing sessions
        session_manager.get_user_sessions.return_value = []
        
        session_state, session_summary = await controller.start_session(session_config.user_id, session_config)
    ...
    @pytest.mark.asyncio
    async def test_start_session_with_recovery(self, controller, session_config, session_manager):
    ...
    @pytest.mark.asyncio
    async def test_pause_session(self, controller, session_state, session_manager, event_bus):
        """Test pausing an active session."""
        session_manager.get_session.return_value = session_state
        
        paused_state = await controller.pause_session(session_state.session_id, "user_requested")
    ...
    @pytest.mark.asyncio
    async def test_resume_session(self, controller, session_state, session_manager, narrative_engine, event_bus):
    ...
    @pytest.mark.asyncio
    async def test_end_session(self, controller, session_state, session_manager, event_bus):
        ...
        session_summary = await controller.end_session(session_state.session_id, "user_completed")
    ...
    @pytest.mark.asyncio
    async def test_detect_break_points_time_based(self, controller, session_state, session_config):
    ...
    @pytest.mark.asyncio
    async def test_detect_break_points_milestone_achievement(self, controller, session_state, session_config):
    ...
    @pytest.mark.asyncio
    async def test_detect_break_points_emotional_processing(self, controller, session_state, session_config):
    ...
    @pytest.mark.asyncio
    async def test_handle_break_response_accept(self, controller, session_state, session_manager):
