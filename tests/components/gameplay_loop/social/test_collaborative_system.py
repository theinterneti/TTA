...
from src.components.gameplay_loop.social.collaborative_system import (
    CollaborativeSystem, CollaborativeSession, CollaborativeParticipant, GroupChoice,
    SupportMessage, ConflictResolution, CollaborativeMode, ParticipantRole,
    PrivacyLevel, ModerationAction
)
...
class TestCollaborativeSystem:
    ...
    @pytest.fixture
    def collaborative_system(self, event_bus, gameplay_controller, replayability_system):
    ...
    async def test_create_collaborative_session(self, collaborative_system, event_bus):
        ...
        session = await collaborative_system.create_collaborative_session(
            "host_user_123", "Test Collaborative Session", CollaborativeMode.COOPERATIVE,
            ["anxiety_management", "communication_skills"], 4
        )
        ...
        assert session.therapeutic_focus == ["anxiety_management", "communication_skills"]
        ...
        assert host_participant.therapeutic_goals == ["anxiety_management", "communication_skills"]
    ...
    async def test_join_collaborative_session(self, collaborative_system, event_bus):
        ...
        participant = await collaborative_system.join_collaborative_session(
            session.session_id, "participant_user_456", ParticipantRole.PARTICIPANT,
        ...
        assert session.session_id in collaborative_system.user_sessions["participant_user_456"]
    ...
    @pytest.mark.asyncio
    async def test_start_collaborative_session(self, collaborative_system, session_state, event_bus):
        ...
        # Check session state
        assert started_session.is_active == True
    ...
    async def test_propose_group_choice(self, collaborative_system, event_bus):
        ...
        session_state = Mock()
        await collaborative_system.start_collaborative_session(session.session_id, session_state)
        
        # Propose choice
        group_choice = await collaborative_system.propose_group_choice(
            session.session_id, "host_user_123", "Let's try a calming approach",
    ...
    async def test_vote_on_group_choice(self, collaborative_system, event_bus):
        ...
        assert decision_made == False
        assert group_choice.is_approved == False
        ...
        assert decision_made == True
        assert group_choice.is_approved == True
    ...
    async def test_send_support_message(self, collaborative_system, event_bus):
        ...
        support_message = await collaborative_system.send_support_message(
            session.session_id, "host_user_123", "participant_user_456",
            "You're doing great! Keep it up!", "encouragement"
        )
        ...
        # Check moderation
        assert support_message.is_approved == True  # Should be approved for positive message
    ...
    async def test_report_conflict(self, collaborative_system, event_bus):
        ...
        conflict_resolution = await collaborative_system.report_conflict(
            session.session_id, "host_user_123", "disagreement",
        ...
        assert conflict_resolution.involved_participants == ["participant_user_456", "participant_user_789"]
        assert conflict_resolution.description == "Participants disagree on approach"
        
        # Check resolution method assignment
        assert conflict_resolution.resolution_method == "mediation"  # Default for disagreement
        
        # Check storage
        assert conflict_resolution.resolution_id in collaborative_system.conflict_resolutions
    ...
    async def test_moderate_participant(self, collaborative_system, event_bus):
        ...
        result = await collaborative_system.moderate_participant(
            session.session_id, "host_user_123", "participant_user_456",
            ModerationAction.WARNING, "Inappropriate language"
        )
    ...
    def test_get_session_analytics(self, collaborative_system):
        ...
        session.support_messages = [{"message_id": "msg_1"}, {"message_id": "msg_2"}, {"message_id": "msg_3"}]
        ...
        assert overview["is_active"] == True
        ...
        assert engagement["avg_choices_per_participant"] == 1.0  # 2 choices / 2 participants
