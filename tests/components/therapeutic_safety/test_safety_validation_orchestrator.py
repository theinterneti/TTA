...
from src.components.therapeutic_safety.orchestrator import (
    SafetyValidationOrchestrator, ValidationPipeline, ValidationTimeout
)
...
from src.components.therapeutic_safety.enums import (
    ...
    CrisisLevel, ContentType, ValidationScope
)
...
class TestValidationPipeline:
    ...
    def test_get_ordered_components(self):
        ...
        pipeline.register_component(ValidationComponent.CONTENT_SAFETY, Mock(), priority=3)
        pipeline.register_component(ValidationComponent.CRISIS_DETECTION, Mock(), priority=10)
        pipeline.register_component(ValidationComponent.BIAS_DETECTION, Mock(), priority=1)
    ...
    async def test_run_component_success(self):
        ...
        pipeline.register_component(ValidationComponent.CONTENT_SAFETY, mock_validator, timeout_ms=100)
        
        content = ContentPayload(content_text="Test content")
        context = ValidationContext(user_id="test_user")
        
        result = await pipeline.run_component(ValidationComponent.CONTENT_SAFETY, content, context)
    ...
    async def test_run_component_timeout(self):
        ...
        pipeline.register_component(ValidationComponent.CONTENT_SAFETY, slow_validator, timeout_ms=50)
...
class TestSafetyValidationOrchestrator:
    ...
    async def test_orchestrator_initialization(self, orchestrator):
        ...
        assert ValidationComponent.THERAPEUTIC_ALIGNMENT in orchestrator.pipeline.components
    
    @pytest.mark.asyncio
    async def test_validate_content_success(self, orchestrator, event_bus):
        """Test successful content validation."""
        # Setup mock validators
        content_validator = AsyncMock(return_value={
            "safety_level": SafetyLevel.SAFE,
            "violations": []
        })
        crisis_detector = AsyncMock(return_value={
    ...
    async def test_validate_content_crisis_detected(self, orchestrator, event_bus):
        ...
        assert result.immediate_intervention_needed == True
    ...
    async def test_validate_content_timeout(self, orchestrator, event_bus):
        ...
        content = ContentPayload(content_text="Test content")
        context = ValidationContext(user_id="test_user", timeout_ms=100)  # 100ms timeout
    ...
    @pytest.mark.asyncio
    async def test_validate_content_with_cache_hit(self, orchestrator, cache_manager, event_bus):
    ...
    @pytest.mark.asyncio
    async def test_validate_content_cache_miss(self, orchestrator, cache_manager, event_bus):
