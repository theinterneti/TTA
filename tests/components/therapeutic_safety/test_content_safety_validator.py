...
from src.components.therapeutic_safety.enums import (
    SafetyLevel, ContentType, AgeGroup, TherapeuticGoalCategory,
    ValidationScope, RiskCategory
)
...
class TestContentSafetyValidator:
    ...
    async def test_content_complexity_estimation(self, validator):
        ...
        # Simple content
        simple_content = ContentPayload(content_text="This is simple. It has short words.")
    ...
    async def test_recommendations_generation(self, validator):
        ...
        if any(v["risk_category"] == RiskCategory.SELF_HARM.value for v in result["violations"]):
            assert "coping strategies" in recommendations_text or "support" in recommendations_text
    ...
    async def test_multiple_violations(self, validator):
        ...
        violation_categories = set(v["risk_category"] for v in result["violations"])
    ...
    async def test_metrics_update(self, validator):
        ...
        content = ContentPayload(
            content_text="I want to hurt myself",
            content_type=ContentType.USER_INPUT
        )
        context = ValidationContext(
            user_id="test_user",
    ...
    async def test_goal_keywords_mapping(self, validator):
        ...
        anxiety_keywords = validator._get_goal_keywords(TherapeuticGoalCategory.ANXIETY_MANAGEMENT)
        depression_keywords = validator._get_goal_keywords(TherapeuticGoalCategory.DEPRESSION_SUPPORT)
    ...
    async def test_prohibited_patterns_detection(self, validator):
        ...
        prohibited_violations = [rule for rule in violation_rules if "prohibited" in rule]
