...
def test_characters_root_requires_auth(client: TestClient) -> None:
    ...
    body = resp.json()
    assert body.get("error") in {"Authentication Required", "Authentication Failed", "Invalid Authorization Header"}
...
def test_update_therapeutic_profile_requires_auth(client: TestClient) -> None:
    resp = client.patch("/api/v1/characters/char-123/therapeutic-profile", json={
        "primary_concerns": [],
        "therapeutic_goals": [],
