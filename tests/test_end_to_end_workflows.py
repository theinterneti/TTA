...
from src.player_experience.models.character import CharacterCreationData, CharacterAppearance, CharacterBackground, TherapeuticProfile
from src.player_experience.models.enums import IntensityLevel, TherapeuticApproach
from src.player_experience.models.world import WorldParameters
...
def create_auth_token(player_id: str = "test-player", username: str = "testuser", email: str = "test@example.com") -> str:
...
class TestEndToEndUserWorkflows:
    ...
    def test_complete_new_user_journey(self, client):
        ...
        response = client.post("/api/v1/characters/", json=character_data, headers=headers)
        assert response.status_code == 201
        ...
        response = client.get(f"/api/v1/worlds/{world_id}/compatibility/{character_id}", headers=headers)
        assert response.status_code == 200
        ...
        response = client.post(f"/api/v1/worlds/{world_id}/customize", json=world_params, headers=headers)
        assert response.status_code == 200
        ...
        with client.websocket_connect(f"/ws/chat?token={token}") as websocket:
            ...
            user_message = {
                "type": "user_message",
                "content": {"text": "Hello, I'm ready to start my therapeutic journey."},
                "metadata": {
                    "character_id": character_id,
                    "world_id": world_id,
    ...
    def test_multi_character_workflow(self, client):
        ...
        for i, character in enumerate(characters):
            ...
            session_data = {
                ...
                "therapeutic_settings": {
                    "intensity_level": "medium",
                    "preferred_approaches": character["therapeutic_profile"]["therapeutic_approaches"],
        ...
        with client.websocket_connect(f"/ws/chat?token={token}") as websocket:
            ...
            for i, (character, session) in enumerate(zip(characters, sessions)):
                switch_message = {
                    "type": "switch_context",
                    "metadata": {
                        "character_id": character["character_id"],
    ...
    def test_therapeutic_settings_adaptation_workflow(self, client):
        ...
        with client.websocket_connect(f"/ws/chat?token={token}") as websocket:
            ...
            user_message = {
                "type": "user_message",
                "content": {"text": "I'm feeling ready for more challenging exercises."},
                "metadata": {
                    "character_id": character_id,
                    "world_id": world_id,
                    "session_id": session["session_id"]
        ...
        assert updated_character["therapeutic_profile"]["preferred_intensity"] == "medium"
...
class TestConcurrentUserScenarios:
    ...
    def test_concurrent_websocket_connections(self, client):
        ...
        response_times = [r["connection_time"] for r in successful_connections if r["connection_time"]]
        if response_times:
    ...
    def test_concurrent_api_requests(self, client):
        ...
        # Check response times
        response_times = [r["request_time"] for r in successful_requests if r["request_time"]]
        if response_times:
            avg_response_time = sum(response_times) / len(response_times)
            max_response_time = max(response_times)
        ...
        def concurrent_session_operation(operation_id: int):
            ...
            try:
                if operation_id % 3 == 0:
                    ...
                    return {"operation": "get_session", "success": response.status_code == 200}
                    ...
                    update_data = {
                        "therapeutic_settings": {
                            "intensity_level": "medium",
                            "preferred_approaches": ["cbt", "mindfulness"],
                            "session_goals": ["personal_growth", "emotional_regulation"],
                            "safety_monitoring": True
                        }
                    }
                    response = client.put(f"/api/v1/sessions/{session_id}", json=update_data, headers=headers)
                    return {"operation": "update_session", "success": response.status_code == 200}
                    
                else:
                    # Get progress for session
                    response = client.get(f"/api/v1/sessions/{session_id}/progress", headers=headers)
                    return {"operation": "get_progress", "success": response.status_code == 200}
                    
            except Exception as e:
                return {"operation": f"operation_{operation_id}", "success": False, "error": str(e)}
        
        # Execute concurrent operations
        num_operations = 15
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(concurrent_session_operation, i) for i in range(num_operations)]
