...
class TTALoadTester:
    ...
    async def initialize(self):
        ...
        logger.info(f"Initialized {len(self.test_users)} test users and {len(self.session_pool)} sessions")
    ...
    async def _create_test_users(self):
        ...
        for i in range(self.config.concurrent_users):
            user = {
                ...
                "therapeutic_framework": random.choice(["CBT", "DBT", "Narrative", "Mindfulness"]),
                "session_count": 0,
    ...
    async def _simulate_user_journey(self, user: Dict[str, Any], delay: float, end_time: datetime):
    ...
    async def _simulate_patient_journey(self, user: Dict[str, Any], session: aiohttp.ClientSession, end_time: datetime):
        ...
        while datetime.utcnow() < end_time:
            try:
                ...
                session_response = await self._test_endpoint(
                    session, "POST", "/api/patient/sessions",
                    data={
                ...
                if session_response and session_response.get("id"):
                    ...
                    await self._simulate_gaming_session(user, session, session_id, end_time)
                
                # Think time between major actions
                await asyncio.sleep(random.uniform(1, self.config.think_time_seconds * 2))
                
            except Exception as e:
                logger.error(f"Error in patient journey: {str(e)}")
                await asyncio.sleep(5)  # Error recovery delay

    async def _simulate_gaming_session(self, user: Dict[str, Any], session: aiohttp.ClientSession, session_id: str, end_time: datetime):
        """Simulate therapeutic gaming session interactions"""
        gaming_actions = [
            "make_choice", "view_progress", "interact_character", 
            "update_emotional_state", "request_intervention"
        ]
        
        session_duration = random.randint(300, 1800)  # 5-30 minutes
        session_end = min(datetime.utcnow() + timedelta(seconds=session_duration), end_time)
        ...
        while datetime.utcnow() < session_end:
            ...
            if action == "make_choice":
                await self._test_endpoint(
                    session, "POST", f"/api/patient/sessions/{session_id}/choices",
                    data={
                ...
                await self._test_endpoint(
                    session, "GET", f"/api/patient/sessions/{session_id}/progress",
                ...
                await self._test_endpoint(
                    session, "POST", f"/api/patient/sessions/{session_id}/interactions",
                    data={
                ...
                await self._test_endpoint(
                    session, "PATCH", f"/api/patient/sessions/{session_id}/emotional-state",
                    data={
                ...
                if random.random() < 0.1:  # 10% chance
                    await self._test_endpoint(
                        session, "POST", f"/api/patient/sessions/{session_id}/interventions",
                        data={
                            "intervention_type": random.choice(["emotional_support", "skill_building", "crisis_support"]),
    ...
    async def _simulate_clinician_journey(self, user: Dict[str, Any], session: aiohttp.ClientSession, end_time: datetime):
        ...
        while datetime.utcnow() < end_time:
            try:
                ...
                await self._test_endpoint(
                    session, "GET", f"/api/clinical/dashboard/{user['user_id']}",
                ...
                await self._test_endpoint(
                    session, "GET", "/api/clinical/alerts",
                ...
                patient_ids = [f"load_test_user_{i:04d}" for i in range(0, len(self.test_users), 4)]
                for patient_id in random.sample(patient_ids, min(5, len(patient_ids))):
                    await self._test_endpoint(
                        session, "GET", f"/api/clinical/patients/{patient_id}/progress",
                ...
                if random.random() < 0.3:  # 30% chance
                    await self._test_endpoint(
                        session, "PATCH", f"/api/clinical/alerts/{random.randint(1, 100)}/acknowledge",
                        test_name="acknowledge_alert",
                        user_id=user["user_id"]
                    )
                
                # Think time between major actions
                await asyncio.sleep(random.uniform(5, self.config.think_time_seconds * 3))
    ...
    async def _test_endpoint(self, session: aiohttp.ClientSession, method: str, endpoint: str, 
                           ...
                           test_name: str = "", user_id: str = "", session_id: Optional[str] = None) -> Optional[Dict]:
    ...
    async def _generate_test_report(self) -> Dict[str, Any]:
        ...
        for test_name, stats in test_stats.items():
            ...
            stats["median_response_time"] = statistics.median(stats["response_times"]) if stats["response_times"] else 0
        ...
        report = {
            ...
            "performance_metrics": {
                ...
                "performance_passed": p95_response_time <= self.config.max_response_time_ms
            },
        ...
        logger.info(f"Load test completed: {success_rate:.2%} success rate, {avg_response_time:.2f}ms avg response time")
        
        return report

    async def _save_results_to_csv(self):
        """Save detailed test results to CSV file"""
        filename = f"load_test_results_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.csv"
        
        with open(filename, 'w', newline='') as csvfile:
            fieldnames = ['test_name', 'endpoint', 'method', 'response_time', 'status_code', 
                         'success', 'timestamp', 'user_id', 'session_id', 'error_message']
    ...
    async def _generate_performance_charts(self):
        ...
        chart_filename = f"load_test_charts_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.png"
