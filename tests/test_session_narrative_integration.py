...
class MockInteractiveNarrativeEngine:
    ...
    async def initialize_session(
        self, session_id: str, character_id: str, world_id: str, therapeutic_settings
    ):
...
class TestSessionNarrativeIntegration(unittest.TestCase):
    ...
    def test_session_creation_with_narrative_initialization(self):
        ...
        async def run_test():
            ...
            self.assertIn(
                result.session_id, self.mock_narrative_engine.initialized_sessions
            )
            init_data = self.mock_narrative_engine.initialized_sessions[
                result.session_id
            ]
    ...
    def test_therapeutic_settings_update_with_narrative_engine(self):
        ...
        async def run_test():
            ...
            self.assertIn(
                "test_session", self.mock_narrative_engine.therapeutic_settings
            )
            engine_settings = self.mock_narrative_engine.therapeutic_settings[
                "test_session"
            ]
            self.assertEqual(engine_settings.intensity_level, 0.8)
            self.assertEqual(
                engine_settings.preferred_approaches[0], TherapeuticApproach.MINDFULNESS
            )
