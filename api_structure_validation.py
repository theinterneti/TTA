...
class APIStructureValidator:
    ...
    def log_result(self, endpoint, method, success, status_code, response_data=None, error=None):
    ...
    def test_endpoint(self, endpoint, method="GET", data=None, expect_auth_required=False):
        ...
        try:
            ...
            if expect_auth_required and response.status_code == 401:
                self.log_result(endpoint, method, True, response.status_code, {"auth_required": True})
                return True
            elif response.status_code in [200, 201, 405]:  # 405 = Method Not Allowed is also informative
                response_data = None
                try:
                ...
                self.log_result(endpoint, method, True, response.status_code, response_data)
                return True
            else:
                try:
                ...
                self.log_result(endpoint, method, False, response.status_code, error_data)
                return False
    ...
    def validate_api_structure(self):
        ...
        self.test_endpoint("/auth/register", "GET")  # Should return 405 Method Not Allowed
        self.test_endpoint("/auth/register", "POST", {"username": "test", "email": "test@example.com", "password": "test123"})
        self.test_endpoint("/auth/login", "POST", {"username": "test", "password": "test123"})
        ...
        self.test_endpoint("/characters/", "POST", {"name": "Test Character"}, expect_auth_required=True)
        self.test_endpoint("/worlds/", "GET", expect_auth_required=True)
        self.test_endpoint("/sessions/", "GET", expect_auth_required=True)
        self.test_endpoint("/sessions/", "POST", {"character_id": "test", "world_id": "test"}, expect_auth_required=True)
        
        # Test specific resource endpoints
        print("\n🎯 Resource-Specific Endpoints:")
        self.test_endpoint("/worlds/therapeutic_world_001", "GET", expect_auth_required=True)
        self.test_endpoint("/worlds/therapeutic_world_001/compatibility/test-char-id", "GET", expect_auth_required=True)
        self.test_endpoint("/worlds/therapeutic_world_001/customize", "POST", {"parameters": {}}, expect_auth_required=True)
        
        # Test session management endpoints
        print("\n⏯️ Session Management Endpoints:")
        self.test_endpoint("/sessions/test-session-id/progress", "PUT", {"progress": 0.5}, expect_auth_required=True)
        self.test_endpoint("/sessions/test-session-id/pause", "POST", expect_auth_required=True)
        self.test_endpoint("/sessions/test-session-id/resume", "POST", expect_auth_required=True)
        
        # Test export endpoints
        print("\n📤 Export Endpoints:")
        self.test_endpoint("/characters/test-char-id/export", "GET", expect_auth_required=True)
        ...
        print("\n" + "=" * 50)
        print(f"🎯 API Structure Validation: {successful_tests}/{total_tests} endpoints responding correctly")
        ...
        if register_response.status_code in [200, 201]:
            ...
            if login_response.status_code == 200:
                ...
                if token:
                    ...
                    test_response = self.session.get(f"{API_BASE}/characters/", headers=headers)
                    
                    print(f"3. Testing token with protected endpoint...")
