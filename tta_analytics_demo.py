...
class TTAAnalyticsDemo:
    ...
    def get_user_progress_data(self, user_id: str, token: str) -> dict:
        ...
        try:
            ...
            return {
                ...
                "visualization": (
                    viz_response.json() if viz_response.status_code == 200 else {}
                ),
                "dashboard": (
    ...
    def create_system_health_visualization(self, metrics: dict):
        ...
        try:
            ...
            colors = [
                "green" if status == "UP" else "red" for status in status_counts.index
            ]
            plt.pie(
            ...
            service_counts = (
                service_df.groupby(["Service", "Status"]).size().unstack(fill_value=0)
            )
    ...
    def create_user_progress_visualization(self, user_data: dict):
        ...
        try:
            ...
            summary_stats = {
                ...
                "Avg Session Length": (
                    df["Duration (minutes)"].mean() if df["Sessions"].sum() > 0 else 0
                ),
            ...
            plt.xticks(
                range(len(summary_stats)), list(summary_stats.keys()), rotation=45
            )
            plt.title("User Progress Summary")

            # Add value labels on bars
            for bar, value in zip(bars, summary_stats.values(), strict=False):
                plt.text(
            ...
            plt.plot(
                df["Date"], df["Sessions_MA"], linewidth=2, label="7-day Moving Average"
            )
    ...
    def generate_analytics_report(self, metrics: dict, user_data: dict) -> str:
        ...
        if health_data:
            ...
            health_percentage = (
                (up_services / total_services * 100) if total_services > 0 else 0
            )

            report.append(
                f"- **System Health**: {health_percentage:.1f}% ({up_services}/{total_services} services up)"
            )
            ...
            for metric in health_data:
                ...
                status = (
                    "UP" if float(metric.get("value", [0, "0"])[1]) == 1 else "DOWN"
                )
        ...
        if progress_data:
            report.append(
                f"- **Total Sessions**: {progress_data.get('total_sessions', 0)}"
            )
            report.append(
                f"- **Total Time**: {progress_data.get('total_time_minutes', 0)} minutes"
            )
            report.append(
                f"- **Milestones Achieved**: {progress_data.get('milestones_achieved', 0)}"
            )
        ...
        if health_data:
            down_services = [
                m.get("metric", {}).get("service", "unknown")
                for m in health_data
                if float(m.get("value", [0, "0"])[1]) == 0
            ]
            if down_services:
