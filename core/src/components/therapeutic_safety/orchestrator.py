...
class ValidationTimeout(Exception):
    ...
    def __init__(
        self, timeout_ms: int, completed_components: list[ValidationComponent]
    ):
    ...
    async def run_component(
        ...
        try:
            result = await asyncio.wait_for(
                validator(content, context), timeout=timeout_ms / 1000.0
            )
            return result
        except asyncio.TimeoutError:
            logger.warning(
                f"Component {component_type.value} timed out after {timeout_ms}ms"
            )
...
class SafetyValidationOrchestrator:
    ...
    async def validate_content(
        ...
        try:
            ...
            logger.debug(
                f"Validation {validation_id} completed in {result.processing_time_ms:.2f}ms"
            )
            ...
            logger.warning(
                f"Validation {validation_id} timed out after {e.timeout_ms}ms"
            )
    ...
    async def _run_validation_pipeline(
        ...
        try:
            ...
            raise ValidationTimeout(timeout_ms, completed_components) from e

    async def _process_component_result(
        self,
        component_type: ValidationComponent,
    ...
    async def _calculate_overall_result(self, result: ValidationResult) -> None:
        ...
        if (
            ...
            or result.crisis_level >= CrisisLevel.HIGH
        ):
        ...
        if component_count > 0:
            ...
            result.confidence_score = min(
                1.0, component_count / len(ValidationComponent)
            )
    ...
    async def _publish_timeout_event(
        ...
        context: ValidationContext,
        result: ValidationResult,
        timeout_error: ValidationTimeout,
    ) -> None:
    ...
    async def _publish_failure_event(
        self, context: ValidationContext, result: ValidationResult, error_message: str
    ) -> None:
    ...
    def _update_average_processing_time(self, processing_time_ms: float) -> None:
        ...
        if completed_count == 1:
            ...
            self.metrics["average_processing_time_ms"] = (
                current_avg * (completed_count - 1) + processing_time_ms
            ) / completed_count
