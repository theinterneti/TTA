...
class PerformanceAnalytics:
    ...
    async def _check_agent_overload(
        self, operation_type: OperationType, stats: Any
    ) -> list[BottleneckIdentification]:
    ...
    def _calculate_trend(
        self, operation_type: OperationType, data_points: list[float]
...
class StepAggregator:
    """Aggregates performance metrics for agent steps."""
    ...
    def record(self, key: str, duration_ms: float, success: bool = True) -> None:
        """Record a step performance metric."""
        try:
            ...
            logging.getLogger(__name__).error(f"Failed to record metric for {key}: {e}")
    ...
    def get_stats(self, key: str) -> dict[str, Any]:
        ...
        durations = [m["duration_ms"] for m in self.metrics[key]]
        total_requests = self.success_counts[key] + self.failure_counts[key]
        ...
        return {
            ...
            "success_rate": (
                self.success_counts[key] / total_requests if total_requests > 0 else 0
            ),
        }


# Global step aggregator instance
_step_aggregator: StepAggregator | None = None


def get_step_aggregator() -> StepAggregator:
    """Get the global step aggregator instance."""
    global _step_aggregator
    if _step_aggregator is None:
        _step_aggregator = StepAggregator()
    return _step_aggregator
