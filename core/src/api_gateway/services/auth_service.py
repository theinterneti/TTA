...
class GatewayAuthService:
    ...
    async def authenticate_request(
        ...
        try:
            ...
            auth_context = await self._create_auth_context_from_validation(
                validation_result, auth_request
            )

            return AuthenticationResponse(success=True, auth_context=auth_context)
    ...
    async def validate_token(self, token: str) -> TokenValidationResult:
        ...
        try:
            ...
            gateway_permissions = (
                auth_middleware._convert_tta_permissions_to_gateway_permissions(
                    authenticated_user.permissions, authenticated_user.role
                )
            )
    ...
    async def validate_user_permissions(
        ...
        try:
            ...
            if required_permission == "read":
                has_permission = (
                    PermissionLevel.READ in auth_context.permissions.permissions
                )
            elif required_permission == "write":
                has_permission = (
                    PermissionLevel.WRITE in auth_context.permissions.permissions
                )
            elif required_permission == "admin":
                has_permission = (
                    PermissionLevel.ADMIN in auth_context.permissions.permissions
                )
            elif required_permission == "therapeutic":
                has_permission = (
                    PermissionLevel.THERAPEUTIC in auth_context.permissions.permissions
                )
            elif required_permission == "crisis":
                has_permission = (
                    PermissionLevel.CRISIS in auth_context.permissions.permissions
                )
            else:
            ...
            if service_name and service_name in auth_context.permissions.service_access:
                service_permissions = auth_context.permissions.service_access[
                    service_name
                ]
    ...
    async def _create_auth_context_from_validation(
        self,
        ...
        is_therapeutic = (
            auth_request.therapeutic_context
            or validation_result.permissions.is_therapeutic_user()
        )
