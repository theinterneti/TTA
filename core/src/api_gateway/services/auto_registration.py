...
from ..models import ServiceEndpoint, ServiceHealthCheck, ServiceInfo, ServiceType
...
class AutoRegistrationService:
        ...
        try:
            ...
            service = ServiceInfo(
                ...
                health_check=ServiceHealthCheck(
                    enabled=True, endpoint="/health", interval=30, timeout=5, retries=3
                ),
    ...
    async def _register_agent_orchestration_service(self) -> None:
        ...
        try:
            if not self.tta_config.get("agent_orchestration_enabled", False):
                logger.info(
                    "Agent Orchestration service not enabled, skipping registration"
                )
                return
            ...
            service = ServiceInfo(
                ...
                service_type=ServiceType.API,
                endpoint=ServiceEndpoint(
                    host="localhost", port=port, path="/api/v1", scheme="http"
                ),
                weight=120,
                priority=80,
    ...
    async def _register_database_services(self) -> None:
        ...
        try:
            ...
            redis_service = ServiceInfo(
                ...
                endpoint=ServiceEndpoint(
                    host="localhost", port=6379, path="/", scheme="redis"
                ),
                weight=100,
                priority=90,
                tags=["cache", "redis", "storage"],
                metadata={"description": "Redis Cache Service", "component": "redis"},
                health_check=ServiceHealthCheck(
            ...
            success = await self.discovery_manager.register_service(
                redis_service, start_health_monitoring=False
            )
            if success:
    ...
    async def refresh_service_registration(self, service_name: str) -> bool:
        ...
        success = await self.discovery_manager.register_service(
            service, start_health_monitoring=False
        )
