...
class TransformationRule:
    ...
    body_transform_function: str | None = None  # JSONPath or function name
    ...
    path_rewrite_pattern: str | None = None
    path_rewrite_replacement: str | None = None
...
class TransformationConfig:
    ...
    default_response_headers: dict[str, str] = field(
        default_factory=lambda: {
    ...
    # Therapeutic transformations
    therapeutic_headers: dict[str, str] = field(
        default_factory=lambda: {
...
class TransformationMiddleware:
        ...
        try:
            ...
            for rule in applicable_rules:
                ...
                try:
                    ...
                    logger.error(
                        f"Error applying transformation rule '{rule.name}': {e}"
                    )
    ...
    def _find_applicable_rules(
        self, path: str, method: str, phase: TransformationPhase
    ...
    async def _apply_transformation_rule(
        self,
        target: GatewayRequest | GatewayResponse,
        rule: TransformationRule,
        is_request: bool = True,
        request_context: GatewayRequest | None = None,
    ) -> GatewayRequest | GatewayResponse:
        """Apply a single transformation rule to request or response."""

        # Header transformations
        if rule.transformation_type in [
            TransformationType.HEADER,
            TransformationType.THERAPEUTIC,
        ]:
            target = await self._transform_headers(
                target, rule, is_request, request_context
            )
        ...
        if rule.transformation_type in [
            ...
            target = await self._transform_body(
                target, rule, is_request, request_context
            )
        ...
        if rule.transformation_type in [
            ...
            target = await self._enrich_context(
                target, rule, is_request, request_context
            )
    ...
    async def _transform_headers(
        ...
        if (
            rule.crisis_mode_headers
            and hasattr(target, "auth_context")
            and target.auth_context
        ):
    ...
    async def _transform_body(
        ...
        try:
            ...
            if (
                rule.body_transform_function
                and rule.body_transform_function in self.custom_functions
            ):
            ...
            logger.debug("Skipping body transformation for non-JSON content")
        except Exception as e:
            logger.error(f"Error transforming body: {e}")

        return target

    async def _transform_query_params(
        self, request: GatewayRequest, rule: TransformationRule
    ) -> GatewayRequest:
    ...
    async def _enrich_context(
        ...
        # Add user context
        if (
            rule.add_user_context
            and hasattr(target, "auth_context")
            and target.auth_context
        ):
            if hasattr(target, "headers") and target.headers is not None:
                target.headers["x-user-id"] = str(
                    target.auth_context.get("user_id", "")
                )
        ...
        if rule.therapeutic_session_enrichment:
            ...
            if hasattr(context_source, "auth_context") and context_source.auth_context:
                ...
                if (
                    session_id
                    and hasattr(target, "headers")
                    and target.headers is not None
                ):
    ...
    async def _resolve_variables(
        ...
        if hasattr(target, "auth_context") and target.auth_context:
            ...
            request_context
            and hasattr(request_context, "auth_context")
            and request_context.auth_context
        ):
            ...
            variables.update(
                {
                    ...
                    "${therapeutic_context}": str(
                        auth_context.get("therapeutic_context", False)
                    ),
                    "${therapeutic_session_id}": auth_context.get(
                        "therapeutic_session_id", ""
                    ),
    ...
    def get_transformation_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "rules_by_type": {
                ttype.value: len(
                    [
                        ...
                        if r.transformation_type == ttype
                    ]
                )
            ...
            "rules_by_phase": {
                phase.value: len(
                    [r for r in self.transformation_rules if r.phase == phase]
                )
