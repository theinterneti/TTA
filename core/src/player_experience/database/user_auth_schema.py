...
class UserAuthSchemaManager:
    ...
    def connect(self) -> None:
        ...
        for attempt in range(attempts):
            try:
                ...
                if ("AuthenticationRateLimit" in emsg) or (
                    ...
                    delay = min(base_delay * (2**attempt), 8.0)
                    logger.debug(
                        f"Neo4j auth rate limit attempt {attempt + 1}/{attempts}; retrying in {delay:.1f}s"
                    )
                    try:
                        if self.driver:
                            self.driver.close()
                    ...
                    raise UserAuthSchemaError(f"Failed to connect to Neo4j: {e}") from e
        ...
        if last_exc:
            raise UserAuthSchemaError(
                f"Failed to connect to Neo4j after {attempts} attempts: {last_exc}"
            )
    ...
    def create_user_auth_constraints(self) -> bool:
        ...
        try:
                ...
                for constraint in constraints:
                    try:
                        ...
                        if (
                            ...
                            or "equivalent constraint already exists" in str(e).lower()
                        ):
                            logger.debug(f"Constraint already exists: {constraint}")
                        else:
                            logger.error(
                                f"Failed to create constraint {constraint}: {e}"
                            )
                            raise UserAuthSchemaError(
            ...
            raise UserAuthSchemaError(f"Error creating constraints: {e}") from e
    ...
    def create_user_auth_indexes(self) -> bool:
        ...
        try:
            ...
            raise UserAuthSchemaError(f"Error creating indexes: {e}") from e
    ...
    def setup_user_auth_schema(self) -> bool:
        ...
        try:
            ...
            raise UserAuthSchemaError(f"Schema setup failed: {e}") from e
    ...
    def verify_schema(self) -> dict[str, Any]:
        ...
        verification_results = {"constraints": [], "indexes": [], "schema_valid": False}
        ...
        try:
            with self.driver.session() as session:
                ...
                verification_results["schema_valid"] = (
                    len(verification_results["constraints"])
                    >= 6  # Minimum expected constraints
                    and len(verification_results["indexes"])
                    >= 12  # Minimum expected indexes
                )

            logger.info(
                f"Schema verification completed: {verification_results['schema_valid']}"
            )
            return verification_results

        except Exception as e:
            logger.error(f"Error verifying user authentication schema: {e}")
            raise UserAuthSchemaError(f"Schema verification failed: {e}") from e


# Utility functions for user authentication schema management
def create_user_auth_schema_manager(
    uri: str = "bolt://localhost:7687",
    username: str = "neo4j",
    password: str = "password",
