...
async def verify_observability_metrics():
    ...
    try:
        ...
        openhands_metrics = [line for line in lines if "openhands" in line.lower()]
...
async def main():
    ...
    logger.info(
        f"  AI context session: {'✓ CREATED' if session_created else '✗ NOT FOUND'}"
    )
    logger.info(
        f"  Observability metrics: {'✓ CREATED' if metrics_created else '✗ NOT FOUND'}"
    )
    logger.info("")
