...
async def test_creative_solutions():
    ...
    for expected_type in expected_types:
        assert any(expected_type in st for st in solution_types), (
...
async def test_retroactive_changes():
    ...
    for change in changes:
        assert change.in_world_explanation, (
...
async def test_storyline_convergence():
    ...
    assert validation.session_id == "test_session_4", (
    ...
    assert "john" in convergence_text.lower(), (
    ...
    if not validation.is_convergent:
        assert len(validation.recommended_adjustments) > 0, (
