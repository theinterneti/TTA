...
def mfa_config():
    ...
    return MFAConfig(enabled=True, email_enabled=True, recovery_codes_enabled=True)
...
class TestMFAService:
    ...
    def test_totp_secret_generation(self, auth_service):
        ...
        secret, qr_code = auth_service.mfa_service.generate_totp_secret(
            user_id, username
        )
...
class TestEnhancedAuthService:
    ...
    def test_security_event_logging(self, auth_service):
        ...
        event = SecurityEvent(
            event_type="test_event", user_id="test_user_123", details={"test": "data"}
        )
...
class TestAuthenticationAPI:
    ...
    def test_login_endpoint(self, client):
        """Test login endpoint."""
        login_data = {"username": "testuser", "password": "StrongP@ss123"}
...
class TestRoleBasedAccessControl:
    ...
    def test_authenticated_user_permission_methods(self):
        ...
        assert user.has_any_permission(
            [Permission.CREATE_CHARACTER, Permission.MANAGE_USERS]
        )
        assert not user.has_all_permissions(
            [Permission.CREATE_CHARACTER, Permission.MANAGE_USERS]
        )
        assert user.has_all_permissions(
            [Permission.CREATE_CHARACTER, Permission.MANAGE_OWN_CHARACTERS]
        )
