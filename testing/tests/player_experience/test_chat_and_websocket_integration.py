...
class TestGameplayChatManager:
    ...
    async def test_message_broadcasting(
        ...
        # Add connection
        await chat_manager.add_connection(mock_websocket, player_id, session_id)
    ...
    async def test_multiple_connections_per_session(self, chat_manager):
        ...
        await chat_manager.add_connection(websocket1, "player_001", session_id)
        await chat_manager.add_connection(websocket2, "player_002", session_id)
...
class TestGameplayWebSocketRouter:
    ...
    async def test_websocket_connection_flow(self):
        ...
        # TODO: Implement per websocket-integration.kiro specification
        # GameplayWebSocketRouter class needs to be implemented in
        # src/player_experience/api/routers/gameplay_websocket.py

        # Mock implementation for testing
        class MockGameplayWebSocketRouter:
            def __init__(self):
                self.chat_manager = MagicMock()

            def _validate_message(self, message):
                return message.get("type") in ["player_input", "connection_init"]

            async def websocket_endpoint(self, websocket, player_id, session_id):
                pass

        router = MockGameplayWebSocketRouter()
