...
from src.api_gateway.core.service_router import (
    FailoverStrategy,
    ServiceRouter,
    ServiceRouterConfig,
)
...
class TestServiceRouter:
    ...
    async def test_select_service_success(self, service_router, sample_gateway_request):
        ...
        selected_service = await service_router.select_service(
            "test-service", sample_gateway_request
        )
    ...
    @pytest.mark.asyncio
    async def test_select_service_no_services_available(
        self, service_router, sample_gateway_request
    ):
        ...
        selected_service = await service_router.select_service(
            "nonexistent-service", sample_gateway_request
        )

        assert selected_service is None

    @pytest.mark.asyncio
    async def test_select_service_therapeutic_priority(
        self, service_router, therapeutic_gateway_request
    ):
        """Test service selection with therapeutic priority."""
        selected_service = await service_router.select_service(
            "test-service", therapeutic_gateway_request
        )
    ...
    @pytest.mark.asyncio
    async def test_select_service_crisis_mode_allows_unhealthy(
        self, service_router, crisis_gateway_request
    ):
        ...
        selected_service = await service_router.select_service(
            "test-service", crisis_gateway_request
        )
    ...
    @pytest.mark.asyncio
    async def test_route_request_with_failover_success(
        self, service_router, sample_gateway_request
    ):
        ...
        result = await service_router.route_request_with_failover(
            "test-service", sample_gateway_request, mock_request_func
        )

        assert "success from" in result

    @pytest.mark.asyncio
    async def test_route_request_with_failover_retry_on_failure(
        self, service_router, sample_gateway_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_route_request_with_failover_max_retries_exceeded(
        self, service_router, sample_gateway_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_route_request_therapeutic_more_retries(
        self, service_router, therapeutic_gateway_request
    ):
        ...
        result = await service_router.route_request_with_failover(
            "test-service", therapeutic_gateway_request, mock_request_func
        )
    ...
    @pytest.mark.asyncio
    async def test_route_request_crisis_most_retries(
        self, service_router, crisis_gateway_request
    ):
        ...
        result = await service_router.route_request_with_failover(
            "test-service", crisis_gateway_request, mock_request_func
        )
    ...
    @pytest.mark.asyncio
    async def test_route_request_circuit_breaker_error_retry(
        self, service_router, sample_gateway_request
    ):
        ...
        service_router.circuit_breaker_manager.call_service = AsyncMock(
            side_effect=[
    ...
    async def test_health_check_healthy(self, service_router):
        ...
        assert (
            health["load_balancing_strategy"]
            == service_router.config.load_balancing_strategy.value
        )
...
class TestServiceRouterCaching:
    ...
    async def test_service_cache_miss(self, service_router):
        ...
        await service_router._get_available_services("test-service")

        # Should call discovery manager
        service_router.discovery_manager.get_services_by_name.assert_called_once_with(
            "test-service"
        )
    ...
    async def test_service_cache_error_fallback(self, service_router):
        ...
        service_router.discovery_manager.get_services_by_name.side_effect = Exception(
            "Discovery failed"
        )
...
class TestServiceRouterConfiguration:
    ...
    def test_default_configuration(self, mock_discovery_manager):
        ...
        assert (
            router.config.load_balancing_strategy == LoadBalancingStrategy.HEALTH_BASED
        )
        assert (
            router.config.failover_strategy == FailoverStrategy.RETRY_DIFFERENT_SERVICE
        )
    ...
    def test_custom_configuration(self, mock_discovery_manager):
        ...
        assert (
            router.config.load_balancing_strategy == LoadBalancingStrategy.ROUND_ROBIN
        )
