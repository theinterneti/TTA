...
class TestAuthenticationMiddleware:
    ...
    async def test_invalid_token(
        self, mock_auth_service_class, auth_middleware, mock_request
    ):
    ...
    def test_convert_tta_role_to_gateway_role(self, auth_middleware):
        ...
        assert (
            auth_middleware._convert_tta_role_to_gateway_role(TTAUserRole.PLAYER)
            == UserRole.PATIENT
        )
        assert (
            auth_middleware._convert_tta_role_to_gateway_role(TTAUserRole.THERAPIST)
            == UserRole.THERAPIST
        )
        assert (
            auth_middleware._convert_tta_role_to_gateway_role(TTAUserRole.ADMIN)
            == UserRole.ADMIN
        )
...
class TestAuthContextCreation:
    ...
    def test_create_auth_context(self, auth_middleware, mock_request, sample_user_data):
        ...
        auth_context = auth_middleware._create_auth_context(
            mock_request, sample_user_data, "test_token"
        )
