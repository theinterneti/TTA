...
def mock_discovery_manager():
    ...
    mock_service = ServiceInfo(
        ...
        endpoint=ServiceEndpoint(
            host="localhost", port=8080, path="/api/v1", scheme="http"
        ),
        weight=100,
        priority=100,
        tags=["test"],
...
def mock_request():
    ...
    request.headers = {"content-type": "application/json", "user-agent": "test-client"}
...
class TestGatewayCore:
    ...
    async def test_create_gateway_request(self, gateway_core, mock_request):
        ...
        gateway_request = await gateway_core._create_gateway_request(
            mock_request, correlation_id
        )
    ...
    async def test_get_target_service(self, gateway_core, mock_discovery_manager):
        ...
        mock_discovery_manager.get_service_for_request.assert_called_once_with(
            service_name="test-service", therapeutic_priority=True
        )

    def test_transform_request_path_no_rewrite(self, gateway_core):
        """Test path transformation without rewrite rule."""
        route_rule = RouteRule(
            name="test-rule", path_pattern="/api/v1/test", target_service="test-service"
        )

        original_path = "/api/v1/test"
        transformed_path = gateway_core._transform_request_path(
            original_path, route_rule
        )
    ...
    async def test_remove_route_rule(self, gateway_core):
        ...
        # Add a rule first
        rule = RouteRule(
            name="temp-rule", path_pattern="/api/v1/temp", target_service="temp-service"
        )
