...
class TestCircuitBreaker:
    ...
    async def test_circuit_opens_after_threshold(
        self, sample_service, circuit_breaker_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_crisis_bypass_open_circuit(
        self, sample_service, circuit_breaker_config
    ):
    ...
    async def test_half_open_to_closed_transition(
        ...
        for _i in range(circuit_breaker_config.success_threshold):
            async with cb.call():
                pass
    ...
    @pytest.mark.asyncio
    async def test_therapeutic_service_lower_threshold(
        self, therapeutic_service, circuit_breaker_config
    ):
...
class TestCircuitBreakerManager:
    ...
    async def test_call_service_success(self, sample_service):
        ...
        result = await manager.call_service(sample_service, test_func)
    ...
    async def test_call_service_failure(self, sample_service):
        ...
        with pytest.raises(Exception):
            await manager.call_service(sample_service, test_func)
    ...
    async def test_call_service_with_therapeutic_context(self, therapeutic_service):
        ...
        result = await manager.call_service(
            therapeutic_service, test_func, therapeutic_request=True
        )
    ...
    async def test_call_service_with_crisis_mode(self, therapeutic_service):
        ...
        result = await manager.call_service(
            therapeutic_service, test_func, therapeutic_request=True, crisis_mode=True
        )

        assert result == "crisis_success"

    def test_get_healthy_services_filters_unhealthy(
        self, sample_service, therapeutic_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_service_health_summary(
        self, sample_service, therapeutic_service
    ):
...
class TestCircuitBreakerIntegration:
    ...
    async def test_circuit_breaker_full_cycle(
        ...
        for _ in range(
            circuit_breaker_config.success_threshold - 1
        ):  # -1 because we already made one
            async with cb.call():
                pass
    ...
    @pytest.mark.asyncio
    async def test_concurrent_circuit_breaker_calls(
        self, sample_service, circuit_breaker_config
    ):
