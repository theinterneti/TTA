...
class TestReplayabilitySystem:
    ...
    def session_state(self):
        ...
        state.choice_history = [
            Mock(
                choice_id="choice_1",
                text="Help the character",
                choice_type=ChoiceType.EMOTIONAL_REGULATION,
            ),
            Mock(
                choice_id="choice_2",
                text="Listen carefully",
                choice_type=ChoiceType.COMMUNICATION,
            ),
        ]
        state.context = {
    ...
    @pytest.mark.asyncio
    async def test_create_exploration_snapshot(
        self, replayability_system, session_state, event_bus
    ):
        ...
        assert (
            snapshot.therapeutic_progress
            == session_state.context["therapeutic_progress"]
        )
        ...
        assert (
            ...
            in replayability_system.user_snapshots[session_state.user_id]
        )
    ...
    async def test_start_exploration_session(self, replayability_system, event_bus):
        ...
        assert (
            ...
            in replayability_system.user_explorations["test_user_456"]
        )
    ...
    @pytest.mark.asyncio
    async def test_create_alternative_path(
        self, replayability_system, session_state, event_bus
    ):
        ...
        exploration = replayability_system.exploration_sessions[
            exploration.exploration_id
        ]
    ...
    async def test_restore_from_snapshot(
        self, replayability_system, session_state, character_development, event_bus
    ):
        """Test restoring session state from snapshots."""
        # Initialize character development for user
        await character_development._initialize_character_attributes(
        ...
        restored_state = await replayability_system.restore_from_snapshot(
            snapshot.snapshot_id, target_state
        )

        # Check restoration
        assert (
            restored_state.session_id == target_state.session_id
        ...
        assert restored_state.context["exploration_mode"]
    ...
    @pytest.mark.asyncio
    async def test_record_path_choice(
        self, replayability_system, session_state, user_choice
    ):
        ...
        await replayability_system.record_path_choice(
            path.path_id, user_choice, outcomes
        )
        ...
        assert (
            choice_record["therapeutic_relevance"] == user_choice.therapeutic_relevance
        )
    ...
    @pytest.mark.asyncio
    async def test_complete_alternative_path(
        self, replayability_system, session_state, event_bus
    ):
        ...
        completed_path = await replayability_system.complete_alternative_path(
            path.path_id, final_outcomes
        )

        # Check completion
        assert completed_path.is_completed
        assert completed_path.completed_at is not None
        assert completed_path.completion_time_minutes > 0

        # Check final outcomes
        assert (
            completed_path.therapeutic_outcomes
            == final_outcomes["therapeutic_outcomes"]
        )
        assert (
            completed_path.character_development
            == final_outcomes["character_development"]
        )
    ...
    async def test_compare_paths(self, replayability_system, session_state, event_bus):
        ...
        await replayability_system.complete_alternative_path(
            path1.path_id,
            {
                "therapeutic_outcomes": {
                    "goals_achieved": 3,
                    "skills_practiced": ["breathing", "mindfulness"],
                },
        ...
        await replayability_system.complete_alternative_path(
            path2.path_id,
            {
                ...
                "character_development": {
                    "attributes_improved": {"courage": 0.7, "wisdom": 0.4}
                },
            },
        )
        ...
        comparison = await replayability_system.compare_paths(
            [path1.path_id, path2.path_id],
            "Therapeutic vs Character Focus",
            [
        ...
        assert (
            ComparisonMetric.THERAPEUTIC_EFFECTIVENESS in comparison.metric_comparisons
        )
    ...
    def test_insight_generators_loading(self, replayability_system):
        ...
        for _generator_type, generator in generators.items():
