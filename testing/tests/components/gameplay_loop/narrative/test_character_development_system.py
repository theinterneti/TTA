...
class TestCharacterDevelopmentSystem:
    ...
    @pytest.mark.asyncio
    async def test_process_character_development_choice_consequence(
        self, development_system, session_state, choice_consequence_context, event_bus
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_character_development_therapeutic_progress(
        self, development_system, session_state, therapeutic_progress_context, event_bus
    ):
        """Test character development from therapeutic progress."""
        event = await development_system.process_character_development(
            session_state,
            DevelopmentTrigger.THERAPEUTIC_PROGRESS,
            therapeutic_progress_context,
        )
        ...
        assert (
            resilience_change > 0.1
        )  # Should be boosted by milestone_related multiplier

    @pytest.mark.asyncio
    async def test_character_attribute_initialization(
        self, development_system, session_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_attribute_change_application(
        self, development_system, session_state
    ):
        ...
        await development_system._apply_attribute_change(
            user_id, attribute, change, DevelopmentTrigger.CHOICE_CONSEQUENCE
        )
    ...
    @pytest.mark.asyncio
    async def test_experience_points_and_level_up(
        self, development_system, session_state
    ):
        ...
        await development_system._apply_experience_points(
            user_id, attribute, 15
        )  # Above threshold for level 2
    ...
    @pytest.mark.asyncio
    async def test_milestone_achievement_detection(
        self, development_system, session_state, event_bus
    ):
        ...
        await development_system.process_character_development(
    ...
    @pytest.mark.asyncio
    async def test_ability_unlock_detection(
        self, development_system, session_state, event_bus
    ):
    ...
    def test_attribute_templates_loading(self, development_system):
        ...
        for _attribute, template in templates.items():
    ...
    def test_ability_templates_loading(self, development_system):
        ...
        # Check template structure
        for _ability_type, abilities in templates.items():
    ...
    def test_milestone_templates_loading(self, development_system):
        ...
        # Check template structure
        for _milestone, template in templates.items():
            assert "name" in template
            assert "description" in template
    ...
    def test_get_character_summary(self, development_system, session_state):
        ...
        for attribute in list(CharacterAttribute)[:3]:  # First 3 attributes
            development_system.character_attributes[user_id][attribute] = (
                CharacterAttributeLevel(
                    attribute=attribute,
                    current_level=2.5
                    + (attribute.value == "courage") * 1.0,  # Courage slightly higher
    ...
    def test_get_character_progression_visualization(
        self, development_system, session_state
    ):
        ...
        visualization = development_system.get_character_progression_visualization(
            user_id
        )
    ...
    async def test_handle_character_regression(self, development_system, session_state):
        ...
        # Get initial levels
        initial_resilience = development_system.character_attributes[user_id][
            CharacterAttribute.RESILIENCE
        ].current_level

        # Handle regression
        event = await development_system.handle_character_regression(
            user_id, session_state, regression_context
        )
        ...
        current_resilience = development_system.character_attributes[user_id][
            CharacterAttribute.RESILIENCE
        ].current_level
        ...
        assert regression_info["is_temporary"]
        assert regression_info["recovery_opportunity"]
