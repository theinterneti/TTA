...
class TestAdaptiveDifficultyEngine:
    ...
    async def test_monitor_performance_normal(self, difficulty_engine, session_state):
        ...
        snapshot = await difficulty_engine.monitor_performance(
    ...
    @pytest.mark.asyncio
    async def test_monitor_performance_poor(
        self, difficulty_engine, poor_performance_state, event_bus
    ):
        ...
        for _ in range(3):
            await difficulty_engine.monitor_performance(
                poor_performance_state, interaction_data
            )
    ...
    @pytest.mark.asyncio
    async def test_monitor_performance_excellent(
        self, difficulty_engine, excellent_performance_state, event_bus
    ):
        ...
        for _ in range(3):
            await difficulty_engine.monitor_performance(
                excellent_performance_state, interaction_data
            )
        ...
        latest_adjustment = difficulty_engine.adjustment_history[user_id][-1]
        assert (
            latest_adjustment.to_difficulty > latest_adjustment.from_difficulty
            or latest_adjustment.strategy == AdaptationStrategy.CONTEXTUAL_SUPPORT
        )
    ...
    def test_calculate_engagement_level(self, difficulty_engine, session_state):
        ...
        engagement = difficulty_engine._calculate_engagement_level(
            session_state, interaction_data
        )
    ...
    def test_determine_adjustment_trigger(
        self, difficulty_engine, poor_performance_state
    ):
        ...
        trigger = difficulty_engine._determine_adjustment_trigger(
            poor_performance_state, snapshot
        )
    ...
    def test_calculate_new_difficulty(self, difficulty_engine, session_state):
        ...
        snapshot = PerformanceSnapshot(current_difficulty=DifficultyLevel.MODERATE)
    ...
    async def test_update_user_preferences(self, difficulty_engine):
        ...
        assert not user_prefs.wants_hints
        assert "anxiety_management" in user_prefs.therapeutic_focus

    @pytest.mark.asyncio
    async def test_request_difficulty_adjustment(
        self, difficulty_engine, session_state
    ):
        ...
        assert session_state.context["difficulty_adjustment_requested"]
        assert (
            session_state.context["requested_difficulty"] == DifficultyLevel.EASY.value
        )
        assert (
            session_state.context["requested_strategy"]
            == AdaptationStrategy.IMMEDIATE_ADJUSTMENT.value
        )
    ...
    def test_pattern_detection(self, difficulty_engine):
        ...
        pattern_detected = difficulty_engine._detect_performance_pattern(
            excellent_snapshots
        )
        assert pattern_detected
