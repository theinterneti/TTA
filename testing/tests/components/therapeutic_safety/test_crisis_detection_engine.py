...
class TestCrisisDetectionEngine:
    ...
    async def test_severe_crisis_level(self, engine):
        ...
        assert result["immediate_intervention"]
        assert (
            "Contact crisis hotline or emergency services" in result["recommendations"]
        )
    ...
    async def test_suicide_risk_detection(self, engine):
        ...
        assert RiskCategory.SUICIDE in list(result["risk_factors"])
        assert result["crisis_level"] >= CrisisLevel.HIGH
        assert (
            "suicide prevention protocols"
            in " ".join(result["recommendations"]).lower()
        )
    ...
    async def test_self_harm_risk_detection(self, engine):
        ...
        assert RiskCategory.SELF_HARM in list(result["risk_factors"])
        assert result["crisis_level"] >= CrisisLevel.MODERATE
        assert (
    ...
    async def test_violence_risk_detection(self, engine):
        ...
        assert RiskCategory.VIOLENCE in list(result["risk_factors"])
        assert result["crisis_level"] >= CrisisLevel.MODERATE
    ...
    async def test_substance_abuse_risk_detection(self, engine):
        ...
        assert RiskCategory.SUBSTANCE_ABUSE in list(result["risk_factors"])
    ...
    async def test_protective_factors_detection(self, engine):
        ...
        assert (
            "strengthen" in recommendations_text or "build on" in recommendations_text
        )
    ...
    async def test_confidence_calculation(self, engine):
        ...
        high_crisis_result = await engine.assess_crisis(
            high_crisis_content, high_crisis_context
        )

        # Ambiguous content should have lower confidence
        ambiguous_content = ContentPayload(
            content_text="Bad day.", content_type=ContentType.USER_INPUT
        )
        ambiguous_context = ValidationContext(user_id="test_user")
        ambiguous_result = await engine.assess_crisis(
            ambiguous_content, ambiguous_context
        )
    ...
    async def test_crisis_recommendations_specificity(self, engine):
        ...
        self_harm_result = await engine.assess_crisis(
            self_harm_content, self_harm_context
        )

        self_harm_recommendations = " ".join(
            self_harm_result["recommendations"]
        ).lower()
    ...
    def test_crisis_patterns_loading(self, engine):
        ...
        for _level, patterns in engine.crisis_patterns.items():
    ...
    def test_risk_indicators_loading(self, engine):
        ...
        for _category, indicators in engine.risk_indicators.items():
    ...
    def test_protective_indicators_loading(self, engine):
        ...
        for _factor, indicators in engine.protective_indicators.items():
            assert len(indicators) > 0
            assert all(isinstance(indicator, str) for indicator in indicators)
    ...
    async def test_metrics_update(self, engine):
        ...
        content = ContentPayload(
            content_text="I want to kill myself", content_type=ContentType.USER_INPUT
        )
        context = ValidationContext(user_id="test_user")
