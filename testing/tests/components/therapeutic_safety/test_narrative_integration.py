...
class TestNarrativeIntegration:
    ...
    def narrative_scene(self):
        ...
        scene.description = (
            "You find yourself in a peaceful garden where you can practice mindfulness."
        )
        scene.choices = [
            {
                "id": "choice_1",
            ...
            {
                "id": "choice_2",
                "text": "Explore the garden and notice the details around you",
            },
        ]
        return scene

    @pytest.mark.asyncio
    async def test_therapeutic_integrator_initialization(
        self, therapeutic_integrator, safety_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_safe_scene_validation(
        self, therapeutic_integrator, session_state, narrative_scene, safety_service
    ):
        ...
        # Validate scene
        is_safe = await therapeutic_integrator.validate_scene_safety(
            session_state, narrative_scene
        )
    ...
    @pytest.mark.asyncio
    async def test_unsafe_scene_validation(
        self, therapeutic_integrator, session_state, safety_service
    ):
        ...
        unsafe_scene.description = (
            "You feel overwhelmed and think about hurting yourself."
        )
    ...
    @pytest.mark.asyncio
    async def test_crisis_intervention_handling(
        self, therapeutic_integrator, session_state, safety_service
    ):
        ...
        assert session_state.context["crisis_detected"]
        assert session_state.context["crisis_level"] == CrisisLevel.CRITICAL.value
        assert session_state.context["intervention_needed"]
    ...
    async def test_user_input_validation(
        ...
        result = await therapeutic_integrator.validate_user_input_safety(
    ...
    @pytest.mark.asyncio
    async def test_user_input_crisis_detection(
        self, therapeutic_integrator, session_state, safety_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_fallback_validation_without_safety_service(
        self, therapeutic_integrator, session_state, narrative_scene
    ):
    ...
    @pytest.mark.asyncio
    async def test_fallback_validation_unsafe_content(
    ...
    @pytest.mark.asyncio
    async def test_session_safety_monitoring(
        self, therapeutic_integrator, session_state
    ):
        ...
        assert session_state.context.get("safety_alert")

        # Safety level should be updated
        safety_monitor = therapeutic_integrator.safety_monitors[
    ...
    async def test_session_finalization(self, therapeutic_integrator, session_state):
        ...
        progress_tracker = therapeutic_integrator.progress_trackers[
            session_state.session_id
        ]
