    ...
    async def validate(
        ...
        for pattern in self.unsafe_patterns:
            if re.search(pattern, text, re.IGNORECASE):
                violations.append(
                    {"pattern": pattern, "risk_category": "high_risk", "severity": 0.9}
                )
...
class SimpleCrisisDetector:
    ...
    def __init__(self):
        self.crisis_patterns = {
            ...
            CrisisLevel.MODERATE: [r"\bfeel\s+hopeless\b", r"\bnothing\s+matters\b"],
            CrisisLevel.LOW: [r"\bfeeling\s+down\b", r"\bhaving\s+a\s+hard\s+time\b"],
        }
...
async def test_therapeutic_safety():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(
            f'   Content: "{test_case["text"][:50]}{"..." if len(test_case["text"]) > 50 else ""}"'
        )

        # Create content and context
        content = ContentPayload(
            content_text=test_case["text"], content_type=ContentType.USER_INPUT
        )
        ...
        print(
            f"   Safety: {safety_result['safety_level']} ({'✅' if safety_passed else '❌'})"
        )
        print(f"   Crisis: Level {crisis_level} ({'✅' if crisis_passed else '❌'})")
        print(
            f"   Intervention: {'Yes' if crisis_result['immediate_intervention'] else 'No'}"
        )
    ...
    if passed_tests == total_tests:
        print(
            "✅ All tests passed! Therapeutic safety validation system is working correctly."
        )
        return True
    else:
...
async def main():
    ...
    print("📊 FINAL RESULTS")
    print(
        f"   Functionality Tests: {'✅ PASSED' if functionality_passed else '❌ FAILED'}"
    )
