...
def test_redis_connectivity() -> dict[str, Any]:
    ...
    try:
        ...
        if retrieved_value == test_value:
            ...
            raise Exception(
                f"Set/get mismatch: expected {test_value}, got {retrieved_value}"
            )
    ...
    try:
        ...
        if manager.validate_schema():
            ...
            results["details"].append(
                "⚠ Neo4jManager schema validation - Schema needs setup"
            )
...
def print_summary(results: dict[str, Any]):
    ...
    if results["overall_status"] == "success":
        print(
            "🎉 ALL TESTS PASSED! Dependencies and integration are working correctly."
        )
    else:
        print(
