...
async def test_creative_solutions():
    ...
    for expected_type in expected_types:
        assert any(
            expected_type in st for st in solution_types
        ), f"Should include {expected_type} solutions"
    ...
    for solution in solutions:
        assert (
            0.0 <= solution.effectiveness_score <= 1.0
        ), "Effectiveness score should be between 0 and 1"
        assert (
            0.0 <= solution.narrative_cost <= 1.0
        ), "Narrative cost should be between 0 and 1"
        assert (
            solution.implementation_steps
        ), "Solution should have implementation steps"
        assert (
            solution.in_world_explanation
        ), "Solution should have in-world explanation"
...
async def test_conflict_resolution():
    ...
    for i, contradiction in enumerate(contradictions):
        ...
        solution = CreativeSolution(
            ...
            solution_type=(
                "temporal" if contradiction.type == "temporal" else "causal_bridge"
            ),
        ...
        resolution = NarrativeResolution(
            ...
            narrative_changes=[
                f"Applied {step}" for step in solution.implementation_steps
            ],
        )
    ...
    for resolution in resolutions:
        ...
        assert resolution.conflict_id in [
            c.contradiction_id for c in contradictions
        ], "Resolution should reference valid conflict"
        assert resolution.solution_used, "Resolution should include the solution used"
        assert (
            resolution.player_explanation
        ), "Resolution should include player explanation"
        assert (
            resolution.implementation_success
        ), "Resolution should be marked as successful"
...
async def test_retroactive_changes():
    ...
    def apply_changes(changes):
        ...
        for change in changes:
            ...
            applied_changes.append(
                f"Applied {change.change_type}: {change.modified_content}"
            )
    ...
    for change in changes:
        assert (
            change.in_world_explanation
        ), "Each change should have in-world explanation"
...
async def test_storyline_convergence():
    ...
    def analyze_convergence(storylines):
        ...
        return ConvergenceValidation(
            ...
            recommended_adjustments=(
                [] if is_convergent else ["Consider adding more shared elements"]
            ),
        )
    ...
    assert (
        validation.session_id == "test_session_4"
    ), "Validation should reference correct session"
    assert validation.storyline_count == len(storylines), "Should count all storylines"
    assert (
        0.0 <= validation.convergence_score <= 1.0
    ), "Convergence score should be between 0 and 1"

    print(
        f"✓ Convergence validation completed with score: {validation.convergence_score:.2f}"
    )
    ...
    assert (
        "john" in convergence_text.lower()
    ), "Should identify John as convergence point"
    ...
    if not validation.is_convergent:
        assert (
            len(validation.recommended_adjustments) > 0
        ), "Should provide recommendations for poor convergence"
        print(
            f"✓ Generated {len(validation.recommended_adjustments)} recommendations for improvement"
        )
...
async def test_solution_types_for_different_conflicts():
    ...
    for conflict_type in conflict_types:
        ...
        assert (
            len(solutions) > 0
        ), f"Should generate solutions for {conflict_type} conflicts"
        ...
        if conflict_type == "direct":
            assert any(
                "character_driven" in st for st in solution_types
            ), "Direct conflicts should have character-driven solutions"
        elif conflict_type == "temporal":
            assert any(
                "temporal" in st for st in solution_types
            ), "Temporal conflicts should have temporal solutions"
        elif conflict_type == "causal":
            assert any(
                "causal_bridge" in st for st in solution_types
            ), "Causal conflicts should have causal bridge solutions"
        elif conflict_type == "implicit":
            assert any(
                "recontextualization" in st for st in solution_types
            ), "Implicit conflicts should have recontextualization solutions"
...
async def main():
    ...
    try:
        ...
        print(
            "✓ Retroactive change management with in-world explanations - IMPLEMENTED"
        )
