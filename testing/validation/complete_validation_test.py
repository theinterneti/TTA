#!/usr/bin/env python3
"""
Complete 10-Turn TTA Therapeutic Gaming Live Database Validation Test
This script executes the full user journey validation sequence.
"""
...
class TTAValidationTest:
    ...
    def turn_1_register_user(self):
        ...
        try:
            user_data = {
                "username": "therapeutic_tester_validation",
                "email": "validation@therapeutic.example",
    ...
    def turn_3_create_character(self):
        """Turn 3: Create detailed therapeutic character."""
        try:
            character_data = {
                "name": "Alex Journey",
                "appearance": {
            ...
            if response.status_code in [200, 201]:
                ...
                self.character_id = character_response.get(
                    "character_id"
                ) or character_response.get("id")
    ...
    def turn_4_browse_worlds(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                worlds = (
                    worlds_response.get("worlds", [])
                    if isinstance(worlds_response, dict)
                    else worlds_response
                )

                # Select Mindful Meadows if available
                for world in worlds:
                    if (
                        world.get("name") == "Mindful Meadows"
                        or world.get("world_id") == "therapeutic_world_001"
                    ):
                        self.world_id = world.get("world_id") or world.get("id")
                        break

                if not self.world_id and worlds:
                    # Select first available world
                    self.world_id = worlds[0].get("world_id") or worlds[0].get("id")

                self.log_result(
                    4,
                    "Browse Therapeutic Worlds",
                    True,
                    {"worlds_count": len(worlds), "selected_world": self.world_id},
                )
                return True
            else:
                self.log_result(
    ...
    def turn_5_check_compatibility(self):
        ...
        if not self.world_id or not self.character_id:
            self.log_result(
                ...
                False,
                error="Missing world_id or character_id",
            )
            return False

        try:
            response = self.session.get(
                f"{API_BASE}/worlds/{self.world_id}/compatibility/{self.character_id}"
            )

            if response.status_code == 200:
                compatibility_data = response.json()
                self.log_result(
                    5, "World-Character Compatibility", True, compatibility_data
                )
                return True
            else:
                self.log_result(
    ...
    def turn_6_customize_world(self):
        ...
        try:
            ...
            response = self.session.post(
                f"{API_BASE}/worlds/{self.world_id}/customize", json=customization_data
            )
    ...
    def turn_7_create_session(self):
        ...
        if not self.character_id or not self.world_id:
            self.log_result(
                7, "Session Creation", False, error="Missing character_id or world_id"
            )
            return False
        ...
        try:
            session_data = {
                ...
                "therapeutic_goals": ["anxiety_management", "social_confidence"],
                "session_parameters": {
                    "duration_minutes": 30,
            ...
            if response.status_code in [200, 201]:
                ...
                self.session_id = session_response.get(
                    "session_id"
                ) or session_response.get("id")
    ...
    def turn_8_update_progress(self):
        ...
        if not self.session_id:
            self.log_result(
                8, "Session Progress Update", False, error="Missing session_id"
            )
            return False
        ...
        try:
            progress_data = {
                "therapeutic_progress": {
                    ...
                    "coping_skills_practiced": [
                        "deep_breathing",
                        "cognitive_reframing",
                    ],
                    "milestones_achieved": [
                        "first_social_interaction",
                        "anxiety_management_technique",
                    ],
                },
                "session_metrics": {
            ...
            response = self.session.put(
                f"{API_BASE}/sessions/{self.session_id}/progress", json=progress_data
            )
    ...
    def turn_9_pause_resume_session(self):
        ...
        if not self.session_id:
            self.log_result(
                9, "Session Pause/Resume", False, error="Missing session_id"
            )
            return False
        ...
        try:
            ...
            pause_response = self.session.post(
                f"{API_BASE}/sessions/{self.session_id}/pause"
            )
            ...
            if pause_response.status_code != 200:
                self.log_result(
                    ...
                    False,
                    pause_response.json(),
                    f"Pause failed: HTTP {pause_response.status_code}",
                )
                return False
            ...
            resume_response = self.session.post(
                f"{API_BASE}/sessions/{self.session_id}/resume"
            )
            ...
            if resume_response.status_code == 200:
                ...
                self.log_result(
                    ...
                    "Session Pause/Resume",
                    True,
                    {"pause_success": True, "resume_data": resume_data},
                )
                return True
            else:
                self.log_result(
                    ...
                    f"Resume failed: HTTP {resume_response.status_code}",
                )
                return False
    ...
    def turn_10_export_character(self):
        ...
        if not self.character_id:
            self.log_result(
                10, "Character Data Export", False, error="Missing character_id"
            )
            return False

        try:
            response = self.session.get(
                f"{API_BASE}/characters/{self.character_id}/export"
            )
            ...
            if response.status_code == 200:
                ...
                has_session_data = (
                    "sessions" in export_data or "therapeutic_progress" in export_data
                )
    ...
    def run_validation(self):
        ...
        try:
            health_response = self.session.get(
                f"{API_BASE}/services/health", headers={}
            )
