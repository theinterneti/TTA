...
def test_model(
    ...
    logger.info(f"Configuration: quantization={quantization}, flash_attention={use_flash_attention}, temperature={temperature}")
    ...
    try:
        ...
        for prompt_type, prompt in TEST_PROMPTS.items():
            ...
            # Generate response
            with torch.no_grad():
                if use_flash_attention and torch.cuda.is_available() and torch.__version__ >= "2.0.0":
        ...
        for quantization in quantizations:
            for use_flash_attention in flash_attention_settings:
                for temperature in temperatures:
                    logger.info(f"Testing {model} with quantization={quantization}, "
        ...
        analysis["task_recommendations"][task] = [
            {
                ...
                "recommended_config": analysis["best_configurations"][model].get(
                    task.replace("_critical", "").replace("_constrained", "_efficiency").replace("_content", "").replace("complex_", "")
                )
            }
...
def print_analysis(analysis: dict[str, Any]):
    ...
    for model, configs in analysis["best_configurations"].items():
        ...
        for metric, config in configs.items():
            if config:
                print(f"  Best for {metric}: quantization={config['quantization']}, flash_attention={config['flash_attention']}, temperature={config['temperature']}")
    ...
    for task, recommendations in analysis["task_recommendations"].items():
        ...
        for i, rec in enumerate(recommendations[:3], 1):
            ...
            config_str = f" (quantization={config['quantization']}, flash_attention={config['flash_attention']}, temperature={config['temperature']})" if config else ""
