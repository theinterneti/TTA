...
def plot_speed_comparison(df: pd.DataFrame, output_dir: str):
    ...
    speed_df = df.groupby(['model', 'quantization', 'temperature'])['tokens_per_second'].mean().reset_index()
...
def plot_temperature_effect(df: pd.DataFrame, output_dir: str):
    ...
    creative_df = df[df['prompt_type'] == 'creative'].groupby(['model', 'temperature'])['lexical_diversity'].mean().reset_index()
...
def plot_task_performance(df: pd.DataFrame, output_dir: str):
    ...
    structured_df = df[df['prompt_type'] == 'structured_output'].groupby('model')['is_valid'].mean().reset_index()
    ...
    # Tool use mentions
    tool_df = df[df['prompt_type'] == 'tool_use'].groupby('model')['tool_mentions'].mean().reset_index()
    ...
    # Reasoning score
    reasoning_df = df[df['prompt_type'] == 'reasoning'].groupby('model')['reasoning_score'].mean().reset_index()
...
def plot_radar_chart(analysis: dict[str, Any], output_dir: str):
    ...
    for _i, model in enumerate(models):
        ...
        values = [
            perf["speed"]["avg_tokens_per_second"] / MAX_TOKENS_PER_SECOND,  # Normalize by max tokens/s
