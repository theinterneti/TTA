...
def get_latest_analysis_file() -> str | None:
    ...
    try:
        ...
        analysis_files.sort(key=lambda f: os.path.getmtime(os.path.join(RESULTS_DIR, f)), reverse=True)
...
def select_model_for_task(
        ...
        if constraints:
            ...
            for constraint_key, constraint_value in constraints.items():
                if constraint_key == "max_memory_mb":
                    model_memory = get_value_from_nested_dict(performance, "memory.avg_model_size_mb")
                    if model_memory and model_memory > constraint_value:
                        skip = True
                        break
                elif constraint_key == "min_speed":
                    model_speed = get_value_from_nested_dict(performance, "speed.avg_tokens_per_second")
    ...
    best_config = analysis.get("best_configurations", {}).get(best_model, {}).get(
        task_type.replace("_critical", "").replace("_constrained", "_efficiency").replace("_content", "").replace("complex_", "")
    )
    ...
    return {
        ...
        "performance": {
            ...
            "structured_output": get_value_from_nested_dict(model_details, "capabilities.structured_output.success_rate"),
            "tool_use": get_value_from_nested_dict(model_details, "capabilities.tool_use.avg_tool_mentions"),
            "creativity": get_value_from_nested_dict(model_details, "capabilities.creativity.avg_lexical_diversity"),
            "reasoning": get_value_from_nested_dict(model_details, "capabilities.reasoning.avg_reasoning_score")
        },
...
def print_model_selection(selection: dict[str, Any]):
    ...
    print(f"  Structured Output: {perf.get('structured_output', 'N/A')*100:.1f}%" if perf.get('structured_output') is not None else "  Structured Output: N/A")
...
def main():
    ...
    parser.add_argument("--agent", choices=["creative", "analytical", "assistant", "chat", "coding", "summarization", "translation"], help="Agent type")
