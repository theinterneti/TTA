...
class DatabaseDeploymentManager:
    ...
    def wait_for_services(self, max_wait_seconds: int = 120) -> bool:
        ...
        while time.time() - start_time < max_wait_seconds:
            ...
            print(f"⏳ Waiting... Neo4j: {'✅' if neo4j_ready else '❌'}, Redis: {'✅' if redis_ready else '❌'}")
            time.sleep(5)
    ...
    def check_redis_health(self) -> bool:
        ...
        try:
            result = self.run_command("docker exec tta-redis redis-cli -a TTA_Redis_2024! ping", check=False)
    ...
    def initialize_neo4j_schema(self):
        ...
        if not schema_file.exists():
            print(f"⚠️ Schema file {schema_file} not found, skipping schema initialization")
            return
        
        try:
            # Execute schema initialization script
            result = self.run_command(
                f"docker exec tta-neo4j cypher-shell -u neo4j -p TTA_Neo4j_2024! -f /docker-entrypoint-initdb.d/01-schema.cypher",
                check=False
            )
    ...
    def validate_api_integration(self) -> bool:
        ...
        try:
            response = requests.get("http://localhost:8080/api/v1/services/health", timeout=10)
            if response.status_code == 200:
                ...
                if using_mocks:
                    print("⚠️ API is still using mock services. Check environment configuration.")
                    return False
                else:
    ...
    def display_deployment_summary(self):
        ...
        print("   💾 Backup data: docker compose exec neo4j neo4j-admin dump --database=tta_therapeutic_gaming")
        
        print("\n🎯 Next Steps:")
        print("   1. Start TTA API: uv run python -m src.player_experience.api.main")
        print("   2. Verify service health: curl http://localhost:8080/api/v1/services/health")
