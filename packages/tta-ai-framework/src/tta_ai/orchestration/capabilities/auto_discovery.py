...
# Logseq: [[TTA.dev/Packages/Tta-ai-framework/Src/Tta_ai/Orchestration/Capabilities/Auto_discovery]]
...
class ComponentInfo:
    ...
    last_discovery_attempt: float | None = None
...
class AutoDiscoveryManager:
    ...
    async def start(self) -> None:
        ...
        if self.config.strategy in [
            DiscoveryStrategy.DELAYED,
            DiscoveryStrategy.HEARTBEAT,
        ]:
    ...
    def register_component(
        ...
        if (
            self.config.strategy == DiscoveryStrategy.IMMEDIATE
            and self.config.auto_register_on_startup
        ):
    ...
    async def _register_with_registry(self, component: ComponentInfo) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to register component {component.component_id} with registry: {e}"
            )
            return False
            ...
            try:
                ...
                for component_id, component in self.components.items():
                    if component.discovery_status == DiscoveryStatus.REGISTERED:
                        ...
                        component.discovery_status == DiscoveryStatus.PENDING
                        and self.config.strategy == DiscoveryStrategy.HEARTBEAT
                    ):
    ...
    def get_component_status(self, component_id: str) -> DiscoveryStatus | None:
    ...
    def get_discovery_statistics(self) -> dict[str, Any]:
        ...
        for status in DiscoveryStatus:
            status_counts[status.value] = sum(
                1 for c in self.components.values() if c.discovery_status == status
            )
...
_auto_discovery_manager: AutoDiscoveryManager | None = None


def get_auto_discovery_manager(
    registry: RedisAgentRegistry | None = None,
