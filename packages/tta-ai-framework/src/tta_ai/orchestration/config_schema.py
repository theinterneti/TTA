...
# Logseq: [[TTA.dev/Packages/Tta-ai-framework/Src/Tta_ai/Orchestration/Config_schema]]
...
class AgentConfig(BaseModel):
    ...
    auto_register_enabled: bool = Field(
        default=False, description="Enable auto-registration for this agent type"
    )
    instance: str | None = Field(
        default=None, description="Explicit instance name (null = auto-generate)"
    )
...
class AgentsConfig(BaseModel):
    ...
    heartbeat_ttl: float = Field(
        default=30.0, ge=5.0, le=300.0, description="Agent heartbeat TTL in seconds"
    )
    heartbeat_interval: float | None = Field(
        default=None, description="Heartbeat interval (null = auto-calculate as ttl/3)"
    )
...
class AgentOrchestrationConfig(BaseModel):
    ...
    max_concurrent_workflows: int = Field(
        default=50, ge=1, le=1000, description="Maximum concurrent workflows"
    )
    workflow_timeout: int = Field(
        default=30, ge=1, le=3600, description="Default workflow timeout in seconds"
    )
    ...
    resources: dict[str, Any] = Field(
        default_factory=dict, description="Resource management settings"
    )
    messaging: dict[str, Any] = Field(
        default_factory=dict, description="Message coordination settings"
    )
...
def validate_agent_orchestration_config(
