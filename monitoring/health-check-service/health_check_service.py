...
# Prometheus metrics
service_up = Gauge('tta_service_up', 'Service health status (1=up, 0=down)', ['service', 'instance', 'environment'])
service_response_time = Gauge('tta_service_response_time_seconds', 'Service response time in seconds', ['service', 'instance', 'environment'])
health_check_total = Counter('tta_health_checks_total', 'Total number of health checks performed', ['service', 'status'])
service_info = Info('tta_service_info', 'Service information', ['service', 'instance', 'environment'])
...
class HealthChecker:
    ...
    async def check_neo4j_health(self, service_name: str, config: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            driver = GraphDatabase.driver(
                ...
                auth=(auth_config.get('username', 'neo4j'), auth_config.get('password', 'password')),
            ...
            return {
                ...
                'response_time': response_time,
                'details': {
                    'components': [dict(record) for record in components] if components else [],
    ...
    async def update_metrics(self, service_name: str, result: dict[str, Any]):
        ...
        service_up.labels(service=service_name, instance=instance, environment=environment).set(status_value)

        # Update response time
        service_response_time.labels(service=service_name, instance=instance, environment=environment).set(result['response_time'])
        ...
        if 'details' in result:
            ...
            service_info.labels(service=service_name, instance=instance, environment=environment).info(info_dict)
    ...
    async def run_health_checks(self):
        ...
        for service_name, task in tasks:
            try:
                ...
                logger.info(f"Health check completed for {service_name}: {result['status']} ({result['response_time']:.3f}s)")
            except Exception as e:
                logger.error(f"Health check failed for {service_name}: {e}")
                # Update metrics with failure
                await self.update_metrics(service_name, {
                    'status': 'down',
                    'response_time': 0,
                    'error': str(e)
                })
