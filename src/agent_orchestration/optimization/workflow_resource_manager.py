...
from ..realtime.workflow_progress import WorkflowProgressTracker, WorkflowStage
from .response_time_monitor import ResponseTimeCollector, ResponseTimeCategory
...
class ResourcePool:
    ...
    def utilization_percentage(self) -> float:
        ...
        return ((self.allocated_capacity + self.reserved_capacity) / self.total_capacity) * 100.0 if self.total_capacity > 0 else 0.0
...
class WorkflowScheduler:
    ...
    def __init__(
        ...
        priority_weights: Optional[Dict[WorkflowPriority, float]] = None,
    ):
    ...
    def enqueue_workflow(self, request: WorkflowResourceRequest) -> bool:
        ...
        logger.info(f"Enqueued workflow {request.workflow_id} with priority {request.priority.value}")
        return True
    
    def get_next_workflow(self) -> Optional[WorkflowResourceRequest]:
        """Get the next workflow to schedule based on priority."""
        if len(self.running_workflows) >= self.max_concurrent_workflows:
            return None
        
        # Check queues in priority order
        for priority in [WorkflowPriority.CRITICAL, WorkflowPriority.HIGH, WorkflowPriority.NORMAL, WorkflowPriority.LOW]:
...
class WorkflowResourceManager:
    ...
    def __init__(
        self,
        workflow_tracker: Optional[WorkflowProgressTracker] = None,
        response_time_collector: Optional[ResponseTimeCollector] = None,
        ...
        self.resource_pools: Dict[ResourceType, ResourcePool] = {
            ...
            ResourceType.AGENT_SLOTS: ResourcePool(ResourceType.AGENT_SLOTS, 50.0),  # 50 agent slots
            ResourceType.CONCURRENT_WORKFLOWS: ResourcePool(ResourceType.CONCURRENT_WORKFLOWS, float(max_concurrent_workflows)),
            ResourceType.MESSAGE_QUEUE_CAPACITY: ResourcePool(ResourceType.MESSAGE_QUEUE_CAPACITY, 10000.0),  # 10k messages
        }
        ...
        for resource_type, pool in self.resource_pools.items():
            ...
            if self.response_time_collector:
                self.response_time_collector.record_duration(
                    ...
                    metadata={"resource_type": resource_type.value, "utilization_percent": utilization}
                )
...
class WorkflowLoadBalancer:
    ...
    def get_load_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_assigned_agents": sum(len(agents) for agents in self.workflow_assignments.values()),
        }
