...
from .optimization_engine import OptimizationEngine, OptimizationResult, OptimizationStrategy
...
class PerformanceAnalytics:
    ...
    def __init__(
        ...
        trend_analysis_periods: Optional[List[str]] = None,
        ...
        self.trend_analysis_periods = trend_analysis_periods or ["1h", "6h", "24h", "7d"]
        ...
        self._effectiveness_task: Optional[asyncio.Task] = None
    ...
    async def _calculate_system_health(self) -> SystemHealthMetrics:
        ...
        response_time_health = statistics.mean(response_time_scores) if response_time_scores else 50.0
        ...
        if self.optimization_engine and self.optimization_effectiveness:
            ...
            optimization_health = statistics.mean(effectiveness_scores) if effectiveness_scores else 75.0
        ...
        for category, category_stat_list in category_stats.items():
            if category_stat_list:
                category_summaries[category] = {
                    ...
                    "avg_mean_duration": statistics.mean([s.mean_duration for s in category_stat_list]),
                    "avg_p95_duration": statistics.mean([s.p95_duration for s in category_stat_list]),
                    "avg_success_rate": statistics.mean([s.success_rate for s in category_stat_list]),
    ...
    async def _get_optimization_analytics(self) -> Dict[str, Any]:
        ...
        if self.optimization_effectiveness:
            effectiveness_scores = [eff.effectiveness_score for eff in self.optimization_effectiveness.values()]
            effectiveness_summary = {
    ...
    async def _get_resource_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            ]) if efficiency_metrics else 0.0,
        }
        ...
        for period in self.trend_analysis_periods:
            ...
            if period_trends:
                trends_by_period[period] = {
                    ...
                    "improving_trends": len([t for t in period_trends.values() if t.trend_direction == "improving"]),
                    "degrading_trends": len([t for t in period_trends.values() if t.trend_direction == "degrading"]),
                    "stable_trends": len([t for t in period_trends.values() if t.trend_direction == "stable"]),
                    "trends": [
                        {
    ...
    async def _generate_recommendations(self) -> List[Dict[str, Any]]:
        ...
        if slow_operations:
            recommendations.append({
                ...
                "affected_operations": [f"{op.category.value}:{op.operation}" for op in slow_operations[:5]],
            })
        ...
        if failing_operations:
            recommendations.append({
                ...
                "affected_operations": [f"{op.category.value}:{op.operation}" for op in failing_operations[:5]],
            })
    ...
    async def _update_performance_trends(self) -> None:
        ...
        for stat_key, stat in current_stats.items():
            ...
            if trend_key in self.performance_trends:
                ...
                change = (stat.mean_duration - previous_trend.current_value) / previous_trend.current_value * 100
    ...
    async def _analyze_optimization_effectiveness(self) -> None:
        ...
        for optimization in recent_optimizations:
            ...
            effectiveness_score = min(1.0, max(0.0, optimization.confidence_score + (hash(optimization.optimization_id) % 40 - 20) / 100.0))
            
            self.optimization_effectiveness[optimization.optimization_id] = OptimizationEffectiveness(
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_hit_rate": 1.0 if time.time() - self.last_cache_update < self.cache_ttl else 0.0,
            "configuration": {
