...
class OptimizationParameter:
    ...
    def adjust(self, delta: int | float) -> int | float:
...
class ConservativeOptimizer(OptimizationAlgorithm):
    ...
    def analyze_performance(
        ...
        for _stat_key, stat in message_stats.items():
...
class AggressiveOptimizer(OptimizationAlgorithm):
    ...
    def analyze_performance(
        ...
        for _stat_key, stat in stats.items():
            if stat.success_rate < 0.9:  # Less than 90% success rate
                ...
                for param in timeout_params:
                    if param.current_value < param.max_value:
                        adjustment = max(
                            param.step_size,
                            param.current_value * self.adjustment_factor,
                        )
        ...
        for _stat_key, stat in high_confidence_stats.items():
            ...
            if cv > 0.5:  # High variability threshold
                ...
                if heartbeat_param:
                    ...
                    adjustment = -max(
                        heartbeat_param.step_size, heartbeat_param.current_value * 0.1
                    )
                    ...
                    result = OptimizationResult(
                        ...
                        new_value=max(
                            heartbeat_param.min_value,
                            heartbeat_param.current_value + adjustment,
                        ),
                        strategy=self.get_strategy(),
                        target=OptimizationTarget.RESPONSE_TIME,
...
class OptimizationEngine:
    ...
    def __init__(
        ...
        enabled_strategies: list[OptimizationStrategy] | None = None,
    ...
    async def run_optimization_cycle(self) -> list[OptimizationResult]:
        ...
        logger.info(
            f"Applied {len(applied_results)} optimizations out of {len(all_results)} suggestions"
        )
