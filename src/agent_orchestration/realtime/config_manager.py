...
class RealtimeConfigManager:
    ...
    def load_config(self) -> RealtimeConfig:
        ...
        config = RealtimeConfig(
            ...
            progressive_feedback=self._load_progressive_feedback_config(realtime_config.get("progressive_feedback", {})),
    ...
    def _get_default_enabled(self) -> bool:
        ...
        if self._environment in [RealtimeEnvironment.DEVELOPMENT, RealtimeEnvironment.TESTING]:
    ...
    def _load_event_config(self, event_config: Dict[str, Any]) -> EventConfig:
        ...
        return EventConfig(
            ...
            broadcast_agent_status=self._get_bool_config("broadcast_agent_status", event_config, True),
            broadcast_workflow_progress=self._get_bool_config("broadcast_workflow_progress", event_config, True),
            broadcast_system_metrics=self._get_bool_config("broadcast_system_metrics", event_config, False),
            event_retention_hours=int(event_config.get("event_retention_hours", 24))
        )
    
    def _load_progressive_feedback_config(self, pf_config: Dict[str, Any]) -> ProgressiveFeedbackConfig:
        ...
        return ProgressiveFeedbackConfig(
            ...
            stream_intermediate_results=self._get_bool_config("stream_intermediate_results", pf_config, True),
    ...
    def _load_optimization_config(self, opt_config: Dict[str, Any]) -> OptimizationConfig:
        ...
        return OptimizationConfig(
            ...
            response_time_monitoring=self._get_bool_config("response_time_monitoring", opt_config, True),
            statistical_analysis=self._get_bool_config("statistical_analysis", opt_config, True),
            auto_parameter_adjustment=self._get_bool_config("auto_parameter_adjustment", opt_config, False),
    ...
    def _validate_config(self, config: RealtimeConfig) -> None:
        ...
        if config.progressive_feedback.enabled and config.progressive_feedback.max_updates_per_workflow <= 0:
...
_config_manager: Optional[RealtimeConfigManager] = None


def get_realtime_config_manager(base_config: Optional[Dict[str, Any]] = None) -> RealtimeConfigManager:
...
def get_realtime_config(base_config: Optional[Dict[str, Any]] = None) -> RealtimeConfig:
