...
class WorkflowAwareMessageCoordinator:
    ...
    def __init__(
        ...
        feedback_manager: Optional[ProgressiveFeedbackManager] = None,
    ...
    async def start_workflow_tracking(
        ...
        try:
            ...
            logger.info(f"Started workflow tracking: {workflow_id} with {len(participating_agents)} agents")
            return True
        ...
        if self.track_message_workflows and workflow_id:
            for i, result in enumerate(results):
                if result.delivered and i < len(recipients):
                    await self._track_workflow_message(workflow_id, message.message_id, sender, recipients[i])
    ...
    async def receive_workflow_message(
        ...
        if received_message and self.track_message_workflows:
            ...
            if workflow_id:
                await self._update_workflow_on_message_received(workflow_id, received_message, agent_id)
    ...
    async def ack_workflow_message(
        ...
        if success and self.track_message_workflows and message_info:
            ...
            if workflow_id:
                await self._update_workflow_on_message_ack(workflow_id, message_info, agent_id, workflow_result)
    ...
    async def nack_workflow_message(
        ...
        if success and self.track_message_workflows and message_info:
            workflow_id = self.message_to_workflow.get(message_info.get("message_id"))
            if workflow_id:
                await self._update_workflow_on_message_nack(workflow_id, message_info, agent_id, failure, error)
        ...
        try:
            if success:
                ...
                await self.workflow_tracker.fail_workflow(
                    workflow_id,
                    error_message or "Workflow failed",
                    final_result
                )
    ...
    def add_workflow_callback(self, workflow_id: str, callback: Callable[[str, Dict[str, Any]], None]) -> bool:
    ...
    def get_workflow_message_stats(self, workflow_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _update_workflow_on_message_ack(
        ...
        await self._call_workflow_callbacks(workflow_id, "message_ack", {
            "message_info": message_info,
            "agent_id": agent_id.model_dump() if hasattr(agent_id, "model_dump") else str(agent_id),
            "workflow_result": workflow_result,
        })
    ...
    async def _update_workflow_on_message_nack(
        ...
        # Call workflow callbacks
        await self._call_workflow_callbacks(workflow_id, "message_nack", {
            "message_info": message_info,
            "agent_id": agent_id.model_dump() if hasattr(agent_id, "model_dump") else str(agent_id),
            "failure_type": failure.value,
            "error": error,
    ...
    async def _get_reserved_message_info(self, agent_id: AgentId, token: str) -> Optional[Dict[str, Any]]:
    ...
    async def _call_workflow_callbacks(self, workflow_id: str, event_type: str, event_data: Dict[str, Any]) -> None:
