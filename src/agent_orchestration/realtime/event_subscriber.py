...
from .models import WebSocketEvent, EventType
...
class EventSubscriber:
    ...
    async def subscribe_to_event_type(self, event_type: EventType, handler: Callable[[WebSocketEvent], None]) -> None:
        """Subscribe to a specific event type."""
        await self.subscribe_to_channel(f"{self.channel_prefix}:{event_type.value}", handler)
    
    async def subscribe_to_agent_events(self, agent_id: str, handler: Callable[[WebSocketEvent], None]) -> None:
        """Subscribe to events for a specific agent."""
        await self.subscribe_to_channel(f"{self.channel_prefix}:agent:{agent_id}", handler)
    
    async def subscribe_to_user_events(self, user_id: str, handler: Callable[[WebSocketEvent], None]) -> None:
        """Subscribe to events for a specific user."""
        await self.subscribe_to_channel(f"{self.channel_prefix}:user:{user_id}", handler)
    
    async def subscribe_to_channel(self, channel: str, handler: Callable[[WebSocketEvent], None]) -> None:
        ...
        logger.debug(f"Added handler to channel {channel}: {len(self.subscriptions[channel])} total handlers")
    
    async def unsubscribe_from_channel(self, channel: str, handler: Optional[Callable[[WebSocketEvent], None]] = None) -> None:
    ...
    async def _call_handler(self, handler: Callable[[WebSocketEvent], None], event: WebSocketEvent) -> None:
