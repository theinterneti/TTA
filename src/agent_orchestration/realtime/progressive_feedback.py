...
class ProgressiveFeedbackManager:
    ...
    async def update_operation_progress(
        ...
        # Send progress event
        await self._send_progress_event(operation, message or f"Progress update: {stage or 'continuing'}")
    ...
    def add_operation_callback(self, operation_id: str, callback: Callable[[OperationProgress], None]) -> bool:
    ...
    def remove_operation_callback(self, operation_id: str, callback: Callable[[OperationProgress], None]) -> bool:
    ...
    def get_active_operations(self, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    async def _call_operation_callbacks(self, operation_id: str, operation: OperationProgress) -> None:
        ...
        while self._is_running:
            try:
                ...
                for operation_id, operation in self.active_operations.items():
                    ...
                    elif current_time - operation.last_update > self.cleanup_interval * 2:
                        stale_operations.append(operation_id)
    ...
    async def update_agent_operation(
        ...
        # Increment update count
        self.operation_update_counts[operation_id] = self.operation_update_counts.get(operation_id, 0) + 1
