...
def validate_test_conventions(test_file: Path) -> list[str]:
    ...
    Example:
        >>> issues = validate_test_conventions(
        ...     Path("tests/agent_orchestration/tools/test_client.py")
        ... )
        >>> print(issues)
        []
    ...
    if not (file_name.startswith("test_") or file_name.endswith("_test.py")):
        issues.append(
            f"Test file name should start with 'test_' or end with '_test.py': {file_name}"
        )
    ...
    if "uvx pytest" in content:
        issues.append(
            "Test file contains 'uvx pytest' anti-pattern (should use 'uv run pytest')"
        )
...
async def _run_tests(
    test_file: Path, workspace_path: Path
