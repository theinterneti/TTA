...
from .response_time_monitor import OperationType, PerformanceLevel
...
class AlertThreshold:
    ...
    operation_type: Optional[OperationType] = None  # None for global
...
class PerformanceAlerting:
    ...
    def __init__(
        self,
        default_thresholds: Optional[List[AlertThreshold]] = None,
    ...
    async def create_bottleneck_alert(
        ...
        alert = Alert(
            ...
            operation_type=bottleneck.affected_operations[0] if bottleneck.affected_operations else None,
    ...
    async def create_trend_alert(
        ...
        if (trend.trend_direction != TrendDirection.DEGRADING or 
            trend.confidence < 0.7 or 
    ...
    def _find_threshold(
        ...
        for threshold in self.thresholds:
            if (threshold.operation_type == operation_type and 
                threshold.metric_name == metric_name):
                return threshold
        
        # Look for global threshold
        for threshold in self.thresholds:
            if (threshold.operation_type is None and 
                threshold.metric_name == metric_name):
                return threshold
