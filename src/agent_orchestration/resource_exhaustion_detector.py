...
class ResourceThresholds:
    ...
    def validate(self) -> List[str]:
        ...
        if not (0 < self.memory_warning_percent < self.memory_critical_percent < self.memory_exhaustion_percent <= 100):
            errors.append("Memory thresholds must be: 0 < warning < critical < exhaustion <= 100")
        
        # Check CPU thresholds
        if not (0 < self.cpu_warning_percent < self.cpu_critical_percent < self.cpu_exhaustion_percent <= 100):
            errors.append("CPU thresholds must be: 0 < warning < critical < exhaustion <= 100")
        
        # Check disk thresholds
        if not (0 < self.disk_warning_percent < self.disk_critical_percent < self.disk_exhaustion_percent <= 100):
...
class ResourceExhaustionDetector:
    ...
    def __init__(
        ...
        self._threshold_breach_times: Dict[str, Optional[float]] = {
            'memory': None,
            'cpu': None,
            'disk': None
        }
        self._last_exhaustion_events: Dict[str, Optional[ResourceExhaustionEvent]] = {
            'memory': None,
            'cpu': None,
            'disk': None
        }
    ...
    async def check_resource_exhaustion(self) -> List[ResourceExhaustionEvent]:
        ...
        try:
            ...
            events.extend(await self._check_resource_type(
                'memory', memory_percent, current_time,
            ...
            events.extend(await self._check_resource_type(
                'disk', disk_percent, current_time,
    ...
    async def _check_resource_type(
        ...
        if severity:
            ...
            # Log event
            logger.log(
                logging.ERROR if severity == 'exhaustion' else 
                logging.WARNING if severity == 'critical' else 
                logging.INFO,
            ...
            if severity == 'exhaustion' and sustained_duration >= self.thresholds.sustained_duration_seconds:
    ...
    async def _trigger_circuit_breakers_for_exhaustion(self, event: ResourceExhaustionEvent) -> None:
    ...
    # ---- Callback management ----
    def register_exhaustion_callback(self, callback: Callable[[ResourceExhaustionEvent], Awaitable[None]]) -> None:
        """Register callback for exhaustion events."""
        self._exhaustion_callbacks.append(callback)
    
    def register_warning_callback(self, callback: Callable[[ResourceExhaustionEvent], Awaitable[None]]) -> None:
        """Register callback for warning events."""
        self._warning_callbacks.append(callback)
    
    def unregister_exhaustion_callback(self, callback: Callable[[ResourceExhaustionEvent], Awaitable[None]]) -> bool:
    ...
    def unregister_warning_callback(self, callback: Callable[[ResourceExhaustionEvent], Awaitable[None]]) -> bool:
