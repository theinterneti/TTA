...
from .models import AgentId, AgentType, AgentMessage, MessageType, MessagePriority
...
from .adapters import AgentAdapterFactory, RetryConfig, AgentCommunicationError
...
class EnhancedRedisMessageCoordinator(RedisMessageCoordinator):
    ...
    async def send_message(
        ...
        if (self.enable_real_agents and 
            self.message_router and 
            recipient.type in [AgentType.IPA, AgentType.WBA, AgentType.NGA]):
    ...
    def configure_real_agents(
        self,
        enable_real_agents: Optional[bool] = None,
...
class ScalableWorkflowCoordinator:
    ...
    async def _execute_ipa_step(self, config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_wba_step(self, config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._batch_processor:
            ...
            # Direct processing
            result = await self.enhanced_coordinator.wba_adapter.process_world_request(world_id, updates)

        return {'wba_result': result, 'step_type': 'wba'}

    async def _execute_nga_step(self, config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._batch_processor:
            ...
            # Direct processing
            result = await self.enhanced_coordinator.nga_adapter.generate_narrative(prompt, narrative_context)

        return {'nga_result': result, 'step_type': 'nga'}

    async def _execute_parallel_steps(self, config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
