...
from .messaging import MessageResult, FailureType
from .adapters import IPAAdapter, WBAAdapter, NGAAdapter, AgentCommunicationError
...
class MessageTranslationResult:
    ...
    success: bool
    translated_message: Optional[Dict[str, Any]] = None
...
class MessageRouter:
    ...
    async def _route_to_ipa(self, message: Union[AgentMessage, Dict[str, Any]]) -> MessageResult:
        """Route message to IPA adapter."""
        try:
            # Translate message to IPA format
            translation = self.translator.translate_message(
                message, ProtocolType.ORCHESTRATION, ProtocolType.REAL_AGENT, AgentType.IPA
            )
    ...
    async def _route_to_wba(self, message: Union[AgentMessage, Dict[str, Any]]) -> MessageResult:
        """Route message to WBA adapter."""
        try:
            # Translate message to WBA format
            translation = self.translator.translate_message(
                message, ProtocolType.ORCHESTRATION, ProtocolType.REAL_AGENT, AgentType.WBA
            )
    ...
    async def _route_to_nga(self, message: Union[AgentMessage, Dict[str, Any]]) -> MessageResult:
        """Route message to NGA adapter."""
        try:
            # Translate message to NGA format
            translation = self.translator.translate_message(
                message, ProtocolType.ORCHESTRATION, ProtocolType.REAL_AGENT, AgentType.NGA
            )
    ...
    def _get_message_id(self, message: Union[AgentMessage, Dict[str, Any]]) -> str:
