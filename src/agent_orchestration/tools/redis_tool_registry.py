...
from .models import ToolSpec, ToolRegistration, ToolStatus
...
class _LRU:
    ...
    async def stats(self) -> Dict[str, int]:
        async with self._lock:
            return {"hits": self.hits, "misses": self.misses, "size": len(self._order), "capacity": self._max}


class RedisToolRegistry:
    def __init__(self, redis: Redis, key_prefix: str = "ao", *, cache_ttl_s: float = 300.0, cache_max_items: int = 512) -> None:
        self._redis = redis
    ...
    async def register_tool(self, spec: ToolSpec) -> bool:
        ...
        async with self._lock_for(key):
            ...
            await self._redis.set(self._status_key(spec.name, spec.version), ToolStatus.ACTIVE.value)
            await self._cache.set(key, spec)
            return True

    async def get_tool(self, name: str, version: Optional[str] = None) -> Optional[ToolSpec]:
        if version is None:
    ...
    async def list_tools(self, prefix: Optional[str] = None) -> List[ToolSpec]:
        members = await self._redis.smembers(self._idx)
        out: List[ToolSpec] = []
        for b in members:
            s = b.decode() if isinstance(b, (bytes, bytearray)) else b
    ...
    async def get_status(self, name: str, version: str) -> str:
        ...
        return (s.decode() if isinstance(s, (bytes, bytearray)) else s) if s else ToolStatus.ACTIVE.value
