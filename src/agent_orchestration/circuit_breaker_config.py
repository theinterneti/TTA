...
class CircuitBreakerConfigSchema(BaseModel):
    ...
    timeout_seconds: int = Field(default=60, ge=1, le=3600, description="Timeout before transitioning from OPEN to HALF_OPEN")
    recovery_timeout_seconds: int = Field(default=300, ge=1, le=7200, description="Maximum time to wait for recovery")
    half_open_max_calls: int = Field(default=3, ge=1, le=20, description="Maximum calls allowed in HALF_OPEN state")
    success_threshold: int = Field(default=2, ge=1, le=10, description="Successful calls needed to close circuit")
    ...
    def success_threshold_must_be_less_than_half_open_max_calls(cls, v, info):
        ...
        if info.data:
            ...
            if v > half_open_max:
                raise ValueError(f'success_threshold ({v}) must be <= half_open_max_calls ({half_open_max})')
        return v
    ...
    def recovery_timeout_must_be_greater_than_timeout(cls, v, info):
        ...
        if info.data:
            ...
            if v <= timeout:
                raise ValueError(f'recovery_timeout_seconds ({v}) must be > timeout_seconds ({timeout})')
        return v
...
class WorkflowErrorHandlingConfigSchema(BaseModel):
    ...
    rollback_retention_days: int = Field(default=30, ge=1, le=365, description="Rollback history retention")
...
class CircuitBreakerConfigManager:
    ...
    validated_config: Optional[WorkflowErrorHandlingConfigSchema] = None
    ...
    def load_from_env(self) -> None:
        ...
        if os.getenv("TTA_CIRCUIT_BREAKER_ENABLED"):
            ...
            env_config["circuit_breaker"]["enabled"] = os.getenv("TTA_CIRCUIT_BREAKER_ENABLED", "true").lower() in ("true", "1", "yes")
        
        if os.getenv("TTA_CIRCUIT_BREAKER_FAILURE_THRESHOLD"):
            env_config["circuit_breaker"] = env_config.get("circuit_breaker", {})
            env_config["circuit_breaker"]["failure_threshold"] = int(os.getenv("TTA_CIRCUIT_BREAKER_FAILURE_THRESHOLD"))
        
        if os.getenv("TTA_CIRCUIT_BREAKER_TIMEOUT_SECONDS"):
            env_config["circuit_breaker"] = env_config.get("circuit_breaker", {})
            env_config["circuit_breaker"]["timeout_seconds"] = int(os.getenv("TTA_CIRCUIT_BREAKER_TIMEOUT_SECONDS"))
        
        if os.getenv("TTA_CIRCUIT_BREAKER_RECOVERY_TIMEOUT_SECONDS"):
            env_config["circuit_breaker"] = env_config.get("circuit_breaker", {})
            env_config["circuit_breaker"]["recovery_timeout_seconds"] = int(os.getenv("TTA_CIRCUIT_BREAKER_RECOVERY_TIMEOUT_SECONDS"))
        
        # Workflow error handling settings
        if os.getenv("TTA_WORKFLOW_ERROR_HANDLING_ENABLED"):
            env_config["enabled"] = os.getenv("TTA_WORKFLOW_ERROR_HANDLING_ENABLED", "true").lower() in ("true", "1", "yes")
        ...
        if os.getenv("TTA_RESOURCE_MEMORY_THRESHOLD"):
            ...
            env_config["resource_monitoring"]["memory_threshold_percent"] = int(os.getenv("TTA_RESOURCE_MEMORY_THRESHOLD"))
        
        if os.getenv("TTA_RESOURCE_CPU_THRESHOLD"):
            env_config["resource_monitoring"] = env_config.get("resource_monitoring", {})
            env_config["resource_monitoring"]["cpu_threshold_percent"] = int(os.getenv("TTA_RESOURCE_CPU_THRESHOLD"))
    ...
    def get_circuit_breaker_config(self, name: Optional[str] = None) -> CircuitBreakerConfig:
...
def load_circuit_breaker_config(config_dict: Optional[Dict[str, Any]] = None) -> CircuitBreakerConfigManager:
...
def create_circuit_breaker_config_from_dict(config_dict: Dict[str, Any]) -> CircuitBreakerConfig:
