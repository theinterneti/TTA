...
class AgentCapability(BaseModel):
    ...
    scope: CapabilityScope = Field(default=CapabilityScope.SESSION, description="Scope of operation")
    ...
    resource_requirements: Dict[str, Any] = Field(default_factory=dict, description="Resource requirements (CPU, memory, etc.)")

    # Compatibility and dependencies
    compatible_versions: Set[str] = Field(default_factory=set, description="Compatible capability versions")
    ...
    def validate_name(cls, v):
        ...
        if not re.match(r'^[a-z][a-z0-9_]*$', v):
            raise ValueError('Capability name must start with lowercase letter and contain only lowercase letters, numbers, and underscores')
        return v
...
class AgentCapabilitySet(BaseModel):
    ...
    priority: int = Field(default=5, description="Priority for capability matching (1-10, higher is better)")
    load_factor: float = Field(default=1.0, description="Current load factor (0.0-1.0)")
    availability: bool = Field(default=True, description="Whether agent is available for new requests")

    def get_capability(self, name: str, version: Optional[str] = None) -> Optional[AgentCapability]:
    ...
    def has_capability(self, name: str, version: Optional[str] = None) -> bool:
...
class CapabilityMatchCriteria(BaseModel):
    ...
    capability_name: Optional[str] = Field(None, description="Specific capability name to match")
    capability_type: Optional[CapabilityType] = Field(None, description="Type of capability required")
    required_inputs: Set[str] = Field(default_factory=set, description="Required input fields")

    # Version constraints
    min_version: Optional[str] = Field(None, description="Minimum capability version")
    max_version: Optional[str] = Field(None, description="Maximum capability version")
    preferred_version: Optional[str] = Field(None, description="Preferred capability version")

    # Performance constraints
    max_duration_ms: Optional[int] = Field(None, description="Maximum acceptable duration")
...
class CapabilityMatchResult(BaseModel):
    ...
    performance_match: bool = Field(default=False, description="Whether performance requirements are met")
    ...
    estimated_wait_time_ms: Optional[int] = Field(None, description="Estimated wait time for agent")
...
class CapabilityDiscoveryRequest(BaseModel):
    ...
    include_degraded: bool = Field(default=False, description="Include agents with degraded performance")
...
class CapabilityDiscoveryResponse(BaseModel):
    ...
    matches: List[CapabilityMatchResult] = Field(default_factory=list, description="Matched capabilities")
    ...
    # Discovery metadata
    discovery_timestamp: datetime = Field(default_factory=datetime.utcnow, description="Discovery timestamp")
