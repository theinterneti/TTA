...
class CoherenceValidator:
    ...
    async def validate_narrative_consistency(self, content: NarrativeContent) -> ValidationResult:
        ...
        try:
            ...
            result.character_consistency = self._calculate_character_consistency_score(character_issues)
            ...
            result.therapeutic_alignment = self._calculate_therapeutic_alignment_score(therapeutic_issues)
            ...
            result.is_valid = (
                result.consistency_score >= self.consistency_threshold and
                result.lore_compliance >= self.lore_compliance_threshold and
                result.character_consistency >= self.character_consistency_threshold
            )

            # Generate suggested corrections
            result.suggested_corrections = await self._generate_corrections(result.detected_issues)

            logger.debug(f"Validation completed: score={result.consistency_score:.2f}, valid={result.is_valid}")
            return result
            ...
            return ValidationResult(
                ...
                detected_issues=[ConsistencyIssue(
    ...
    async def _validate_lore_compliance(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        ...
        try:
            ...
            for character in content.characters:
                ...
                if character_lore:
                    character_issues = await self._check_character_lore_compliance(content, character, character_lore)
            ...
            for location in content.locations:
                ...
                if location_lore:
                    location_issues = await self._check_location_lore_compliance(content, location, location_lore)
            ...
            for theme in content.themes:
                ...
                if theme_lore:
                    theme_issues = await self._check_theme_lore_compliance(content, theme, theme_lore)
    ...
    async def _validate_character_consistency(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        ...
        try:
            for character in content.characters:
                ...
                personality_issues = await self._check_personality_consistency(content, character, character_profile)
                issues.extend(personality_issues)

                # Check dialogue consistency
                dialogue_issues = await self._check_dialogue_consistency(content, character, character_profile)
                issues.extend(dialogue_issues)

                # Check behavioral consistency
                behavior_issues = await self._check_behavioral_consistency(content, character, character_profile)
    ...
    async def _validate_therapeutic_alignment(self, content: NarrativeContent) -> List[ConsistencyIssue]:
    ...
    def _get_character_lore(self, character: str) -> Optional[LoreEntry]:
        """Get lore entry for a character."""
        return self.lore_database.get(f"character_{character}")

    def _get_location_lore(self, location: str) -> Optional[LoreEntry]:
        """Get lore entry for a location."""
        return self.lore_database.get(f"location_{location}")

    def _get_theme_lore(self, theme: str) -> Optional[LoreEntry]:
        """Get lore entry for a theme."""
        return self.lore_database.get(f"theme_{theme}")

    async def _check_character_lore_compliance(self, content: NarrativeContent, character: str, lore: LoreEntry) -> List[ConsistencyIssue]:
    ...
    async def _check_location_lore_compliance(self, content: NarrativeContent, location: str, lore: LoreEntry) -> List[ConsistencyIssue]:
    ...
    async def _check_theme_lore_compliance(self, content: NarrativeContent, theme: str, lore: LoreEntry) -> List[ConsistencyIssue]:
    ...
    async def _check_constraint_compliance(self, content: NarrativeContent, constraint: str) -> bool:
    ...
    async def _check_personality_consistency(self, content: NarrativeContent, character: str, profile: Dict[str, Any]) -> List[ConsistencyIssue]:
        ...
        for trait, value in personality_traits.items():
            if not await self._check_personality_trait_consistency(content, character, trait, value):
                issues.append(ConsistencyIssue(
                    issue_id=str(uuid.uuid4()),
                    issue_type=ConsistencyIssueType.CHARACTER_INCONSISTENCY,
                    severity=ValidationSeverity.WARNING,
    ...
    async def _check_dialogue_consistency(self, content: NarrativeContent, character: str, profile: Dict[str, Any]) -> List[ConsistencyIssue]:
        ...
        if character_dialogue:
            ...
            for pattern, expected in speech_patterns.items():
                if not await self._check_speech_pattern_consistency(character_dialogue, pattern, expected):
                    issues.append(ConsistencyIssue(
                        issue_id=str(uuid.uuid4()),
                        issue_type=ConsistencyIssueType.CHARACTER_INCONSISTENCY,
    ...
    async def _check_behavioral_consistency(self, content: NarrativeContent, character: str, profile: Dict[str, Any]) -> List[ConsistencyIssue]:
        ...
        if character_actions:
            ...
            for pattern, expected in behavioral_patterns.items():
                if not await self._check_behavioral_pattern_consistency(character_actions, pattern, expected):
                    issues.append(ConsistencyIssue(
                        issue_id=str(uuid.uuid4()),
                        issue_type=ConsistencyIssueType.CHARACTER_INCONSISTENCY,
                        severity=ValidationSeverity.WARNING,
    ...
    async def _generate_generic_correction(self, issue: ConsistencyIssue) -> Optional[str]:
        ...
        if issue.issue_type == ConsistencyIssueType.LORE_VIOLATION:
            ...
            return f"Revise content to ensure therapeutic appropriateness and safety"
        else:
    ...
    async def _check_personality_trait_consistency(self, content: NarrativeContent, character: str, trait: str, value: Any) -> bool:
        """Check if content is consistent with a character's personality trait."""
        # Placeholder implementation
        return True

    async def _extract_character_dialogue(self, content: NarrativeContent, character: str) -> List[str]:
        """Extract dialogue lines for a specific character."""
        # Placeholder implementation
        return []

    async def _check_speech_pattern_consistency(self, dialogue: List[str], pattern: str, expected: Any) -> bool:
        """Check if dialogue matches expected speech patterns."""
        # Placeholder implementation
        return True

    async def _extract_character_actions(self, content: NarrativeContent, character: str) -> List[str]:
        """Extract action descriptions for a specific character."""
        # Placeholder implementation
        return []

    async def _check_behavioral_pattern_consistency(self, actions: List[str], pattern: str, expected: Any) -> bool:
        """Check if actions match expected behavioral patterns."""
        # Placeholder implementation
        return True

    async def _check_physics_rules(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        """Check content against physics rules."""
        # Placeholder implementation
        return []

    async def _check_supernatural_rules(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        """Check content against supernatural/magic rules."""
        # Placeholder implementation
        return []

    async def _check_social_rules(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        """Check content against social/cultural rules."""
        # Placeholder implementation
        return []

    async def _check_technological_rules(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        """Check content against technological rules."""
        # Placeholder implementation
        return []

    async def _check_harmful_content(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        """Check for potentially harmful content."""
        # Placeholder implementation
        return []

    async def _check_therapeutic_concepts(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        """Check therapeutic concept consistency."""
        # Placeholder implementation
        return []

    async def _check_emotional_safety(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        """Check emotional safety of content."""
        # Placeholder implementation
        return []

    async def _check_therapeutic_progression(self, content: NarrativeContent) -> List[ConsistencyIssue]:
        """Check therapeutic progression appropriateness."""
        # Placeholder implementation
        return []
...
class ContradictionDetector:
    ...
    def _load_temporal_markers(self):
        ...
        self.temporal_markers = [
            ...
            "during", "while", "when", "since", "until", "ago", "later",
    ...
    def _load_causal_indicators(self):
        ...
        self.causal_indicators = [
            ...
            "consequently", "hence", "so", "caused by", "led to", "resulted in"
        ]
...
class CausalValidator:
    ...
    async def validate_causal_logic(self, narrative_branch: List[NarrativeContent]) -> ValidationResult:
        ...
        """
        try:
            logger.debug(f"Validating causal logic for branch with {len(narrative_branch)} elements")
            ...
            result.causal_consistency = self._calculate_causal_consistency_score(result.detected_issues)
            ...
            result.suggested_corrections = await self._generate_causal_corrections(result.detected_issues)
    ...
    def _load_logical_operators(self):
        ...
        self.logical_operators = [
            "if", "then", "because", "since", "therefore", "thus", "hence",
            "as a result", "consequently", "due to", "caused by", "led to"
        ]
    ...
    async def _validate_causal_chains(self, narrative_branch: List[NarrativeContent]) -> List[ConsistencyIssue]:
        ...
        try:
            ...
            for relationship in causal_relationships:
                relationship_issues = await self._validate_causal_relationship(relationship)
    ...
    async def _validate_logical_consistency(self, narrative_branch: List[NarrativeContent]) -> List[ConsistencyIssue]:
        ...
        try:
            ...
            impossibility_issues = await self._check_impossible_scenarios(narrative_branch)
    ...
    async def _validate_consequences(self, narrative_branch: List[NarrativeContent]) -> List[ConsistencyIssue]:
        ...
        try:
            ...
            proportionality_issues = await self._check_consequence_proportionality(narrative_branch)
            ...
            believability_issues = await self._check_consequence_believability(narrative_branch)
...
class NarrativeCoherenceEngine(Component):
    ...
    async def validate_narrative_consistency(self, content: NarrativeContent, session_id: str) -> ValidationResult:
        """
        ...
        try:
            ...
            # Perform validation
            result = await self.coherence_validator.validate_narrative_consistency(content)
            ...
            logger.debug(f"Validation completed: {len(result.detected_issues)} issues found")
            return result
    ...
    async def detect_contradictions(self, session_id: str) -> List[Contradiction]:
        ...
        try:
            ...
            contradictions = await self.contradiction_detector.detect_contradictions(content_history)

            logger.info(f"Detected {len(contradictions)} contradictions for session {session_id}")
            return contradictions
    ...
    async def get_coherence_status(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            latest_validation = await self.validate_narrative_consistency(latest_content, session_id)
    ...
    async def resolve_narrative_conflicts(self, conflicts: List[Contradiction], session_id: str) -> List['NarrativeResolution']:
        """
        ...
        """
        try:
            logger.info(f"Resolving {len(conflicts)} narrative conflicts for session {session_id}")

            resolutions = []

            for conflict in conflicts:
                logger.debug(f"Resolving conflict {conflict.contradiction_id}: {conflict.type}")

                # Generate creative solutions for the conflict
                creative_solutions = await self.generate_creative_solutions(conflict, session_id)

                # Select the best solution based on narrative cost and effectiveness
                best_solution = await self._select_best_solution(creative_solutions, conflict)

                if best_solution:
                    # Implement the resolution
                    resolution = await self._implement_conflict_resolution(best_solution, conflict, session_id)
                    if resolution:
                        resolutions.append(resolution)
                        logger.debug(f"Successfully resolved conflict {conflict.contradiction_id}")
                    else:
                        logger.warning(f"Failed to implement resolution for conflict {conflict.contradiction_id}")
                else:
                    logger.warning(f"No viable solution found for conflict {conflict.contradiction_id}")

            logger.info(f"Successfully resolved {len(resolutions)} out of {len(conflicts)} conflicts")
            return resolutions
    ...
    async def generate_creative_solutions(self, conflict: Contradiction, session_id: str) -> List['CreativeSolution']:
        """
        ...
        """
        try:
            logger.debug(f"Generating creative solutions for conflict {conflict.contradiction_id}")
            ...
            if conflict.type == "direct":
                ...
                solutions.extend(await self._generate_character_driven_solutions(conflict, content_history))
                solutions.extend(await self._generate_perspective_based_solutions(conflict, content_history))

            elif conflict.type == "temporal":
                # Temporal conflicts need time-based explanations
                solutions.extend(await self._generate_temporal_solutions(conflict, content_history))
                solutions.extend(await self._generate_memory_based_solutions(conflict, content_history))

            elif conflict.type == "causal":
                # Causal conflicts need logical bridging
                solutions.extend(await self._generate_causal_bridge_solutions(conflict, content_history))
                solutions.extend(await self._generate_hidden_factor_solutions(conflict, content_history))

            elif conflict.type == "implicit":
                # Implicit conflicts need subtle recontextualization
                solutions.extend(await self._generate_recontextualization_solutions(conflict, content_history))
                solutions.extend(await self._generate_subtext_solutions(conflict, content_history))

            # Add universal solution types that work for any conflict
            solutions.extend(await self._generate_universal_solutions(conflict, content_history))

            # Score and rank solutions
            for solution in solutions:
                solution.effectiveness_score = await self._calculate_solution_effectiveness(solution, conflict)
                solution.narrative_cost = await self._calculate_narrative_cost(solution, content_history)
                solution.player_impact = await self._calculate_player_impact(solution, session_id)

            # Sort by effectiveness and narrative cost
            solutions.sort(key=lambda s: (s.effectiveness_score, -s.narrative_cost), reverse=True)
    ...
    async def manage_retroactive_changes(self, changes: List['RetroactiveChange'], session_id: str) -> bool:
        """
        ...
        """
        try:
            logger.info(f"Managing {len(changes)} retroactive changes for session {session_id}")

            # Validate changes don't create new conflicts
            validation_result = await self._validate_retroactive_changes(changes, session_id)
            if not validation_result.is_valid:
                logger.warning("Retroactive changes would create new conflicts, aborting")
                return False

            # Generate in-world explanations for each change
            explanations = []
            for change in changes:
                explanation = await self._generate_in_world_explanation(change, session_id)
                if explanation:
                    explanations.append(explanation)
                else:
                    logger.warning(f"Could not generate explanation for change {change.change_id}")
                    return False

            # Apply changes with explanations
            for change, explanation in zip(changes, explanations):
                success = await self._apply_retroactive_change(change, explanation, session_id)
                if not success:
                    logger.error(f"Failed to apply retroactive change {change.change_id}")
                    return False
    ...
    async def validate_storyline_convergence(self, storylines: List['StorylineThread'], session_id: str) -> 'ConvergenceValidation':
        """
        ...
        """
        try:
            logger.debug(f"Validating convergence of {len(storylines)} storylines for session {session_id}")
            ...
            character_issues = await self._validate_cross_storyline_character_consistency(storylines)
            validation.integration_issues.extend(character_issues)

            # Validate thematic coherence
            thematic_issues = await self._validate_thematic_coherence_across_storylines(storylines)
            ...
            validation.convergence_score = await self._calculate_convergence_score(validation)
            validation.is_convergent = validation.convergence_score >= 0.7  # Threshold for acceptable convergence

            # Generate recommendations if convergence is poor
            if not validation.is_convergent:
                validation.recommended_adjustments = await self._generate_convergence_recommendations(validation, storylines)

            logger.debug(f"Convergence validation completed: score={validation.convergence_score:.2f}, convergent={validation.is_convergent}")
            return validation
    ...
    async def _select_best_solution(self, solutions: List[CreativeSolution], conflict: Contradiction) -> Optional[CreativeSolution]:
        ...
        for solution in solutions:
            ...
            score = (
                ...
                narrative_cost_weight * (1.0 - solution.narrative_cost) +  # Lower cost is better
                player_impact_weight * (1.0 - abs(solution.player_impact))  # Minimal impact is better
            )
        ...
        logger.debug(f"Selected solution {best_solution.solution_id} with score {best_score:.2f}")
        return best_solution

    async def _implement_conflict_resolution(self, solution: CreativeSolution, conflict: Contradiction, session_id: str) -> Optional[NarrativeResolution]:
        ...
        try:
            ...
            player_explanation = await self._generate_player_explanation(solution, conflict)
    ...
    async def _generate_character_driven_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _generate_perspective_based_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _generate_temporal_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _generate_memory_based_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _generate_causal_bridge_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _generate_hidden_factor_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _generate_recontextualization_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _generate_subtext_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _generate_universal_solutions(self, conflict: Contradiction, content_history: List[NarrativeContent]) -> List[CreativeSolution]:
    ...
    async def _calculate_solution_effectiveness(self, solution: CreativeSolution, conflict: Contradiction) -> float:
        ...
        if conflict.type == "direct" and solution.solution_type in ["character_driven", "perspective_based"]:
            compatibility_bonus = 0.3
        elif conflict.type == "temporal" and solution.solution_type in ["temporal", "memory_based"]:
            compatibility_bonus = 0.3
        elif conflict.type == "causal" and solution.solution_type in ["causal_bridge", "hidden_factor"]:
            compatibility_bonus = 0.3
        elif conflict.type == "implicit" and solution.solution_type in ["recontextualization", "subtext"]:
            compatibility_bonus = 0.3
    ...
    async def _calculate_narrative_cost(self, solution: CreativeSolution, content_history: List[NarrativeContent]) -> float:
        ...
        affected_content_ratio = len(solution.required_changes) / max(1, len(content_history))
        base_cost += affected_content_ratio * 0.2

        return min(1.0, max(0.0, base_cost))

    async def _calculate_player_impact(self, solution: CreativeSolution, session_id: str) -> float:
    ...
    async def _apply_implementation_step(self, step: str, session_id: str) -> Optional[str]:
    ...
    async def _generate_player_explanation(self, solution: CreativeSolution, conflict: Contradiction) -> str:
    ...
    async def _validate_retroactive_changes(self, changes: List[RetroactiveChange], session_id: str) -> ValidationResult:
        """Validate that retroactive changes don't create new conflicts."""
        try:
            logger.debug(f"Validating {len(changes)} retroactive changes for session {session_id}")
            ...
            simulated_history = await self._simulate_retroactive_changes(content_history, changes)
            ...
            if new_contradictions:
                ...
                original_contradictions = await self.detect_contradictions(content_history)
                original_ids = {c.contradiction_id for c in original_contradictions}

                genuinely_new = [c for c in new_contradictions if c.contradiction_id not in original_ids]

                if genuinely_new:
                    result.is_valid = False
                    result.consistency_score = max(0.0, 1.0 - (len(genuinely_new) * 0.2))

                    for contradiction in genuinely_new:
                        result.detected_issues.append(ConsistencyIssue(
                            issue_id=str(uuid.uuid4()),
                            issue_type=ConsistencyIssueType.CAUSAL_INCONSISTENCY,
                            severity=ValidationSeverity.ERROR,
            ...
            causal_issues = await self._validate_causal_consistency_after_changes(simulated_history, changes)
            result.detected_issues.extend(causal_issues)

            # Check for character consistency after changes
            character_issues = await self._validate_character_consistency_after_changes(simulated_history, changes)
            result.detected_issues.extend(character_issues)

            # Update overall score based on all issues
            if result.detected_issues:
                issue_penalty = len(result.detected_issues) * 0.1
                result.consistency_score = max(0.0, result.consistency_score - issue_penalty)
                result.is_valid = result.consistency_score >= 0.7

            logger.debug(f"Retroactive change validation completed: valid={result.is_valid}, score={result.consistency_score:.2f}")
            return result
    ...
    async def _generate_in_world_explanation(self, change: RetroactiveChange, session_id: str) -> Optional[str]:
    ...
    async def _apply_retroactive_change(self, change: RetroactiveChange, explanation: str, session_id: str) -> bool:
        """Apply a retroactive change with its explanation."""
        try:
            logger.debug(f"Applying retroactive change {change.change_id}: {explanation}")
            ...
            if not target_content:
                logger.error(f"Target content {change.target_content_id} not found for retroactive change")
                return False
            ...
            logger.info(f"Successfully applied retroactive change {change.change_id} to content {change.target_content_id}")
    ...
    async def _update_content_history_with_changes(self, changes: List[RetroactiveChange], session_id: str) -> None:
        """Update content history to reflect retroactive changes."""
        try:
            logger.debug(f"Updating content history with {len(changes)} retroactive changes")

            # Get current content history
            content_history = self.content_history.get(session_id, [])

            # Create a mapping of content IDs to their positions
            content_map = {content.content_id: i for i, content in enumerate(content_history)}
                ...
                if change.target_content_id in content_map:
                    ...
                    updated_metadata = {
                        **original_content.metadata,
                        "retroactive_changes": original_content.metadata.get("retroactive_changes", []) + [{
                            "change_id": change.change_id,
                    ...
                    logger.debug(f"Updated content {change.target_content_id} with retroactive change metadata")
                else:
                    logger.warning(f"Target content {change.target_content_id} not found in history")
            ...
            logger.info(f"Successfully updated content history with {len(changes)} retroactive changes")
    ...
    async def _detect_convergence_conflicts(self, storylines: List[StorylineThread]) -> List[str]:
        """Detect conflicts in storyline convergence."""
        conflicts = []

        # Check for character conflicts across storylines
        character_conflicts = await self._detect_character_conflicts_across_storylines(storylines)
        conflicts.extend(character_conflicts)

        # Check for thematic conflicts
        thematic_conflicts = await self._detect_thematic_conflicts_across_storylines(storylines)
        conflicts.extend(thematic_conflicts)

        # Check for pacing conflicts
        pacing_conflicts = await self._detect_pacing_conflicts_across_storylines(storylines)
        conflicts.extend(pacing_conflicts)

        return conflicts

    async def _identify_convergence_points(self, storylines: List[StorylineThread]) -> List[str]:
    ...
    async def _validate_cross_storyline_character_consistency(self, storylines: List[StorylineThread]) -> List[str]:
        """Validate character consistency across storylines."""
        issues = []
        ...
        try:
            ...
            for character, char_storylines in character_storylines.items():
                if len(char_storylines) > 1:
                    consistency_issues = await self._check_character_consistency_across_storylines(character, char_storylines)
    ...
    async def _validate_thematic_coherence_across_storylines(self, storylines: List[StorylineThread]) -> List[str]:
        ...
        try:
            ...
            conflicting_themes = await self._identify_conflicting_themes(list(all_themes))
            ...
            for theme1, theme2 in conflicting_themes:
                ...
                if storylines1.intersection(storylines2):
                    issues.append(f"Conflicting themes '{theme1}' and '{theme2}' appear in overlapping storylines")
            ...
            for theme, theme_storylines_list in theme_storylines.items():
                if len(theme_storylines_list) > 1:
                    consistency_issues = await self._check_thematic_consistency_across_storylines(theme, theme_storylines_list)
                    issues.extend(consistency_issues)
    ...
    async def _validate_convergence_pacing(self, storylines: List[StorylineThread]) -> List[str]:
        """Validate pacing balance in storyline convergence."""
        # Placeholder implementation
        return []

    async def _calculate_convergence_score(self, validation: ConvergenceValidation) -> float:
        """Calculate overall convergence score."""
        base_score = 1.0
    ...
    async def _generate_convergence_recommendations(self, validation: ConvergenceValidation, storylines: List[StorylineThread]) -> List[str]:
        """Generate recommendations for improving storyline convergence."""
        recommendations = []

        if len(validation.convergence_points) == 0:
            recommendations.append("Consider adding shared characters or themes to create natural convergence points")

        if len(validation.integration_issues) > 3:
            recommendations.append("Reduce the number of conflicting elements between storylines")

        if validation.convergence_score < 0.5:
            recommendations.append("Consider simplifying the convergence by focusing on fewer storylines")

        return recommendations

    # Placeholder helper methods for detailed conflict detection

    async def _detect_character_conflicts_across_storylines(self, storylines: List[StorylineThread]) -> List[str]:
        """Detect character conflicts across storylines."""
        return []

    async def _detect_thematic_conflicts_across_storylines(self, storylines: List[StorylineThread]) -> List[str]:
        """Detect thematic conflicts across storylines."""
        return []

    async def _detect_pacing_conflicts_across_storylines(self, storylines: List[StorylineThread]) -> List[str]:
        """Detect pacing conflicts across storylines."""
        return []

    # Helper methods for retroactive change validation

    async def _simulate_retroactive_changes(self, content_history: List[NarrativeContent], changes: List[RetroactiveChange]) -> List[NarrativeContent]:
                ...
                for i, content in enumerate(simulated_history):
                    if content.content_id == change.target_content_id:
                        if change.change_type == "modification":
                            ...
                            simulated_history[i].text += f"\n\n{change.modified_content}"
                        elif change.change_type == "recontextualization":
                            # Add context metadata
                            simulated_history[i].metadata["recontextualization"] = change.modified_content
                        break
    ...
    async def _validate_causal_consistency_after_changes(self, simulated_history: List[NarrativeContent], changes: List[RetroactiveChange]) -> List[ConsistencyIssue]:
        ...
        try:
            ...
            for change in changes:
                ...
                dependent_content = [
                    content for content in simulated_history
                ...
                for dependent in dependent_content:
                    ...
                    if await self._change_breaks_causal_dependency(change, dependent):
                        issues.append(ConsistencyIssue(
                            ...
                            affected_elements=[change.target_content_id, dependent.content_id],
                            confidence_score=0.7
                        ))
    ...
    async def _validate_character_consistency_after_changes(self, simulated_history: List[NarrativeContent], changes: List[RetroactiveChange]) -> List[ConsistencyIssue]:
                ...
                if changed_content:
                    ...
                    for character in changed_content.characters:
                        ...
                        if character_profile:
                            consistency_issues = await self._check_character_consistency_with_change(
                                changed_content, character, character_profile, change
                            )
                            issues.extend(consistency_issues)
    ...
    async def _change_breaks_causal_dependency(self, change: RetroactiveChange, dependent_content: NarrativeContent) -> bool:
    ...
    async def _check_character_consistency_with_change(self, content: NarrativeContent, character: str, profile: Dict[str, Any], change: RetroactiveChange) -> List[ConsistencyIssue]:
        ...
        try:
            ...
            if character in content.text.lower():
                ...
                character_issues = await self._check_personality_consistency(content, character, profile)
                ...
                for issue in character_issues:
                    ...
                    issue.description += f" (after retroactive change {change.change_id})"
    ...
    async def _notify_dependent_systems_of_changes(self, changes: List[RetroactiveChange], session_id: str) -> None:
        ...
        try:
            ...
            logger.debug(f"Notifying dependent systems of {len(changes)} retroactive changes for session {session_id}")
...
try:
    from .narrative_coherence.coherence_validator import CoherenceValidator as _ExtractedCoherenceValidator
    from .narrative_coherence.contradiction_detector import ContradictionDetector as _ExtractedContradictionDetector
    from .narrative_coherence.causal_validator import CausalValidator as _ExtractedCausalValidator
