...
class AgentOrchestrationComponent(Component):
    ...
    def _validate_configuration(self):
        ...
        try:
            from src.agent_orchestration.config_schema import validate_agent_orchestration_config

            # Extract agent orchestration config section
            ao_config_dict = {}
            for key in ["enabled", "port", "max_concurrent_workflows", "workflow_timeout",
                       "agents", "resources", "messaging", "monitoring", "diagnostics", "realtime"]:
            ...
            if validated_config.agents.auto_register:
                logger.warning("Global auto-registration is ENABLED - ensure this is intended for your environment")
            else:
                logger.info("Global auto-registration is disabled (secure default)")

            # Check per-agent auto-registration
            enabled_agents = []
            for agent_type in ["input_processor", "world_builder", "narrative_generator"]:
                if validated_config.agents.is_auto_registration_enabled(agent_type):
                    enabled_agents.append(agent_type)

            if enabled_agents:
                logger.warning(f"Per-agent auto-registration enabled for: {', '.join(enabled_agents)}")
            else:
                logger.info("Per-agent auto-registration disabled for all agents (secure default)")
    ...
    def _start_impl(self) -> bool:
        ...
        try:
            ...
            try:
                if bool(self.config.get("agent_orchestration.error_handling.enabled", True)):
                    ...
                    tt = float(self.config.get("agent_orchestration.workflow.timeouts.total_seconds", 300.0))
                    st = float(self.config.get("agent_orchestration.workflow.timeouts.per_step_seconds", 60.0))
                    ard = int(self.config.get("agent_orchestration.workflow.audit_retention_days", 30))
                    self._workflow_monitor = WorkflowMonitor(self._redis_client, key_prefix="ao", default_total_timeout_s=tt, default_step_timeout_s=st, audit_retention_days=ard)
                    # Background timeout checks
                    import asyncio as _aio
                    loop = _aio.get_event_loop()
                    if loop.is_running():
                        self._wf_timeout_task = loop.create_task(self._workflow_monitor.check_timeouts_once())  # one-shot kickoff
                    # Start periodic background checks only if explicitly enabled (default off for tests)
                    try:
                        iv = float(self.config.get("agent_orchestration.workflow.timeout_check_interval_s", 1.0))
                    except Exception:
                        iv = 1.0
                    if bool(self.config.get("agent_orchestration.workflow.background_checks_enabled", False)):
                        try:
                            self._workflow_monitor.start_background_checks(iv)
                        except Exception:
                            pass
                    # Periodic state validation
                    self._state_validator = StateValidator(self._redis_client, key_prefix="ao")
                    svi = float(self.config.get("agent_orchestration.workflow.state_validation_interval_s", 10.0))
            ...
            try:
                ...
                self._agent_router = AgentRouter(self._agent_registry, self._redis_client, getattr(self, "_tool_policy", None))
                ...
                try:
                    win = int(self.config.get("agent_orchestration.router.success_window_size", 100))
            ...
            if bool(self.config.get("agent_orchestration.workflow.auto_recover_on_start", False)):
                import asyncio
            ...
            if bool(self.config.get("agent_orchestration.metrics.background_polling_enabled", False)):
            ...
            try:
                from src.agent_orchestration.therapeutic_safety import SafetyRulesProvider, SafetyService
                key_prefix = str(self.config.get("agent_orchestration.tools.redis_key_prefix", "ao")) if isinstance(getattr(self, "_tools_cfg", None), dict) else str(self.config.get("agent_orchestration.tools", {}).get("redis_key_prefix", "ao"))
                redis_key = f"{key_prefix}:safety:rules"
                enabled = bool(self.config.get("agent_orchestration.safety.enabled", False))
                provider = SafetyRulesProvider(redis_client=self._redis_client, redis_key=redis_key)
            ...
            if bool(self.config.get("agent_orchestration.realtime.events.enabled", False)):
                try:
                    ...
                    self._event_publisher = EventPublisher(
                        ...
                        enabled=True,
                        buffer_size=int(self.config.get("agent_orchestration.realtime.events.buffer_size", 1000)),
                        broadcast_agent_status=bool(self.config.get("agent_orchestration.realtime.events.broadcast_agent_status", True)),
                        broadcast_workflow_progress=bool(self.config.get("agent_orchestration.realtime.events.broadcast_workflow_progress", True)),
                        broadcast_system_metrics=bool(self.config.get("agent_orchestration.realtime.events.broadcast_system_metrics", False)),
                    )
                    logger.info("Event publisher initialized for real-time communication")
                except Exception as e:
            ...
            if bool(self.config.get("agent_orchestration.realtime.progressive_feedback.enabled", False)):
                try:
                    from src.agent_orchestration.realtime.progressive_feedback import ProgressiveFeedbackManager
                    self._feedback_manager = ProgressiveFeedbackManager(
                        event_publisher=self._event_publisher,
                        update_interval=float(self.config.get("agent_orchestration.realtime.progressive_feedback.update_interval", 1.0)),
                        max_updates_per_operation=int(self.config.get("agent_orchestration.realtime.progressive_feedback.max_updates_per_workflow", 100)),
                        stream_intermediate_results=bool(self.config.get("agent_orchestration.realtime.progressive_feedback.stream_intermediate_results", True)),
                    )
                    logger.info("Progressive feedback manager initialized")
                except Exception as e:
                    logger.warning(f"Failed to initialize progressive feedback manager: {e}")
            ...
            if bool(self.config.get("agent_orchestration.realtime.enabled", False)):
                try:
                    from src.agent_orchestration.realtime.workflow_progress import WorkflowProgressTracker
                    self._workflow_tracker = WorkflowProgressTracker(
                        event_publisher=self._event_publisher,
                        update_interval=float(self.config.get("agent_orchestration.realtime.progressive_feedback.update_interval", 2.0)),
                        auto_publish_updates=True,
                    )
                    logger.info("Workflow progress tracker initialized")
                except Exception as e:
                    logger.warning(f"Failed to initialize workflow progress tracker: {e}")
            ...
            if bool(self.config.get("agent_orchestration.optimization.enabled", False)):
                try:
                    ...
                    self._response_time_collector = ResponseTimeCollector(
                        max_metrics_per_operation=int(self.config.get("agent_orchestration.optimization.max_metrics_per_operation", 1000)),
                        cleanup_interval=float(self.config.get("agent_orchestration.optimization.cleanup_interval", 300.0)),
                        metric_retention_hours=float(self.config.get("agent_orchestration.optimization.metric_retention_hours", 24.0)),
                    )

                    # Initialize optimization engine
                    enabled_strategies_config = self.config.get("agent_orchestration.optimization.enabled_strategies", ["conservative"])
                    enabled_strategies = [OptimizationStrategy(s) for s in enabled_strategies_config]

                    self._optimization_engine = OptimizationEngine(
                        response_time_collector=self._response_time_collector,
                        event_publisher=self._event_publisher,
                        optimization_interval=float(self.config.get("agent_orchestration.optimization.optimization_interval", 300.0)),
                        enabled_strategies=enabled_strategies,
                        max_adjustments_per_cycle=int(self.config.get("agent_orchestration.optimization.max_adjustments_per_cycle", 3)),
                    )
                    ...
                    self._resource_manager = WorkflowResourceManager(
                        ...
                        response_time_collector=self._response_time_collector,
                        event_publisher=self._event_publisher,
                        max_concurrent_workflows=int(self.config.get("agent_orchestration.optimization.max_concurrent_workflows", 10)),
                        resource_monitoring_interval=float(self.config.get("agent_orchestration.optimization.resource_monitoring_interval", 30.0)),
                    )
                    ...
                    self._performance_analytics = PerformanceAnalytics(
                        ...
                        analytics_interval=float(self.config.get("agent_orchestration.optimization.analytics_interval", 60.0)),
                    )
            ...
            try:
                ...
                enable_events = bool(self.config.get("agent_orchestration.realtime.events.enabled", False))
                ...
                if hasattr(self, "_feedback_manager") and self._feedback_manager:
                    try:
                        ...
                        logger.warning(f"Failed to start progressive feedback manager: {e}")
                ...
                if hasattr(self, "_workflow_tracker") and self._workflow_tracker:
                    try:
                        ...
                        logger.warning(f"Failed to start workflow progress tracker: {e}")

                # Start optimization services
                if hasattr(self, "_response_time_collector") and self._response_time_collector:
                    try:
                ...
                if hasattr(self, "_performance_analytics") and self._performance_analytics:
                    try:
                ...
                async def _restart_agent(agent: Agent) -> bool:
                    try:
                        ...
                        try:
                            ...
                            pass
                        try:
                            self._restarts_total = int(getattr(self, "_restarts_total", 0)) + 1
                        except Exception:
                            pass
                        try:
                            if bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
                                import time as _t
                                ev = {"ts": _t.time(), "event": "restart", "agent": getattr(agent, "name", None)}
                ...
                async def _fallback_route(unhealthy: Agent) -> bool:
                    try:
                        ...
                        try:
                            self._fallbacks_total = int(getattr(self, "_fallbacks_total", 0)) + 1
                        except Exception:
                            pass
                        try:
                            if bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
                                import time as _t
                                ev = {"ts": _t.time(), "event": "fallback", "from": getattr(unhealthy, "name", None), "to": getattr(backups[0], "name", None)}
                                self._ao_events.append(ev); self._ao_events = self._ao_events[-500:]
                        except Exception:
                            pass
                        return True
                ...
                fd_enabled = bool(self.config.get("agent_orchestration.monitoring.failure_detection_enabled", True))
                fd_interval = float(self.config.get("agent_orchestration.monitoring.failure_detection_interval", max(5.0, float(self.config.get("agent_orchestration.monitoring.health_check_interval", 15)))))
                if fd_enabled:
                    self._fd_task = loop.create_task(self._failure_detection_loop(fd_interval))
            except Exception:
                ...
                try:
                    ...
                    from src.agent_orchestration.agents import Agent, AgentRegistry  # type: ignore
                    ...
                    async def _restart_agent(agent: Agent) -> bool:
                        ...
                        try:
                            ...
                            logger.info("Agent restart attempted: %s", getattr(agent, "name", "unknown"))
                            return True
                        except Exception as e:
                            logger.warning("Agent restart failed: %s", e)
                            return False
                    ...
                    async def _fallback_route(unhealthy: Agent) -> bool:
                        ...
                        try:
                            ...
                            backups = [a for a in reg.all() if a.agent_id.type == t and a is not unhealthy and a._running and not a._degraded]
                            if not backups:
                                return False
                            ...
                            try:
                                ...
                                pass
                            logger.warning("Fallback activated for %s -> %s", unhealthy.name, backup.name)
                            return True
                        except Exception:
                            return False
                    ...
                    fd_enabled = bool(self.config.get("agent_orchestration.monitoring.failure_detection_enabled", True))
                    fd_interval = float(self.config.get("agent_orchestration.monitoring.failure_detection_interval", max(5.0, float(self.config.get("agent_orchestration.monitoring.health_check_interval", 15)))))
                    diag_enabled = bool(self.config.get("agent_orchestration.diagnostics.enabled", False))
            ...
            try:
                ...
                if self._validated_config:
                    ...
                    auto_register_enabled = bool(self.config.get("agent_orchestration.agents.auto_register", False))
                ...
                if auto_register_enabled:
                    ...
                    def _inst(agent_type: str) -> str:
                        ...
                        if self._validated_config:
                            agent_config = self._validated_config.agents.get_agent_config(agent_type)
                            if agent_config and agent_config.instance:
                                return agent_config.instance

                        # Fallback to raw config
                        explicit = self.config.get(f"agent_orchestration.agents.{agent_type}.instance")
                        if explicit:
                            return str(explicit)
                    ...
                    def _should_register_agent(agent_type: str) -> bool:
                        ...
                        if self._validated_config:
                            return self._validated_config.agents.is_auto_registration_enabled(agent_type)
                        else:
                            # Fallback to raw config - both enabled and auto_register_enabled must be true
                            enabled = bool(self.config.get(f"agent_orchestration.agents.{agent_type}.enabled", False))
                            auto_reg = bool(self.config.get(f"agent_orchestration.agents.{agent_type}.auto_register_enabled", False))
                            return enabled and auto_reg

                    # Register Input Processor Agent
                    if _should_register_agent("input_processor"):
                        ipa = InputProcessorAgentProxy(coordinator=self._message_coordinator, instance=_inst("input_processor"))
                        if loop.is_running():
                    ...
                    if _should_register_agent("world_builder"):
                        wba = WorldBuilderAgentProxy(coordinator=self._message_coordinator, instance=_inst("world_builder"))
                        if loop.is_running():
                    ...
                    if _should_register_agent("narrative_generator"):
                        nga = NarrativeGeneratorAgentProxy(coordinator=self._message_coordinator, instance=_inst("narrative_generator"))
                        if loop.is_running():
            ...
            if bool(self.config.get("agent_orchestration.diagnostics.enabled", False)) and bool(self.config.get("agent_orchestration.diagnostics.start_server", False)):
                self._start_diagnostics_server(loop)
            elif bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
                logger.info("Diagnostics enabled; server not started (agent_orchestration.diagnostics.start_server=false)")
            else:
    ...
    def _stop_impl(self) -> bool:
        ...
        try:
                        ...
                        if agents_list:
                            if loop.is_running():
                                ...
                                for agent in agents_list:
                                    try:
                                        fut = _asyncio.run_coroutine_threadsafe(agent.stop(), loop)
                                        futures.append(fut)
                                    except Exception:
                                        pass
                                for fut in futures:
            ...
            try:
                if hasattr(self, "_orchestration_service") and self._orchestration_service:
                    loop = asyncio.get_event_loop()
                    if loop.is_running():
    ...
    def _create_diagnostics_app(self):
        ...
        try:
            ...
            if extra_allow:
                try:
                    ...
                    merged_allow = list(set((merged.get("callable_allowlist") or []) + extra_allow))
                    merged["callable_allowlist"] = merged_allow
                    # Reconstruct model
                    from src.agent_orchestration.tools.policy_config import ToolPolicyConfig as _TPC
            ...
            if self._policy_cfg_path and bool(self.config.get("agent_orchestration.diagnostics.policy_live_reload_enabled", False)):
                try:
                    import time as _t
                    from src.agent_orchestration.tools.policy_config import _load_from_file, validate_tool_policy_config, ToolPolicyConfig as _TPC
                    def _watcher():
                        while True:
                            try:
                                _t.sleep(float(self.config.get("agent_orchestration.diagnostics.policy_live_reload_interval_s", 2.0)))
                                try:
                                ...
                                if mtime and mtime != self._policy_cfg_mtime:
                                    ...
                                    try:
                                        self._policy_reload_audit.append({"ts": _t.time(), "ok": ok, "source": self._policy_cfg_path, "error": err})
                                        # keep last 10
                                        self._policy_reload_audit = self._policy_reload_audit[-10:]
                                        # log at most once per change to avoid noisy loops on malformed files
                                        if ok:
                                            logger.info("Policy live-reload applied from %s", self._policy_cfg_path)
                                        else:
        ...
        async def metrics() -> dict:
            ...
            try:
                if bool(self.config.get("agent_orchestration.diagnostics.enabled", False)) and bool(self.config.get("agent_orchestration.error_handling.enabled", True)):
            ...
            try:
                ...
                if pol_cfg is not None:
                    ...
                    data["policy"] = redact_policy_config_dict({
                        "callable_allowlist": list(getattr(self._tool_policy, "allowed_callables", [])),
                        "allow_network_tools": bool(getattr(self._tool_policy, "allow_network_tools", False)),
                        "allow_filesystem_tools": bool(getattr(self._tool_policy, "allow_filesystem_tools", False)),
                        "allow_process_tools": bool(getattr(self._tool_policy, "allow_process_tools", False)),
                    })
            except Exception:
                pass
        ...
        async def metrics_prometheus() -> str:
            ...
            try:
                if bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
                    ...
                    if reg and isinstance(reg, AgentRegistry):  # type: ignore
                        ...
                        for d in snap_agents.values():
                            ...
                            if st not in ("IDLE", "BUSY", "healthy", "initializing"):
                                unhealthy += 1
                    # Expose as a gauge
                    g_unhealthy = Gauge("agent_orchestration_agents_unhealthy", "Number of unhealthy agents", registry=prom_reg)
                    ...
                    c_restarts = Counter("agent_orchestration_agents_restarts_total", "Total agent restarts attempted", registry=prom_reg)
                    c_fallbacks = Counter("agent_orchestration_agents_fallbacks_total", "Total agent fallbacks attempted", registry=prom_reg)
                    # Increment to absolute values (counter inc by delta)
                    last_rf = getattr(self, "_prom_last_rf", {"restarts": 0, "fallbacks": 0})
                    inc_r = max(0, restarts_total - last_rf.get("restarts", 0))
                    inc_f = max(0, fallbacks_total - last_rf.get("fallbacks", 0))
                    if inc_r: c_restarts.inc(inc_r)
                    if inc_f: c_fallbacks.inc(inc_f)
                    self._prom_last_rf = {"restarts": restarts_total, "fallbacks": fallbacks_total}
            except Exception:
                pass
        ...
        async def routing_preview(agent_type: str, exclude_degraded: bool = True, show_all_candidates: bool = False) -> dict:
            if not bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
            ...
            try:
                ...
                preview = await router.preview(at, exclude_degraded=bool(exclude_degraded), show_all_candidates=bool(show_all_candidates))
                ...
                preview["configured_heartbeat_fresh_seconds"] = float(rcfg.get("heartbeat_fresh_seconds", 30.0))
        ...
        async def policy_snapshot() -> dict:
            try:
                ...
                pol_cfg = getattr(self._tool_policy, "config", None)
                snap = {
                    "policy": redact_policy_config_dict(pol_cfg.model_dump() if pol_cfg else {
                        "callable_allowlist": list(getattr(self._tool_policy, "allowed_callables", [])),
                        "allow_network_tools": bool(getattr(self._tool_policy, "allow_network_tools", False)),
                        "allow_filesystem_tools": bool(getattr(self._tool_policy, "allow_filesystem_tools", False)),
                        "allow_process_tools": bool(getattr(self._tool_policy, "allow_process_tools", False)),
                ...
                # attach audit log
                try:
                    snap["reload_audit"] = list(getattr(self, "_policy_reload_audit", []))
                except Exception:
                    pass
                return snap
            except Exception as e:
        ...
        async def policy_reload(x_ao_diag_key: str | None = Header(default=None, alias="X-AO-DIAG-KEY")) -> dict:
            if not bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
                return {"error": "diagnostics disabled"}
            ...
            try:
                import os, time
                from src.agent_orchestration.tools.policy_config import load_tool_policy_config, load_tool_policy_config_from, ToolPolicyConfig as _TPC
                ...
                with getattr(self, "_policy_lock", None) or threading.Lock():
                    if isinstance(new_cfg, _TPC):
                        ...
                        ok = True
                        try:
                            self._policy_cfg_mtime = os.path.getmtime(cfg_path) if cfg_path and os.path.exists(cfg_path) else None
                        except Exception:
                            pass
                try:
                    self._policy_reload_audit.append({"ts": time.time(), "ok": ok, "source": cfg_path or "env", "error": err})
        ...
        async def policy_validate(payload: dict | None = None, x_ao_diag_key: str | None = Header(default=None, alias="X-AO-DIAG-KEY")) -> dict:
            if not bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
                return {"error": "diagnostics disabled"}
            api_key = self.config.get("agent_orchestration.diagnostics.admin_api_key")
            if api_key and (x_ao_diag_key != api_key):
                ...
                return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401)
            try:
                from src.agent_orchestration.tools.policy_config import validate_tool_policy_config
        ...
        async def workflows_rollback(run_id: str, payload: dict | None = None, x_ao_diag_key: str | None = Header(default=None, alias="X-AO-DIAG-KEY")) -> dict:
            ...
            if api_key and (x_ao_diag_key != api_key):
                ...
                return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401)
            try:
                if not bool(self.config.get("agent_orchestration.error_handling.enabled", True)):
                ...
                redis_url = self.config.get("player_experience.api.redis_url", "redis://localhost:6379/0")
                rclient = aioredis.from_url(redis_url)
                tx = WorkflowTransaction(rclient, key_prefix=str(self.config.get("agent_orchestration.tools.redis_key_prefix", "ao")))
        ...
        async def workflows_rollback_history(run_id: str, x_ao_diag_key: str | None = Header(default=None, alias="X-AO-DIAG-KEY")) -> dict:
            if not bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
                return {"error": "diagnostics disabled"}
            api_key = self.config.get("agent_orchestration.diagnostics.admin_api_key")
            if api_key and (x_ao_diag_key != api_key):
        ...
        try:
            ...
            if _safety is None:
                from src.agent_orchestration.therapeutic_safety import get_global_safety_service
                ...
                try:
                    ...
                    try:
                        ...
                        if b:
                            ...
                            if prov is not None:
                                try:
                                    setattr(prov, "_cached_raw", raw)
                                    setattr(prov, "_cached_at", _t.time())
                                    setattr(prov, "_last_source", f"redis:{redis_key}")
                                except Exception:
                                    pass
                    except Exception:
                        cfg = {}
            ...
            async def safety_reload(x_ao_diag_key: str | None = Header(default=None, alias="X-AO-DIAG-KEY")) -> dict:
                if not bool(self.config.get("agent_orchestration.diagnostics.enabled", False)):
                    return {"error": "diagnostics disabled"}
                api_key = self.config.get("agent_orchestration.diagnostics.admin_api_key")
                if api_key and (x_ao_diag_key != api_key):
                    ...
                    return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401)
                try:
                    import asyncio as _asyncio, time as _t, json as _json
                    ...
                    async def _do_reload():
                        ...
                        try:
                            setattr(_safety, "_validator", None)
                        except Exception:
                            pass
                    ...
                    return {"ok": True, "status": getattr(prov, "status", lambda: {})() if prov else {}}
                except Exception as e:
        ...
        async def policy_status() -> dict:
            try:
                ...
                return {
                    "source": src,
                    "last_reload_ts": (getattr(self, "_policy_reload_audit", []) or [{}])[-1].get("ts") if getattr(self, "_policy_reload_audit", None) else None,
                    "live_reload": bool(self.config.get("agent_orchestration.diagnostics.policy_live_reload_enabled", False)),
                }
            except Exception as e:
                return {"error": str(e)}
        ...
        if bool(self.config.get("agent_orchestration.diagnostics.allow_tool_execution", False)):
            from fastapi import Header
            ...
            async def tools_execute(payload: dict, x_ao_diag_key: str | None = Header(default=None, alias="X-AO-DIAG-KEY")) -> dict:
                ...
                try:
                    name = str(payload.get("tool_name")); version = payload.get("version"); args = payload.get("arguments") or {}
                    if not name:
        ...
        if bool(self.config.get("agent_orchestration.realtime.enabled", False)) and bool(self.config.get("agent_orchestration.realtime.websocket.enabled", False)):
            ...
            if not hasattr(self, "_ws_connection_manager"):
                ...
                if hasattr(self, "_event_publisher") and self._event_publisher:
                    self._event_publisher.add_websocket_manager(self._ws_connection_manager)
        ...
        try:
            ...
            if hasattr(self, "_message_coordinator"):
                self._optimization_engine.register_parameter(
                    ...
                    current_value=getattr(self._message_coordinator, "_queue_size", 10000),
                    min_value=1000,
                    max_value=50000,
                    step_size=1000,
                    parameter_type="int",
            ...
            if hasattr(self, "_agent_registry"):
                heartbeat_ttl = float(self.config.get("agent_orchestration.agents.heartbeat_ttl", 30.0))
                self._optimization_engine.register_parameter(
    ...
    def _update_heartbeat_interval(self, parameter_name: str, new_value: Any) -> None:
        ...
        try:
            ...
            logger.info(f"Optimization suggests updating {parameter_name} to {new_value}")
        except Exception as e:
            logger.error(f"Failed to update {parameter_name}: {e}")
