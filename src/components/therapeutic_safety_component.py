...
class TherapeuticSafetyComponent(Component):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Core components
        self.orchestrator: SafetyValidationOrchestrator | None = None
        self.content_validator: ContentSafetyValidator | None = None
        self.crisis_detector: CrisisDetectionEngine | None = None
        self.bias_detector: BiasDetectionValidator | None = None
        self.alignment_validator: TherapeuticAlignmentValidator | None = None

        # Caching
        self.validation_cache: ValidationCache | None = None
        self.safety_cache: SafetyResultCache | None = None
    ...
    async def _initialize_caching(self) -> None:
        ...
        try:
            ...
            self.validation_cache = ValidationCache(
                redis_client=redis_client, key_prefix=cache_prefix
            )
    ...
    async def _initialize_orchestrator(self) -> None:
        ...
        self.orchestrator = SafetyValidationOrchestrator(
            event_bus=self.event_bus, cache_manager=self.validation_cache
        )
        ...
        self.orchestrator.default_timeout_ms = orchestrator_config.get(
            "timeout_ms", 200
        )
        self.orchestrator.max_concurrent_validations = orchestrator_config.get(
            "max_concurrent", 100
        )
    ...
    async def validate_user_input(
        ...
        **context_kwargs,
        ...
        content = ContentPayload(
            content_text=user_input, content_type="user_input", source="user_generated"
        )

        # Create validation context
        context = ValidationContext(
            user_id=user_id, session_id=session_id, **context_kwargs
        )
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        if self.alignment_validator:
            validator_metrics["therapeutic_alignment"] = (
                self.alignment_validator.get_metrics()
            )
