...
try:
    from core.character_development_system import (
        CharacterDevelopmentSystem, Interaction, PersonalityTrait
    )
    from models.data_models import CharacterState, MemoryFragment, DialogueStyle
except ImportError as e:
    logging.warning(f"Could not import tta.prototype modules: {e}")
    # Create mock classes for development
    class CharacterDevelopmentSystem:
        def __init__(self): pass
        def get_character_state(self, character_id: str): return None
        def update_character_from_interaction(self, character_id: str, interaction): return True
        def create_character(self, character_id: str, name: str, **kwargs): return None
...
from .character_arc_manager import (
    CharacterArcManagerComponent, CharacterArc, PlayerInteraction, 
    InteractionContext, ArcStage, RelationshipState, RelationshipType
)
...
class CharacterDataSynchronizer:
    ...
    async def sync_character_to_development_system(self, character_arc: CharacterArc) -> bool:
        """Sync CharacterArc data to Character Development System."""
        try:
            logger.debug(f"Syncing character arc {character_arc.character_id} to development system")
            
            # Get existing character state or create new one
            character_state = self.character_development_system.get_character_state(character_arc.character_id)
            ...
            if not character_state:
                ...
                character_state = self.character_development_system.create_character(
                    ...
                    therapeutic_role=character_arc.metadata.get("therapeutic_role", "companion")
                )
                
                if not character_state:
                    logger.error(f"Failed to create character {character_arc.character_id} in development system")
                    return False
    ...
    async def sync_character_from_development_system(self, character_arc: CharacterArc) -> bool:
        """Sync data from Character Development System to CharacterArc."""
        try:
            logger.debug(f"Syncing character {character_arc.character_id} from development system")
            
            character_state = self.character_development_system.get_character_state(character_arc.character_id)
            if not character_state:
                logger.warning(f"Character {character_arc.character_id} not found in development system")
                return False
            ...
            if player_relationship and "player" in character_state.relationship_scores:
                ...
                player_relationship.trust_level = max(0.0, min(1.0, (dev_score + 1.0) / 2.0))
                player_relationship.affection_level = max(0.0, min(1.0, (dev_score + 1.0) / 2.0))
            
            # Update last sync timestamp
            character_arc.last_updated = datetime.now()
            
            logger.debug(f"Successfully synced character {character_arc.character_id} from development system")
            return True
    ...
    async def _sync_personality_traits(self, character_arc: CharacterArc, character_state: CharacterState) -> None:
        ...
        try:
            ...
            logger.debug(f"Synced {len(character_arc.personality_evolution)} personality traits")
            
        except Exception as e:
            logger.error(f"Error syncing personality traits: {e}")
    
    async def _sync_relationship_data(self, character_arc: CharacterArc, character_state: CharacterState) -> None:
        """Sync relationship data between systems."""
        try:
            # Convert our relationship states to development system relationship scores
            for character_id, relationship_state in character_arc.relationship_dynamics.items():
                ...
                character_state.relationship_scores[character_id] = max(-1.0, min(1.0, overall_score))
            
            logger.debug(f"Synced {len(character_arc.relationship_dynamics)} relationships")
            
        except Exception as e:
            logger.error(f"Error syncing relationship data: {e}")
    
    async def _sync_memory_data(self, character_arc: CharacterArc, character_state: CharacterState) -> None:
        """Sync memory data between systems."""
        try:
            # Convert character history to memory fragments
            for history_entry in character_arc.character_history[-10:]:  # Last 10 interactions
                memory_content = f"Interaction: {history_entry.get('player_choice', 'Unknown choice')}"
                emotional_weight = sum(history_entry.get('emotional_impact', {}).values()) / max(1, len(history_entry.get('emotional_impact', {})))
            ...
            logger.debug(f"Synced memory data for character {character_arc.character_id}")
...
class RelationshipDynamicsManager:
    ...
    async def evolve_relationships_across_scales(self, character_arc: CharacterArc, 
        ...
        try:
            ...
            for player_interaction in interactions:
                dev_interaction = Interaction(
                    ...
                    emotional_impact=sum(player_interaction.emotional_impact.values()) / max(1, len(player_interaction.emotional_impact)),
            ...
            for interaction in dev_interactions:
                success = self.character_development_system.update_character_from_interaction(
                    character_arc.character_id, interaction
                )
                if not success:
                    logger.warning(f"Failed to update character {character_arc.character_id} in development system")
            ...
            for interaction in interactions:
                ...
                for character_id, changes in interaction.relationship_changes.items():
                    ...
                    relationship_changes[character_id] += sum(changes.values()) if isinstance(changes, dict) else changes
            
            logger.debug(f"Evolved relationships for character {character_arc.character_id}")
    ...
    async def _update_arc_relationships(self, character_arc: CharacterArc, interaction: PlayerInteraction) -> None:
        ...
        try:
            ...
            for metric, change in interaction.relationship_changes.items():
                if hasattr(relationship, metric):
                    ...
                    new_value = max(0.0, min(1.0, current_value + change * self.relationship_evolution_rate))
            ...
            logger.debug(f"Updated relationship for character {character_arc.character_id}")
            
        except Exception as e:
            logger.error(f"Error updating arc relationships: {e}")
    
    async def calculate_relationship_compatibility(self, character_arc: CharacterArc, 
                                                 other_character_id: str) -> float:
        """Calculate relationship compatibility between characters."""
        try:
            # Get character states from development system
            char1_state = self.character_development_system.get_character_state(character_arc.character_id)
            char2_state = self.character_development_system.get_character_state(other_character_id)
            ...
            compatibility = self.character_development_system.personality_manager.calculate_personality_compatibility(
                char1_state.personality_traits,
                char2_state.personality_traits
            )
...
class CharacterArcIntegration:
    ...
    def __init__(self, character_arc_manager: CharacterArcManagerComponent):
        ...
        self.data_synchronizer = CharacterDataSynchronizer(self.character_development_system)
        self.relationship_manager = RelationshipDynamicsManager(self.character_development_system)
    ...
    async def initialize_character_integration(self, character_id: str, 
                                             base_personality: Dict[str, Any]) -> Tuple[CharacterArc, Any]:
        ...
        try:
            ...
            # Initial sync
            await self.data_synchronizer.sync_character_to_development_system(character_arc)
    ...
    async def process_integrated_interaction(self, character_id: str, 
                                           player_interaction: PlayerInteraction) -> bool:
        """Process an interaction through both systems with synchronization."""
        try:
            logger.debug(f"Processing integrated interaction for character {character_id}")
            ...
            # Update development system
            dev_success = self.character_development_system.update_character_from_interaction(
            ...
            if character_arc:
                sync_success = await self.data_synchronizer.sync_character_to_development_system(character_arc)
            else:
                sync_success = False
            ...
            if success:
                logger.debug(f"Successfully processed integrated interaction for character {character_id}")
            else:
                logger.warning(f"Partial failure processing interaction for character {character_id}")
            
            return success
            
        except Exception as e:
            logger.error(f"Error processing integrated interaction for character {character_id}: {e}")
            return False
    
    async def generate_integrated_character_response(self, character_id: str, 
        ...
        try:
            ...
            dev_context = self.character_development_system.generate_character_dialogue_context(
                character_id, dialogue_context
            )
            
            # Validate consistency between systems
            character_state = self.character_development_system.get_character_state(character_id)
            if character_state:
                is_consistent, consistency_message = self.character_development_system.validate_character_consistency(
                    character_id, arc_response.response_text, dev_context
                )
                
                if not is_consistent:
                    logger.warning(f"Consistency issue for character {character_id}: {consistency_message}")
            ...
            logger.error(f"Error generating integrated response for character {character_id}: {e}")
            return {
    ...
    async def sync_character_data(self, character_id: str, force_sync: bool = False) -> bool:
        """Synchronize character data between systems."""
        try:
            # Check if sync is needed
            last_sync = self.last_sync_times.get(character_id)
            if not force_sync and last_sync and (datetime.now() - last_sync) < self.sync_interval:
            ...
            sync_to_success = await self.data_synchronizer.sync_character_to_development_system(character_arc)
            
            # Sync from development system (to get any updates)
            sync_from_success = await self.data_synchronizer.sync_character_from_development_system(character_arc)
    ...
    async def evolve_integrated_relationships(self, character_id: str, 
                                            interactions: List[PlayerInteraction],
                                            time_period: timedelta = None) -> Dict[str, Any]:
        """Evolve relationships across both systems."""
        try:
            logger.debug(f"Evolving integrated relationships for character {character_id}")
            ...
            relationship_changes = await self.relationship_manager.evolve_relationships_across_scales(
            ...
            if character_state:
                for other_char_id, score in character_state.relationship_scores.items():
                    relationship_summary[other_char_id] = {
                        "score": score,
                        "description": self.character_development_system.relationship_tracker.get_relationship_description(score),
            ...
            logger.debug(f"Successfully evolved relationships for character {character_id}")
            return result
            
        except Exception as e:
            logger.error(f"Error evolving integrated relationships for character {character_id}: {e}")
            return {}
    ...
    async def get_integrated_character_summary(self, character_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            dev_summary = self.character_development_system.get_character_development_summary(character_id)
            
            # Combine data
            integrated_summary = {
                "character_id": character_id,
                "arc_manager_data": {
                    "current_stage": character_arc.current_stage.value if character_arc else None,
                    "completed_milestones": len(character_arc.completed_milestones) if character_arc else 0,
                    "active_goals": len(character_arc.current_goals) if character_arc else 0,
                    "therapeutic_modeling": len(character_arc.therapeutic_modeling) if character_arc else 0,
                    "relationship_count": len(character_arc.relationship_dynamics) if character_arc else 0
                },
                "development_system_data": dev_summary,
                "integration_status": {
                    "last_sync": self.last_sync_times.get(character_id),
                    "systems_connected": character_arc is not None and bool(dev_summary),
                    "data_consistency": await self._check_data_consistency(character_id)
                },
            ...
            logger.error(f"Error getting integrated summary for character {character_id}: {e}")
            return {"error": str(e)}
    ...
    async def _check_data_consistency(self, character_id: str) -> Dict[str, bool]:
        ...
        try:
            ...
            consistency_checks = {
                "personality_traits": self._check_personality_consistency(character_arc, character_state),
                "relationships": self._check_relationship_consistency(character_arc, character_state),
    ...
    def _check_personality_consistency(self, character_arc: CharacterArc, character_state) -> bool:
    ...
    def _check_relationship_consistency(self, character_arc: CharacterArc, character_state) -> bool:
        ...
        try:
            ...
            if player_relationship and player_score is not None:
                ...
                arc_overall = (player_relationship.trust_level + player_relationship.affection_level) / 2.0
