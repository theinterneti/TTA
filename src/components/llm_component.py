...
class LLMComponent(Component):
    ...
    def __init__(self, config: Any, repository: str = "tta.dev"):
        ...
        self.model = self.config.get(
            f"{repository}.components.llm.model", "qwen2.5-7b-instruct"
        )
        self.api_base = self.config.get(
            f"{repository}.components.llm.api_base", "http://localhost:1234/v1"
        )
        self.use_gpu = self.config.get("docker.use_gpu", False)

        logger.info(
            f"Initialized LLM component for {repository} with model {self.model}"
        )

    @log_entry_exit
    @timing_decorator
    @require_config(["tta.dev.components.llm.model", "tta.dev.components.llm.api_base"])
    def _start_impl(self) -> bool:
        ...
        try:
            logger.info(
                f"Starting LLM service using Docker Compose in {self.repository}"
            )
            ...
            logger.error(
                f"Timed out waiting for LLM service to start at {self.api_base}"
            )
            return False
    ...
    def _stop_impl(self) -> bool:
        ...
        try:
            logger.info(
                f"Stopping LLM service using Docker Compose in {self.repository}"
            )
            ...
            logger.error(
                f"Timed out waiting for LLM service to stop at {self.api_base}"
            )
            return False
    ...
    def _run_docker_compose(
        ...
        full_command = [
            "docker-compose",
            "-f",
            str(self.repo_dir / "docker-compose.yml"),
        ] + command
