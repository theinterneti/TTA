...
try:
    from components.character_arc_manager import CharacterArcManagerComponent, PlayerInteraction, InteractionContext
...
async def test_character_arc_integration():
    ...
    try:
        ...
        if hasattr(arc_manager, 'generate_integrated_character_response'):
            ...
            response = await arc_manager.generate_character_response(character_id, context)
            response_data = {
        ...
        logger.info(f"Generated response: {response_data.get('response_text', 'No response')[:100]}...")
        
        # Test data synchronization
        if hasattr(arc_manager, 'sync_with_character_development_system'):
            logger.info("Testing data synchronization")
            sync_success = await arc_manager.sync_with_character_development_system(character_id)
            if sync_success:
        ...
        if hasattr(arc_manager, 'get_integrated_character_summary'):
            ...
            if summary and "error" not in summary:
                ...
                logger.info(f"Character stage: {summary.get('arc_manager_data', {}).get('current_stage', 'Unknown')}")
                logger.info(f"Integration status: {summary.get('integration_status', {}).get('systems_connected', False)}")
            else:
                logger.warning("Failed to get integrated summary or integration not available")
        ...
        if milestones:
            ...
            if resolution_result.get("success", False):
                ...
                logger.warning(f"Failed to resolve milestone: {resolution_result.get('reason', 'Unknown')}")
...
async def test_integration_fallback():
    ...
    try:
        ...
        character_arc = await arc_manager.initialize_character_arc(character_id, base_personality)
