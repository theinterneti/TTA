...
class StateTransition:
    ...
    def is_valid(self, current_context: Dict[str, Any]) -> bool:
        ...
        valid_transitions = {
            SessionStateType.INITIALIZING: [SessionStateType.ACTIVE, SessionStateType.ERROR],
            SessionStateType.ACTIVE: [SessionStateType.PAUSED, SessionStateType.TRANSITIONING, 
                                    SessionStateType.COMPLETED, SessionStateType.ERROR],
            SessionStateType.PAUSED: [SessionStateType.ACTIVE, SessionStateType.COMPLETED, 
                                    SessionStateType.EXPIRED, SessionStateType.ERROR],
            SessionStateType.TRANSITIONING: [SessionStateType.ACTIVE, SessionStateType.ERROR],
            SessionStateType.COMPLETED: [],  # Terminal state
            SessionStateType.ERROR: [SessionStateType.INITIALIZING, SessionStateType.ACTIVE],
...
class SessionState(BaseModel):
    ...
    # Core session data
    current_scene_id: Optional[str] = Field(None, description="Current active scene")
    scene_history: List[str] = Field(default_factory=list, description="Scene ID history")
    choice_history: List[str] = Field(default_factory=list, description="Choice ID history")
    ...
    last_persisted: Optional[datetime] = Field(None, description="Last persistence timestamp")
    dirty_fields: List[str] = Field(default_factory=list, description="Fields needing persistence")
    ...
    def update_emotional_state(self, emotion: str, intensity: float) -> None:
        ...
        if not 0.0 <= intensity <= 1.0:
            raise ValueError(f'Emotional intensity must be between 0.0 and 1.0')
    ...
    def is_active(self) -> bool:
        ...
        return self.state_type in [SessionStateType.ACTIVE, SessionStateType.TRANSITIONING]
    ...
    def record_state_change(self, from_state: SessionStateType, to_state: SessionStateType, 
                          reason: str = "") -> None:
...
class StateValidator:
    ...
    def _define_transition_rules(self) -> List[StateTransition]:
        ...
        return [
            ...
            StateTransition(
                SessionStateType.INITIALIZING, SessionStateType.ACTIVE, TransitionType.START,
            ...
            StateTransition(
                SessionStateType.ACTIVE, SessionStateType.PAUSED, TransitionType.PAUSE,
                conditions=["has_active_scene"],
                side_effects=["save_current_state", "set_pause_time"]
            ),
            
            StateTransition(
                SessionStateType.ACTIVE, SessionStateType.TRANSITIONING, TransitionType.START,
                conditions=["scene_transition_requested"],
                side_effects=["prepare_scene_transition"]
            ),
            
            StateTransition(
                SessionStateType.ACTIVE, SessionStateType.COMPLETED, TransitionType.COMPLETE,
            ...
            StateTransition(
                SessionStateType.PAUSED, SessionStateType.ACTIVE, TransitionType.RESUME,
            ...
            StateTransition(
                SessionStateType.TRANSITIONING, SessionStateType.ACTIVE, TransitionType.COMPLETE,
            ...
            StateTransition(
                SessionStateType.ERROR, SessionStateType.ACTIVE, TransitionType.RESET,
            ...
            StateTransition(
                SessionStateType.PAUSED, SessionStateType.EXPIRED, TransitionType.EXPIRE,
                conditions=["session_expired"],
                side_effects=["cleanup_session_data", "notify_expiration"]
            )
        ]
    
    def validate_transition(self, current_state: SessionState, 
                          new_state_type: SessionStateType,
                          transition_type: TransitionType) -> tuple[bool, str]:
        """Validate a state transition."""
        # Find applicable transition rule
        applicable_rule = None
        for rule in self.transition_rules:
            if (rule.from_state == current_state.state_type and 
                rule.to_state == new_state_type and 
                rule.transition_type == transition_type):
                applicable_rule = rule
                break
        
        if not applicable_rule:
            return False, f"No transition rule found for {current_state.state_type} -> {new_state_type}"
        ...
        if not applicable_rule.is_valid(context):
            return False, f"Transition conditions not met for {current_state.state_type} -> {new_state_type}"
    ...
    def validate_state_consistency(self, state: SessionState) -> tuple[bool, List[str]]:
        ...
        if state.is_expired() and state.state_type not in [SessionStateType.EXPIRED, SessionStateType.COMPLETED]:
        ...
        if state.safety_level == "crisis" and state.state_type == SessionStateType.ACTIVE:
...
class SessionStateManager:
    ...
    def transition_state(self, session_state: SessionState, 
                        new_state_type: SessionStateType,
                        transition_type: TransitionType,
        ...
        return True, f"Successfully transitioned from {old_state.value} to {new_state_type.value}"
    ...
    def auto_expire_check(self, session_state: SessionState, 
                         max_idle_time: timedelta = timedelta(hours=2)) -> bool:
        """Check if session should be auto-expired."""
        if session_state.state_type in [SessionStateType.COMPLETED, SessionStateType.EXPIRED]:
            return False
        
        if session_state.get_idle_time() > max_idle_time:
            success, _ = self.transition_state(
                session_state, SessionStateType.EXPIRED, TransitionType.EXPIRE,
