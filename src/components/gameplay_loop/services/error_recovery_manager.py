...
class ErrorSeverity(str, Enum):
    ...
    THERAPEUTIC_CRITICAL = "therapeutic_critical"  # Issues that could affect therapeutic safety
...
class ErrorRecoveryManager:
    ...
    async def handle_error(self, exception: Exception, component: str, function: str,
                          user_id: Optional[str] = None, session_id: Optional[str] = None,
                          therapeutic_context: Dict[str, Any] = None) -> RecoveryResult:
    ...
    @asynccontextmanager
    async def error_handling_context(self, component: str, function: str,
                                   user_id: Optional[str] = None, session_id: Optional[str] = None,
    ...
    async def create_system_backup(self, user_id: str, session_id: str,
                                 backup_type: str = "full") -> SystemBackup:
    ...
    async def _categorize_error(self, error_context: ErrorContext) -> None:
        ...
        try:
            ...
            elif "data" in error_type or "corruption" in error_context.error_message.lower():
                error_context.error_category = ErrorCategory.DATA_ERROR
            elif "network" in error_type or "connection" in error_context.error_message.lower():
                error_context.error_category = ErrorCategory.NETWORK_ERROR
            elif "validation" in error_type or "invalid" in error_context.error_message.lower():
            ...
            elif "performance" in error_type or "timeout" in error_context.error_message.lower():
    ...
    async def _assess_error_severity(self, error_context: ErrorContext) -> None:
        ...
        try:
            ...
            critical_indicators = [
                ...
                "therapeutic_safety", "crisis", "emergency"
            ]

            # High severity indicators
            high_indicators = [
                "session", "data_loss", "therapeutic", "safety",
                "character_development", "progress"
            ]

            # Medium severity indicators
            medium_indicators = [
                "validation", "processing", "integration", "performance"
            ]

            if any(indicator in error_message or indicator in component for indicator in critical_indicators):
                error_context.severity = ErrorSeverity.CRITICAL
            elif any(indicator in error_message or indicator in component for indicator in high_indicators):
                error_context.severity = ErrorSeverity.HIGH
            elif any(indicator in error_message or indicator in component for indicator in medium_indicators):
    ...
    async def _assess_therapeutic_impact(self, error_context: ErrorContext) -> None:
        ...
        try:
            therapeutic_components = [
                "therapeutic", "safety", "emotional", "crisis", "character",
                "session", "narrative", "choice"
            ]

            safety_keywords = [
                "safety", "crisis", "intervention", "emergency", "critical",
                "therapeutic_boundary", "emotional_distress"
            ]

            # Check if error affects therapeutic components
            if any(comp in error_context.component.lower() for comp in therapeutic_components):
                error_context.affects_therapeutic_safety = True

            # Check if error message contains safety keywords
            if any(keyword in error_context.error_message.lower() for keyword in safety_keywords):
                error_context.affects_therapeutic_safety = True
            ...
            if error_context.therapeutic_context:
                ...
                if error_context.therapeutic_context.get("emotional_distress_level", 0) > 0.7:
                    error_context.affects_therapeutic_safety = True
    ...
    async def _attempt_recovery(self, error_context: ErrorContext) -> RecoveryResult:
        ...
        try:
            ...
            for strategy in strategies:
                ...
                if recovery_result.success:
                    recovery_result.recovery_time_seconds = (datetime.utcnow() - recovery_start_time).total_seconds()
                    return recovery_result
            ...
            return RecoveryResult(
                ...
                escalation_needed=True,
                recovery_time_seconds=(datetime.utcnow() - recovery_start_time).total_seconds()
            )
    ...
    async def _use_fallback_mechanism(self, error_context: ErrorContext) -> RecoveryResult:
        ...
        try:
            ...
            return RecoveryResult(
                ...
                therapeutic_message=fallback.get("therapeutic_message", "We can adapt and continue our therapeutic work even when things don't go as planned.")
            )
    ...
    async def _recover_session_state(self, error_context: ErrorContext) -> RecoveryResult:
    ...
    async def _therapeutic_intervention(self, error_context: ErrorContext) -> RecoveryResult:
        ...
        try:
            ...
            return RecoveryResult(
                ...
                user_message=intervention.get("message", "We're here to support you through this technical difficulty."),
                therapeutic_message=intervention.get("reassurance", "Your therapeutic journey continues to be valuable and meaningful."),
    ...
    async def _notify_user(self, error_context: ErrorContext) -> RecoveryResult:
        ...
        try:
            ...
            return RecoveryResult(
                ...
                requires_user_action=error_context.severity in [ErrorSeverity.HIGH, ErrorSeverity.CRITICAL]
            )
    ...
    async def _escalate_error(self, error_context: ErrorContext) -> RecoveryResult:
        ...
        try:
            # Log escalation
            logger.critical(f"Error escalated: {error_context.error_id} - {error_context.error_message}")
    ...
    async def _restart_system_component(self, error_context: ErrorContext) -> RecoveryResult:
    ...
    def _calculate_backup_checksum(self, backup: SystemBackup) -> str:
        ...
        try:
            ...
            data_str = json.dumps(backup.session_state, sort_keys=True) if backup.session_state else ""
    ...
    def get_system_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for error in self.active_errors.values():
                ...
                active_errors_by_severity[severity] = active_errors_by_severity.get(severity, 0) + 1
            ...
            return {
                ...
                "recent_errors": len([e for e in self.error_history[-10:] if e.timestamp > datetime.utcnow() - timedelta(hours=1)]),
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy",
            "recovery_strategies_loaded": sum(len(strategies) for strategies in self.recovery_strategies.values()),
