...
from src.components.gameplay_loop.services.session_state import SessionState, SessionStateManager
from src.components.gameplay_loop.services.cache_strategies import (
    CacheStrategy, TTLCacheStrategy, LRUCacheStrategy, WriteBackCacheStrategy
)
...
class RedisConnectionManager:
    ...
    async def connect(self) -> bool:
        ...
        for attempt in range(self.retry_attempts):
            try:
                logger.debug(f"Redis connection attempt {attempt + 1}/{self.retry_attempts}")
...
class RedisSessionManager:
    ...
    async def create_session(self, session_state: SessionState, 
        ...
        try:
            ...
            async with self.connection_manager.get_client() as client:
                ...
                if result:
                    ...
                    logger.warning(f"Session already exists: {session_state.session_id}")
                    return False
    ...
    async def update_session(self, session_state: SessionState, 
                           ttl: Optional[timedelta] = None) -> bool:
        ...
        try:
            ...
            async with self.connection_manager.get_client() as client:
                ...
                if result:
                    ...
                    logger.warning(f"Failed to update session: {session_state.session_id}")
                    return False
...
class SessionCacheManager:
    ...
    async def _persist_to_redis(self, session_id: str, session_state: Optional[SessionState]):
    ...
    async def set_session(self, session_state: SessionState, 
                         ttl: Optional[timedelta] = None) -> bool:
...
class NarrativeContextCache:
    ...
    async def set_context(self, session_id: str, context_type: str, 
        ...
        try:
            ...
            logger.error(f"Failed to set narrative context {context_type} for session {session_id}: {e}")
            return False
    
    async def get_context(self, session_id: str, context_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get narrative context {context_type} for session {session_id}: {e}")
            return None
    ...
    async def invalidate_context(self, session_id: str, context_type: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to invalidate narrative context {context_type} for session {session_id}: {e}")
            return False
    ...
    async def invalidate_all_context(self, session_id: str) -> int:
        ...
        try:
            ...
            logger.error(f"Failed to invalidate all narrative context for session {session_id}: {e}")
            return 0
...
class ProgressCacheManager:
    ...
    async def cache_progress_snapshot(self, user_id: str, 
    ...
    async def get_progress_snapshot(self, user_id: str) -> Optional[Dict[str, Any]]:
