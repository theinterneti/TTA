...
class ChoiceContext:
    ...
    skill_practice_opportunity: Optional[str] = None
...
class ChoiceConsequence:
    ...
    def _evaluate_condition(self, condition: str, session_state: SessionState) -> bool:
        ...
        if condition.startswith("has_goal:"):
            ...
            if len(parts) == 3:
                emotion, operator, threshold = parts[1], parts[2][0], float(parts[2][1:])
...
class ChoiceValidator:
    ...
    def validate_choice(self, choice: UserChoice, session_state: SessionState) -> ChoiceContext:
        ...
        context.therapeutic_alignment = self._calculate_therapeutic_alignment(choice, session_state)
        if context.therapeutic_alignment < self.validation_rules["min_therapeutic_relevance"]:
        ...
        if (session_state.safety_level == "crisis" and
            self.validation_rules["crisis_mode_restrictions"]):
            if choice.choice_type not in [ChoiceType.THERAPEUTIC]:
                context.validation_result = ChoiceValidationResult.INVALID
                context.validation_issues.append("Only therapeutic choices allowed in crisis mode")
        
        # Blocked choice types
        if choice.choice_type in self.validation_rules["blocked_choice_types"]:
            context.validation_result = ChoiceValidationResult.INVALID
            context.validation_issues.append(f"Choice type {choice.choice_type} is blocked")
    ...
    def _calculate_safety_score(self, choice: UserChoice, session_state: SessionState) -> float:
    ...
    def _calculate_therapeutic_alignment(self, choice: UserChoice, session_state: SessionState) -> float:
...
class ChoiceProcessor:
    ...
    def __init__(self, narrative_engine):
        ...
        self.delayed_consequences: List[Tuple[datetime, ChoiceConsequence, SessionState]] = []
    ...
    async def load_choice(self, choice_id: str) -> Optional[UserChoice]:
        """Load a choice by ID."""
        try:
            # Check cache first
            ...
            # Load from database
            choice_data = await self.narrative_engine.database_manager.narrative_manager.get_choice_by_id(choice_id)
            ...
            choice = UserChoice(
                ...
                choice_type=ChoiceType(choice_data.get("choice_type", ChoiceType.NARRATIVE.value)),
    ...
    async def process_choice(self, session_state: SessionState, choice_id: str) -> bool:
        ...
        try:
            ...
            if context.validation_result != ChoiceValidationResult.VALID:
                logger.warning(f"Choice validation failed: {choice_id} - {context.validation_issues}")
            ...
            consequence_set = await self.consequence_system.generate_consequences(consequence_context)
            ...
            for consequence in legacy_consequences:
                if consequence.is_applicable(session_state):
                    if consequence.immediate:
                        ...
                        self.delayed_consequences.append((execute_at, consequence, session_state))
            ...
            logger.debug(f"Processed choice {choice_id} for session {session_state.session_id}")
            return True
    ...
    async def _apply_choice_effects(self, context: ChoiceContext) -> None:
        ...
        for goal, impact in context.progress_impact.items():
            ...
            session_state.set_narrative_variable(f"progress_{goal}", 
                session_state.get_narrative_variable(f"progress_{goal}", 0.0) + impact)
        
        # Set choice-related narrative variables
        session_state.set_narrative_variable("last_choice_type", choice.choice_type.value)
        session_state.set_narrative_variable("last_choice_therapeutic_relevance", choice.therapeutic_relevance)
    
    async def _load_choice_consequences(self, choice_id: str) -> List[ChoiceConsequence]:
        ...
        try:
            ...
            # Load from database
            consequences_data = await self.narrative_engine.database_manager.narrative_manager.get_choice_consequences(choice_id)
    ...
    async def _apply_consequence(self, consequence: ChoiceConsequence, session_state: SessionState) -> None:
        ...
        try:
            ...
            for goal, impact in consequence.therapeutic_impact.items():
                current_progress = session_state.get_narrative_variable(f"progress_{goal}", 0.0)
                session_state.set_narrative_variable(f"progress_{goal}", current_progress + impact)
            
            logger.debug(f"Applied consequence {consequence.consequence_id}")
            
        except Exception as e:
            logger.error(f"Failed to apply consequence {consequence.consequence_id}: {e}")
    
    async def _apply_consequence_set(self, consequence_set: ConsequenceSet, session_state: SessionState) -> None:
        ...
        try:
            ...
            for outcome in consequence_set.therapeutic_outcomes:
                ...
                current_progress = session_state.progress_metrics.get(outcome.outcome_type, 0.0)
                ...
                logger.debug(f"Applied therapeutic outcome: {outcome.outcome_type} +{outcome.therapeutic_value}")
            ...
            for character, changes in consequence_set.character_changes.items():
                for attribute, change in changes.items():
                    current_value = session_state.get_narrative_variable(f"character_{character}_{attribute}", 0.0)
                    session_state.set_narrative_variable(f"character_{character}_{attribute}", current_value + change)
            ...
            if consequence_set.learning_opportunities:
                current_opportunities = session_state.context.get("learning_opportunities", [])
                current_opportunities.extend(consequence_set.learning_opportunities)
                session_state.context["learning_opportunities"] = current_opportunities[-10:]  # Keep last 10
            ...
            session_state.context["consequence_history"] = consequence_history[-20:]  # Keep last 20

            self.metrics["consequences_applied"] += 1
            logger.debug(f"Applied consequence set {consequence_set.consequence_id}")

        except Exception as e:
            logger.error(f"Failed to apply consequence set {consequence_set.consequence_id}: {e}")
