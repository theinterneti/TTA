...
from src.components.gameplay_loop.models.core import NarrativeScene, SceneType, UserChoice
...
class SceneContext:
    ...
    def record_therapeutic_moment(self, moment_type: str, description: str, 
...
class SceneValidator:
    ...
    def validate_scene(self, scene: NarrativeScene) -> Tuple[bool, List[str]]:
        ...
        if (self.validation_rules["therapeutic_focus_required"] and 
            not scene.therapeutic_focus):
            issues.append("Therapeutic focus is required")
        
        return len(issues) == 0, issues
    
    def validate_transition(self, transition: SceneTransition, 
        ...
        if session_state.safety_level == "crisis" and transition.trigger_type != "safety":
...
class SceneManager:
    ...
    async def load_scene(self, scene_id: str) -> Optional[NarrativeScene]:
        """Load a scene by ID."""
        try:
            # Check cache first
            ...
            # Load from database
            scene_data = await self.narrative_engine.database_manager.narrative_manager.get_scene_by_id(scene_id)
            ...
            scene = NarrativeScene(
                ...
                scene_type=SceneType(scene_data.get("scene_type", SceneType.EXPLORATION.value)),
    ...
    async def enter_scene(self, session_state: SessionState, scene: NarrativeScene) -> bool:
        ...
        try:
            ...
            logger.debug(f"Entered scene {scene.scene_id} for session {session_state.session_id}")
    ...
    async def exit_scene(self, session_state: SessionState, scene_id: str, 
                        completion_status: SceneStatus = SceneStatus.COMPLETED) -> bool:
        """Exit a scene."""
        try:
            ...
            logger.debug(f"Exited scene {scene_id} for session {session_state.session_id}")
            return True
    ...
    async def get_scene_context(self, session_id: str, scene_id: str) -> Optional[SceneContext]:
        """Get scene context."""
    ...
    async def _apply_scene_effects(self, context: SceneContext) -> None:
        ...
        session_state.set_narrative_variable(f"current_scene_type", scene.scene_type.value)
        session_state.set_narrative_variable(f"scene_difficulty", scene.difficulty_level)
    ...
    async def _preload_scene_choices(self, context: SceneContext) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to preload choices for scene {context.scene.scene_id}: {e}")
    ...
    def _calculate_completion_rate(self, context: SceneContext) -> float:
        ...
        time_factor = min(actual_time / expected_time, 2.0) / 2.0  # Cap at 2x expected time
    ...
    def _calculate_engagement_score(self, context: SceneContext) -> float:
        ...
        if context.emotional_responses:
            avg_intensity = sum(context.emotional_responses.values()) / len(context.emotional_responses)
    ...
    async def _handle_scene_entered(self, event) -> None:
        ...
        logger.debug(f"Scene entered: {event.context.get('scene_id')} in session {event.session_id}")
    
    async def _handle_scene_completed(self, event) -> None:
        """Handle scene completed events."""
        logger.debug(f"Scene completed: {event.context.get('scene_id')} in session {event.session_id}")
