...
class CharacterAttributeLevel:
    ...
    last_increased: Optional[datetime] = None
...
class CharacterMilestoneAchievement:
    ...
    # Achievement context
    triggering_choice: Optional[str] = None
    story_context: str = ""
    attribute_levels_at_achievement: Dict[CharacterAttribute, float] = field(default_factory=dict)
...
class CharacterDevelopmentSystem:
    ...
    def __init__(self, event_bus: EventBus):
        ...
        self.character_attributes: Dict[str, Dict[CharacterAttribute, CharacterAttributeLevel]] = {}
        ...
        self.level_thresholds = [0, 10, 25, 50, 100, 175, 275, 400, 550, 750, 1000]  # XP needed for each level
            ...
            AbilityType.THERAPEUTIC_SKILL: [
                {
                    ...
                    "usage_examples": ["Using deep breathing before difficult conversations", "Teaching breathing techniques to anxious characters"]
                },
                {
                    "name": "Cognitive Reframing",
                    "description": "The ability to challenge and reframe negative thought patterns",
                    "required_attributes": {CharacterAttribute.WISDOM: 4.0, CharacterAttribute.SELF_AWARENESS: 3.0},
                    "unlock_story": "Your growing wisdom allows you to see situations from multiple perspectives and help others do the same.",
                    "usage_examples": ["Helping characters see challenges as opportunities", "Reframing setbacks as learning experiences"]
                }
            ],
            AbilityType.SOCIAL_ABILITY: [
                {
                    ...
                    "required_attributes": {CharacterAttribute.COMPASSION: 3.0, CharacterAttribute.COMMUNICATION: 2.0},
                    "unlock_story": "Your compassionate heart and growing communication skills allow you to truly connect with others.",
                    "usage_examples": ["Providing comfort to distressed characters", "Mediating conflicts through understanding"]
                },
                {
                    "name": "Inspiring Leadership",
                    "description": "The ability to motivate and guide others toward positive outcomes",
                    "required_attributes": {CharacterAttribute.LEADERSHIP: 4.0, CharacterAttribute.COURAGE: 3.0},
                    ...
                    "usage_examples": ["Rallying characters to work together", "Inspiring hope in difficult situations"]
                }
            ]
        }
    ...
    def _load_milestone_templates(self) -> Dict[CharacterMilestone, Dict[str, Any]]:
        ...
        return {
            CharacterMilestone.FIRST_BRAVE_ACT: {
                ...
                "attribute_bonuses": {CharacterAttribute.COURAGE: 0.5, CharacterAttribute.SELF_AWARENESS: 0.2},
                "story_unlocks": ["courage_based_story_paths", "leadership_opportunities"]
            },
            CharacterMilestone.WISE_DECISION_MAKER: {
                ...
                "attribute_bonuses": {CharacterAttribute.WISDOM: 0.5, CharacterAttribute.LEADERSHIP: 0.3},
                "story_unlocks": ["advisor_role_opportunities", "complex_decision_scenarios"]
            },
            CharacterMilestone.COMPASSIONATE_HELPER: {
                ...
                "attribute_bonuses": {CharacterAttribute.COMPASSION: 0.5, CharacterAttribute.EMOTIONAL_INTELLIGENCE: 0.3},
                "story_unlocks": ["healing_role_opportunities", "community_building_scenarios"]
            }
        }
    ...
    def _load_milestone_thresholds(self) -> Dict[CharacterMilestone, Dict[str, Any]]:
        ...
        return {
            CharacterMilestone.FIRST_BRAVE_ACT: {
                ...
                "story_contexts": ["conflict_resolution", "personal_challenge", "helping_others"]
            },
            CharacterMilestone.WISE_DECISION_MAKER: {
                "min_wisdom": 3.0,
                "required_actions": ["thoughtful_choice", "learn_from_mistake", "give_advice"],
                "consecutive_wise_decisions": 3
            },
            CharacterMilestone.COMPASSIONATE_HELPER: {
                "min_compassion": 3.0,
                "required_actions": ["help_others", "show_empathy", "sacrifice_for_others"],
    ...
    def _load_ability_unlock_conditions(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "cognitive_reframing": {
                ...
                "story_conditions": ["challenged_negative_thoughts", "helped_others_reframe"]
            },
            "empathetic_listening": {
                "required_attributes": {CharacterAttribute.COMPASSION: 3.0, CharacterAttribute.COMMUNICATION: 2.0},
                "required_milestones": [CharacterMilestone.COMPASSIONATE_HELPER],
                "story_conditions": ["listened_to_others", "provided_emotional_support"]
            }
        }

    async def process_character_development(self, session_state: SessionState,
                                          trigger: DevelopmentTrigger,
                                          context: Dict[str, Any]) -> CharacterDevelopmentEvent:
        ...
        try:
            ...
            event = CharacterDevelopmentEvent(
                ...
                event_type=trigger,
                description=context.get("description", f"Character development triggered by {trigger.value}"),
            ...
            attribute_changes = await self._calculate_attribute_changes(trigger, context, session_state)
            ...
            experience_gained = await self._calculate_experience_gained(trigger, context, attribute_changes)
            ...
            event.character_feedback = await self._generate_character_feedback(attribute_changes, context)

            # Generate narrative impact
            event.narrative_impact = await self._generate_narrative_impact(attribute_changes, context)
            ...
            session_state.context["recent_character_development"] = {
                ...
                "attribute_changes": {attr.value: change for attr, change in attribute_changes.items()},
            ...
            logger.error(f"Failed to process character development for user {session_state.user_id}: {e}")
    ...
    async def _calculate_attribute_changes(self, trigger: DevelopmentTrigger,
                                         context: Dict[str, Any],
                                         session_state: SessionState) -> Dict[CharacterAttribute, float]:
        """Calculate attribute changes based on trigger and context."""
        changes = {}

        # Get base gain rate for trigger type
        base_rate = self.development_rules["attribute_gain_rates"].get(trigger.value, 0.1)
        ...
        if trigger == DevelopmentTrigger.CHOICE_CONSEQUENCE:
            ...
            if choice_type == ChoiceType.EMOTIONAL_REGULATION:
                changes[CharacterAttribute.EMOTIONAL_INTELLIGENCE] = base_rate * therapeutic_relevance
                changes[CharacterAttribute.RESILIENCE] = base_rate * 0.5 * therapeutic_relevance
            elif choice_type == ChoiceType.COMMUNICATION:
                changes[CharacterAttribute.COMMUNICATION] = base_rate * therapeutic_relevance
                changes[CharacterAttribute.COMPASSION] = base_rate * 0.3 * therapeutic_relevance
            elif choice_type == ChoiceType.PROBLEM_SOLVING:
                changes[CharacterAttribute.PROBLEM_SOLVING] = base_rate * therapeutic_relevance
                changes[CharacterAttribute.WISDOM] = base_rate * 0.4 * therapeutic_relevance
            elif choice_type == ChoiceType.SELF_REFLECTION:
                changes[CharacterAttribute.SELF_AWARENESS] = base_rate * therapeutic_relevance
                changes[CharacterAttribute.WISDOM] = base_rate * 0.3 * therapeutic_relevance
            else:
            ...
            if "anxiety" in therapeutic_concept.lower():
                ...
                changes[CharacterAttribute.EMOTIONAL_INTELLIGENCE] = base_rate * 0.7 * progress_level
            elif "communication" in therapeutic_concept.lower():
                changes[CharacterAttribute.COMMUNICATION] = base_rate * progress_level
                changes[CharacterAttribute.COMPASSION] = base_rate * 0.5 * progress_level
            elif "emotional" in therapeutic_concept.lower():
                changes[CharacterAttribute.EMOTIONAL_INTELLIGENCE] = base_rate * progress_level
                changes[CharacterAttribute.SELF_AWARENESS] = base_rate * 0.6 * progress_level
            else:
                # General therapeutic progress
                changes[CharacterAttribute.WISDOM] = base_rate * progress_level
                changes[CharacterAttribute.RESILIENCE] = base_rate * 0.5 * progress_level
            ...
            changes[CharacterAttribute.DETERMINATION] = base_rate * 0.6 * challenge_difficulty
            ...
            if skill_type == "communication":
                ...
                changes[CharacterAttribute.EMOTIONAL_INTELLIGENCE] = base_rate * effectiveness
    ...
    async def _apply_attribute_change(self, user_id: str, attribute: CharacterAttribute,
                                    change: float, trigger: DevelopmentTrigger) -> None:
        ...
        attr_level.current_level = max(0.0, min(10.0, attr_level.current_level + change))
        ...
        if attr_level.last_increased:
            ...
            if time_diff.total_seconds() > 0:
                attr_level.development_rate = change / (time_diff.total_seconds() / 3600)  # Per hour
    ...
    async def _calculate_experience_gained(self, trigger: DevelopmentTrigger,
                                         context: Dict[str, Any],
                                         attribute_changes: Dict[CharacterAttribute, float]) -> Dict[CharacterAttribute, int]:
    ...
    async def _apply_experience_points(self, user_id: str, attribute: CharacterAttribute, xp: int) -> None:
        ...
        if current_level_index < len(self.level_thresholds) - 1:
            ...
            if attr_level.experience_points >= next_level_threshold:
                ...
                current_level_threshold = self.level_thresholds[current_level_index - 1] if current_level_index > 0 else 0
                progress = (attr_level.experience_points - current_level_threshold) / (next_level_threshold - current_level_threshold)
                attr_level.level_progress = min(1.0, progress)

    async def _generate_character_feedback(self, attribute_changes: Dict[CharacterAttribute, float],
        ...
        primary_attribute = max(attribute_changes.keys(), key=lambda attr: attribute_changes[attr])
    ...
    async def _generate_narrative_impact(self, attribute_changes: Dict[CharacterAttribute, float],
                                       context: Dict[str, Any]) -> str:
    ...
    async def _check_milestone_achievements(self, user_id: str, session_state: SessionState,
                                          event: CharacterDevelopmentEvent) -> None:
        ...
        if user_id in self.milestone_achievements:
            existing_milestones = {achievement.milestone for achievement in self.milestone_achievements[user_id]}
        ...
        for milestone, template in self.milestone_templates.items():
            ...
            if await self._check_milestone_requirements(milestone, template, user_attributes, event):
                await self._achieve_milestone(user_id, session_state, milestone, template, event)

    async def _check_milestone_requirements(self, milestone: CharacterMilestone,
                                          template: Dict[str, Any],
                                          user_attributes: Dict[CharacterAttribute, CharacterAttributeLevel],
                                          event: CharacterDevelopmentEvent) -> bool:
        ...
        for attr, min_level in required_attributes.items():
            if attr not in user_attributes or user_attributes[attr].current_level < min_level:
                return False
        ...
        if milestone == CharacterMilestone.FIRST_BRAVE_ACT:
            ...
            if CharacterAttribute.COMPASSION in event.attribute_changes:
                compassion_change = event.attribute_changes[CharacterAttribute.COMPASSION]
                if compassion_change > 0.1 and ("help" in event.story_context.lower() or "support" in event.story_context.lower()):
                    return True

        return False

    async def _achieve_milestone(self, user_id: str, session_state: SessionState,
                               milestone: CharacterMilestone, template: Dict[str, Any],
                               triggering_event: CharacterDevelopmentEvent) -> None:
        ...
        try:
            ...
            for attr, bonus in achievement.attribute_bonuses.items():
                await self._apply_attribute_change(user_id, attr, bonus, DevelopmentTrigger.MILESTONE_ACHIEVEMENT)
            ...
            session_state.context["recent_milestone_achievement"] = {
                ...
                "attribute_bonuses": {attr.value: bonus for attr, bonus in achievement.attribute_bonuses.items()},
            ...
            logger.error(f"Failed to achieve milestone {milestone} for user {user_id}: {e}")

    async def _check_ability_unlocks(self, user_id: str, session_state: SessionState,
                                   event: CharacterDevelopmentEvent) -> None:
        ...
        if user_id in self.milestone_achievements:
            user_milestones = {achievement.milestone for achievement in self.milestone_achievements[user_id]}

        # Get existing abilities
        existing_abilities = set()
        if user_id in self.character_abilities:
            existing_abilities = {ability.name for ability in self.character_abilities[user_id]}
        ...
        for ability_type, abilities in self.ability_templates.items():
            for ability_template in abilities:
                ...
                if await self._check_ability_unlock_conditions(ability_template, user_attributes, user_milestones, event):
                    await self._unlock_ability(user_id, session_state, ability_type, ability_template)

    async def _check_ability_unlock_conditions(self, ability_template: Dict[str, Any],
                                             user_attributes: Dict[CharacterAttribute, CharacterAttributeLevel],
    ...
    async def _unlock_ability(self, user_id: str, session_state: SessionState,
                             ability_type: AbilityType, ability_template: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to unlock ability {ability_template['name']} for user {user_id}: {e}")

    async def _publish_development_event(self, session_state: SessionState,
                                       event: CharacterDevelopmentEvent) -> None:
        ...
        narrative_event = NarrativeEvent(
            ...
            context={
                ...
                "attribute_changes": {attr.value: change for attr, change in event.attribute_changes.items()},
                "experience_gained": {attr.value: xp for attr, xp in event.experience_gained.items()},
                "character_feedback": event.character_feedback,
                "narrative_impact": event.narrative_impact
            }
        )
    ...
    async def _publish_milestone_event(self, session_state: SessionState,
                                     achievement: CharacterMilestoneAchievement) -> None:
        """Publish milestone achievement event."""
        narrative_event = NarrativeEvent(
    ...
    async def _publish_ability_unlock_event(self, session_state: SessionState,
    ...
    def get_character_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_experience": sum(attr.experience_points for attr in user_attributes.values()),
            "attributes": {
                attr.value: {
            ...
            "top_attributes": [
                {
                    ...
                    "name": self.attribute_templates.get(attr, {}).get("name", attr.value)
                }
            ...
            "milestones": [
                {
                    "milestone": achievement.milestone.value,
                    "name": self.milestone_templates.get(achievement.milestone, {}).get("name", achievement.milestone.value),
    ...
    def get_character_progression_visualization(self, user_id: str) -> Dict[str, Any]:
        ...
        for attribute, attr_level in user_attributes.items():
            ...
            attribute_data.append({
                ...
                "story_manifestations": attr_level.story_manifestations[-3:],  # Last 3
                "character_reactions": attr_level.character_reactions[-3:]  # Last 3
            })
    ...
    async def handle_character_regression(self, user_id: str, session_state: SessionState,
                                        regression_context: Dict[str, Any]) -> CharacterDevelopmentEvent:
        ...
        try:
            ...
            event = CharacterDevelopmentEvent(
                ...
                description=regression_context.get("description", "Character faces a temporary setback"),
            ...
            if regression_rules["enabled"]:
                affected_attributes = regression_context.get("affected_attributes", [CharacterAttribute.RESILIENCE])
                max_regression = regression_rules["max_regression"]

                for attribute in affected_attributes:
                    if user_id in self.character_attributes and attribute in self.character_attributes[user_id]:
                        # Apply small regression
                        regression_amount = -min(max_regression, 0.2)  # Small regression
                        await self._apply_attribute_change(user_id, attribute, regression_amount, DevelopmentTrigger.CHALLENGE_OVERCOME)
            ...
            logger.error(f"Failed to handle character regression for user {user_id}: {e}")
            return CharacterDevelopmentEvent(
                user_id=user_id,
                session_id=session_state.session_id,
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.metrics,
            ...
            "total_abilities_unlocked": sum(len(abilities) for abilities in self.character_abilities.values()),
            "total_milestones_achieved": sum(len(achievements) for achievements in self.milestone_achievements.values()),
            "total_development_events": sum(len(events) for events in self.development_events.values()),
            "attribute_templates_loaded": len(self.attribute_templates),
            "ability_templates_loaded": sum(len(abilities) for abilities in self.ability_templates.values()),
