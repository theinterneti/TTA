...
class PathComparison:
    ...
    # Comparison metrics
    metric_comparisons: dict[ComparisonMetric, dict[str, float]] = field(
    ...
    # Recommendations
    recommended_approach: str | None = None  # Path ID
...
class ReplayabilitySystem:
    ...
    def __init__(
        self, event_bus: EventBus, character_development: CharacterDevelopmentSystem
    ):
    ...
    async def create_exploration_snapshot(
        ...
        try:
            ...
            snapshot = ExplorationSnapshot(
                ...
                or f"Snapshot at {datetime.utcnow().strftime('%H:%M')}",
            ...
            if session_state.user_id in self.character_development.character_attributes:
                snapshot.character_attributes = {
                    ...
                    for attr, attr_level in self.character_development.character_attributes[
                        session_state.user_id
                    ].items()
                }

            # Capture therapeutic progress
            snapshot.therapeutic_progress = session_state.context.get(
            ...
            snapshot.scene_context = {
                ...
                "narrative_state": session_state.context.get(
                    "narrative_state", {}
                ).copy(),
            }

            snapshot.narrative_context = session_state.context.get(
                "narrative_context", {}
            ).copy()
            ...
            snapshot.choice_history = [
                {
                    "choice_id": (
                        choice.choice_id if hasattr(choice, "choice_id") else str(i)
                    ),
            ...
            await self._publish_exploration_event(
                session_state,
                "snapshot_created",
                {
            ...
            logger.error(
                f"Failed to create exploration snapshot for session {session_state.session_id}: {e}"
            )
            raise
    ...
    async def start_exploration_session(
        ...
        try:
            ...
            await self._publish_exploration_event_by_user(
                user_id,
                "exploration_started",
                {
    ...
    async def create_alternative_path(
        ...
        try:
            ...
            path = AlternativePath(
                ...
                or f"Alternative path exploring {path_type.value}",
            ...
            logger.error(
                f"Failed to create alternative path for exploration {exploration_id}: {e}"
            )
            raise

    async def restore_from_snapshot(
        self, snapshot_id: str, target_session_state: SessionState
    ) -> SessionState:
        ...
        try:
            ...
            if (
                snapshot.character_attributes
                and snapshot.user_id in self.character_development.character_attributes
            ):
                for attr_name, attr_data in snapshot.character_attributes.items():
                    try:
                        ...
                        if (
                            attr
                            in self.character_development.character_attributes[
                                snapshot.user_id
                            ]
                        ):
                            attr_level = (
                                self.character_development.character_attributes[
                                    snapshot.user_id
                                ][attr]
                            )
                            attr_level.current_level = attr_data["current_level"]
                            attr_level.experience_points = attr_data[
                                "experience_points"
                            ]
                        ...
                        logger.warning(
                            f"Failed to restore character attribute {attr_name}: {e}"
                        )

            # Restore therapeutic progress
            if snapshot.therapeutic_progress:
                restored_state.context["therapeutic_progress"] = (
                    snapshot.therapeutic_progress.copy()
                )

            # Restore scene and narrative context
            if snapshot.scene_context:
                restored_state.current_scene_id = snapshot.scene_context.get(
                    "current_scene_id"
                )
                restored_state.scene_history = snapshot.scene_context.get(
                    "scene_history", []
                ).copy()
                if "narrative_state" in snapshot.scene_context:
                    restored_state.context["narrative_state"] = snapshot.scene_context[
                        "narrative_state"
                    ].copy()

            if snapshot.narrative_context:
                restored_state.context["narrative_context"] = (
                    snapshot.narrative_context.copy()
                )
            ...
            restored_state.context["exploration_timestamp"] = (
    ...
    async def record_path_choice(
        self, path_id: str, choice: UserChoice, outcomes: list[dict[str, Any]]
    ) -> None:
        ...
        try:
            ...
            choice_record = {
                "choice_id": (
                    choice.choice_id if hasattr(choice, "choice_id") else str(uuid4())
                ),
                "text": choice.text,
    ...
    async def complete_alternative_path(
        self, path_id: str, final_outcomes: dict[str, Any]
    ) -> AlternativePath:
        ...
        try:
            ...
            path.completion_time_minutes = (
                path.completed_at - path.created_at
            ).total_seconds() / 60
    ...
    async def compare_paths(
        ...
        path_ids: list[str],
        comparison_name: str = "",
        focus_metrics: list[ComparisonMetric] = None,
    ) -> PathComparison:
        ...
        try:
            ...
            comparison.therapeutic_insights = await self._generate_therapeutic_insights(
                paths, comparison.metric_comparisons
            )
        ...
        try:
            ...
            if therapeutic_outcomes:
                avg_therapeutic_value = sum(
                    outcome["therapeutic_value"] for outcome in therapeutic_outcomes
            ...
            logger.error(
                f"Failed to update path analytics for path {path.path_id}: {e}"
            )
    ...
    async def _calculate_final_path_analytics(self, path: AlternativePath) -> None:
        ...
        try:
            ...
            if path.character_development:
                char_growth_score = sum(
                    path.character_development.get("attributes_improved", {}).values()
                )
                if char_growth_score > 0:
                    path.therapeutic_effectiveness = min(
                        1.0, path.therapeutic_effectiveness + (char_growth_score * 0.1)
                    )
            ...
            logger.error(
                f"Failed to calculate final path analytics for path {path.path_id}: {e}"
            )

    async def _calculate_path_metric(
        self, path: AlternativePath, metric: ComparisonMetric
    ) -> float:
        ...
        try:
            ...
            if metric == ComparisonMetric.THERAPEUTIC_EFFECTIVENESS:
                ...
                score += weight_factors.get("therapeutic_goals_achieved", 0.3) * min(
                    1.0, therapeutic_goals / 3
                )

                # Skills practiced
                skills_practiced = len(
                    path.therapeutic_outcomes.get("skills_practiced", [])
                )
                score += weight_factors.get("skills_practiced", 0.2) * min(
                    1.0, skills_practiced / 5
                )

                # Emotional regulation events
                emotional_regulation = path.therapeutic_outcomes.get(
                    "emotional_regulation_events", 0
                )
                score += weight_factors.get("emotional_regulation", 0.2) * min(
                    1.0, emotional_regulation / 3
                )
                ...
                attr_improvements = sum(
                    path.character_development.get("attributes_improved", {}).values()
                )
                score += weight_factors.get("attribute_improvements", 0.4) * min(
                    1.0, attr_improvements / 2
                )

                # Milestones achieved
                milestones = len(
                    path.character_development.get("milestones_achieved", [])
                )
                score += weight_factors.get("milestones_achieved", 0.3) * min(
                    1.0, milestones / 2
                )

                # Abilities unlocked
                abilities = len(
                    path.character_development.get("abilities_unlocked", [])
                )
                score += weight_factors.get("abilities_unlocked", 0.2) * min(
                    1.0, abilities / 3
                )
                ...
                intensities = [
                    event.get("intensity", 0.5) for event in path.emotional_journey
                ]
                avg_intensity = (
            ...
            logger.error(
                f"Failed to calculate metric {metric} for path {path.path_id}: {e}"
            )
            return 0.0
    ...
    async def _identify_key_differences(
        ...
        try:
            ...
            for choice_type in all_choice_types:
                paths_with_type = [
                    path_id
                    for path_id, types in choice_types_by_path.items()
                    if choice_type in types
                ]
                if len(paths_with_type) < len(paths):
                    differences.append(
                        f"Choice type '{choice_type}' used in {len(paths_with_type)} of {len(paths)} paths"
                    )

            # Compare completion times
            completion_times = [
                path.completion_time_minutes for path in paths if path.is_completed
            ]
            if len(completion_times) > 1:
                ...
                if max_time - min_time > 10:  # Significant difference
                    differences.append(
                        f"Completion times varied significantly: {min_time:.1f} to {max_time:.1f} minutes"
                    )
    ...
    async def _generate_therapeutic_insights(
        ...
        metric_comparisons: dict[ComparisonMetric, dict[str, float]],
    ) -> list[str]:
        ...
        try:
            ...
            if ComparisonMetric.THERAPEUTIC_EFFECTIVENESS in metric_comparisons:
                effectiveness_scores = metric_comparisons[
                    ComparisonMetric.THERAPEUTIC_EFFECTIVENESS
                ]
                best_path_id = max(
                ...
                insights.append(
                    f"The '{best_path.path_name}' approach was most therapeutically effective"
                )
                ...
                if best_path.therapeutic_outcomes.get("skills_practiced"):
                    ...
                    insights.append(
                        f"Practicing {', '.join(skills[:2])} contributed to therapeutic success"
                    )
            ...
            for path in paths:
                therapeutic_choices = [
                    choice
                    for choice in path.choices_made
                    if choice.get("therapeutic_relevance", 0) > 0.7
                ]
                if len(therapeutic_choices) > len(path.choices_made) * 0.6:
                    insights.append(
                        f"The '{path.path_name}' path focused heavily on therapeutic choices"
                    )
        ...
        try:
            ...
            for path in paths:
                ...
                if char_dev.get("attributes_improved"):
                    top_attribute = max(
                        char_dev["attributes_improved"].items(), key=lambda x: x[1]
                    )
                    insights.append(
                        f"The '{path.path_name}' path most improved {top_attribute[0]} (+{top_attribute[1]:.1f})"
                    )

                if char_dev.get("milestones_achieved"):
                    milestones = char_dev["milestones_achieved"]
                    insights.append(
                        f"The '{path.path_name}' path achieved {len(milestones)} character milestones"
                    )
        ...
        try:
            ...
            recommended_path = next(
                (
                    path
                    for path in paths
                    if path.path_id == comparison.recommended_approach
                ),
                None,
            )
                ...
                if comparison.recommended_approach in scores:
                    ...
                    if score > 0.7:  # High score
                        reasoning_parts.append(
                            f"high {metric.value.replace('_', ' ')} ({score:.1f})"
                        )

            if reasoning_parts:
                return f"Recommended due to {', '.join(reasoning_parts[:2])}"
            else:
                return "Recommended based on overall balanced performance"
    ...
    async def _identify_learning_opportunities(
        ...
        try:
                ...
                if len(scores) >= 2:
                    ...
                    if max_score - min_score > 0.3:  # Significant difference
                        opportunities.append(
                            f"Explore different approaches to improve {metric.value.replace('_', ' ')}"
                        )
            ...
            for choice_type in list(unexplored)[:2]:  # Limit to 2
                opportunities.append(
                    f"Try more {choice_type.replace('_', ' ')} focused choices"
                )
    ...
    async def _publish_exploration_event(
        ...
        try:
            narrative_event = NarrativeEvent(
                ...
                session_id=session_state.session_id,
                user_id=session_state.user_id,
                context={"exploration_event_type": event_type, **context},
            )
    ...
    async def _publish_exploration_event_by_user(
        self, user_id: str, event_type: str, context: dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to publish exploration event {event_type} for user {user_id}: {e}"
            )
    ...
    def get_user_exploration_summary(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Get path details
            paths = [
                self.alternative_paths[path_id]
                for path_id in user_paths
                if path_id in self.alternative_paths
            ]
            completed_paths = [path for path in paths if path.is_completed]

            # Calculate summary statistics
            total_exploration_time = sum(
                path.completion_time_minutes for path in completed_paths
            )
            avg_therapeutic_effectiveness = (
                sum(path.therapeutic_effectiveness for path in completed_paths)
                / len(completed_paths)
                if completed_paths
                else 0.0
            )
            avg_engagement = (
                sum(path.engagement_score for path in completed_paths)
                / len(completed_paths)
                if completed_paths
                else 0.0
            )
            ...
            user_comparisons = [
                comp
                for comp in self.path_comparisons.values()
            ...
            return {
                ...
                "exploration_analytics": {
                    ...
                    "preferred_exploration_mode": self._get_preferred_exploration_mode(
                        user_explorations
                    ),
                },
                "recent_paths": [
                    ...
                    for path in sorted(paths, key=lambda p: p.created_at, reverse=True)[
                        :5
                    ]
                ],
                "recent_comparisons": [
                    ...
                    for comp in sorted(
                        user_comparisons, key=lambda c: c.created_at, reverse=True
                    )[:3]
                ],
            }
    ...
    def _get_most_explored_path_type(self, paths: list[AlternativePath]) -> str:
        ...
        path_type_counts = {}
        for path in paths:
            path_type_counts[path.path_type.value] = (
                path_type_counts.get(path.path_type.value, 0) + 1
            )
    ...
    def _get_preferred_exploration_mode(self, exploration_ids: list[str]) -> str:
        ...
        explorations = [
            self.exploration_sessions[exp_id]
            for exp_id in exploration_ids
            if exp_id in self.exploration_sessions
        ]
        if not explorations:
            return "none"
        ...
        for exploration in explorations:
            mode_counts[exploration.exploration_mode.value] = (
                mode_counts.get(exploration.exploration_mode.value, 0) + 1
            )
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            **self.metrics,
            ...
            "active_exploration_sessions": len(
                [exp for exp in self.exploration_sessions.values() if exp.is_active]
            ),
