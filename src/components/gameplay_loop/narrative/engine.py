...
from src.components.gameplay_loop.models.core import GameplaySession, NarrativeScene, UserChoice
...
from .events import EventBus, EventType, NarrativeEvent, event_bus
...
class NarrativeEngine:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            await self.event_bus.publish(NarrativeEvent(
                event_type=EventType.NARRATIVE_STARTED,
                data={"engine_state": self.state.value, "config": self.config.__dict__}
            ))
    ...
    async def enter_scene(self, session_id: str, scene_id: str) -> bool:
        ...
        try:
            ...
            async with self.session_locks[session_id]:
                ...
                if not await self.flow_controller.can_enter_scene(session_state, scene_id):
                    logger.warning(f"Cannot enter scene {scene_id} for session {session_id}")
                    return False
            ...
            logger.error(f"Failed to enter scene {scene_id} for session {session_id}: {e}")
            return False
    ...
    async def _transition_session_state(self, session_id: str, 
        ...
        try:
            ...
            logger.debug(f"Session {session_id} transitioned from {old_state} to {new_state}")
            return True
    ...
    def _setup_event_handlers(self) -> None:
        ...
        self.event_bus.subscribe(EventType.SAFETY_CONCERN_DETECTED, self._handle_safety_concern)
        self.event_bus.subscribe(EventType.CRISIS_INTERVENTION_NEEDED, self._handle_crisis)
        
        # Progress event handlers
        self.event_bus.subscribe(EventType.MILESTONE_ACHIEVED, self._handle_milestone)
        self.event_bus.subscribe(EventType.THERAPEUTIC_MOMENT, self._handle_therapeutic_moment)
    ...
    async def _safety_monitoring_loop(self) -> None:
        ...
        while self.state not in [EngineState.SHUTDOWN, EngineState.ERROR]:
            try:
                for session_id, session_state in self.active_sessions.items():
                    await self.therapeutic_integrator.monitor_session_safety(session_state)
