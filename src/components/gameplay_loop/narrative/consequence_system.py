...
class ConsequenceSystem:
    ...
    async def generate_consequences(
        self, context: ConsequenceGenerationContext
    ) -> ConsequenceSet:
        ...
        try:
            ...
            self.metrics["learning_opportunities_created"] += len(
                learning_opportunities
            )
            ...
            logger.debug(
                f"Generated consequences for choice {context.choice.choice_id} in {processing_time.total_seconds():.3f}s"
            )
            ...
            logger.error(
                f"Failed to generate consequences for choice {context.choice.choice_id}: {e}"
            )
            # Return minimal consequence set
            return ConsequenceSet(
                choice_id=context.choice.choice_id,
                consequences=[
                    {"type": "error", "description": "Unable to generate consequences"}
                ],
                consequence_type=ConsequenceType.IMMEDIATE,
            )

    async def _analyze_choice(
        self, context: ConsequenceGenerationContext
    ) -> dict[str, Any]:
        """Analyze choice for consequence generation."""
        ...
        if context.therapeutic_goals:
            ...
            for goal in context.therapeutic_goals:
                ...
                if any(
                    keyword in choice_text for keyword in self._get_goal_keywords(goal)
                ):
                    alignment_scores.append(0.8)
                else:
                    alignment_scores.append(0.2)
            analysis["alignment_with_goals"] = sum(alignment_scores) / len(
                alignment_scores
            )
    ...
    async def _generate_immediate_consequences(
        self, context: ConsequenceGenerationContext, analysis: dict[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    def _select_consequence_template(
        self, choice: UserChoice, analysis: dict[str, Any]
    ) -> str:
    ...
    async def _create_learning_opportunities(
        self, context: ConsequenceGenerationContext, analysis: dict[str, Any]
    ) -> list[str]:
        ...
        if template and "learning_opportunities" in template:
            for opportunity_type in template["learning_opportunities"]:
                opportunities.append(
                    self._format_learning_opportunity(opportunity_type, context)
                )
        ...
        if framework in self.learning_frameworks:
            ...
            if "learning_prompts" in framework_data:
                opportunities.extend(
                    framework_data["learning_prompts"][:2]
                )  # Add 2 prompts

        return opportunities

    def _format_learning_opportunity(
        self,
        ...
        opportunity_formats = {
            LearningOpportunityType.EMOTIONAL_REGULATION: "This situation offers a chance to practice recognizing and managing emotions.",
            LearningOpportunityType.COMMUNICATION_SKILLS: "This is an opportunity to practice expressing yourself clearly and listening actively.",
            LearningOpportunityType.PROBLEM_SOLVING: "This challenge can help you develop problem-solving strategies.",
            LearningOpportunityType.SELF_AWARENESS: "This moment offers insight into your patterns and reactions.",
            LearningOpportunityType.RELATIONSHIP_BUILDING: "This interaction can strengthen your connection with others.",
            LearningOpportunityType.COPING_STRATEGIES: "This situation allows you to practice healthy coping techniques.",
            LearningOpportunityType.BOUNDARY_SETTING: "This is a chance to practice setting healthy boundaries.",
            LearningOpportunityType.CONFLICT_RESOLUTION: "This conflict offers an opportunity to practice resolution skills.",
        }
        return opportunity_formats.get(
            opportunity_type, "This situation offers a learning opportunity."
        )

    def _select_therapeutic_framework(
        self, context: ConsequenceGenerationContext
    ) -> str:
    ...
    async def _frame_consequences_therapeutically(
        self,
        ...
        for consequence in consequences:
            ...
            if framing_approach == ConsequenceFraming.LEARNING_OPPORTUNITY:
                framed_consequence["therapeutic_frame"] = self._create_learning_frame(
                    consequence, context
                )
            elif framing_approach == ConsequenceFraming.POSITIVE_REINFORCEMENT:
                framed_consequence["therapeutic_frame"] = (
                    self._create_reinforcement_frame(consequence, context)
                )
            elif framing_approach == ConsequenceFraming.GROWTH_MOMENT:
                framed_consequence["therapeutic_frame"] = self._create_growth_frame(
                    consequence, context
                )
            else:
                framed_consequence["therapeutic_frame"] = (
                    "This outcome reflects the natural result of your choice."
                )

            # Add reflection prompts
            framed_consequence["reflection_prompts"] = (
                self._generate_reflection_prompts(consequence, context)
            )
    ...
    def _create_learning_frame(
        self, consequence: dict[str, Any], context: ConsequenceGenerationContext
    ) -> str:
    ...
    async def _update_choice_patterns(
        self, context: ConsequenceGenerationContext, consequence_set: ConsequenceSet
    ) -> None:
        ...
        if pattern_key not in self.choice_patterns:
            self.choice_patterns[pattern_key] = ConsequencePattern(
                choice_pattern=choice_type, consequence_theme="developing", frequency=0
            )
        ...
        if consequence_set.learning_opportunities:
            for opportunity in consequence_set.learning_opportunities:
                ...
                if "emotional" in opportunity.lower():
                    if (
                        LearningOpportunityType.EMOTIONAL_REGULATION
                        not in pattern.learning_opportunities
                    ):
                        pattern.learning_opportunities.append(
                            LearningOpportunityType.EMOTIONAL_REGULATION
                        )
    ...
    async def _publish_consequence_event(
        self, context: ConsequenceGenerationContext, consequence_set: ConsequenceSet
    ) -> None:
    ...
    async def explain_causality(
        self, choice_id: str, consequence_id: str, context: ConsequenceGenerationContext
        ...
        explanation = CausalityExplanation(
            choice_id=choice_id, consequence_id=consequence_id
        )
    ...
    def _generate_therapeutic_insight(
        self, choice: UserChoice, context: ConsequenceGenerationContext
    ) -> str:
    ...
    async def recognize_patterns(
        self, user_id: str, session_count: int = 10
    ) -> list[ConsequencePattern]:
        """Recognize patterns in user's choice-consequence history."""
        user_patterns = [
            pattern
            for pattern_key, pattern in self.choice_patterns.items()
        ...
        significant_patterns.sort(
            key=lambda p: p.therapeutic_significance, reverse=True
        )
