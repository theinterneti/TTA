...
class GameplayLoopComponent(Component, ABC):
    ...
    def __init__(
        ...
        self._agent_orchestration: Optional[AgentOrchestrationService] = None
        ...
        # Configuration
        self._max_concurrent_sessions = self.gameplay_config.get("max_concurrent_sessions", 100)
        self._session_timeout_minutes = self.gameplay_config.get("session_timeout_minutes", 30)
        self._enable_performance_tracking = self.gameplay_config.get("enable_performance_tracking", True)
        
        logger.info(f"GameplayLoopComponent {name} initialized with priority {priority.name}")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Component {self.name} could not connect to agent orchestration: {e}")
    ...
    async def get_session_context(self, session_id: str) -> Optional[GameplayLoopContext]:
    ...
    def _update_performance_metrics(self, context: GameplayLoopContext) -> None:
        ...
        session_duration = (datetime.utcnow() - context.session_start_time).total_seconds()
    ...
    async def get_agent_orchestration(self) -> Optional[AgentOrchestrationService]:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        gameplay_health = {
            "active_sessions": self.get_active_session_count(),
            "session_capacity_usage": self.get_active_session_count() / self._max_concurrent_sessions,
...
class GameplayLoopManager:
    ...
    def register_component(self, component: GameplayLoopComponent) -> None:
        ...
        logger.info(f"Registered component {component.name} with priority {component.priority.name}")
    
    def get_component(self, name: str) -> Optional[GameplayLoopComponent]:
        """Get a registered component by name."""
        return self._components.get(name)
    
    def get_components_by_priority(self, priority: ComponentPriority) -> List[GameplayLoopComponent]:
