...
class ProgressMetric(BaseModel):
    ...
    current_value: float = Field(
        ..., ge=0.0, le=1.0, description="Current progress value (0.0-1.0)"
    )
    baseline_value: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Starting baseline value"
    )
    target_value: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Target progress value"
    )
    ...
    confidence_level: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Confidence in measurement"
    )
    last_updated: datetime = Field(default_factory=datetime.utcnow)
    update_frequency: timedelta = Field(
        default=timedelta(days=7), description="How often to update"
    )

    @field_validator(
        "current_value", "baseline_value", "target_value", "confidence_level"
    )
    @classmethod
...
class SkillDevelopment(BaseModel):
    ...
    successful_applications: int = Field(
        default=0, description="Successful real-world applications"
    )
...
class EmotionalGrowth(BaseModel):
    ...
    emotion_category: str = Field(
        ..., description="Category of emotion (e.g., anxiety, anger)"
    )
    regulation_score: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Emotional regulation ability"
    )
    awareness_score: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Emotional awareness level"
    )
    expression_score: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Healthy expression ability"
    )
    ...
    emotional_incidents: int = Field(
        default=0, description="Number of emotional incidents"
    )
    successful_regulations: int = Field(
        default=0, description="Successful regulation instances"
    )
    breakthrough_moments: list[str] = Field(default_factory=list)
    last_incident: datetime | None = Field(None)
    created_at: datetime = Field(default_factory=datetime.utcnow)
    ...
    def calculate_overall_growth(self) -> float:
        ...
        return (
            self.regulation_score + self.awareness_score + self.expression_score
        ) / 3.0
...
class BehavioralChange(BaseModel):
    ...
    change_type: str = Field(
        ..., description="Type of change (increase, decrease, modify)"
    )
    target_behavior: str = Field(..., description="Desired behavior outcome")
    baseline_frequency: float = Field(
        default=0.0, description="Baseline frequency of behavior"
    )
    current_frequency: float = Field(
        default=0.0, description="Current frequency of behavior"
    )
    ...
    last_occurrence: datetime | None = Field(None)
    created_at: datetime = Field(default_factory=datetime.utcnow)
    ...
    def calculate_change_percentage(self) -> float:
        ...
        return (
            self.current_frequency - self.baseline_frequency
...
class ProgressMilestone(BaseModel):
    ...
    therapeutic_significance: float = Field(
        ..., ge=0.0, le=1.0, description="Therapeutic significance"
    )
    celebration_content: str | None = Field(
        None, description="Content for celebrating milestone"
    )
...
class TherapeuticProgress(BaseModel):
    ...
    @field_validator(
        "overall_progress_score", "therapeutic_momentum", "engagement_level"
    )
    @classmethod
    def validate_scores(cls, v):
        if not 0.0 <= v <= 1.0:
    ...
    def calculate_overall_progress(self) -> float:
        ...
        total_progress = sum(
            metric.calculate_progress_percentage() for metric in self.progress_metrics
        )
    ...
    def needs_attention(self) -> bool:
        ...
        return (
            self.overall_progress_score < 0.3
            or self.therapeutic_momentum < 0.3
            or self.engagement_level < 0.4
        )
