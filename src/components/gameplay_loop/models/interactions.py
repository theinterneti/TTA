...
class InteractionValidation(BaseModel):
    ...
    interaction_id: str = Field(
        ..., description="ID of the interaction being validated"
    )
    status: ValidationStatus = Field(..., description="Validation status")
    safety_score: float = Field(
        ..., ge=0.0, le=1.0, description="Safety assessment score"
    )
    therapeutic_relevance: float = Field(
        ..., ge=0.0, le=1.0, description="Therapeutic relevance score"
    )
    emotional_appropriateness: float = Field(
        ..., ge=0.0, le=1.0, description="Emotional appropriateness score"
    )
    ...
    @field_validator(
        "safety_score", "therapeutic_relevance", "emotional_appropriateness"
    )
    @classmethod
...
class UserInteraction(BaseModel):
    ...
    validation: InteractionValidation | None = Field(
        None, description="Validation result"
    )
    response: InteractionResponse | None = Field(
        None, description="Response to the interaction"
    )
    processing_time_ms: float | None = Field(
        None, description="Time taken to process interaction"
    )
    metadata: dict[str, Any] = Field(default_factory=dict)
    timestamp: datetime = Field(default_factory=datetime.utcnow)
    processed_at: datetime | None = Field(
        None, description="When interaction was processed"
    )
    ...
    def is_safe(self) -> bool:
        ...
        return (
            self.validation.status not in [ValidationStatus.SAFETY_CONCERN]
            and self.validation.safety_score >= 0.7
        )
    ...
    def requires_therapeutic_attention(self) -> bool:
        ...
        if self.validation is None:
            return False
        return (
            self.validation.status == ValidationStatus.THERAPEUTIC_OPPORTUNITY
            or self.validation.therapeutic_relevance >= 0.8
        )
    ...
    def get_emotional_intensity(self) -> float:
        ...
        if not self.context.user_emotional_state:
            return 0.0
        return sum(self.context.user_emotional_state.values()) / len(
            self.context.user_emotional_state
        )
