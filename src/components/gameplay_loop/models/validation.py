...
class ValidationRule(BaseModel):
    ...
    age_restrictions: dict[str, Any] | None = Field(None)
...
class SafetyCheck(BaseModel):
    ...
    safety_score: float = Field(
        ..., ge=0.0, le=1.0, description="Numerical safety score"
    )
    ...
    def is_safe_for_user(self, user_context: dict[str, Any]) -> bool:
        ...
        if self.safety_level == SafetyLevel.CONCERN:
            ...
            if any(
                emotion in ["distress", "crisis", "panic"]
    ...
    def requires_intervention(self) -> bool:
        ...
        return (
            self.safety_level in [SafetyLevel.DANGER, SafetyLevel.CRISIS]
            or self.escalation_required
            or len(self.crisis_indicators) > 0
        )
...
class TherapeuticAlignment(BaseModel):
    ...
    alignment_type: TherapeuticAlignmentType = Field(
        ..., description="Type of therapeutic alignment"
    )
    ...
    def is_therapeutically_beneficial(self) -> bool:
        ...
        return (
            self.alignment_score >= 0.6
            and len(self.contraindications) == 0
            and len(self.therapeutic_goals) > 0
        )
...
class ContentValidation(BaseModel):
    ...
    content_type: str = Field(
        ..., description="Type of content (scene, choice, response, etc.)"
    )
    ...
    therapeutic_alignment: TherapeuticAlignment = Field(
        ..., description="Therapeutic alignment"
    )
    ...
    validated_by: str = Field(
        default="system", description="Who/what performed validation"
    )
    ...
    def calculate_overall_score(self) -> float:
        ...
        therapeutic_component = (
            self.therapeutic_alignment.alignment_score * therapeutic_weight
        )
    ...
    def is_ready_for_use(self) -> bool:
        ...
        return (
            self.is_approved
            and not self.requires_revision
            and len(self.validation_errors) == 0
            and self.safety_check.safety_level
            not in [SafetyLevel.DANGER, SafetyLevel.CRISIS]
        )
        ...
        for error in self.validation_errors:
            if error.get("severity") == ValidationLevel.CRITICAL:
                blocking_issues.append(
                    error.get("message", "Critical validation error")
                )
...
class ValidationResult(BaseModel):
    ...
    def _update_batch_scores(self) -> None:
        ...
        safety_scores = [
            cv.safety_check.safety_score for cv in self.content_validations
        ]
        therapeutic_scores = [
            cv.therapeutic_alignment.alignment_score for cv in self.content_validations
        ]
