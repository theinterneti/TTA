...
from src.components.gameplay_loop.models.core import (
    GameplaySession, NarrativeScene, UserChoice, ConsequenceSet, TherapeuticOutcome
)
from src.components.gameplay_loop.models.progress import (
    TherapeuticProgress, ProgressMetric, SkillDevelopment, EmotionalGrowth, BehavioralChange
)
from src.components.gameplay_loop.database.schema import GraphSchema, NodeType, RelationshipType
from src.components.gameplay_loop.database.queries import NarrativeQueries, ProgressQueries, SessionQueries
...
class Neo4jConnectionManager:
    ...
    async def connect(self) -> bool:
        ...
        for attempt in range(self._max_retries):
            try:
                logger.debug(f"Neo4j connection attempt {attempt + 1}/{self._max_retries}")
                ...
                if attempt < self._max_retries - 1:
                    ...
                    logger.error(f"Failed to connect to Neo4j after {self._max_retries} attempts")
                    raise e
...
class Neo4jGameplayManager:
    ...
    async def create_session_node(self, session: GameplaySession) -> bool:
        ...
        try:
            ...
            parameters = {
                ...
                "session_metrics": session.session_metrics.dict() if session.session_metrics else {}
            }
    ...
    async def update_session_state(self, session_id: str, new_state: str, completed_at: Optional[datetime] = None) -> bool:
...
class NarrativeGraphManager:
    ...
    async def create_narrative_flow(self, from_scene_id: str, to_scene_id: str, 
                                  choice_id: Optional[str] = None, 
                                  flow_properties: Optional[Dict[str, Any]] = None) -> bool:
...
class ProgressGraphManager:
    ...
    async def create_progress_metric_node(self, metric: ProgressMetric, user_id: str) -> bool:
    ...
    async def update_progress_metric(self, metric_id: str, new_value: float, 
    ...
    async def get_user_progress_metrics(self, user_id: str, 
                                      progress_type: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    async def create_skill_development_node(self, skill: SkillDevelopment, user_id: str) -> bool:
...
class RelationshipManager:
    ...
    async def create_relationship(self, from_node_id: str, from_node_type: NodeType,
                                to_node_id: str, to_node_type: NodeType,
        ...
        try:
            ...
            logger.debug(f"Created relationship: {from_node_id} -[{relationship_type.value}]-> {to_node_id}")
            return True
    ...
    async def get_related_nodes(self, node_id: str, node_type: NodeType,
                              relationship_type: RelationshipType,
