...
class GraphSchema:
    ...
    NODE_PROPERTIES: Dict[NodeType, NodeProperties] = {
        NodeType.SESSION: NodeProperties(
            ...
            optional={"character_id", "world_id", "therapeutic_goals", "safety_level", 
                     "completed_at", "last_activity", "session_metrics"},
        ...
        NodeType.SCENE: NodeProperties(
            ...
            optional={"description", "narrative_content", "therapeutic_focus", 
                     "emotional_tone", "completion_criteria", "completed_at"},
        ...
        NodeType.CHOICE: NodeProperties(
            ...
            optional={"therapeutic_relevance", "emotional_weight", "difficulty_level",
        ...
        NodeType.THERAPEUTIC_GOAL: NodeProperties(
            ...
            optional={"description", "target_metrics", "completion_criteria", "priority"},
        ...
        NodeType.SKILL: NodeProperties(
            ...
            optional={"description", "learning_objectives", "mastery_indicators", 
        ...
        NodeType.PROGRESS_METRIC: NodeProperties(
            ...
            optional={"baseline_value", "target_value", "measurement_method", 
        ...
        NodeType.MILESTONE: NodeProperties(
            ...
            optional={"description", "achievement_criteria", "therapeutic_significance",
                     "celebration_content", "related_skills"},
            indexed={"milestone_id", "milestone_type", "achieved_at"},
            unique={"milestone_id"}
        ),
        
        NodeType.SAFETY_CHECK: NodeProperties(
            required={"check_id", "content_id", "safety_level", "safety_score", "checked_at"},
            optional={"risk_factors", "protective_factors", "crisis_indicators",
                     "intervention_recommendations", "requires_human_review"},
    ...
    def get_node_indexes(cls) -> List[str]:
        ...
        for node_type, properties in cls.NODE_PROPERTIES.items():
            ...
            for indexed_prop in properties.indexed:
                if indexed_prop not in properties.unique:  # Unique constraints create indexes automatically
            ...
            if node_type == NodeType.SESSION:
                indexes.append(f"CREATE INDEX session_user_state_index IF NOT EXISTS FOR (n:Session) ON (n.user_id, n.session_state)")
                indexes.append(f"CREATE INDEX session_created_state_index IF NOT EXISTS FOR (n:Session) ON (n.created_at, n.session_state)")
            
            elif node_type == NodeType.SCENE:
                indexes.append(f"CREATE INDEX scene_session_type_index IF NOT EXISTS FOR (n:Scene) ON (n.session_id, n.scene_type)")
            
            elif node_type == NodeType.PROGRESS_METRIC:
                indexes.append(f"CREATE INDEX metric_type_updated_index IF NOT EXISTS FOR (n:ProgressMetric) ON (n.progress_type, n.last_updated)")
