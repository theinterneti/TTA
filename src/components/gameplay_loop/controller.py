...
from .models.core import SessionState, Scene, Choice, ConsequenceSet, EmotionalState
from .models.interactions import UserChoice, ChoiceOutcome, GameplaySession
...
class GameplayLoopController:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.choice_architecture = ChoiceArchitectureManager(config.get("choice_architecture", {}))
        self.consequence_system = ConsequenceSystem(config.get("consequence_system", {}))
        ...
        self.response_time_target = self.config.get("response_time_target", 2.0)  # 2 seconds
    ...
    async def start_session(self,
                          user_id: str,
                          therapeutic_context: Optional[Dict[str, Any]] = None) -> GameplaySession:
        """
        ...
        try:
            ...
            opening_scene = await self.narrative_engine.generate_opening_scene(session_state)
            if opening_scene:
                session_state.current_scene = opening_scene
            
            # Generate initial choices
            initial_choices = await self.choice_architecture.generate_choices_for_scene(
    ...
    async def resume_session(self, session_id: str) -> Optional[GameplaySession]:
        """
        ...
        try:
            ...
            if not current_scene:
                current_scene = await self.narrative_engine.generate_next_scene(session_state)
                session_state.current_scene = current_scene
            
            # Generate current choices
            available_choices = await self.choice_architecture.generate_choices_for_scene(
    ...
    async def end_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            if session_id in self.active_sessions:
                ...
                await self.database_manager.save_session_summary(session_id, session_summary)
    ...
    async def process_user_choice(self,
                                session_id: str,
                                choice_id: str) -> Tuple[Optional[Scene], List[Choice], Optional[ConsequenceSet]]:
        """
        ...
        try:
            ...
            if consequences.emotional_impact:
                new_emotion = consequences.emotional_impact.get("primary_emotion", "neutral")
            ...
            new_choices = await self.choice_architecture.generate_choices_for_scene(
            ...
            if processing_time > self.response_time_target:
                logger.warning(f"Choice processing took {processing_time:.2f}s (target: {self.response_time_target}s)")
            
            logger.info(f"Choice processed successfully in {processing_time:.2f}s")
            return next_scene, new_choices, consequences
            
        except Exception as e:
            logger.error(f"Failed to process choice {choice_id} for session {session_id}: {e}")
            return None, [], None
    ...
    async def _generate_session_recap(self, session_state: SessionState) -> str:
        ...
        try:
            ...
            recent_choices = session_state.choice_history[-3:] if choice_count >= 3 else session_state.choice_history
            therapeutic_themes = []
    ...
    async def _generate_session_summary(self, session: GameplaySession) -> Dict[str, Any]:
        ...
        try:
            ...
            if choice_history:
                therapeutic_values = [choice.get("therapeutic_value", 0) for choice in choice_history]
                summary["therapeutic_engagement"] = sum(therapeutic_values) / len(therapeutic_values)
            ...
            if summary["therapeutic_engagement"] > 0.7:
                summary["key_insights"].append("Demonstrated strong therapeutic engagement")

            if len(summary["skills_practiced"]) >= 3:
                summary["key_insights"].append("Explored multiple therapeutic approaches")

            if summary["total_choices"] >= 10:
                summary["key_insights"].append("Sustained participation in therapeutic process")

            return summary

        except Exception as e:
            logger.error(f"Failed to generate session summary: {e}")
            return {"session_id": session.session_id, "error": "Summary generation failed"}
    ...
    async def get_session_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            status = {
                ...
                "current_scene_id": session_state.current_scene.scene_id if session_state.current_scene else None,
            ...
            if session_state.choice_history:
                therapeutic_values = [choice.get("therapeutic_value", 0) for choice in session_state.choice_history]
                status["therapeutic_engagement"] = sum(therapeutic_values) / len(therapeutic_values)
