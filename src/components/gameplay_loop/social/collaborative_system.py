...
from src.components.gameplay_loop.controllers.gameplay_loop_controller import GameplayLoopController
from src.components.gameplay_loop.narrative.replayability_system import ReplayabilitySystem
...
class CollaborativeSession:
    ...
    # Session state
    base_session_state: Optional[SessionState] = None
    ...
    human_moderator_id: Optional[str] = None
...
class GroupChoice:
    ...
    votes: Dict[str, str] = field(default_factory=dict)  # user_id -> vote (support/oppose/abstain)
...
class SupportMessage:
    ...
    # Context
    related_choice_id: Optional[str] = None
...
class ConflictResolution:
    ...
    # Conflict details
    conflict_type: str = "disagreement"  # disagreement, inappropriate_behavior, therapeutic_concern
    involved_participants: List[str] = field(default_factory=list)  # user_ids
    description: str = ""
    
    # Resolution process
    resolution_method: str = "mediation"  # mediation, voting, facilitator_decision, therapeutic_intervention
...
class CollaborativeSystem:
    ...
    def __init__(self, event_bus: EventBus, gameplay_controller: GameplayLoopController, 
    ...
    async def create_collaborative_session(self, host_user_id: str, session_name: str,
                                         collaborative_mode: CollaborativeMode = CollaborativeMode.COOPERATIVE,
                                         therapeutic_focus: List[str] = None,
                                         max_participants: int = 4) -> CollaborativeSession:
        ...
        try:
            ...
            session = CollaborativeSession(
                ...
                session_name=session_name or f"{collaborative_mode.value.title()} Session",
            ...
            await self._publish_collaborative_event(session, "collaborative_session_created", {
            ...
            logger.error(f"Failed to create collaborative session for user {host_user_id}: {e}")
            raise

    async def join_collaborative_session(self, session_id: str, user_id: str,
                                       role: ParticipantRole = ParticipantRole.PARTICIPANT,
                                       therapeutic_goals: List[str] = None) -> CollaborativeParticipant:
        ...
        try:
            ...
            if len(session.participants) >= session.max_participants:
                ...
                raise ValueError(f"Session {session_id} is full. Added to waiting list.")

            # Check if facilitator is required and available
            if session.requires_facilitator and role != ParticipantRole.FACILITATOR:
                has_facilitator = any(p.role == ParticipantRole.FACILITATOR for p in session.participants.values())
                if not has_facilitator:
                    raise ValueError(f"Session {session_id} requires a facilitator to be present")
            ...
            # Publish join event
            await self._publish_collaborative_event(session, "participant_joined", {
                "user_id": user_id,
                "role": role.value,
            ...
            logger.error(f"Failed to join collaborative session {session_id} for user {user_id}: {e}")
            raise

    async def start_collaborative_session(self, session_id: str, base_session_state: SessionState) -> CollaborativeSession:
        ...
        try:
            ...
            if len(session.participants) < 2 and session.collaborative_mode != CollaborativeMode.SOLO:
            ...
            if session.requires_facilitator:
                has_facilitator = any(p.role == ParticipantRole.FACILITATOR for p in session.participants.values())
                if not has_facilitator:
                    raise ValueError(f"Session {session_id} requires a facilitator to start")
            ...
            await self._publish_collaborative_event(session, "collaborative_session_started", {
                "participant_count": len(session.participants),
                "therapeutic_focus": session.therapeutic_focus,
                "collaborative_mode": session.collaborative_mode.value
            })
    ...
    async def propose_group_choice(self, session_id: str, proposer_user_id: str,
                                 choice_text: str, choice_type: ChoiceType = ChoiceType.GENERAL) -> GroupChoice:
        ...
        try:
            ...
            if not participant or not participant.can_make_choices:
                raise ValueError(f"User {proposer_user_id} cannot propose choices in this session")
            ...
            group_choice = GroupChoice(
                ...
                approval_threshold=session.shared_context.get("consensus_threshold", 0.6)
            )

            # Assess therapeutic impact and safety
            group_choice.therapeutic_impact = await self._assess_choice_therapeutic_impact(
            ...
            await self._publish_collaborative_event(session, "group_choice_proposed", {
                "choice_id": group_choice.choice_id,
    ...
    async def vote_on_group_choice(self, choice_id: str, voter_user_id: str, vote: str) -> bool:
        ...
        try:
            ...
            if not session or not session.is_active:
                raise ValueError(f"Session not found or not active")
            ...
            total_participants = len([p for p in session.participants.values() if p.can_make_choices])
            votes_cast = len(group_choice.votes)
            support_votes = len([v for v in group_choice.votes.values() if v == "support"])

            # Calculate approval percentage
            approval_percentage = support_votes / total_participants if total_participants > 0 else 0

            # Check if threshold is met or all votes are in
            if approval_percentage >= group_choice.approval_threshold or votes_cast >= total_participants:
                group_choice.is_approved = approval_percentage >= group_choice.approval_threshold
                ...
                await self._publish_collaborative_event(session, "group_choice_decided", {
                    "choice_id": choice_id,
            ...
            # Publish vote event
            await self._publish_collaborative_event(session, "group_choice_vote_cast", {
                "choice_id": choice_id,
    ...
    async def send_support_message(self, session_id: str, sender_id: str, recipient_id: str,
                                 message_text: str, message_type: str = "encouragement") -> SupportMessage:
        ...
        try:
            ...
            if not sender or not sender.can_communicate:
                raise ValueError(f"User {sender_id} cannot send messages in this session")
            ...
            if support_message.is_approved:
                ...
                await self._publish_collaborative_event(session, "support_message_sent", {
                    "message_id": support_message.message_id,
                    "sender_id": sender_id,
                    "recipient_id": recipient_id,
                    "message_type": message_type,
    ...
    async def report_conflict(self, session_id: str, reporter_id: str, conflict_type: str,
                            involved_participants: List[str], description: str) -> ConflictResolution:
        ...
        try:
            ...
            if conflict_type == "therapeutic_concern":
                ...
                # Find facilitator
                facilitator = next((p for p in session.participants.values()
                                  if p.role == ParticipantRole.FACILITATOR), None)
                if facilitator:
            ...
            self.conflict_resolutions[conflict_resolution.resolution_id] = conflict_resolution
            ...
            await self._publish_collaborative_event(session, "conflict_reported", {
                "resolution_id": conflict_resolution.resolution_id,
                "conflict_type": conflict_type,
                "involved_participants": involved_participants,
    ...
    async def moderate_participant(self, session_id: str, moderator_id: str, target_user_id: str,
                                 action: ModerationAction, reason: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not moderator or moderator.role not in [ParticipantRole.HOST, ParticipantRole.FACILITATOR]:
                raise ValueError(f"User {moderator_id} does not have moderation permissions")
            ...
            if action == ModerationAction.WARNING:
                ...
                await self._trigger_therapeutic_intervention(target_user_id, session, reason)
            ...
            await self._publish_collaborative_event(session, "moderation_action_applied", {
                "action": action.value,
                "target_user_id": target_user_id,
                "moderator_id": moderator_id,
                "reason": reason
            ...
            logger.error(f"Failed to moderate participant {target_user_id} in session {session_id}: {e}")
            raise

    async def _initialize_group_therapeutic_context(self, session: CollaborativeSession) -> None:
        ...
        try:
            ...
            session.shared_context["group_therapeutic_framework"] = {
                ...
                "support_mechanisms": ["peer_encouragement", "shared_experiences", "group_reflection"],
                "safety_protocols": ["crisis_intervention", "boundary_respect", "confidentiality"],
                "progress_tracking": "collective_and_individual"
            }

            # Initialize group dynamics
            session.shared_context["group_dynamics"]["established_at"] = datetime.utcnow().isoformat()
            session.shared_context["group_dynamics"]["trust_level"] = 0.5  # Starting trust level
            session.shared_context["group_dynamics"]["cohesion_score"] = 0.3  # Starting cohesion

        except Exception as e:
            logger.error(f"Failed to initialize group therapeutic context: {e}")

    async def _assess_choice_therapeutic_impact(self, choice_text: str, choice_type: ChoiceType,
                                              therapeutic_focus: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            focus_keywords = {
                "anxiety_management": ["calm", "breathe", "relax", "peaceful", "mindful"],
                "communication_skills": ["talk", "listen", "express", "share", "communicate"],
                "emotional_regulation": ["feel", "emotion", "manage", "cope", "process"],
                "problem_solving": ["solve", "plan", "strategy", "approach", "solution"],
                "self_awareness": ["reflect", "understand", "realize", "insight", "awareness"]
            }
            ...
            for focus_area in therapeutic_focus:
                if focus_area in focus_keywords:
                    ...
                    if matches > 0:
                        ...
                        impact_assessment["potential_benefits"].append(f"Supports {focus_area}")

            if therapeutic_focus:
                impact_assessment["alignment_with_focus"] = min(1.0, alignment_score / len(therapeutic_focus))
            ...
            impact_assessment["therapeutic_relevance"] = therapeutic_choice_types.get(choice_type, 0.5)
            ...
            if any(indicator in choice_lower for indicator in concern_indicators):
                impact_assessment["potential_concerns"].append("May discourage therapeutic progress")
            ...
            return {"therapeutic_relevance": 0.5, "alignment_with_focus": 0.5, "potential_benefits": [], "potential_concerns": [], "group_suitability": 0.5}

    async def _assess_choice_safety(self, choice_text: str, safety_level: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for indicator in self.safety_keywords["crisis_indicators"]:
                if indicator in choice_lower:
                    ...
                    safety_assessment["safety_concerns"].append("Contains crisis language")
                ...
                if category != "crisis_indicators":
                    for keyword in keywords:
                        if keyword in choice_lower:
                            ...
                            safety_assessment["safety_concerns"].append(f"Contains {category.replace('_', ' ')}")
            ...
            return {"safety_score": 0.5, "safety_concerns": ["Assessment failed"], "requires_immediate_review": True, "recommended_modifications": []}

    async def _moderate_message(self, message: SupportMessage, session: CollaborativeSession) -> Dict[str, Any]:
        ...
        try:
            ...
            for category, keywords in self.safety_keywords.items():
                for keyword in keywords:
                    if keyword in message_lower:
                        ...
                        moderation_result["notes"] = f"Contains {category.replace('_', ' ')}"
            ...
            if message.message_type == "advice" and session.requires_facilitator:
                ...
                if sender and sender.role not in [ParticipantRole.FACILITATOR, ParticipantRole.MENTOR]:
                    moderation_result["approved"] = False
                    moderation_result["notes"] = "Advice should come from facilitators in this session type"
            ...
            return {"approved": False, "confidence": 0.0, "notes": "Moderation failed", "required_actions": ["human_review"]}

    async def _execute_group_choice(self, group_choice: GroupChoice, session: CollaborativeSession) -> None:
        ...
        try:
            ...
            user_choice = UserChoice(
                ...
                therapeutic_relevance=group_choice.therapeutic_impact.get("therapeutic_relevance", 0.5)
            )
            ...
            logger.error(f"Failed to execute group choice {group_choice.choice_id}: {e}")

    async def _notify_participants_of_choice(self, session: CollaborativeSession, group_choice: GroupChoice) -> None:
        ...
        try:
            ...
            for participant in session.participants.values():
                if participant.can_make_choices and participant.user_id != group_choice.proposed_by:
    ...
    async def _initiate_conflict_resolution(self, conflict_resolution: ConflictResolution,
                                          session: CollaborativeSession) -> None:
        ...
        try:
            if conflict_resolution.resolution_method == "therapeutic_intervention":
                ...
                for participant_id in conflict_resolution.involved_participants:
                    await self._trigger_therapeutic_intervention(participant_id, session,
    ...
    async def _trigger_therapeutic_intervention(self, user_id: str, session: CollaborativeSession, reason: str) -> None:
    ...
    async def _escalate_to_human_moderator(self, session: CollaborativeSession, user_id: str, reason: str) -> None:
    ...
    async def _publish_collaborative_event(self, session: CollaborativeSession, event_type: str, context: Dict[str, Any]) -> None:
    ...
    def get_user_collaborative_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get session details
            sessions = [self.collaborative_sessions[session_id] for session_id in user_sessions
            ...
            modes_used = list(set(session.collaborative_mode.value for session in sessions))

            # Get roles played
            roles_played = list(set(session.participants[user_id].role.value for session in sessions
                                  if user_id in session.participants))
            ...
            return {
                ...
                "collaborative_summary": {
                    ...
                    "sessions_hosted": len([s for s in sessions if s.host_user_id == user_id])
                },
                "participation_metrics": {
                    ...
                    "support_ratio": total_support_given / max(1, total_support_received)
                },
                "collaborative_experience": {
                    "modes_experienced": modes_used,
                    "roles_played": roles_played,
                    "preferred_mode": self._get_preferred_collaborative_mode(sessions, user_id),
                    "collaboration_comfort_level": self._calculate_collaboration_comfort(sessions, user_id)
                },
                "recent_sessions": [
                    {
                        "session_id": session.session_id,
                        ...
                        "collaborative_mode": session.collaborative_mode.value,
                        "role": session.participants[user_id].role.value if user_id in session.participants else "unknown",
                        "is_active": session.is_active,
                        "created_at": session.created_at.isoformat()
                    }
                    for session in sorted(sessions, key=lambda s: s.created_at, reverse=True)[:5]
                ]
            }
    ...
    def _get_preferred_collaborative_mode(self, sessions: List[CollaborativeSession], user_id: str) -> str:
        ...
        return max(mode_counts.keys(), key=lambda k: mode_counts[k]) if mode_counts else "cooperative"

    def _calculate_collaboration_comfort(self, sessions: List[CollaborativeSession], user_id: str) -> float:
        ...
        for session in sessions:
            if user_id in session.participants:
                ...
                participation_score = min(1.0, (participant.choices_made + participant.messages_sent) / 10)
    ...
    def get_session_analytics(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            active_participants = len([p for p in session.participants.values() if p.is_active])

            # Calculate engagement metrics
            total_choices = len(session.group_choices)
            total_messages = len(session.support_messages)
            total_support_given = sum(p.support_given for p in session.participants.values())
            ...
            avg_comfort_level = sum(p.comfort_level for p in session.participants.values()) / max(1, total_participants)
            ...
            return {
                ...
                "engagement_metrics": {
                    ...
                    "total_support_given": total_support_given,
                    "avg_choices_per_participant": total_choices / max(1, total_participants),
                    "avg_messages_per_participant": total_messages / max(1, total_participants)
                },
                "therapeutic_metrics": {
                    "unique_therapeutic_goals": len(therapeutic_goals),
                    "avg_comfort_level": avg_comfort_level,
                    "support_ratio": support_ratio,
                    "therapeutic_interventions": len([log for log in session.moderation_log
                                                    if log.get("type") == "therapeutic_intervention"])
                },
                "safety_metrics": {
                    "moderation_actions": len(session.moderation_log),
                    "conflicts_reported": len([log for log in session.moderation_log
                                             if log.get("type") == "conflict_reported"]),
                    "safety_escalations": len([log for log in session.moderation_log
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.metrics,
            ...
            "active_collaborative_sessions": len([s for s in self.collaborative_sessions.values() if s.is_active]),
            ...
            "avg_participants_per_session": sum(len(s.participants) for s in self.collaborative_sessions.values()) / max(1, len(self.collaborative_sessions))
        }
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "safety_keywords_loaded": sum(len(keywords) for keywords in self.safety_keywords.values()),
    ...
    def _load_safety_keywords(self) -> Dict[str, List[str]]:
        ...
        return {
            "crisis_indicators": [
                "hurt myself", "end it all", "can't go on", "no point", "give up",
                "suicide", "kill myself", "better off dead", "worthless", "hopeless"
            ],
            "inappropriate_content": [
                "explicit_content", "harassment_terms", "discriminatory_language"
            ],
            "therapeutic_boundaries": [
                "personal_information", "contact_details", "meeting_requests"
            ]
        }
