...
from .models import ValidationResult, ContentPayload, ValidationContext
from .enums import ValidationStatus, SafetyLevel
...
class ValidationCache:
    ...
    def _generate_cache_key(self, content: ContentPayload, context: ValidationContext) -> str:
        ...
        context_factors = {
            "user_age_group": context.user_age_group.value if context.user_age_group else None,
            "validation_scope": context.validation_scope.value,
            "strict_mode": context.strict_mode,
            "user_therapeutic_goals": [goal.value for goal in context.user_therapeutic_goals],
    ...
    async def get(self, content: ContentPayload, context: ValidationContext) -> Optional[ValidationResult]:
    ...
    async def set(self, content: ContentPayload, context: ValidationContext, 
                  result: ValidationResult, ttl: Optional[timedelta] = None) -> bool:
    ...
    async def invalidate(self, content: ContentPayload, context: ValidationContext) -> bool:
...
class SafetyResultCache:
    ...
    async def get_or_validate(self, content: ContentPayload, context: ValidationContext,
                             validator_func) -> ValidationResult:
    ...
    async def cache_batch_results(self, results: List[Tuple[ContentPayload, ValidationContext, ValidationResult]],
    ...
    async def get_similar_results(self, content: ContentPayload, 
                                 similarity_threshold: float = 0.8) -> List[ValidationResult]:
    ...
    def analyze_cache_patterns(self) -> Dict[str, Any]:
        ...
        patterns = {
            ...
            "cache_effectiveness": "high" if stats["hit_rate_percent"] > 70 else "medium" if stats["hit_rate_percent"] > 40 else "low",
            "total_validations_cached": stats["cache_sets"],
            "cache_errors_rate": (stats["cache_errors"] / max(stats["total_requests"], 1)) * 100
        }
    ...
    def _generate_cache_recommendations(self, patterns: Dict[str, Any]) -> List[str]:
        ...
        if patterns["cache_errors_rate"] > 5:
            recommendations.append("High cache error rate detected - check Redis connection")
        
        if patterns["total_validations_cached"] < 100:
            recommendations.append("Low cache usage - consider enabling caching for more validation types")
        
        return recommendations
    
    async def warm_cache(self, common_content: List[Tuple[ContentPayload, ValidationContext]],
                        validator_func) -> int:
