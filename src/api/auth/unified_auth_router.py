...
async def get_current_user(
    ...
    try:
        ...
        auth_service.audit_logger.log_authentication_event(
            user.id, user.username, "api_access", True,
            session.interface_type, ip_address, session.session_id
        )
...
async def login(credentials: LoginCredentials, request: Request):
    ...
    try:
        ...
        logger.info(f"Successful login: {credentials.username} -> {credentials.interface_type.value}")
        ...
        "logs": [
            {
                ...
                "interfaceType": log.interface_type.value if log.interface_type else None,
                "success": log.success,
