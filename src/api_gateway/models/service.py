...
class ServiceEndpoint(BaseModel):
    ...
    scheme: str = Field(default="http", description="Service scheme (http/https/ws/wss)")
...
class ServiceMetrics(BaseModel):
    ...
    last_response_time: float = Field(default=0.0, description="Last response time in seconds")
...
class ServiceInfo(BaseModel):
    ...
    tags: List[str] = Field(default_factory=list, description="Service tags for filtering")
    ...
    health_check: ServiceHealthCheck = Field(default_factory=ServiceHealthCheck, description="Health check configuration")
    metrics: ServiceMetrics = Field(default_factory=ServiceMetrics, description="Service metrics")
    
    # Timestamps
    registered_at: datetime = Field(default_factory=datetime.utcnow, description="Service registration timestamp")
    last_seen: datetime = Field(default_factory=datetime.utcnow, description="Last heartbeat timestamp")
    
    # Therapeutic-specific fields
    therapeutic_priority: bool = Field(default=False, description="High priority for therapeutic sessions")
    crisis_support: bool = Field(default=False, description="Supports crisis intervention")
    safety_validated: bool = Field(default=False, description="Validated for therapeutic safety")
    ...
    def is_therapeutic(self) -> bool:
        ...
        return (
            self.service_type == ServiceType.THERAPEUTIC or
            self.therapeutic_priority or
            'therapeutic' in self.tags
        )
...
class ServiceRegistry(BaseModel):
    ...
    services: Dict[str, ServiceInfo] = Field(default_factory=dict, description="Registered services by ID")
    load_balancing: LoadBalancingAlgorithm = Field(default=LoadBalancingAlgorithm.ROUND_ROBIN, description="Load balancing algorithm")
