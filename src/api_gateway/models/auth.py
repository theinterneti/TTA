...
class TherapeuticPermission(BaseModel):
    ...
    can_access_sessions: bool = Field(default=False, description="Can access therapeutic sessions")
    can_view_progress: bool = Field(default=False, description="Can view patient progress")
    can_modify_treatment: bool = Field(default=False, description="Can modify treatment plans")
    can_handle_crisis: bool = Field(default=False, description="Can handle crisis situations")
    can_access_sensitive_data: bool = Field(default=False, description="Can access sensitive patient data")
    supervision_required: bool = Field(default=True, description="Requires supervision for therapeutic actions")


class UserPermissions(BaseModel):
    """User permissions and access control."""
    role: UserRole = Field(..., description="Primary user role")
    permissions: List[PermissionLevel] = Field(default_factory=list, description="General permissions")
    therapeutic: TherapeuticPermission = Field(default_factory=TherapeuticPermission, description="Therapeutic permissions")
    service_access: Dict[str, List[str]] = Field(default_factory=dict, description="Service-specific access permissions")
    rate_limit_multiplier: float = Field(default=1.0, ge=0.1, le=10.0, description="Rate limit multiplier")
    ...
    def is_therapeutic_user(self) -> bool:
        ...
        return (
            self.role in [UserRole.THERAPIST, UserRole.PATIENT] or
            self.therapeutic.can_access_sessions
        )
...
class AuthContext(BaseModel):
    ...
    therapeutic_session_id: Optional[UUID] = Field(default=None, description="Active therapeutic session ID")
    ...
    # Therapeutic context
    in_therapeutic_session: bool = Field(default=False, description="Currently in therapeutic session")
    crisis_mode: bool = Field(default=False, description="Crisis mode activated")
    safety_level: int = Field(default=1, ge=1, le=5, description="Safety monitoring level (1-5)")
    ...
    def is_therapeutic_context(self) -> bool:
        ...
        return (
            self.in_therapeutic_session or
            self.therapeutic_session_id is not None or
            self.permissions.is_therapeutic_user()
        )
    ...
    def requires_elevated_permissions(self) -> bool:
        ...
        return (
            self.crisis_mode or
            self.safety_level >= 4 or
            self.permissions.therapeutic.can_handle_crisis
        )
...
class AuthenticationRequest(BaseModel):
    ...
    client_ip: str = Field(..., description="Client IP address")
    user_agent: str = Field(default="unknown", description="Client user agent")
    service_name: Optional[str] = Field(default=None, description="Requesting service name")
    therapeutic_context: bool = Field(default=False, description="Request is in therapeutic context")


class AuthenticationResponse(BaseModel):
    """Authentication response model."""
    success: bool = Field(..., description="Authentication success status")
    auth_context: Optional[AuthContext] = Field(default=None, description="Authentication context if successful")
...
class TokenValidationResult(BaseModel):
    ...
    permissions: Optional[UserPermissions] = Field(default=None, description="User permissions if valid")
    expires_at: Optional[datetime] = Field(default=None, description="Token expiration if valid")
