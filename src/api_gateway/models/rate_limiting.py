...
class RateLimitRule(BaseModel):
    ...
    path_patterns: list[str] = Field(
        default_factory=list, description="Path patterns to match"
    )
    methods: list[str] = Field(
        default_factory=list, description="HTTP methods to match"
    )
    user_roles: list[str] = Field(
        default_factory=list, description="User roles to match"
    )
    service_names: list[str] = Field(
        default_factory=list, description="Service names to match"
    )

    # Priority and exceptions
    priority: TrafficPriority = Field(
        default=TrafficPriority.NORMAL, description="Traffic priority"
    )
    bypass_therapeutic: bool = Field(
        default=False, description="Bypass for therapeutic sessions"
    )
    bypass_crisis: bool = Field(
        default=True, description="Bypass for crisis situations"
    )

    # Actions
    block_on_exceed: bool = Field(
        default=True, description="Block requests when limit exceeded"
    )
    delay_on_exceed: float | None = Field(
        default=None, description="Delay in seconds when limit exceeded"
    )
    custom_response: dict[str, Any] | None = Field(
        default=None, description="Custom response when limit exceeded"
    )
    ...
    def matches_request(
        ...
        if self.path_patterns and "*" not in self.path_patterns:
            ...
            if not any(
                fnmatch.fnmatch(path, pattern) for pattern in self.path_patterns
            ):
                return False
...
class RateLimitConfig(BaseModel):
    ...
    burst_limit: int = Field(
        default=200, ge=1, description="Burst limit for short periods"
    )

    # Therapeutic-specific configuration
    therapeutic_multiplier: float = Field(
        default=2.0,
        ge=1.0,
        description="Rate limit multiplier for therapeutic sessions",
    )
    crisis_bypass: bool = Field(
        default=True, description="Bypass rate limits for crisis situations"
    )
    ...
    storage_backend: str = Field(
        default="redis", description="Storage backend for rate limit data"
    )
    key_prefix: str = Field(
        default="tta:gateway:ratelimit", description="Key prefix for storage"
    )

    # Monitoring
    log_violations: bool = Field(default=True, description="Log rate limit violations")
    alert_on_violations: bool = Field(
        default=True, description="Alert on rate limit violations"
    )
    ...
    def get_applicable_rules(
        ...
        return [
            ...
            if rule.enabled
            and rule.matches_request(path, method, user_role, service_name)
        ]
...
class RateLimitStatus(BaseModel):
    ...
    first_request: datetime = Field(
        default_factory=datetime.utcnow, description="First request in window"
    )
...
class TherapeuticEvent(BaseModel):
    ...
    session_id: UUID | None = Field(
        default=None, description="Therapeutic session identifier"
    )
    ...
    crisis_level: int | None = Field(
        default=None, ge=1, le=5, description="Crisis severity level (1-5)"
    )
    intervention_required: bool = Field(
        default=False, description="Requires immediate intervention"
    )
