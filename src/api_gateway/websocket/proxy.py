...
from ..models import ServiceInfo, AuthContext
from ..services import ServiceDiscoveryManager
from ..core.service_router import ServiceRouter
from .connection_manager import WebSocketConnectionManager, ConnectionType, WebSocketConnection
...
class WebSocketProxy:
    ...
    def __init__(self, connection_manager: WebSocketConnectionManager,
                 service_router: ServiceRouter, discovery_manager: ServiceDiscoveryManager):
        """
        ...
        self.backend_to_connections: Dict[str, List[str]] = {}  # backend_key -> connection_ids
    ...
    async def handle_connection(self, websocket: WebSocket, connection_type: ConnectionType,
                              service_name: str, auth_context: Optional[AuthContext] = None,
                              therapeutic_session_id: Optional[str] = None) -> None:
        """
        ...
        try:
            # Get target service
            target_service = await self._select_backend_service(service_name, auth_context)
            ...
            backend_key = await self._connect_to_backend(target_service, connection_id, auth_context)
            if not backend_key:
                await self.connection_manager.disconnect(connection_id, "backend_unavailable")
                return
            ...
            # Cleanup
            if connection_id:
                await self.connection_manager.disconnect(connection_id, "connection_ended")
            if backend_key:
                await self._disconnect_from_backend(backend_key, connection_id)
    
    async def _select_backend_service(self, service_name: str, 
                                    auth_context: Optional[AuthContext]) -> Optional[ServiceInfo]:
        ...
        try:
            ...
            gateway_request = GatewayRequest(
                ...
                client_ip="websocket",
                is_therapeutic=auth_context.is_therapeutic_context() if auth_context else False,
                ...
                auth_context={
                    ...
                    "role": auth_context.permissions.role.value if auth_context else None,
                    "therapeutic_context": auth_context.is_therapeutic_context() if auth_context else False,
                    "crisis_mode": auth_context.crisis_mode if auth_context else False
                } if auth_context else None
            )
            
            return await self.service_router.select_service(service_name, gateway_request)
            
        except Exception as e:
            logger.error(f"Error selecting backend service {service_name}: {e}")
            return None
    
    async def _connect_to_backend(self, target_service: ServiceInfo, connection_id: str,
                                auth_context: Optional[AuthContext]) -> Optional[str]:
        """
        ...
        try:
            ...
            backend_url = f"ws://{target_service.endpoint.host}:{target_service.endpoint.port}/ws"
            
            # Prepare headers
            headers = {}
            if auth_context:
                headers.update({
                    "X-User-ID": str(auth_context.user_id) if auth_context.user_id else "",
                    "X-Username": auth_context.username or "",
                    "X-User-Role": auth_context.permissions.role.value if auth_context.permissions else "",
                    "X-Therapeutic-Context": str(auth_context.is_therapeutic_context()),
            ...
            logger.error(f"Failed to connect to backend service {target_service.name}: {e}")
            return None
    
    async def _disconnect_from_backend(self, backend_key: str, connection_id: str) -> None:
    ...
    async def _start_message_routing(self, connection_id: str, backend_key: str) -> None:
        """
    ...
    async def _route_client_to_backend(self, connection: WebSocketConnection,
                                     backend_ws: aiohttp.ClientWebSocketResponse) -> None:
    ...
    async def _route_backend_to_client(self, connection: WebSocketConnection,
                                     backend_ws: aiohttp.ClientWebSocketResponse) -> None:
    ...
    async def _handle_gateway_message(self, connection: WebSocketConnection, 
    ...
    async def _handle_authenticate(self, connection: WebSocketConnection, message: Dict[str, Any]) -> None:
    ...
    async def _handle_join_session(self, connection: WebSocketConnection, message: Dict[str, Any]) -> None:
        """Handle join therapeutic session message."""
        session_id = message.get("session_id")
        if not session_id:
            await self._send_error_response(connection, "join_session", "Missing session_id")
            return
        ...
        self.connection_manager.session_connections[session_id].add(connection.connection_id)
    ...
    async def _handle_leave_session(self, connection: WebSocketConnection, message: Dict[str, Any]) -> None:
        ...
        if session_id and session_id in self.connection_manager.session_connections:
            self.connection_manager.session_connections[session_id].discard(connection.connection_id)
    ...
    async def _handle_therapeutic_event(self, connection: WebSocketConnection, message: Dict[str, Any]) -> None:
        """Handle therapeutic event message."""
        event_type = message.get("event_type")
        if not event_type:
            await self._send_error_response(connection, "therapeutic_event", "Missing event_type")
            return
    ...
    async def _handle_crisis_alert(self, connection: WebSocketConnection, message: Dict[str, Any]) -> None:
    ...
    async def _send_error_response(self, connection: WebSocketConnection, 
