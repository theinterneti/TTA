...
class WebSocketConnection:
    ...
    target_service: ServiceInfo | None = None
    ...
    def is_therapeutic(self) -> bool:
        ...
        return (
            ...
            in [ConnectionType.THERAPEUTIC_SESSION, ConnectionType.CRISIS_SUPPORT]
            or self.therapeutic_session_id is not None
        )

    def is_crisis(self) -> bool:
        """Check if this is a crisis connection."""
        return self.connection_type == ConnectionType.CRISIS_SUPPORT or self.crisis_mode
...
class WebSocketConnectionManager:
    ...
    def __init__(self):
        ...
        self.service_connections: dict[str, set[str]] = (
            {}
        )  # service_name -> connection_ids
    ...
    async def connect(
        ...
        if auth_context and auth_context.user_id:
            ...
            if user_connection_count >= self.max_connections_per_user:
                ...
                raise Exception(
                    f"Maximum connections per user exceeded: {user_connection_count}"
                )
    ...
    async def disconnect(
        ...
        if (
            connection.therapeutic_session_id
            and connection.therapeutic_session_id in self.session_connections
        ):
            self.session_connections[connection.therapeutic_session_id].discard(
                connection_id
            )
    ...
    def _get_user_agent(self, websocket: WebSocket) -> str | None:
