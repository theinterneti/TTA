...
from .connection_manager import WebSocketConnectionManager, ConnectionType
...
class WebSocketRouter:
    ...
    def __init__(self, connection_manager: WebSocketConnectionManager,
        ...
        self.proxy = WebSocketProxy(connection_manager, service_router, discovery_manager)
    ...
    def _setup_routes(self) -> None:
        ...
        async def narrative_websocket(websocket: WebSocket, token: Optional[str] = None):
        ...
        async def therapeutic_websocket(websocket: WebSocket, session_id: str, token: Optional[str] = None):
        ...
        async def crisis_websocket(websocket: WebSocket, token: Optional[str] = None):
        ...
        async def monitoring_websocket(websocket: WebSocket, token: Optional[str] = None):
            """Monitoring WebSocket endpoint."""
            auth_context = await self._get_auth_context(websocket, token, required=True)
            
            # Verify monitoring access
            if not auth_context or not (auth_context.permissions.is_admin() or auth_context.permissions.is_therapist()):
        ...
        async def websocket_stats(auth_context: AuthContext = Depends(get_websocket_auth_context)):
        ...
        async def broadcast_to_user(user_id: str, message: Dict[str, Any],
            ...
            sent_count = await self.connection_manager.broadcast_to_user(user_id, message)
        ...
        async def broadcast_to_session(session_id: str, message: Dict[str, Any],
            ...
            if not (auth_context.permissions.is_admin() or auth_context.permissions.is_therapist()):
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Admin or therapist access required"
                )
            
            sent_count = await self.connection_manager.broadcast_to_session(session_id, message)
            return {"sent_to_connections": sent_count}
    ...
    async def _get_auth_context(self, websocket: WebSocket, token: Optional[str] = None,
                              required: bool = False) -> Optional[AuthContext]:
        """
...
def create_websocket_router(connection_manager: WebSocketConnectionManager,
                          service_router: ServiceRouter,
                          discovery_manager: ServiceDiscoveryManager) -> APIRouter:
    """
    ...
    """
    websocket_router = WebSocketRouter(connection_manager, service_router, discovery_manager)
