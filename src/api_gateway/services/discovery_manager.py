...
from ..models import ServiceInfo, ServiceType, LoadBalancingAlgorithm
...
class ServiceDiscoveryManager:
    ...
    async def close(self) -> None:
        ...
        if self._health_monitors:
            await asyncio.gather(*self._health_monitors.values(), return_exceptions=True)
        
        await self.registry.close()
        logger.info("Service discovery manager closed")
    
    async def register_service(self, service: ServiceInfo, start_health_monitoring: bool = True) -> bool:
        """
    ...
    async def get_service_for_request(self, service_type: Optional[ServiceType] = None,
                                    service_name: Optional[str] = None,
                                    tags: Optional[List[str]] = None,
                                    therapeutic_priority: bool = False) -> Optional[ServiceInfo]:
        """
    ...
    async def get_all_healthy_services(self, service_type: Optional[ServiceType] = None) -> List[ServiceInfo]:
        """
    ...
    async def update_service_metrics(self, service_id: UUID, response_time: float, success: bool = True) -> None:
        """
    ...
    async def _monitor_service_health(self, service: ServiceInfo) -> None:
        ...
        while True:
            try:
                ...
                health_url = f"{service.endpoint.url.rstrip('/')}{service.health_check.endpoint}"
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            ...
                            is_healthy = response.status == service.health_check.expected_status
                            
                            if is_healthy:
                                consecutive_failures = 0
                                await self.registry.update_service_health(service.id, True)
                            else:
    ...
    async def _select_service_with_load_balancing(self, services: List[ServiceInfo]) -> Optional[ServiceInfo]:
    ...
    def _health_based_select(self, services: List[ServiceInfo]) -> ServiceInfo:
        ...
        sorted_services = sorted(services, key=lambda s: s.metrics.average_response_time)
    ...
    async def _notify_service_event(self, event_type: str, service: ServiceInfo) -> None:
