...
class TransformationRule(BaseModel):
    ...
    request_body_transform: str | None = None  # JSONPath or regex pattern

    # Response transformations
    response_headers_add: dict[str, str] = {}
    response_headers_remove: list[str] = []
    response_body_transform: str | None = None

    # Validation rules
    request_validation_schema: dict[str, Any] | None = None
    response_validation_schema: dict[str, Any] | None = None
...
class RequestTransformer:
    ...
    async def transform_request(
        ...
        try:
            ...
            for rule in applicable_rules:
                ...
                # Transform headers
                transformed_request = self._transform_request_headers(
                    transformed_request, rule
                )

                # Transform body
                transformed_request = await self._transform_request_body(
                    transformed_request, rule
                )
    ...
    async def transform_response(
        self, response_data: bytes, content_type: str, gateway_request: GatewayRequest
    ) -> bytes:
        """
        ...
        try:
            ...
            for rule in applicable_rules:
                ...
                if rule.sensitive_data_mask:
                    transformed_data = self._mask_sensitive_data(
                        transformed_data, content_type
                    )
    ...
    def _transform_request_headers(
                ...
                if rule.request_body_transform == "add_timestamp":
                    ...
                    if request.auth_context:
                        body_data["_user_context"] = {
                            ...
                            "therapeutic": request.auth_context.get(
    ...
    async def _transform_response_body(
        self, response_data: bytes, content_type: str, rule: TransformationRule
    ) -> bytes:
    ...
    async def _process_therapeutic_safety(
        self, request: GatewayRequest, rule: TransformationRule
    ) -> None:
    ...
    def remove_transformation_rule(self, rule_name: str) -> bool:
        ...
        self.transformation_rules = [
            rule for rule in self.transformation_rules if rule.name != rule_name
        ]
