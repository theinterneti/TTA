...
from ..models import ServiceInfo, GatewayRequest, RouteRule
from ..services import ServiceDiscoveryManager
from .load_balancer import LoadBalancer, LoadBalancingStrategy, create_load_balancer
from .circuit_breaker import CircuitBreakerManager, CircuitBreakerError, CircuitBreakerConfig
...
class ServiceRouter:
    ...
    def __init__(self, discovery_manager: ServiceDiscoveryManager, 
                 config: Optional[ServiceRouterConfig] = None):
        """
    ...
    async def select_service(self, service_name: str, gateway_request: GatewayRequest) -> Optional[ServiceInfo]:
        """
        ...
        try:
            ...
            healthy_services = self.circuit_breaker_manager.get_healthy_services(services)
            ...
            if not healthy_services:
                ...
                if gateway_request.crisis_mode:
                    logger.warning(f"Crisis mode: using potentially unhealthy services for {service_name}")
    ...
    async def route_request_with_failover(self, service_name: str, gateway_request: GatewayRequest,
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                selected_service = await self.select_service(service_name, gateway_request)
                ...
                try:
                    ...
                    self.load_balancer.update_metrics(
                        service_id, 0.0, True,  # Response time will be updated elsewhere
                    ...
                    logger.warning(f"Circuit breaker open for {selected_service.name}: {e}")
                    last_exception = e
                    continue
        ...
        if last_exception:
            ...
            raise Exception(f"Failed to route request to {service_name} after {max_retries + 1} attempts")
    ...
    async def _get_available_services(self, service_name: str) -> List[ServiceInfo]:
        ...
        if (service_name in self._service_cache and 
            service_name in self._last_cache_update and
            current_time - self._last_cache_update[service_name] < self._cache_ttl):
    ...
    async def update_service_metrics(self, service_id: str, response_time: float, 
                                   success: bool, therapeutic: bool = False,
                                   crisis: bool = False) -> None:
        """
    ...
    def clear_service_cache(self, service_name: Optional[str] = None) -> None:
        """
        Clear service cache.
