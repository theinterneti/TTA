...
from ..models import (
    GatewayRequest, GatewayResponse, RouteRule, RoutingConfig, 
    ServiceInfo, RequestMethod, RouteType, AuthContext
)
...
class GatewayCore:
    ...
    async def process_request(self, request: Request) -> Response:
        ...
        try:
            ...
            gateway_request = await self._create_gateway_request(request, correlation_id)
            ...
            # Process the request
            response = await self._route_request(gateway_request, route_rule, target_service)
    ...
    async def _create_gateway_request(self, request: Request, correlation_id: str) -> GatewayRequest:
        """
        Convert FastAPI request to gateway request.
        ...
        return GatewayRequest(
            ...
            therapeutic_session_id=getattr(request.state, "therapeutic_session_id", None),
            is_therapeutic=getattr(request.state, "is_therapeutic_context", False),
            crisis_mode=getattr(request.state, "crisis_mode", False)
        )
    
    async def _find_matching_route(self, gateway_request: GatewayRequest) -> Optional[RouteRule]:
        """
    ...
    async def _create_dynamic_route(self, gateway_request: GatewayRequest) -> Optional[RouteRule]:
        """
        ...
        if len(path_parts) >= 2 and path_parts[0] == "api":
            ...
            target_service = next((s for s in services if s.name == target_service_name), None)
    ...
    def _prepare_request_headers(self, gateway_request: GatewayRequest,
                               target_service: ServiceInfo) -> Dict[str, str]:
        """
        ...
        headers["X-Forwarded-Proto"] = "https" if self.settings.is_production else "http"
        ...
        if gateway_request.auth_context:
            ...
            headers["X-Therapeutic-Context"] = str(gateway_request.auth_context.get("therapeutic_context", False))
        ...
        if gateway_request.is_therapeutic:
            ...
            if gateway_request.therapeutic_session_id:
                headers["X-Therapeutic-Session-ID"] = str(gateway_request.therapeutic_session_id)
    ...
    async def _transform_response(self, aiohttp_response: aiohttp.ClientResponse,
                                response_body: bytes, gateway_request: GatewayRequest,
                                target_service: ServiceInfo) -> Response:
        """
        ...
        safe_headers = {
            ...
            "last-modified", "etag", "vary", "content-encoding", "content-language"
        }
    ...
    async def remove_route_rule(self, rule_name: str) -> bool:
        ...
        self.routing_config.rules = [
            rule for rule in self.routing_config.rules
