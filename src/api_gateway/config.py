...
class GatewaySettings(BaseSettings):
    ...
    def __init__(self, **kwargs):
        ...
        if tta_config:
            ...
            if "websocket_timeout" in tta_config:
                kwargs.setdefault(
                    "websocket_timeout", int(tta_config["websocket_timeout"])
                )
    ...
    trusted_hosts: list[str] | None = Field(
        default=None, description="List of trusted hostnames for security"
    )
    ...
    service_registry_backend: str = Field(
        default="redis", description="Service registry backend (redis, consul)"
    )
    ...
    default_rate_limit: str = Field(
        default="100/minute", description="Default rate limit (requests/time_window)"
    )
    therapeutic_rate_limit: str = Field(
        default="200/minute", description="Rate limit for therapeutic sessions"
    )
    ...
    therapeutic_safety_enabled: bool = Field(
        default=True, description="Enable therapeutic safety monitoring"
    )
    crisis_detection_enabled: bool = Field(
        default=True, description="Enable crisis detection"
    )
    safety_monitoring_interval: int = Field(
        default=300, description="Safety monitoring interval in seconds"
    )
    ...
    websocket_enabled: bool = Field(
        default=True, description="Enable WebSocket proxying"
    )
...
def load_tta_config_integration() -> dict:
    ...
    try:
        ...
        if agent_orchestration.get("enabled", False):
            ...
            gateway_config["agent_orchestration_port"] = agent_orchestration.get(
                "port", 8503
            )
        ...
        if realtime.get("enabled", False):
            ...
            gateway_config["websocket_timeout"] = websocket_config.get(
