...
class SanitizationMiddleware:
    ...
    def _load_default_rules(self) -> None:
        ...
        # PII masking rule
        self.sanitization_rules.append(SanitizationRule(
            name="pii_masking",
    ...
    def _load_pii_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "medical_id": re.compile(r'\b(MRN|MR|MEDICAL|PATIENT)[-:\s]?\d+\b', re.IGNORECASE),
    ...
    async def sanitize_response(self, gateway_response: GatewayResponse, 
        ...
        try:
            ...
            for rule in applicable_rules:
                ...
                try:
                    ...
                    logger.error(f"Error applying sanitization rule '{rule.name}' to response: {e}")
                    # Continue with other rules
    ...
    async def _apply_sanitization_rule(self, target: Any, rule: SanitizationRule,
    ...
    def _mask_pii_in_dict(self, data: Dict[str, Any], rule: SanitizationRule) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                masked_data[key] = [
                    self._mask_pii_in_dict(item, rule) if isinstance(item, dict)
                    else self._mask_pii_in_text(str(item), rule) if isinstance(item, str)
                    else item
                    for item in value
                ]
    ...
    def _mask_pii_in_text(self, text: str, rule: SanitizationRule) -> str:
        ...
        if rule.mask_email and "email" in self.pii_patterns:
            ...
            self.sanitization_stats["pii_instances_masked"] += len(self.pii_patterns["email"].findall(text))

        if rule.mask_phone and "phone" in self.pii_patterns:
            masked_text = self.pii_patterns["phone"].sub(self._mask_phone_match, masked_text)
            self.sanitization_stats["pii_instances_masked"] += len(self.pii_patterns["phone"].findall(text))

        if rule.mask_ssn and "ssn" in self.pii_patterns:
            masked_text = self.pii_patterns["ssn"].sub("***-**-****", masked_text)
            self.sanitization_stats["pii_instances_masked"] += len(self.pii_patterns["ssn"].findall(text))

        if rule.mask_credit_card and "credit_card" in self.pii_patterns:
            masked_text = self.pii_patterns["credit_card"].sub("****-****-****-****", masked_text)
            self.sanitization_stats["pii_instances_masked"] += len(self.pii_patterns["credit_card"].findall(text))

        if rule.mask_medical_id and "medical_id" in self.pii_patterns:
            masked_text = self.pii_patterns["medical_id"].sub("MRN: ****", masked_text)
            self.sanitization_stats["pii_instances_masked"] += len(self.pii_patterns["medical_id"].findall(text))
        ...
        for pattern_name, replacement in rule.mask_custom_patterns.items():
            if pattern_name in self.pii_patterns:
                masked_text = self.pii_patterns[pattern_name].sub(replacement, masked_text)

        return masked_text

    async def _apply_content_filtering(self, target: Any, rule: SanitizationRule) -> Any:
    ...
    def _filter_content_in_dict(self, data: Dict[str, Any], rule: SanitizationRule) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                filtered_data[key] = [
                    self._filter_content_in_dict(item, rule) if isinstance(item, dict)
                    else self._filter_content_in_text(str(item), rule) if isinstance(item, str)
                    else item
                    for item in value
                ]
            elif isinstance(value, str):
    ...
    def _filter_content_in_text(self, text: str, rule: SanitizationRule) -> str:
        ...
        for term in rule.blocked_terms:
            if term.lower() in filtered_text.lower():
                filtered_text = re.sub(re.escape(term), rule.replacement_text, filtered_text, flags=re.IGNORECASE)
                self.sanitization_stats["content_filtered"] += 1
    ...
    def _redact_data_in_dict(self, data: Dict[str, Any], rule: SanitizationRule) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if key.lower() in [field.lower() for field in rule.redact_fields]:
                ...
                redacted_data[key] = [
                    self._redact_data_in_dict(item, rule) if isinstance(item, dict) else item
                    for item in value
                ]
            else:
    ...
    async def _apply_format_normalization(self, target: Any, rule: SanitizationRule) -> Any:
        """Apply format normalization to target data."""
        # Implementation for format normalization
        return target

    async def _apply_therapeutic_privacy(self, target: Any, rule: SanitizationRule) -> Any:
    ...
    def get_sanitization_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rules_by_type": {
                stype.value: len([r for r in self.sanitization_rules if r.sanitization_type == stype])
