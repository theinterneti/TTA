...
class ValidationMiddleware:
    ...
    def _load_therapeutic_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            "crisis_level": re.compile(r"^(none|low|medium|high|critical)$", re.IGNORECASE),
        }
        ...
        try:
            ...
            for rule in applicable_rules:
                ...
                try:
                    ...
                    if self.config.fail_fast and rule.severity in [ValidationSeverity.ERROR, ValidationSeverity.CRITICAL]:
                        break
    ...
    async def _validate_schema(self, request: GatewayRequest, rule: ValidationRule) -> None:
    ...
    async def _validate_format(self, request: GatewayRequest, rule: ValidationRule) -> None:
            ...
            for field, pattern in rule.format_patterns.items():
                if field in body_data:
                    ...
                    if not re.match(pattern, value):
                        raise ValidationError(f"Field '{field}' format validation failed")
                        
        except json.JSONDecodeError:
            pass  # Skip format validation for non-JSON content
    
    async def _validate_content(self, request: GatewayRequest, rule: ValidationRule) -> None:
            ...
            for field, max_length in rule.max_field_length.items():
                if field in body_data:
                    ...
                    if len(value) > max_length:
                        raise ValidationError(f"Field '{field}' exceeds maximum length of {max_length}")
                        
        except json.JSONDecodeError:
            pass  # Skip content validation for non-JSON content

    async def _validate_size(self, request: GatewayRequest, rule: ValidationRule) -> None:
        """Validate request size limits."""
        # Check body size
        if rule.max_body_size and request.body:
            body_size = len(str(request.body).encode('utf-8'))
            if body_size > rule.max_body_size:
                raise ValidationError(f"Request body size {body_size} exceeds limit of {rule.max_body_size}")
            ...
            try:
                ...
                if rule.max_object_depth:
                    ...
                    if depth > rule.max_object_depth:
                        raise ValidationError(f"JSON depth {depth} exceeds limit of {rule.max_object_depth}")
    ...
    async def _validate_therapeutic(self, request: GatewayRequest, rule: ValidationRule) -> None:
                ...
                if field in body_data:
                    ...
                    for keyword in self.crisis_keywords:
                        if keyword in content:
                            ...
                            crisis_content.append(f"Field '{field}' contains crisis indicator: '{keyword}'")

            if crisis_detected:
                # Log crisis detection
                logger.critical(
                    f"Crisis content detected in request",
                    extra={
                        "path": request.path,
                ...
                logger.warning("Crisis content detected - request marked for priority handling")
            ...
            for field, pattern in self.therapeutic_field_patterns.items():
                if field in body_data:
                    ...
                    if not pattern.match(value):
                        raise ValidationError(f"Therapeutic field '{field}' has invalid format")

        except json.JSONDecodeError:
            pass  # Skip therapeutic validation for non-JSON content

    async def _validate_security(self, request: GatewayRequest, rule: ValidationRule) -> None:
    ...
    async def _check_xss(self, content: str) -> None:
        ...
        for pattern in self.xss_patterns:
            if pattern.search(content):
                raise ValidationError(f"Potential XSS attack detected: {pattern.pattern}")

    async def _check_path_traversal(self, content: str) -> None:
        """Check for path traversal patterns."""
        for pattern in self.path_traversal_patterns:
            if pattern.search(content):
                raise ValidationError(f"Potential path traversal attack detected: {pattern.pattern}")
    ...
    def _check_array_lengths(self, obj: Any, max_length: int) -> None:
        ...
        if isinstance(obj, list):
            if len(obj) > max_length:
                raise ValidationError(f"Array length {len(obj)} exceeds limit of {max_length}")
            for item in obj:
    ...
    async def _handle_validation_errors(self, errors: List[Dict[str, Any]],
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rules_by_type": {
                vtype.value: len([r for r in self.validation_rules if r.validation_type == vtype])
                for vtype in ValidationType
            },
            "rules_by_severity": {
                severity.value: len([r for r in self.validation_rules if r.severity == severity])
