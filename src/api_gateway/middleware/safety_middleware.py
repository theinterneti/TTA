...
class SafetyRule:
    ...
    # Content scanning
    scan_fields: List[str] = field(default_factory=lambda: ["message", "content", "description", "notes"])
...
class TherapeuticSafetyMiddleware:
    ...
    def _load_crisis_patterns(self) -> Dict[CrisisIndicator, List[re.Pattern]]:
        ...
        patterns = {
            CrisisIndicator.SELF_HARM: [
                re.compile(r'\b(cut|cutting|hurt myself|self.?harm|mutilate)\b', re.IGNORECASE),
                re.compile(r'\b(razor|blade|knife|burn|burning)\b.*\b(myself|skin)\b', re.IGNORECASE),
                re.compile(r'\b(want to hurt|need to hurt|going to hurt)\b.*\bmyself\b', re.IGNORECASE),
            ],
            
            CrisisIndicator.SUICIDE_IDEATION: [
                re.compile(r'\b(suicide|kill myself|end it all|take my life)\b', re.IGNORECASE),
                re.compile(r'\b(want to die|wish I was dead|better off dead)\b', re.IGNORECASE),
                re.compile(r'\b(jump off|hang myself|overdose|pills)\b', re.IGNORECASE),
                re.compile(r'\b(planning to|going to|about to)\b.*\b(kill|die|end)\b', re.IGNORECASE),
            ],
            
            CrisisIndicator.SUBSTANCE_ABUSE: [
                re.compile(r'\b(overdosed|too many pills|drinking too much)\b', re.IGNORECASE),
                re.compile(r'\b(can\'t stop using|relapsed|using again)\b', re.IGNORECASE),
                re.compile(r'\b(high right now|drunk|wasted|stoned)\b', re.IGNORECASE),
            ],
            
            CrisisIndicator.VIOLENCE: [
                re.compile(r'\b(want to hurt|going to hurt|kill)\b.*\b(someone|others|them)\b', re.IGNORECASE),
            ...
            CrisisIndicator.SEVERE_DEPRESSION: [
                ...
                re.compile(r'\b(can\'t go on|no point|give up|trapped)\b', re.IGNORECASE),
                re.compile(r'\b(unbearable pain|can\'t take it|desperate)\b', re.IGNORECASE),
            ],
            
            CrisisIndicator.PANIC_DISORDER: [
                re.compile(r'\b(panic attack|can\'t breathe|heart racing)\b', re.IGNORECASE),
        ...
        try:
            ...
            for rule in applicable_rules:
                ...
                try:
                    ...
                    if rule_assessment["safety_level"].value > overall_safety_level.value:
            ...
            gateway_request.metadata.update({
                "safety_assessment": {
                    ...
                    "crisis_indicators": [indicator.value for indicator in detected_indicators],
                    "assessment_timestamp": datetime.utcnow().isoformat(),
                    "requires_intervention": overall_safety_level in [SafetyLevel.DANGER, SafetyLevel.CRITICAL]
                }
            })
            
            # Log safety event for compliance
            if self.config.compliance_logging:
                await self._log_safety_event(gateway_request, overall_safety_level, detected_indicators)
    ...
    async def _assess_rule(self, request: GatewayRequest, rule: SafetyRule) -> Dict[str, Any]:
        ...
        if rule.require_therapeutic_context:
            if not request.auth_context or not request.auth_context.get('therapeutic_context'):
    ...
    async def _analyze_content(self, content: str) -> Dict[str, Any]:
        ...
        if total_crisis_patterns > 0:
            analysis["crisis_score"] = min(1.0, crisis_matches / total_crisis_patterns * 2.0)
        ...
        if self.warning_patterns:
            analysis["warning_score"] = min(1.0, warning_matches / len(self.warning_patterns))

        # Combine scores (crisis score takes precedence)
        analysis["crisis_score"] = max(analysis["crisis_score"], analysis["warning_score"] * 0.5)

        return analysis

    async def _handle_intervention(self, request: GatewayRequest, rule: SafetyRule,
                                 assessment: Dict[str, Any]) -> None:
        """Handle safety intervention based on rule configuration."""
        intervention_id = f"{request.correlation_id}_{rule.name}_{datetime.utcnow().timestamp()}"
        ...
        intervention_data = {
            ...
            "crisis_indicators": [indicator.value for indicator in assessment["crisis_indicators"]],
            "timestamp": datetime.utcnow().isoformat(),
            "user_id": request.auth_context.get("user_id") if request.auth_context else None,
            "session_id": request.auth_context.get("therapeutic_session_id") if request.auth_context else None,
            "path": request.path
        }
        ...
        if rule.intervention_type == InterventionType.LOG_ONLY:
            ...
            logger.critical(f"Request blocked due to safety concerns: {intervention_data}")
    ...
    async def _notify_staff(self, intervention_data: Dict[str, Any], notify_roles: List[str]) -> None:
        """Notify staff members about safety concerns."""
        notification = {
            "type": "safety_alert",
            "priority": "high" if intervention_data["safety_level"] in ["danger", "critical"] else "medium",
    ...
    async def _trigger_emergency_response(self, intervention_data: Dict[str, Any]) -> None:
    ...
    async def _log_safety_event(self, request: GatewayRequest, safety_level: SafetyLevel,
                               crisis_indicators: List[CrisisIndicator]) -> None:
        ...
        safety_event = {
            ...
            "therapeutic_context": request.auth_context.get("therapeutic_context") if request.auth_context else False
        }
        ...
        cutoff_time = datetime.utcnow() - timedelta(days=self.config.content_retention_days)
        self.safety_events = [
    ...
    def get_safety_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "events_by_level": {
                level.value: len([e for e in recent_events if e["safety_level"] == level.value])
    ...
    def clear_old_interventions(self, hours: int = 168) -> int:  # Default 1 week
        ...
        old_interventions = [
            intervention_id for intervention_id, intervention in self.active_interventions.items()
            if datetime.fromisoformat(intervention["timestamp"]) <= cutoff_time
        ]
