...
from ..models import AuthContext, UserRole, PermissionLevel
...
def require_permission(request: Request, required_permissions: List[PermissionLevel]) -> AuthContext:
    """
...
def require_therapeutic_access(request: Request) -> AuthContext:
    ...
    if not auth_context.permissions.therapeutic.can_access_sessions:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Therapeutic access required"
        )
...
def require_service_access(request: Request, service_name: str, action: str = "read") -> AuthContext:
    """
...
def check_mfa_required(auth_context: AuthContext, sensitive_operation: bool = False) -> bool:
    """
...
def get_user_context(request: Request) -> Optional[AuthContext]:
    """
...
def has_permission(request: Request, permission: PermissionLevel) -> bool:
    ...
    return (
        ...
        permission in auth_context.permissions.permissions
    )
