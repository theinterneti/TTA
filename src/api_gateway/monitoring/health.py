...
class HealthMonitor:
    ...
    async def check_redis_health(self) -> ServiceHealth:
        ...
        try:
            ...
            return ServiceHealth(
                status="healthy", response_time=response_time, last_check=time.time()
            )
    ...
    async def get_overall_health(self) -> HealthStatus:
        ...
        checks["service_discovery"] = (
            service_discovery_health.dict()
            if isinstance(service_discovery_health, ServiceHealth)
            else {"status": "error", "error": str(service_discovery_health)}
        )
        checks["authentication"] = (
            auth_health.dict()
            if isinstance(auth_health, ServiceHealth)
            else {"status": "error", "error": str(auth_health)}
        )
