...
from .enums import TherapeuticApproach, IntensityLevel
...
class CharacterBackground:
    ...
    relationships: Dict[str, str] = field(default_factory=dict)  # relationship_type -> description
    ...
    def __post_init__(self):
        ...
        if not re.match(r"^[a-zA-Z\s\-']+$", self.name.strip()):
            raise ValueError("Character name can only contain letters, spaces, hyphens, and apostrophes")
...
class CharacterUpdates:
    ...
    therapeutic_profile: Optional[TherapeuticProfile] = None
