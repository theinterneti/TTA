...
class WorldParameters:
    ...
    def __post_init__(self):
        ...
        if self.interaction_frequency not in valid_frequencies:
            raise ValueError(f"Interaction frequency must be one of: {valid_frequencies}")
        
        if not 10 <= self.session_length_preference <= 120:
            raise ValueError("Session length preference must be between 10 and 120 minutes")
...
class CompatibilityReport:
    ...
    def _recalculate_overall_score(self) -> None:
        ...
        weighted_sum = sum(factor.score * factor.weight for factor in self.compatibility_factors)
...
class WorldDetails:
    ...
    def __post_init__(self):
        ...
        if not 0.0 <= self.recommended_therapeutic_readiness <= 1.0:
            raise ValueError("Recommended therapeutic readiness must be between 0.0 and 1.0")
        ...
        if not 0.0 <= self.therapeutic_effectiveness_score <= 1.0:
            raise ValueError("Therapeutic effectiveness score must be between 0.0 and 1.0")
    
    def check_prerequisites(self, character_therapeutic_profile) -> List[WorldPrerequisite]:
        ...
        for prerequisite in self.prerequisites:
            if prerequisite.prerequisite_type == "therapeutic_readiness":
                if character_therapeutic_profile.readiness_level < prerequisite.required_value:
    ...
    def get_compatibility_score(self, character_therapeutic_profile) -> float:
        ...
        readiness_diff = abs(character_therapeutic_profile.readiness_level - self.recommended_therapeutic_readiness)
        readiness_score = max(0.0, 1.0 - readiness_diff)
        score_factors.append(readiness_score * 0.3)
        
        # Therapeutic approach alignment
        character_approaches = set(approach.value for approach in character_therapeutic_profile.therapeutic_goals)
        world_approaches = set(approach.value for approach in self.therapeutic_approaches)
        approach_overlap = len(character_approaches.intersection(world_approaches))
        approach_score = approach_overlap / max(len(world_approaches), 1) if world_approaches else 0.0
        score_factors.append(approach_score * 0.4)
        
        # Content safety (avoid trigger topics)
        trigger_overlap = set(character_therapeutic_profile.trigger_topics).intersection(set(self.content_warnings))
        safety_score = 1.0 - (len(trigger_overlap) / max(len(character_therapeutic_profile.trigger_topics), 1))
