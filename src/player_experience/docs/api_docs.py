...
class APIDocumentationGenerator:
    ...
    def __init__(self, app: FastAPI, output_dir: str = "./docs/api"):
        ...
        self.metadata = {
            ...
            "contact": {"name": "TTA Development Team", "email": "dev@tta.example.com"},
    ...
    def generate_all_documentation(self) -> dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"Generated API documentation files: {list(generated_files.keys())}"
            )
    ...
    def generate_openapi_spec(self) -> Path:
        ...
        openapi_schema["servers"] = [
            ...
            {"url": "https://api.tta.example.com", "description": "Production server"},
        ]
        ...
        openapi_schema["tags"] = [
            ...
            {"name": "Worlds", "description": "World selection and customization"},
            {"name": "Sessions", "description": "Therapeutic session management"},
            {"name": "Chat", "description": "Real-time chat interface"},
    ...
    def _generate_markdown_content(self) -> str:
            ...
            for endpoint in endpoints:
                ...
                if "requestBody" in operation:
                    ...
                    if "application/json" in request_body.get("content", {}):
                        schema = request_body["content"]["application/json"].get(
                            "schema", {}
                        )
                        content.append("```json")
                        content.append(
                            json.dumps(
                                self._generate_example_from_schema(schema), indent=2
                            )
                        )
                        content.append("```")
                        content.append("")
        ...
        if "components" in openapi_schema and "schemas" in openapi_schema["components"]:
            for schema_name, schema in openapi_schema["components"]["schemas"].items():
                ...
                content.append("```json")
                content.append(
                    json.dumps(self._generate_example_from_schema(schema), indent=2)
                )
                content.append("```")
                content.append("")
...
<body>
    ...
    def _generate_endpoint_reference_content(
            ...
            for endpoint in tag_endpoints:
                ...
                if endpoint.request_body:
                    ...
                    if "content" in endpoint.request_body:
                        for content_type, content_schema in endpoint.request_body[
                            "content"
                        ].items():
    ...
    def _extract_schema_documentation(self) -> list[SchemaDocumentation]:
        ...
        if "components" in openapi_schema and "schemas" in openapi_schema["components"]:
            for schema_name, schema_def in openapi_schema["components"][
                "schemas"
            ].items():
    ...
    def _generate_schema_content(self, schemas: list[SchemaDocumentation]) -> str:
        ...
        content.append("")
        content.append(
            "This document describes all data schemas used in the Player Experience Interface API."
        )
        content.append("")
