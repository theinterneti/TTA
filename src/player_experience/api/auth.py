...
# Configuration
SECRET_KEY = os.getenv("JWT_SECRET_KEY", "TTA_JWT_Secret_Key_Change_In_Production_2024!")
ALGORITHM = "HS256"
...
async def get_current_player(credentials: HTTPAuthorizationCredentials = Depends(security)) -> TokenData:
    """
...
async def get_current_active_player(current_player: TokenData = Depends(get_current_player)) -> TokenData:
    """
...
def require_player_access(player_id: str):
    ...
    """
    async def check_player_access(current_player: TokenData = Depends(get_current_active_player)) -> TokenData:
...
class AuthService:
    ...
    @staticmethod
    async def authenticate_player(username: str, password: str) -> Optional[PlayerProfile]:
        """
