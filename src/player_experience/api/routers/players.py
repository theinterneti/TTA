...
from ..auth import TokenData, get_current_active_player, require_player_access
...
from ...database.player_profile_repository import PlayerProfileRepository, PlayerProfileRepositoryError
from ...models.player import PlayerProfile, TherapeuticPreferences, PrivacySettings, CrisisContactInfo
...
def get_player_manager() -> PlayerProfileManager:
    ...
    from ...managers.player_profile_manager import PlayerProfileManager
    class _InMemoryRepo:
        def __init__(self):
            self.profiles = {}
...
class CrisisContactInfoRequest(BaseModel):
    ...
    primary_contact_name: Optional[str] = Field(None, max_length=100, description="Primary emergency contact name")
    primary_contact_phone: Optional[str] = Field(None, max_length=20, description="Primary emergency contact phone")
    therapist_name: Optional[str] = Field(None, max_length=100, description="Therapist name")
    therapist_phone: Optional[str] = Field(None, max_length=20, description="Therapist phone number")
    crisis_hotline_preference: Optional[str] = Field(None, max_length=20, description="Preferred crisis hotline")
    emergency_instructions: Optional[str] = Field(None, max_length=500, description="Special emergency instructions")


class TherapeuticPreferencesRequest(BaseModel):
    """Request model for therapeutic preferences."""
    intensity_level: IntensityLevel = Field(IntensityLevel.MEDIUM, description="Therapeutic intensity level")
    preferred_approaches: List[TherapeuticApproach] = Field(default_factory=list, description="Preferred therapeutic approaches")
    trigger_warnings: List[str] = Field(default_factory=list, description="Topics to avoid or handle carefully")
    comfort_topics: List[str] = Field(default_factory=list, description="Topics that provide comfort")
    avoid_topics: List[str] = Field(default_factory=list, description="Topics to completely avoid")
    crisis_contact_info: Optional[CrisisContactInfoRequest] = Field(None, description="Emergency contact information")
    session_duration_preference: int = Field(30, ge=10, le=120, description="Preferred session duration in minutes")
    reminder_frequency: str = Field("weekly", description="Frequency of session reminders")
...
class PrivacySettingsRequest(BaseModel):
    ...
    data_collection_consent: bool = Field(True, description="Consent to data collection")
    research_participation_consent: bool = Field(False, description="Consent to participate in research")
    progress_sharing_enabled: bool = Field(False, description="Allow sharing progress with others")
    anonymous_analytics_enabled: bool = Field(True, description="Allow anonymous analytics")
    session_recording_enabled: bool = Field(False, description="Allow session recording")
    data_retention_period_days: int = Field(365, ge=30, le=2555, description="Data retention period in days")
    third_party_sharing_consent: bool = Field(False, description="Consent to third-party data sharing")
    collect_interaction_patterns: bool = Field(True, description="Collect interaction pattern data")
    collect_emotional_responses: bool = Field(True, description="Collect emotional response data")
    collect_therapeutic_outcomes: bool = Field(True, description="Collect therapeutic outcome data")
...
class CreatePlayerRequest(BaseModel):
    ...
    password: Optional[str] = Field(None, min_length=8, description="Player's password (optional for profile creation)")
    therapeutic_preferences: Optional[Union[TherapeuticPreferencesRequest, Dict[str, Any]]] = Field(None, description="Therapeutic preferences")
    privacy_settings: Optional[Union[PrivacySettingsRequest, Dict[str, Any]]] = Field(None, description="Privacy settings")
...
class UpdatePlayerRequest(BaseModel):
    ...
    therapeutic_preferences: Optional[TherapeuticPreferencesRequest] = Field(None, description="Updated therapeutic preferences")
    privacy_settings: Optional[PrivacySettingsRequest] = Field(None, description="Updated privacy settings")
...
def _normalize_privacy_settings_dict(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    therapeutic_preferences: Dict[str, Any] = Field(..., description="Therapeutic preferences")
...
def convert_player_to_response(player: PlayerProfile) -> PlayerResponse:
    ...
    return PlayerResponse(
        ...
        therapeutic_preferences={
            ...
            "preferred_approaches": [approach.value for approach in player.therapeutic_preferences.preferred_approaches],
            ...
            "crisis_contact_info": {
                "primary_contact_name": player.therapeutic_preferences.crisis_contact_info.primary_contact_name if player.therapeutic_preferences.crisis_contact_info else None,
                "primary_contact_phone": player.therapeutic_preferences.crisis_contact_info.primary_contact_phone if player.therapeutic_preferences.crisis_contact_info else None,
                "therapist_name": player.therapeutic_preferences.crisis_contact_info.therapist_name if player.therapeutic_preferences.crisis_contact_info else None,
                "therapist_phone": player.therapeutic_preferences.crisis_contact_info.therapist_phone if player.therapeutic_preferences.crisis_contact_info else None,
                "crisis_hotline_preference": player.therapeutic_preferences.crisis_contact_info.crisis_hotline_preference if player.therapeutic_preferences.crisis_contact_info else None,
                "emergency_instructions": player.therapeutic_preferences.crisis_contact_info.emergency_instructions if player.therapeutic_preferences.crisis_contact_info else None,
            } if player.therapeutic_preferences.crisis_contact_info else None,
        },
        privacy_settings={
        ...
        progress_summary={
            ...
            "favorite_therapeutic_approach": player.progress_summary.favorite_therapeutic_approach.value if player.progress_summary.favorite_therapeutic_approach else None,
            "most_effective_world_type": player.progress_summary.most_effective_world_type,
            "last_session_date": player.progress_summary.last_session_date.isoformat() if player.progress_summary.last_session_date else None,
            "next_recommended_session": player.progress_summary.next_recommended_session.isoformat() if player.progress_summary.next_recommended_session else None,
        }
    )


def convert_request_to_therapeutic_preferences(request: TherapeuticPreferencesRequest) -> TherapeuticPreferences:
...
def convert_request_to_privacy_settings(request: PrivacySettingsRequest) -> PrivacySettings:
...
async def players_root(current_player: TokenData = Depends(get_current_active_player)) -> dict[str, str]:
    ...
    try:
        ...
        if request.therapeutic_preferences:
            if isinstance(request.therapeutic_preferences, dict):
                ...
                norm = _normalize_therapeutic_preferences_dict(request.therapeutic_preferences)
        ...
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"detail": f"Privacy violation: {str(e)}"})
    except Exception:
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": "Internal server error occurred while creating player profile"})
...
async def get_player(
    ...
    try:
        ...
        if not player:
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"detail": "Player profile not found"})
        ...
        if current_player.player_id != player_id:
            return JSONResponse(status_code=status.HTTP_403_FORBIDDEN, content={"detail": "Access denied: You can only access your own profile"})
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"detail": str(e)})
    except Exception:
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": "Internal server error occurred while retrieving player profile"})
...
async def update_player(
    ...
    try:
        ...
        if request.therapeutic_preferences is not None:
            updates["therapeutic_preferences"] = convert_request_to_therapeutic_preferences(request.therapeutic_preferences)

        if request.privacy_settings is not None:
            updates["privacy_settings"] = convert_request_to_privacy_settings(request.privacy_settings)

        # Perform update
        success = manager.update_player_profile(player_id, updates, current_player.player_id)
        ...
        if not updated_player:
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={"detail": "Player profile not found after update"})
        ...
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": "Internal server error occurred while updating player profile"})
...
async def delete_player(
    ...
    try:
        ...
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": "Internal server error occurred while deleting player profile"})
...
async def get_player_progress(player_id: str, current_player: TokenData = Depends(get_current_active_player)) -> dict:
...
async def export_player_data(
    ...
    try:
        ...
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": "Internal server error occurred while exporting player data"})
...
async def update_therapeutic_preferences(
    ...
    try:
        ...
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": "Internal server error occurred while updating therapeutic preferences"})
...
async def update_privacy_settings(
    ...
    try:
        ...
        return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"detail": "Internal server error occurred while updating privacy settings"})