...
from ..services.connection_manager import get_service_manager, ServiceConnectionManager
from ..config import APISettings, get_settings
from ..auth import get_current_active_player, TokenData
...
class ServiceHealthResponse(BaseModel):
    ...
    mock_metrics: Optional[Dict[str, Any]] = None
...
async def reconnect_services(
    ...
    try:
        ...
        if success:
            ...
            message = "Some services failed to reconnect - check service health for details"
...
async def reconnect_service(
    ...
    """
    try:
        logger.info(f"Player {current_player.username} requested reconnection for service {service_name}")
...
async def get_service_configuration(
    ...
    try:
        config = {
            ...
            "redis": {
                "url": settings.redis_url.split("@")[-1] if "@" in settings.redis_url else settings.redis_url  # Mask credentials
            },
            "security": {
