...
class CharacterNameValidator:
    ...
    def validate(name: str) -> str:
        ...
        if not name or len(name.strip()) < 2:
            raise ValidationError("Character name must be at least 2 characters", "name")
        if len(name) > 50:
            raise ValidationError("Character name must not exceed 50 characters", "name")
...
class PersonalityTraitsValidator:
    ...
    def validate(traits: List[str]) -> List[str]:
        ...
        if not traits or len(traits) == 0:
            raise ValidationError("At least one personality trait is required", "personality_traits")
        if len(traits) > 10:
            raise ValidationError("Maximum 10 personality traits allowed", "personality_traits")
        
        validated_traits = []
        for trait in traits:
            trait = trait.strip()
            if len(trait) < 2:
                raise ValidationError("Each trait must be at least 2 characters", "personality_traits")
            if len(trait) > 50:
                raise ValidationError("Each trait must not exceed 50 characters", "personality_traits")
...
class TherapeuticGoalValidator:
    ...
    def validate_description(description: str) -> str:
        ...
        if not description or len(description.strip()) < 5:
            raise ValidationError("Goal description must be at least 5 characters", "description")
        if len(description) > 500:
            raise ValidationError("Goal description must not exceed 500 characters", "description")
        return description.strip()
    
    @staticmethod
    def validate_target_date(target_date: Optional[str]) -> Optional[str]:
        """Validate target date format."""
        if target_date:
            try:
                datetime.fromisoformat(target_date.replace('Z', '+00:00'))
            except ValueError:
                raise ValidationError("Invalid date format. Use ISO 8601 format", "target_date")
        return target_date
