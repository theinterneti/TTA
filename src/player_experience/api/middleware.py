...
from ..security.rate_limiter import get_security_validator
...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if cp is not None:
            try:
                user_id = cp.get('player_id') if isinstance(cp, dict) else getattr(cp, 'player_id', None)
            except Exception:
        ...
        with performance_monitor.request_tracker.track_request(
            ...
            try:
                ...
                # Record metrics
                record_request_metric(endpoint, method, response.status_code, process_time)
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        while client_requests and client_requests[0] < now - timedelta(seconds=self.period):
...
class CrisisDetectionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if crisis_detected:
            ...
            logger.warning(f"Crisis indicators detected in request from {request.client.host if request.client else 'unknown'}")
...
class TherapeuticSafetyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        response.headers["X-Crisis-Hotline"] = "988"  # National Suicide Prevention Lifeline
