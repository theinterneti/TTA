...
class ConnectionHealth:
    ...
    def get_health_info(self) -> Dict[str, Any]:
        ...
        if self.connection_attempts > 0:
            success_rate = self.successful_operations / (self.successful_operations + self.failed_operations)
            ...
            for attempt in range(self._max_retries):
                try:
                    ...
                    if attempt == self._max_retries - 1:
                        ...
                        logger.error(f"Failed to connect to Neo4j after {self._max_retries} attempts: {e}")
                        return False
    ...
    async def _test_connection(self) -> bool:
        ...
        try:
            ...
            self.health.average_response_time = (
                ...
                if self.health.average_response_time > 0 else response_time
            )
            return True
        except Exception as e:
    ...
    async def connect(self) -> bool:
        ...
        async with self._connection_lock:
            ...
            for attempt in range(self._max_retries):
                try:
                    self.health.connection_attempts += 1
                    logger.debug(f"Redis connection attempt {attempt + 1}/{self._max_retries}")
                    ...
                    if attempt == self._max_retries - 1:
                        self.health.update_status(ServiceStatus.ERROR, str(e))
                        logger.error(f"Failed to connect to Redis after {self._max_retries} attempts: {e}")
                        return False
...
class ServiceConnectionManager:
    ...
    def __init__(self, settings: APISettings):
        ...
        if self.use_mocks:
            ...
            mock_config = MockServiceConfig(
                base_latency_ms=1.0,
                log_operations=settings.debug
            )
            self.neo4j = MockNeo4jDriver(settings.neo4j_url, (settings.neo4j_username, settings.neo4j_password), mock_config)
    ...
    async def initialize(self) -> bool:
        ...
        if success:
            ...
            logger.warning(f"Service initialization partial: Neo4j={neo4j_connected}, Redis={redis_connected}")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if hasattr(self.neo4j, 'get_health'):
            ...
            health_info["services"]["neo4j"] = {
                "service": "neo4j",
                "status": "mock",
                "mock_metrics": self.neo4j.get_metrics() if hasattr(self.neo4j, 'get_metrics') else {}
            }
        ...
        if hasattr(self.redis, 'get_health'):
            ...
            health_info["services"]["redis"] = {
                "service": "redis",
                "status": "mock",
                "mock_metrics": self.redis.get_metrics() if hasattr(self.redis, 'get_metrics') else {}
            }
...
_service_manager: Optional[ServiceConnectionManager] = None


def get_service_manager(settings: Optional[APISettings] = None) -> ServiceConnectionManager:
...
async def initialize_services(settings: Optional[APISettings] = None) -> bool:
