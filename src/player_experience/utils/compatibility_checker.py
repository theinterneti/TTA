...
from ..models.world import WorldDetails, CompatibilityReport, CompatibilityFactor, WorldPrerequisite
from ..models.character import Character, TherapeuticProfile
from ..models.enums import TherapeuticApproach, DifficultyLevel, IntensityLevel
...
class CompatibilityChecker:
    ...
    def calculate_comprehensive_compatibility(self, character: Character, world: WorldDetails) -> CompatibilityReport:
        """
        ...
        """
        logger.info(f"Calculating compatibility between character {character.character_id} and world {world.world_id}")
        ...
        # Check prerequisites
        unmet_prerequisites = self._check_prerequisites(character.therapeutic_profile, world)
        ...
        self._generate_recommendations_and_warnings(compatibility_report, character, world)
        
        logger.info(f"Compatibility score: {compatibility_report.overall_score:.3f}")
        return compatibility_report
    
    def _calculate_therapeutic_readiness_compatibility(self, profile: TherapeuticProfile, world: WorldDetails) -> CompatibilityFactor:
        """Calculate compatibility based on therapeutic readiness levels."""
        readiness_diff = abs(profile.readiness_level - world.recommended_therapeutic_readiness)
    ...
    def _calculate_therapeutic_approach_alignment(self, profile: TherapeuticProfile, world: WorldDetails) -> CompatibilityFactor:
        ...
        for goal in profile.therapeutic_goals:
            character_approaches.update(approach.value for approach in goal.therapeutic_approaches)
        
        world_approaches = set(approach.value for approach in world.therapeutic_approaches)
        ...
        if not character_approaches:
            ...
            coverage_percentage = len(overlap) / len(world_approaches) if world_approaches else 0
    ...
    def _calculate_content_safety_compatibility(self, profile: TherapeuticProfile, world: WorldDetails) -> CompatibilityFactor:
        ...
        if not character_triggers:
            ...
            if trigger_overlap:
                ...
                score = max(0.0, 1.0 - (overlap_ratio * 0.8))  # Don't go below 0.2 for partial overlaps
                explanation = f"Potential triggers detected: {', '.join(trigger_overlap)}"
            else:
                score = 1.0
    ...
    def _calculate_difficulty_appropriateness(self, profile: TherapeuticProfile, world: WorldDetails) -> CompatibilityFactor:
    ...
    def _calculate_prerequisite_fulfillment(self, profile: TherapeuticProfile, world: WorldDetails) -> CompatibilityFactor:
    ...
    def _check_prerequisites(self, profile: TherapeuticProfile, world: WorldDetails) -> List[WorldPrerequisite]:
    ...
    def _generate_recommendations_and_warnings(self, report: CompatibilityReport, 
                                             character: Character, world: WorldDetails) -> None:
        ...
        for factor in report.compatibility_factors:
            if factor.factor_name == "Therapeutic Readiness" and factor.score < 0.5:
                if character.therapeutic_profile.readiness_level < world.recommended_therapeutic_readiness:
                    report.recommendations.append(
            ...
            elif factor.factor_name == "Difficulty Appropriateness" and factor.score < 0.5:
    ...
    def get_world_recommendations(self, character: Character, available_worlds: List[WorldDetails], 
                                max_recommendations: int = 5) -> List[Tuple[WorldDetails, CompatibilityReport]]:
        """
        ...
        for world in available_worlds:
            compatibility_report = self.calculate_comprehensive_compatibility(character, world)
    ...
    def assess_world_suitability(self, character: Character, world: WorldDetails) -> Dict[str, any]:
        """
        ...
        return {
            ...
            "is_recommended": compatibility_report.overall_score >= self._compatibility_threshold,
