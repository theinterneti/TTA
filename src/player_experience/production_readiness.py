...
from .monitoring.logging_config import get_logger, setup_logging, LogCategory
from .security.vulnerability_scanner import VulnerabilityScanner
from .security.rate_limiter import create_rate_limiter, RateLimitConfig
...
class ProductionReadinessValidator:
    ...
    def __init__(self, base_url: str = "http://localhost:8080", config_path: Optional[str] = None):
    ...
    async def _run_performance_checks(self):
        ...
        try:
            ...
            if not check.passed:
                check.recommendations.append(f"Optimize API endpoints to respond within {max_allowed}s")
        ...
        try:
            ...
            check.score = (
                (50 if throughput_ok else 0) +
                (50 if error_rate_ok else 0)
            )
            check.details = load_test_results
            
            if not throughput_ok:
                check.recommendations.append(f"Improve throughput to at least {min_throughput} RPS")
            if not error_rate_ok:
                check.recommendations.append(f"Reduce error rate to below {max_error_rate}%")
        ...
        try:
            ...
            check.score = (
                ...
                (50 if memory_ok else max(0, 50 - (resource_usage["memory_percent"] - max_memory)))
            )
            ...
            if not memory_ok:
                check.recommendations.append(f"Optimize memory usage to below {max_memory}%")
    ...
    async def _run_security_checks(self):
        ...
        try:
            ...
            if not check.passed:
                check.recommendations.append("Configure HTTPS with valid SSL certificates")
        ...
        try:
            ...
            if not check.passed:
                ...
                check.recommendations.append("Configure appropriate rate limits per endpoint")
        ...
        try:
            ...
            if not check.passed:
                check.recommendations.extend(validation_status.get("recommendations", []))
        ...
        try:
            ...
            if critical_vulns > 0:
                check.recommendations.append(f"Fix {critical_vulns} critical vulnerabilities")
            if high_vulns > 0:
                check.recommendations.append(f"Fix {high_vulns} high-severity vulnerabilities")
    ...
    async def _run_monitoring_checks(self):
        ...
        try:
            ...
            if not check.passed:
                check.recommendations.append("Implement comprehensive health check endpoints")
    ...
    async def _run_documentation_checks(self):
        ...
        try:
            ...
            if not check.passed:
                check.recommendations.extend(deploy_docs_status.get("recommendations", []))
    ...
    async def _measure_api_response_times(self) -> List[float]:
        ...
        endpoints = ["/health", "/api/v1/players/", "/api/v1/characters/", "/api/v1/worlds/"]
        response_times = []
    ...
    async def _run_load_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config = LoadTester.create_load_test(self.base_url, concurrent_users=5, duration=30)
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{https_url}/health") as response:
                    return {
                        ...
                        "ssl_info": str(response.connection.transport.get_extra_info('ssl_object'))
                    }
        except Exception as e:
            return {
                "enabled": False,
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                has_rate_limit_headers = any(
                    'x-ratelimit-limit' in resp.get('headers', {}) or
                    'x-ratelimit-remaining' in resp.get('headers', {})
                    for resp in responses
                )
                
                has_429_responses = any(
                    resp.get('status') == 429 for resp in responses
                )
    ...
    async def _check_input_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                for test_input in test_inputs:
                    try:
                        ...
                        async with session.post(f"{self.base_url}/api/v1/test", json=data) as response:
            ...
            blocked_count = sum(1 for result in validation_results if result.get("blocked", False))
    ...
    def _check_metrics_collection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            has_metrics = (
                metrics.request_count > 0 or
                metrics.average_response_time > 0 or
            ...
            return {
                ...
                "metrics_available": {
                    ...
                    "database": bool(self.metrics_collector.get_database_metrics().query_count > 0),
                    "cache": bool(self.metrics_collector.get_cache_metrics().hit_count > 0),
                },
                "recommendations": [
    ...
    def _check_docker_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if has_dockerfile:
                ...
                with open(dockerfile_path, 'r') as f:
                    ...
                    if 'USER' not in dockerfile_content:
                        recommendations.append("Add non-root USER instruction to Dockerfile")
                    if 'HEALTHCHECK' not in dockerfile_content:
                        recommendations.append("Add HEALTHCHECK instruction to Dockerfile")
            else:
            ...
            if has_compose:
                ...
                recommendations.append("Create docker-compose.yml for local development")
    ...
    def _check_environment_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            score = ((len(required_envs) - len(missing_envs)) / len(required_envs)) * 100
    ...
    def _check_gdpr_compliance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            privacy_service_path = Path("./src/player_experience/services/privacy_service.py")
    ...
    def _generate_report(self) -> ReadinessReport:
        ...
        for category in CheckCategory:
            ...
            if category_checks:
                category_scores[category] = sum(check.score for check in category_checks) / len(category_checks)
            else:
        ...
        critical_issues = []
        for check in self.checks:
            if not check.passed and check.required_level in [ReadinessLevel.BASIC, ReadinessLevel.PRODUCTION]:
...
async def main():
    ...
    parser.add_argument("--output", default="./production_readiness_reports", help="Output directory for reports")
    ...
    # Print summary
    print(f"\n{'='*60}")
    print(f"PRODUCTION READINESS ASSESSMENT SUMMARY")
    print(f"{'='*60}")
