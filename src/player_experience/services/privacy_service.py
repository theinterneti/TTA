...
class DataExportRequest(BaseModel):
    ...
    include_categories: List[DataCategory] = Field(default_factory=lambda: list(DataCategory))
...
class DataDeletionRequest(BaseModel):
    ...
    user_id: str
    deletion_reason: str
    categories_to_delete: List[DataCategory] = Field(default_factory=lambda: list(DataCategory))
...
class EncryptionService:
    ...
    def encrypt_data(self, data: Union[str, bytes], key_id: str, data_category: DataCategory) -> EncryptedData:
        ...
        return EncryptedData(
            ...
            is_sensitive=data_category in [DataCategory.THERAPEUTIC_CONTENT, DataCategory.PERSONAL_IDENTIFIABLE]
        )
...
class AnonymizationService:
    ...
    def anonymize_therapeutic_content(self, content: str) -> str:
        ...
        anonymized = re.sub(r'\b\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Drive|Dr|Lane|Ln)\b', '[ADDRESS]', anonymized, flags=re.IGNORECASE)
    ...
    def k_anonymize_dataset(self, dataset: List[Dict[str, Any]], k: int = 5, quasi_identifiers: List[str] = None) -> List[Dict[str, Any]]:
...
class DataPrivacyService:
    ...
    def decrypt_therapeutic_content(self, data_id: str) -> Optional[str]:
    ...
    def export_user_data(self, request: DataExportRequest) -> bytes:
        ...
        export_data = {
            "export_metadata": {
                ...
                "categories_included": [cat.value for cat in request.include_categories],
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            zip_file.writestr(f"user_data_export.{request.export_format}", export_content)
            zip_file.writestr("export_info.txt", f"GDPR Data Export for User {request.user_id}\nGenerated: {datetime.utcnow()}")
    ...
    def delete_user_data(self, request: DataDeletionRequest) -> Dict[str, Any]:
        ...
        for category in request.categories_to_delete:
            if category == DataCategory.THERAPEUTIC_CONTENT:
                deleted_count = self._delete_therapeutic_content(request.user_id, request.preserve_anonymized)
        ...
        deletion_log_entry = {
            "user_id": request.user_id,
            "deletion_request": request.model_dump() if hasattr(request, 'model_dump') else request.dict(),
    ...
    def anonymize_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        for item_id, content in therapeutic_items.items():
            anonymized_content = self.anonymization_service.anonymize_therapeutic_content(content)
    ...
    def _collect_user_data(self, user_id: str, categories: List[DataCategory]) -> Dict[str, Any]:
        ...
        for category in categories:
            if category == DataCategory.THERAPEUTIC_CONTENT:
                user_data["therapeutic_content"] = self._get_user_therapeutic_content(user_id)
            elif category == DataCategory.PERSONAL_IDENTIFIABLE:
    ...
    def _delete_therapeutic_content(self, user_id: str, preserve_anonymized: bool) -> int:
        ...
        items_to_delete = [
            data_id for data_id, encrypted_data in self.encrypted_data_store.items()
        ...
        for data_id in items_to_delete:
            if preserve_anonymized:
                ...
                if content:
                    anonymous_id = self.anonymization_service.anonymize_user_id(user_id)
                    anonymized_content = self.anonymization_service.anonymize_therapeutic_content(content)
                    self._store_anonymized_content(anonymous_id, data_id, anonymized_content)
