...
from ..models.auth import (
    UserRole, Permission, MFAMethod, MFAConfig, MFASecret, MFAChallenge,
    MFAVerification, AuthenticatedUser, SecurityEvent, SessionInfo,
    SecuritySettings, DEFAULT_ROLE_PERMISSIONS, UserCredentials,
...
class SecurityService:
    ...
    def validate_password_strength(self, password: str) -> Tuple[bool, List[str]]:
        ...
        if self.settings.password_require_special and not any(c in "!@#$%^&*()_+-=[]{}|;:,.<>?" for c in password):
...
class EnhancedAuthService:
    ...
    def register_user(self, registration: UserRegistration) -> Tuple[bool, List[str]]:
        ...
        is_valid, password_errors = self.security_service.validate_password_strength(registration.password)
        if not is_valid:
        ...
        self.log_security_event(SecurityEvent(
            ...
            details={"username": registration.username, "email": registration.email, "role": registration.role.value}
        ))
        
        return True, []
    
    def authenticate_user(self, credentials: UserCredentials, ip_address: Optional[str] = None) -> Optional[AuthenticatedUser]:
        """
    ...
    def create_session(self, user: AuthenticatedUser, ip_address: Optional[str] = None, user_agent: Optional[str] = None) -> str:
        """
    ...
    def require_permission(self, user: AuthenticatedUser, permission: Permission) -> bool:
        """
        ...
        if not user.has_permission(permission):
            ...
            raise AuthorizationError(f"User lacks required permission: {permission.value}")
        return True
    ...
    def require_mfa_verification(self, user: AuthenticatedUser) -> bool:
        ...
        if user.mfa_enabled and not user.mfa_verified:
            raise AuthenticationError("Multi-factor authentication verification required")
        
        # Check if MFA is required for this user's role
        if user.role in self.security_settings.require_mfa_for_roles and not user.mfa_verified:
