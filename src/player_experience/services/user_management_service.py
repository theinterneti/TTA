...
from ..models.auth import UserRegistration, UserRole
from ..models.player import PlayerProfile, TherapeuticPreferences, PrivacySettings
...
class UserManagementService:
    ...
    def __init__(self, 
                 ...
                 player_repository: Optional[PlayerProfileRepository] = None,
        ...
        try:
            ...
            if self.auth_service:
                ...
                if self.user_repository:
                    user = self.user_repository.get_user_by_username(registration.username)
                    if user:
            ...
            if self.player_repository and user_id:
                try:
                    ...
                    therapeutic_prefs = TherapeuticPreferences(
                        ...
                        therapeutic_goals=registration.therapeutic_preferences.get("goals", []) if registration.therapeutic_preferences else [],
                        intensity_preference=registration.therapeutic_preferences.get("intensity", "medium") if registration.therapeutic_preferences else "medium",
                        session_frequency=registration.therapeutic_preferences.get("frequency", "weekly") if registration.therapeutic_preferences else "weekly",
                        preferred_techniques=registration.therapeutic_preferences.get("techniques", []) if registration.therapeutic_preferences else [],
                        trigger_warnings=registration.therapeutic_preferences.get("triggers", []) if registration.therapeutic_preferences else [],
                    ...
                    privacy_settings = PrivacySettings(
                        ...
                        data_sharing_consent=registration.privacy_settings.get("data_sharing", False) if registration.privacy_settings else False,
                        analytics_consent=registration.privacy_settings.get("analytics", False) if registration.privacy_settings else False,
                        research_participation_consent=registration.privacy_settings.get("research", False) if registration.privacy_settings else False,
                        marketing_consent=registration.privacy_settings.get("marketing", False) if registration.privacy_settings else False,
                        data_retention_period=registration.privacy_settings.get("retention_days", 365) if registration.privacy_settings else 365
                    )
                    ...
                    profile_success = self.player_repository.create_player_profile(player_profile)
                    if profile_success:
                        profile_created = True
                        logger.info(f"Successfully created user and player profile: {user_id}")
                        return True, [], user_id
                    else:
            ...
            if user_created and not profile_created:
                logger.warning(f"Rolling back user creation due to profile creation failure: {user_id}")
                try:
                    if self.user_repository and user_id:
                        self.user_repository.delete_user(user_id)
                        logger.info(f"Successfully rolled back user creation: {user_id}")
    ...
    def delete_user_with_profile(self, user_id: str) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if self.player_repository:
                try:
                    profile_success = self.player_repository.delete_player_profile(user_id)
                    if profile_success:
                        profile_deleted = True
                    else:
                        logger.warning(f"Player profile not found or already deleted: {user_id}")
    ...
    def update_user_profile_coordination(self, user_id: str, **kwargs) -> Tuple[bool, List[str]]:
        """
