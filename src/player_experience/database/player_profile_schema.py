...
class PlayerProfileSchemaManager:
                ...
                for constraint in constraints:
                    try:
                        ...
                        if "already exists" in str(e).lower():
                            logger.debug(f"Player profile constraint already exists: {constraint}")
                        else:
                            logger.error(f"Failed to create player profile constraint: {constraint}, Error: {e}")
                            return False
            ...
            with self.driver.session() as session:
                for index in indexes:
                    try:
                        ...
                        if "already exists" in str(e).lower():
                            logger.debug(f"Player profile index already exists: {index}")
                        else:
                            logger.error(f"Failed to create player profile index: {index}, Error: {e}")
                            return False
    ...
    def _record_player_schema_version(self) -> None:
        ...
        try:
            ...
            logger.info(f"Recorded player profile schema version: {self.player_schema_version}")
        except Exception as e:
            logger.error(f"Failed to record player profile schema version: {e}")
    
    def get_player_schema_version(self) -> Optional[str]:
        """
        ...
        try:
            with self.driver.session() as session:
                ...
                expected_constraints = [
                    "player_profile_id", "player_username", "player_email",
                    "therapeutic_prefs_id", "privacy_settings_id"
                ]
                ...
                if missing_constraints:
                    logger.error(f"Missing player profile constraints: {missing_constraints}")
                    return False
                ...
                expected_indexes = [
                    "player_username_idx", "player_email_idx", "player_created_at_idx"
                ]
                ...
                if missing_indexes:
                    logger.warning(f"Missing player profile indexes (non-critical): {missing_indexes}")
    ...
    def drop_player_profile_schema(self) -> bool:
        ...
        logger.warning("Dropping all player profile schema elements - this is destructive!")
        ...
        try:
            with self.driver.session() as session:
                ...
                player_constraints = [
                    "player_profile_id", "player_username", "player_email",
                    "therapeutic_prefs_id", "privacy_settings_id", "crisis_contact_id",
                    "progress_summary_id"
                ]
                ...
                for record in constraints_result:
                    ...
                    if any(pc in constraint_name for pc in player_constraints):
                        ...
                        logger.debug(f"Dropped player profile constraint: {constraint_name}")
                
                # Drop player profile specific indexes
                player_indexes = [
                    "player_username_idx", "player_email_idx", "player_created_at_idx",
                    "player_last_login_idx", "player_is_active_idx", "therapeutic_intensity_idx",
                    "privacy_consent_idx", "progress_total_sessions_idx"
                ]
...
def setup_player_profile_schema(uri: str = "bolt://localhost:7687", username: str = "neo4j", password: str = "password") -> bool:
    """
