...
from ..models.session import SessionContext, SessionSummary, Recommendation
from ..models.progress import (
    ProgressSummary, ProgressHighlight, Milestone, EngagementMetrics, ProgressVizSeries,
    TherapeuticMetric, TherapeuticEffectivenessReport
)
...
class ProgressTrackingService:
    ...
    def __init__(self, session_repository: SessionRepository, therapeutic_engine=None):
        ...
        self.therapeutic_engine = therapeutic_engine  # Optional integration with therapeutic components
    ...
    async def compute_progress_summary(self, player_id: str, *, summaries_limit: int = 30) -> ProgressSummary:
        """
        ...
        summaries: List[SessionSummary] = await self.session_repository.get_session_summaries(player_id, summaries_limit)
        active_sessions: List[SessionContext] = await self.session_repository.get_player_active_sessions(player_id)
        ...
        engagement.average_session_length = (total_minutes / engagement.total_sessions) if engagement.total_sessions else 0.0
        ...
        engagement.sessions_this_week = sum(1 for date in session_dates if date >= week_ago)
        engagement.sessions_this_month = sum(1 for date in session_dates if date >= month_ago)

        # Calculate dropout risk
        engagement.dropout_risk_score = self._calculate_dropout_risk(engagement, session_dates)
        ...
        new_milestones, milestone_highlights = await self._detect_comprehensive_milestones(
        ...
        summary.progress_trend = self._analyze_progress_trend(marker_analysis, engagement)
        summary.engagement_trend = self._analyze_engagement_trend(session_dates, engagement)

        # Therapeutic insights
        summary.favorite_therapeutic_approach = therapeutic_approaches_used.most_common(1)[0][0] if therapeutic_approaches_used else None
        summary.therapeutic_momentum = self._calculate_therapeutic_momentum(marker_analysis, engagement)
        summary.readiness_for_advancement = self._calculate_readiness_for_advancement(summary)

        # Identify strengths and challenges
        summary.strength_areas = self._identify_strength_areas(marker_analysis, engagement)
        summary.challenge_areas = self._identify_challenge_areas(marker_analysis, engagement)

        # Generate future planning recommendations
        summary.next_recommended_goals = self._generate_next_goals(summary, marker_analysis)
        summary.suggested_therapeutic_adjustments = self._suggest_therapeutic_adjustments(summary)

        summary.last_updated = datetime.now()
        return summary

    async def detect_and_update_milestones(self, player_id: str) -> Tuple[List[Milestone], List[ProgressHighlight]]:
        """
        ...
        """
        summaries: List[SessionSummary] = await self.session_repository.get_session_summaries(player_id, 90)
        active_sessions: List[SessionContext] = await self.session_repository.get_player_active_sessions(player_id)
        ...
        logger.info(f"Detected {len(new_milestones)} new milestones and {len(highlights)} highlights for player {player_id}")
    ...
    async def generate_progress_insights(self, player_id: str) -> List[Recommendation]:
        ...
        if summary.engagement_metrics.current_streak_days >= 7 and summary.therapeutic_momentum >= 0.7:
            recs.append(
                Recommendation(
        ...
        elif summary.engagement_metrics.current_streak_days >= 3 and summary.therapeutic_momentum >= 0.6:
            recs.append(
                Recommendation(
    ...
    async def generate_therapeutic_effectiveness_report(self, player_id: str, character_id: Optional[str] = None) -> TherapeuticEffectivenessReport:
        """
        ...
        active_sessions = await self.session_repository.get_player_active_sessions(player_id)

        # Calculate therapeutic metrics
        therapeutic_metrics = []

        # Engagement effectiveness
        engagement_score = min(1.0, len(summaries) / 20.0)  # Target: 20 sessions per month
        ...
        marker_analysis = self._analyze_progress_markers(active_sessions)
        skill_rate = min(1.0, marker_analysis['skills_acquired'] / 10.0)  # Target: 10 skills
        therapeutic_metrics.append(
            TherapeuticMetric(
        ...
        breakthrough_rate = min(1.0, marker_analysis['breakthroughs'] / 3.0)  # Target: 3 breakthroughs
        therapeutic_metrics.append(
            TherapeuticMetric(
        ...
        overall_score = sum(metric.value * metric.confidence_level for metric in therapeutic_metrics) / len(therapeutic_metrics)
        ...
        if overall_score < self.effectiveness_thresholds["improvement_needed"]:
            recommendations.append("Consider adjusting therapeutic intensity or approach")
        ...
        return TherapeuticEffectivenessReport(
            ...
            emotional_regulation_improvement=min(1.0, marker_analysis['skills_acquired'] * 0.1),
            coping_skills_development=min(1.0, marker_analysis['skills_acquired'] * 0.15),
    ...
    async def celebrate_milestone_achievement(self, player_id: str, milestone: Milestone) -> Dict[str, Any]:
        """
        ...
        celebration_data = {
            ...
            "suggested_next_milestone": await self._suggest_next_milestone(player_id, milestone),
            "celebration_type": self._determine_celebration_type(milestone),
            "timestamp": datetime.utcnow().isoformat()
        }

        logger.info(f"Celebrating milestone achievement for player {player_id}: {milestone.title}")
    ...
    async def _suggest_next_milestone(self, player_id: str, achieved_milestone: Milestone) -> Optional[str]:
    ...
    def _determine_celebration_type(self, milestone: Milestone) -> str:
        ...
        elif "streak" in milestone.title.lower() and any(num in milestone.title for num in ["30", "60", "90"]):
            return "major_achievement"
        elif "skill" in milestone.title.lower():
    ...
    async def get_visualization_data(self, player_id: str, *, days: int = 14) -> ProgressVizSeries:
        """Return daily buckets for counts and durations for the last `days` days."""
        days = max(1, min(days, 60))
        summaries: List[SessionSummary] = await self.session_repository.get_session_summaries(player_id, days * 2)

        # Build bucket keys YYYY-MM-DD
        today = datetime.utcnow().date()
        bucket_keys = [(today - timedelta(days=i)).strftime("%Y-%m-%d") for i in range(days)][::-1]
        counts = {k: 0 for k in bucket_keys}
        durations = {k: 0.0 for k in bucket_keys}
    ...
    def _assess_therapeutic_value(self, achievement_title: str, session_summary: SessionSummary) -> float:
        ...
        if any(keyword in achievement_title.lower() for keyword in ['breakthrough', 'insight', 'realization']):
            base_value += 0.3
        elif any(keyword in achievement_title.lower() for keyword in ['skill', 'technique', 'coping']):
            base_value += 0.2
        elif any(keyword in achievement_title.lower() for keyword in ['goal', 'milestone', 'progress']):
            base_value += 0.15
    ...
    def _enhance_achievement_description(self, title: str, session_summary: SessionSummary) -> str:
    ...
    def _calculate_current_streak(self, session_dates: List[datetime]) -> int:
        ...
        from src.common.time_utils import pick_reference_today, consecutive_streak_ending_today
    ...
    def _calculate_dropout_risk(self, engagement: EngagementMetrics, session_dates: List[datetime]) -> float:
        ...
        if engagement.last_session_date:
            days_since_last = (datetime.now().date() - engagement.last_session_date).days
    ...
    def _analyze_progress_markers(self, active_sessions: List[SessionContext]) -> Dict[str, int]:
    ...
    async def _detect_comprehensive_milestones(
        ...
        for threshold in self.milestone_thresholds["engagement_streak"]:
            if engagement.current_streak_days >= threshold:
                ...
                milestone = Milestone(
                    ...
                    is_achieved=True,
                    achieved_date=now,
                    therapeutic_approaches_involved=[TherapeuticApproach.BEHAVIORAL_ACTIVATION],
                    reward_description=f"Unlocked advanced therapeutic techniques and personalized content"
                )
        ...
        if engagement.current_streak_days >= 7:
            threshold = max(t for t in self.milestone_thresholds["engagement_streak"] if t <= engagement.current_streak_days)
            milestone_id = f"ms_{player_id}_streak_{threshold}"
            highlights.append(
                ProgressHighlight(
                    highlight_id=f"hl_{milestone_id}",
        ...
        for threshold in self.milestone_thresholds["skills_acquired"]:
            if skills_count >= threshold:
                ...
                milestone = Milestone(
                    ...
                    is_achieved=True,
                    achieved_date=now,
                    therapeutic_approaches_involved=[TherapeuticApproach.CBT, TherapeuticApproach.SKILL_BUILDING],
        ...
        for threshold in self.milestone_thresholds["breakthroughs"]:
            if breakthroughs_count >= threshold:
                ...
                milestone = Milestone(
                    ...
                    is_achieved=True,
                    achieved_date=now,
                    therapeutic_approaches_involved=[TherapeuticApproach.INSIGHT_ORIENTED],
    ...
    def _analyze_progress_trend(self, marker_analysis: Dict[str, int], engagement: EngagementMetrics) -> str:
    ...
    def _analyze_engagement_trend(self, session_dates: List[datetime], engagement: EngagementMetrics) -> str:
    ...
    def _calculate_therapeutic_momentum(self, marker_analysis: Dict[str, int], engagement: EngagementMetrics) -> float:
    ...
    def _identify_strength_areas(self, marker_analysis: Dict[str, int], engagement: EngagementMetrics) -> List[str]:
    ...
    def _identify_challenge_areas(self, marker_analysis: Dict[str, int], engagement: EngagementMetrics) -> List[str]:
    ...
    def _generate_next_goals(self, summary: ProgressSummary, marker_analysis: Dict[str, int]) -> List[str]:
    ...
    def _suggest_therapeutic_adjustments(self, summary: ProgressSummary) -> List[str]:
        ...
        if summary.engagement_metrics.dropout_risk_score > 0.6:
            adjustments.append("Consider shorter, more frequent sessions to maintain engagement")

        if summary.engagement_metrics.average_session_length < 15:
            adjustments.append("Gradually increase session length for deeper therapeutic work")
        ...
        if "Breakthrough Achievement" in summary.challenge_areas:
            adjustments.append("Incorporate more insight-oriented therapeutic approaches")

        # Readiness-based adjustments
        if summary.readiness_for_advancement > 0.7:
            adjustments.append("Ready for more advanced therapeutic techniques and challenges")
