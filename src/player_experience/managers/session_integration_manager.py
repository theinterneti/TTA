...
from ..models.session import SessionContext, TherapeuticSettings, ProgressMarker
from ..models.enums import SessionStatus, TherapeuticApproach
...
class SessionIntegrationManager:
    ...
    def __init__(self, session_repository: SessionRepository, interactive_narrative_engine=None):
        """
    ...
    async def switch_character_world_context(
        ...
        try:
            ...
            if existing_session:
                ...
                therapeutic_settings = current_session.therapeutic_settings if current_session else None
    ...
    async def get_active_session(self, player_id: str) -> Optional[SessionContext]:
        """
    ...
    async def update_session_interaction(self, player_id: str, interaction_data: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            if 'therapeutic_intervention' in interaction_data:
                session.add_therapeutic_intervention(interaction_data['therapeutic_intervention'])
            ...
            logger.error(f"Failed to update session interaction for player {player_id}: {e}")
            return False
    
    async def add_progress_marker(
        self, 
        player_id: str, 
    ...
    async def update_therapeutic_settings(
        ...
        try:
            ...
            logger.error(f"Failed to update therapeutic settings for player {player_id}: {e}")
            return False
    ...
    async def get_session_continuity_data(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'emotional_state_history': session.emotional_state_history[-5:],  # Last 5 states
    ...
    async def _initialize_narrative_context(self, session: SessionContext) -> None:
        ...
        try:
            ...
            logger.debug(f"Initialized narrative context for session {session.session_id}")
    ...
    async def _restore_narrative_state(self, session: SessionContext) -> None:
        ...
        try:
            ...
            if narrative_state:
                ...
                logger.debug(f"Restored narrative state for session {session.session_id}")
    ...
    async def _finalize_narrative_session(self, session: SessionContext) -> None:
        ...
        try:
            ...
            await self.interactive_narrative_engine.finalize_session(
                session.session_id
            )
    ...
    async def _update_narrative_therapeutic_settings(
        ...
        try:
            ...
            logger.debug(f"Updated narrative therapeutic settings for session {session.session_id}")
