...
class PlayerProfileManager:
    ...
    def create_player_profile(
        ...
        username: str,
        email: str,
        therapeutic_preferences: Optional[TherapeuticPreferences] = None,
        ...
        try:
            ...
            if not success:
                raise PlayerProfileManagerError("Failed to create player profile in database")
            ...
            raise PlayerProfileManagerError(f"Unexpected error creating player profile: {e}")
    
    def get_player_profile(self, player_id: str, accessor_id: Optional[str] = None) -> Optional[PlayerProfile]:
        """
        ...
        try:
            ...
            # Log access for audit
            self._log_access(player_id, "READ", f"Profile accessed by {accessor_id or 'system'}")
            ...
            raise PlayerProfileManagerError(f"Unexpected error retrieving player profile: {e}")
    ...
    def update_player_profile(
        ...
        try:
            ...
            profile = self.repository.get_player_profile(player_id)
            if not profile:
                raise PlayerProfileManagerError(f"Player profile not found: {player_id}")
            ...
            if not success:
                raise PlayerProfileManagerError("Failed to update player profile in database")
            
            # Log access for audit
            self._log_access(player_id, "UPDATE", f"Profile updated by {accessor_id or 'system'}: {list(updates.keys())}")
            ...
            raise PlayerProfileManagerError(f"Unexpected error updating player profile: {e}")
    
    def delete_player_profile(self, player_id: str, accessor_id: Optional[str] = None) -> bool:
        """
        ...
        try:
            ...
            if not success:
                raise PlayerProfileManagerError("Failed to delete player profile from database")
            
            # Log deletion for audit (before profile is gone)
            self._log_access(player_id, "DELETE", f"Profile deleted by {accessor_id or 'system'}")
            ...
            raise PlayerProfileManagerError(f"Unexpected error deleting player profile: {e}")
    
    def get_player_by_username(self, username: str, accessor_id: Optional[str] = None) -> Optional[PlayerProfile]:
        """
    ...
    def get_player_by_email(self, email: str, accessor_id: Optional[str] = None) -> Optional[PlayerProfile]:
        """
    ...
    def update_therapeutic_preferences(
        ...
        return self.update_player_profile(
            player_id,
            {"therapeutic_preferences": preferences},
            accessor_id
        )
    ...
    def export_player_data(self, player_id: str, accessor_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if accessor_id != player_id and not profile.privacy_settings.data_collection_consent:
                raise DataAccessRestrictedError("Data export not permitted without consent")
            ...
            export_data = {
                ...
                "therapeutic_preferences": {
                    ...
                    "preferred_approaches": [approach.value for approach in profile.therapeutic_preferences.preferred_approaches],
                    ...
                    "crisis_contact_info": {
                        "primary_contact_name": profile.therapeutic_preferences.crisis_contact_info.primary_contact_name if profile.therapeutic_preferences.crisis_contact_info else None,
                        "primary_contact_phone": profile.therapeutic_preferences.crisis_contact_info.primary_contact_phone if profile.therapeutic_preferences.crisis_contact_info else None,
                        "therapist_name": profile.therapeutic_preferences.crisis_contact_info.therapist_name if profile.therapeutic_preferences.crisis_contact_info else None,
                        "therapist_phone": profile.therapeutic_preferences.crisis_contact_info.therapist_phone if profile.therapeutic_preferences.crisis_contact_info else None,
                        "crisis_hotline_preference": profile.therapeutic_preferences.crisis_contact_info.crisis_hotline_preference if profile.therapeutic_preferences.crisis_contact_info else None,
                        "emergency_instructions": profile.therapeutic_preferences.crisis_contact_info.emergency_instructions if profile.therapeutic_preferences.crisis_contact_info else None,
                    } if profile.therapeutic_preferences.crisis_contact_info else None,
                },
                "privacy_settings": {
                ...
                "progress_summary": {
                    ...
                    "favorite_therapeutic_approach": profile.progress_summary.favorite_therapeutic_approach.value if profile.progress_summary.favorite_therapeutic_approach else None,
                    "most_effective_world_type": profile.progress_summary.most_effective_world_type,
                    "last_session_date": profile.progress_summary.last_session_date.isoformat() if profile.progress_summary.last_session_date else None,
                    "next_recommended_session": profile.progress_summary.next_recommended_session.isoformat() if profile.progress_summary.next_recommended_session else None,
                },
                "export_metadata": {
            ...
            self._log_access(player_id, "EXPORT", f"Data exported by {accessor_id or 'system'}")
    ...
    def get_access_log(self, player_id: str, accessor_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        if accessor_id != player_id:
            raise DataAccessRestrictedError("Access log viewing restricted to profile owner")
    ...
    def _validate_username(self, username: str) -> None:
        ...
        if not username or len(username) < 3:
            raise PlayerProfileManagerError("Username must be at least 3 characters long")
        
        if len(username) > 50:
            raise PlayerProfileManagerError("Username cannot exceed 50 characters")
        
        if not re.match(r'^[a-zA-Z0-9_-]+$', username):
            raise PlayerProfileManagerError("Username can only contain letters, numbers, underscores, and hyphens")
    ...
    def _validate_privacy_compliance(self, privacy_settings: PrivacySettings) -> None:
        ...
        if privacy_settings.data_retention_period_days < 30:
            raise PrivacyViolationError("Data retention period must be at least 30 days")
    ...
    def _check_data_access_permissions(self, profile: PlayerProfile, accessor_id: Optional[str]) -> None:
        ...
        if not profile.privacy_settings.data_collection_consent:
            raise DataAccessRestrictedError("Data access restricted: no data collection consent")
        
        # Additional checks can be added here based on specific access patterns
    
    def _check_update_permissions(self, profile: PlayerProfile, updates: Dict[str, Any], accessor_id: Optional[str]) -> None:
        """Check if profile updates are permitted."""
        # Only profile owner can update their profile
        if accessor_id != profile.player_id:
            raise DataAccessRestrictedError("Profile updates restricted to profile owner")
    ...
    def _check_deletion_permissions(self, profile: PlayerProfile, accessor_id: Optional[str]) -> None:
        """Check if profile deletion is permitted."""
        # Only profile owner can delete their profile
        if accessor_id != profile.player_id:
            raise DataAccessRestrictedError("Profile deletion restricted to profile owner")
    
    def _apply_privacy_filtering(self, profile: PlayerProfile, accessor_id: Optional[str]) -> PlayerProfile:
        ...
        filtered_profile = PlayerProfile(
            player_id=profile.player_id,
            username=profile.username,
            email=profile.email if profile.privacy_settings.progress_sharing_enabled else "***@***.***",
            created_at=profile.created_at,
            therapeutic_preferences=profile.therapeutic_preferences,
            privacy_settings=profile.privacy_settings,
            characters=profile.characters,
            active_sessions=profile.active_sessions if profile.privacy_settings.progress_sharing_enabled else {},
            progress_summary=profile.progress_summary if profile.privacy_settings.progress_sharing_enabled else ProgressSummary(),
            last_login=profile.last_login if profile.privacy_settings.collect_usage_statistics else None,
            is_active=profile.is_active
        )
    ...
    def _apply_profile_updates(self, profile: PlayerProfile, updates: Dict[str, Any]) -> PlayerProfile:
        ...
        updated_profile = PlayerProfile(
            ...
            created_at=profile.created_at,
            therapeutic_preferences=updates.get("therapeutic_preferences", profile.therapeutic_preferences),
...
def create_player_profile_manager(repository: PlayerProfileRepository) -> PlayerProfileManager:
    """
