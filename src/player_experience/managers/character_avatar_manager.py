...
class PersonalizationEngine:
    ...
    def create_personalization_profile(self, character_id: str, **kwargs):
        logger.info(f"Mock: Created personalization profile for character {character_id}")
        return {"character_id": character_id, "profile_created": True}
    
    def update_personalization_profile(self, character_id: str, **kwargs):
        logger.info(f"Mock: Updated personalization profile for character {character_id}")
...
class CharacterAvatarManager:
    ...
    def __init__(self, character_repository: Optional[CharacterRepository] = None):
    ...
    def create_character(self, player_id: str, character_data: CharacterCreationData) -> Character:
        ...
        enhanced_profile = self.therapeutic_integration_service.create_therapeutic_profile_from_character(character)
        ...
        try:
            ...
            self.therapeutic_integration_service.create_personalization_context(character)
            
            # Generate personalization recommendations
            self.therapeutic_integration_service.generate_personalization_recommendations(character)
        ...
        if updates.therapeutic_profile:
            ...
            try:
                self.personalization_engine.update_personalization_profile(
                    character_id,
                    therapeutic_preferences=updates.therapeutic_profile
                )
            except Exception as e:
    ...
    def get_character_therapeutic_profile(self, character_id: str) -> Optional[TherapeuticProfile]:
        """
    ...
    def update_character_therapeutic_profile(self, character_id: str, 
                                           therapeutic_profile: TherapeuticProfile) -> bool:
        """
        ...
        try:
            self.personalization_engine.update_personalization_profile(
                character_id,
                therapeutic_preferences=therapeutic_profile
            )
        except Exception as e:
            logger.warning(f"Failed to update personalization profile: {e}")
    ...
    def get_character_statistics(self, character_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "active_therapeutic_goals": len(character.therapeutic_profile.get_active_goals()),
            "is_active": character.is_active
        }
    
    def _validate_character_creation_data(self, character_data: CharacterCreationData) -> None:
        """
        ...
        if character_data.therapeutic_profile.readiness_level < 0.0 or character_data.therapeutic_profile.readiness_level > 1.0:
            raise ValueError("Therapeutic readiness level must be between 0.0 and 1.0")
    
    def _extract_personality_traits(self, character_data: CharacterCreationData) -> Dict[str, float]:
        """
        ...
        if character_data.therapeutic_profile.preferred_intensity == IntensityLevel.HIGH:
            traits["openness"] = traits.get("openness", 0.0) + 0.3
        elif character_data.therapeutic_profile.preferred_intensity == IntensityLevel.LOW:
    ...
    def get_character_personalization_recommendations(self, character_id: str) -> List[Dict[str, Any]]:
        """
        ...
        """
        recommendations = self.therapeutic_integration_service.get_character_recommendations(character_id)
    ...
    def adapt_therapeutic_content_for_character(self, character_id: str, content: str, 
                                              content_type: str = "general") -> Optional[Dict[str, Any]]:
        """
