...
from ..models.therapeutic_settings import (
    EnhancedTherapeuticSettings, TherapeuticPreferencesValidator, 
    SettingsConflict, SettingsConflictType
)
from ..models.enums import IntensityLevel, TherapeuticApproach, CrisisType
from ..models.player import PlayerProfile, TherapeuticPreferences
from ..models.session import SessionContext, Recommendation
...
try:
    import sys
    import os
    prototype_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'tta.prototype', 'core')
    ...
    logger.warning(f"Could not import TTA prototype components: {e}. Using mock implementations.")
    
    class PersonalizationEngine:
        def __init__(self, llm_client=None):
            self.llm_client = llm_client
        
        def personalize_content(self, user_id, content, session_state, profile, progress=None):
            return {
...
class CrisisDetectionSystem:
    ...
    def __init__(self):
        ...
        self.crisis_keywords = {
            ...
            CrisisType.PANIC_ATTACK: [
                ...
                "going crazy", "chest pain", "dizzy", "hyperventilating"
            ],
            CrisisType.SEVERE_DEPRESSION: [
                "hopeless", "worthless", "nothing matters", "can't go on",
                "empty inside", "numb", "no point", "give up"
            ],
            CrisisType.TRAUMA_RESPONSE: [
                "flashback", "nightmare", "triggered", "reliving",
                "can't stop thinking", "haunted", "traumatized"
            ]
        }
    ...
    def _initialize_crisis_resources(self) -> List[CrisisSupportResource]:
        ...
        return [
            ...
            CrisisSupportResource(
                ...
                availability="24/7",
                crisis_types=[CrisisType.SUICIDAL_IDEATION, CrisisType.SELF_HARM, CrisisType.PANIC_ATTACK],
                priority=1,
                is_emergency=True
            ),
            CrisisSupportResource(
            ...
            CrisisSupportResource(
                ...
                crisis_types=[CrisisType.SEVERE_DEPRESSION, CrisisType.GENERAL_DISTRESS],
                priority=2
            ),
            CrisisSupportResource(
    ...
    def detect_crisis_indicators(self, text: str, context: Dict[str, Any]) -> Tuple[bool, List[CrisisType], float]:
        """
        ...
        if context:
            ...
            crisis_behaviors = ["withdrawal", "agitation", "sleep_disruption", "appetite_changes"]
    ...
    def get_crisis_resources(self, crisis_types: List[CrisisType], 
                           emergency_only: bool = False) -> List[CrisisSupportResource]:
        ...
        for resource in self.crisis_resources:
            ...
            if any(crisis_type in resource.crisis_types for crisis_type in crisis_types):
...
class PersonalizationServiceManager:
    ...
    def update_therapeutic_settings(self, player_id: str, 
                                  settings: EnhancedTherapeuticSettings) -> Tuple[bool, List[SettingsConflict]]:
        """
        ...
        try:
            ...
            if not is_valid:
                ...
                for conflict in conflicts:
                    if conflict.auto_resolvable and conflict.resolution_options:
                        try:
                            resolved_settings = self.settings_validator.resolve_conflict(
                                resolved_settings, conflict, conflict.resolution_options[0]
                            )
                            logger.info(f"Auto-resolved conflict {conflict.conflict_id} for player {player_id}")
                            resolved_any = True
                ...
                if resolved_any:
                    is_valid, remaining_conflicts = self.settings_validator.validate_settings(resolved_settings)
                    if is_valid:
                        settings = resolved_settings
                        conflicts = remaining_conflicts
                    else:
                        # Check if there are still critical unresolvable conflicts
                        critical_unresolvable = [c for c in remaining_conflicts 
                                               if c.severity == "critical" and not c.auto_resolvable]
                        if critical_unresolvable:
                            logger.warning(f"Critical unresolvable conflicts found in settings for player {player_id}")
                    ...
                    if critical_conflicts:
                        logger.warning(f"Critical conflicts found in settings for player {player_id}")
            ...
            logger.error(f"Error updating therapeutic settings for player {player_id}: {e}")
            return False, []
    
    def get_adaptive_recommendations(self, player_id: str, 
        ...
        try:
            ...
            if player_feedback:
                recent_feedback = [f for f in player_feedback if 
                                 f.timestamp > datetime.now() - timedelta(days=7)]
                
                # Analyze feedback patterns
                if recent_feedback:
                    recommendations.extend(self._generate_feedback_based_recommendations(
                        player_id, recent_feedback
                    ))
            
            # Generate recommendations based on adaptation patterns
            if player_adaptations:
                recent_adaptations = [a for a in player_adaptations if 
                                    a.timestamp > datetime.now() - timedelta(days=14)]
                
                if recent_adaptations:
                    recommendations.extend(self._generate_adaptation_based_recommendations(
                        player_id, recent_adaptations
                    ))
            
            # Generate contextual recommendations
            if context:
                recommendations.extend(self._generate_contextual_recommendations(
                    player_id, context
                ))
            ...
            logger.error(f"Error generating adaptive recommendations for player {player_id}: {e}")
            return []
    ...
    def process_feedback(self, player_id: str, feedback: PlayerFeedback) -> AdaptationResult:
        ...
        try:
            ...
            if feedback.feedback_type == "rating":
                changes_made, confidence_score, reasoning = self._process_rating_feedback(feedback)
            elif feedback.feedback_type == "text":
                changes_made, confidence_score, reasoning = self._process_text_feedback(feedback)
            elif feedback.feedback_type == "preference_change":
                changes_made, confidence_score, reasoning = self._process_preference_feedback(feedback)
                requires_approval = True
            elif feedback.feedback_type == "crisis_indicator":
                changes_made, confidence_score, reasoning = self._process_crisis_feedback(feedback)
            ...
            logger.info(f"Processed feedback {feedback.feedback_id} for player {player_id}")
    ...
    def get_crisis_support_resources(self, player_id: str, 
        ...
        try:
            ...
            resources = self.crisis_detector.get_crisis_resources(crisis_types, emergency_only)
            
            logger.info(f"Retrieved {len(resources)} crisis support resources for player {player_id}")
            return resources
            
        except Exception as e:
            logger.error(f"Error getting crisis support resources for player {player_id}: {e}")
            return []
    
    def detect_crisis_situation(self, player_id: str, text: str, 
                              context: Optional[Dict[str, Any]] = None) -> Tuple[bool, List[CrisisType], List[CrisisSupportResource]]:
        """
        ...
        try:
            ...
            crisis_detected, crisis_types, confidence = self.crisis_detector.detect_crisis_indicators(
                text, context or {}
            )
            ...
            if crisis_detected:
                logger.warning(f"Crisis detected for player {player_id}: {crisis_types} (confidence: {confidence:.2f})")
            ...
            logger.error(f"Error detecting crisis situation for player {player_id}: {e}")
            return False, [], []
    
    def _generate_feedback_based_recommendations(self, player_id: str, 
                                               feedback_list: List[PlayerFeedback]) -> List[Recommendation]:
        ...
        if rating_feedback:
            avg_rating = sum(f.content.get("rating", 3) for f in rating_feedback) / len(rating_feedback)
            if avg_rating < 3:
        ...
        if text_feedback:
            ...
            all_text = " ".join(f.content.get("text", "") for f in text_feedback).lower()
    ...
    def _generate_adaptation_based_recommendations(self, player_id: str, 
                                                 adaptations: List[AdaptationResult]) -> List[Recommendation]:
        """Generate recommendations based on adaptation patterns."""
        recommendations = []
    ...
    def _generate_contextual_recommendations(self, player_id: str, 
    ...
    def _process_rating_feedback(self, feedback: PlayerFeedback) -> Tuple[List[str], float, str]:
    ...
    def _process_text_feedback(self, feedback: PlayerFeedback) -> Tuple[List[str], float, str]:
    ...
    def _process_preference_feedback(self, feedback: PlayerFeedback) -> Tuple[List[str], float, str]:
    ...
    def _process_crisis_feedback(self, feedback: PlayerFeedback) -> Tuple[List[str], float, str]:
        """Process crisis indicator feedback."""
        crisis_types = feedback.content.get("crisis_types", [])
        changes_made = ["enable_crisis_monitoring", "reduce_intensity", "provide_support_resources"]
