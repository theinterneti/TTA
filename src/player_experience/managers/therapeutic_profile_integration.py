...
from ..models.character import (
    Character, TherapeuticProfile, TherapeuticGoal, CharacterBackground,
    CharacterAppearance
)
...
class TherapeuticAdaptation:
    ...
    character_id: str = ""
    adaptation_type: str = ""  # content_tone, difficulty_level, approach_selection, etc.
...
class PersonalizationRecommendation:
    ...
    character_id: str = ""
    recommendation_type: str = ""  # therapeutic_approach, content_focus, intensity_adjustment
    title: str = ""
    description: str = ""
    rationale: str = ""
...
class TherapeuticProfileIntegrationService:
    ...
    def create_therapeutic_profile_from_character(self, character: Character) -> TherapeuticProfile:
        """
        ...
        readiness_adjustment = self._calculate_readiness_adjustment(character.background)
        profile.readiness_level = max(0.0, min(1.0, profile.readiness_level + readiness_adjustment))
        
        logger.info(f"Enhanced therapeutic profile for character {character.character_id}")
        return profile
    
    def create_personalization_context(self, character: Character) -> PersonalizationContext:
        """
        ...
        logger.info(f"Created personalization context for character {character.character_id}")
        return context
    
    def generate_personalization_recommendations(self, character: Character) -> List[PersonalizationRecommendation]:
        """
        ...
        logger.info(f"Generated {len(recommendations)} personalization recommendations for character {character.character_id}")
        return recommendations
    
    def adapt_therapeutic_content(self, character: Character, content: str, 
                                content_type: str = "general") -> TherapeuticAdaptation:
        """
        ...
        adaptation_strategy = self._determine_adaptation_strategy(character, content_type)
        
        # Apply adaptations
        adapted_content = self._apply_content_adaptations(content, adaptation_strategy, context)
        ...
        logger.info(f"Adapted therapeutic content for character {character.character_id}")
        return adaptation
    
    def get_character_adaptations(self, character_id: str) -> List[TherapeuticAdaptation]:
        """Get all adaptations for a character."""
        return self.adaptations.get(character_id, [])
    
    def get_character_recommendations(self, character_id: str) -> List[PersonalizationRecommendation]:
        """Get all recommendations for a character."""
        return self.recommendations.get(character_id, [])
    
    def update_adaptation_effectiveness(self, adaptation_id: str, effectiveness_score: float) -> bool:
        """
        ...
        for character_adaptations in self.adaptations.values():
            for adaptation in character_adaptations:
                if adaptation.adaptation_id == adaptation_id:
                    adaptation.effectiveness_score = max(0.0, min(1.0, effectiveness_score))
                    logger.info(f"Updated effectiveness score for adaptation {adaptation_id}")
                    return True
        return False
    
    def _extract_therapeutic_insights(self, background: CharacterBackground) -> Dict[str, List[str]]:
        """Extract therapeutic insights from character background."""
        insights = {
            "primary_concerns": [],
            "comfort_zones": [],
    ...
    def _generate_therapeutic_goals(self, character: Character) -> List[TherapeuticGoal]:
        ...
        for concern in character.therapeutic_profile.primary_concerns:
            if concern == "anxiety":
                goal = TherapeuticGoal(
                    ...
                    milestones=["Learn breathing techniques", "Practice grounding exercises", "Identify anxiety triggers"]
                )
                goals.append(goal)
                ...
                goal = TherapeuticGoal(
                    ...
                    therapeutic_approaches=[TherapeuticApproach.CBT, TherapeuticApproach.SOLUTION_FOCUSED],
                    milestones=["Set healthy boundaries", "Develop time management skills", "Practice stress reduction"]
                )
                goals.append(goal)
        ...
        for challenge in character.therapeutic_profile.challenge_areas:
            if challenge == "self-acceptance":
                goal = TherapeuticGoal(
                    ...
                    therapeutic_approaches=[TherapeuticApproach.HUMANISTIC, TherapeuticApproach.MINDFULNESS],
                    milestones=["Practice self-compassion", "Challenge negative self-talk", "Celebrate achievements"]
                )
                goals.append(goal)
    ...
    def _calculate_readiness_adjustment(self, background: CharacterBackground) -> float:
        ...
        positive_traits = ["empathetic", "patient", "open-minded", "resilient", "supportive"]
        for trait in background.personality_traits:
    ...
    def _recommend_therapeutic_approaches(self, character: Character) -> List[PersonalizationRecommendation]:
        """Recommend therapeutic approaches based on character data."""
        recommendations = []
        
        # Analyze personality traits for approach recommendations
        if any("anxious" in trait.lower() for trait in character.background.personality_traits):
        ...
        if any("creative" in trait.lower() for trait in character.background.personality_traits):
            rec = PersonalizationRecommendation(
                character_id=character.character_id,
                recommendation_type="therapeutic_approach",
    ...
    def _recommend_content_focus(self, character: Character) -> List[PersonalizationRecommendation]:
    ...
    def _recommend_intensity_adjustments(self, character: Character) -> List[PersonalizationRecommendation]:
    ...
    def _determine_adaptation_strategy(self, character: Character, content_type: str) -> Dict[str, Any]:
    ...
    def _apply_content_adaptations(self, content: str, strategy: Dict[str, Any], 
        ...
        if context.character_name:
            adapted_content = adapted_content.replace("[CHARACTER_NAME]", context.character_name)
