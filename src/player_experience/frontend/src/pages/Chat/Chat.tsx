...
const Chat: React.FC = () => {
  ...
  const { currentSession, messageHistory, isConnected, isTyping, connectionError } = useSelector(
        ...
        <div className={`${isMobile ? '' : 'max-w-4xl mx-auto'}`}>
          ...
          >
            <div className="flex items-center space-x-4">
              {!isConnected && (
                <span className="text-red-600 flex items-center space-x-1" role="status" aria-live="polite">
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                    <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                  </svg>
                  <span>{connectionError || 'Disconnected - Attempting to reconnect...'}</span>
                </span>
              )}
              {isConnected && (
                <span className="text-green-600 flex items-center space-x-1" role="status">
                  <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
