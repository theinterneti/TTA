...
export const useStatusMonitoring = (
  ...
  const performHealthCheck = useCallback(async () => {
    ...
    try {
      ...
      if (responseTime > 5000) {
        apiStatus = 'degraded';
        createAlert('warning', 'Slow Response', 
          `API response time is ${responseTime}ms (>5s)`, { autoHide: 10000 });
      ...
      if (healthResponse && typeof healthResponse === 'object' && 
          'maintenance_mode' in healthResponse && healthResponse.maintenance_mode) {
        nexusHubStatus = 'maintenance';
        createAlert('info', 'Maintenance Mode', 
          'The system is currently in maintenance mode', { dismissible: false });
      }
      ...
      if (systemStatus?.api_status === 'critical' || systemStatus?.api_status === 'offline') {
        createAlert('success', 'System Recovered', 
          'Connection to the system has been restored', { autoHide: 5000 });
      }
      ...
      if (retryCountRef.current < maxRetries) {
        ...
        createAlert('info', 'Retrying Connection', 
        ...
        createAlert('error', 'Connection Failed', 
  ...
  const startMonitoring = useCallback(() => {
    ...
    createAlert('info', 'Monitoring Started', 
      'System health monitoring is now active', { autoHide: 3000 });
  }, [enableHealthChecks, isMonitoring, performHealthCheck, healthCheckInterval, createAlert]);
  ...
  const stopMonitoring = useCallback(() => {
    ...
    createAlert('info', 'Monitoring Stopped', 
      'System health monitoring has been disabled', { autoHide: 3000 });
  }, [createAlert]);
  ...
  // Computed values
  const isHealthy = systemStatus?.api_status === 'healthy' && 
