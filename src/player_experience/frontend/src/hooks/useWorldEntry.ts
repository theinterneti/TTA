...
export interface UseWorldEntryResult {
  enterWorld: (worldId: string, entryData?: WorldEntryData) => Promise<WorldEntryResponse>;
  entering: boolean;
  error: string | null;
 ...
 */
export const useWorldEntry = (options: UseWorldEntryOptions = {}): UseWorldEntryResult => {
  const {
    onSuccess,
    onError,
    autoNavigate = false,
  ...
  const [lastEntryResponse, setLastEntryResponse] = useState<WorldEntryResponse | null>(null);
  ...
  const enterWorld = useCallback(async (
    ...
    try {
      ...
      const entryResponse: WorldEntryResponse = {
        ...
        entry_point: response.entry_point || 'default',
        session_config: {
          difficulty_level: response.session_config?.difficulty_level || 'normal',
          therapeutic_focus: response.session_config?.therapeutic_focus || [],
          estimated_duration: response.session_config?.estimated_duration || 30,
          privacy_mode: response.session_config?.privacy_mode || false,
        },
        initial_state: {
          location: response.initial_state?.location || 'Starting Area',
          narrative_context: response.initial_state?.narrative_context || 'Welcome to the world!',
          available_actions: response.initial_state?.available_actions || [],
        },
...
export const useBatchWorldEntry = () => {
  ...
  const enterWorldsBatch = useCallback(async (
    entries: Array<{ worldId: string; entryData?: WorldEntryData }>
  ) => {
...
export const useWorldEntryHistory = () => {
  const [entryHistory, setEntryHistory] = useState<Array<{
    id: string;
    worldId: string;
    worldTitle: string;
    enteredAt: string;
  ...
  const addToHistory = useCallback((entry: {
    ...
    try {
      ...
      localStorage.setItem('tta_world_entry_history', JSON.stringify(updated));
    } catch (error) {
