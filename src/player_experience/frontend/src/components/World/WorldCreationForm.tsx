...
const GENRES = [
  ...
  'Thriller', 'Drama', 'Comedy', 'Historical', 'Psychological', 'Educational'
];

const THERAPEUTIC_FOCUSES = [
  'Anxiety Management', 'Depression Support', 'Trauma Recovery', 'Social Skills',
  'Emotional Regulation', 'Self-Esteem', 'Grief Processing', 'Addiction Recovery',
  'Relationship Building', 'Stress Management', 'Mindfulness', 'Communication Skills'
];
...
const WorldCreationForm: React.FC<WorldCreationFormProps> = ({
    ...
    switch (step) {
      ...
      case 2:
        if (formData.therapeutic_focus.length === 0) {
          errors.therapeutic_focus = 'Please select at least one therapeutic focus';
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const worldId = response.world_id || response.id;
      ...
      setSubmitError(error.message || 'Failed to create world. Please try again.');
    } finally {
    ...
    <div className="space-y-6">
      <div>
        ...
        {validationErrors.world_title && (
          <p className="mt-1 text-sm text-red-600">{validationErrors.world_title}</p>
        )}
      </div>
      ...
      <div>
        ...
        {validationErrors.world_genre && (
          <p className="mt-1 text-sm text-red-600">{validationErrors.world_genre}</p>
        )}
      </div>
    ...
    <div className="space-y-6">
      <div>
        ...
        {validationErrors.therapeutic_focus && (
          <p className="mt-1 text-sm text-red-600">{validationErrors.therapeutic_focus}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            max={300}
            className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              validationErrors.estimated_duration ? 'border-red-300' : 'border-gray-300'
            }`}
          />
        ...
        <div>
          ...
          <input
            ...
            max={10}
            className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              validationErrors.max_players ? 'border-red-300' : 'border-gray-300'
            }`}
          />
          {validationErrors.max_players && (
            <p className="mt-1 text-sm text-red-600">{validationErrors.max_players}</p>
          )}
        </div>
      </div>
