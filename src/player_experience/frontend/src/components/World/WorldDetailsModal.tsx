...
const WorldDetailsModal: React.FC<WorldDetailsModalProps> = ({
  ...
  const { selectedWorld, isLoading } = useSelector(
  ...
  if (isLoading || !selectedWorld) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl p-8 text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto mb-4"></div>
          <p className="text-gray-600">Loading world details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              ...
              {selectedCharacter && (
                <span
                  className={`px-3 py-1 rounded-full text-sm font-medium ${getCompatibilityColor(
                    selectedWorld.compatibility_score
                  )}`}
                >
                  {Math.round(selectedWorld.compatibility_score * 100)}% -{" "}
                  {getCompatibilityText(selectedWorld.compatibility_score)}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {selectedWorld.therapeutic_themes.map((theme, index) => (
                <span
                  key={index}
                  className="px-3 py-1 bg-therapeutic-calm text-blue-600 rounded-full text-sm"
                >
                  {theme}
                </span>
              ))}
          ...
          {/* Prerequisites */}
          {selectedWorld.prerequisites &&
            selectedWorld.prerequisites.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3">
                  Prerequisites
                </h3>
          ...
          {selectedWorld.customizable_parameters && (
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(selectedWorld.customizable_parameters).map(
                  ([param, available]) => (
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${
                      selectedWorld.compatibility_score >= 0.8
                        ? "bg-green-500"
                        : selectedWorld.compatibility_score >= 0.6
                        ? "bg-yellow-500"
                        : "bg-red-500"
                    }`}
                    style={{
                      width: `${selectedWorld.compatibility_score * 100}%`,
                    }}
                  />
                </div>
                <p className="text-sm text-gray-600 mt-2">
                  Based on your character's therapeutic preferences and current
                  goals
                </p>
              </div>
            </div>
