...
class RequestTracker:
    ...
    @contextmanager
    def track_request(self, request_id: str, endpoint: str, method: str, 
    ...
    def _finish_request(self, context: RequestContext):
        ...
        self.metrics_collector.record_timer(f"response_time_{context.endpoint}", duration)
    ...
    def _update_concurrent_users(self):
        ...
        with self.lock:
            unique_users = len(set(
                ctx.user_id for ctx in self.active_requests.values() 
                if ctx.user_id
            ))
    ...
    def get_slow_requests(self, threshold: float = 2.0) -> List[RequestContext]:
        ...
        with self.lock:
            return [
                ctx for ctx in self.active_requests.values()
...
class DatabaseQueryTracker:
    ...
    @contextmanager
    def track_query(self, query_id: str, query_type: str, query_text: Optional[str] = None,
                   connection_id: Optional[str] = None, **metadata):
    ...
    def _finish_query(self, context: QueryContext, success: bool):
        ...
        self.metrics_collector.record_timer(f"db_query_time_{context.query_type}", duration)
        ...
        with self.lock:
            ...
            if len(self.query_patterns[context.query_type]) > 1000:
                self.query_patterns[context.query_type] = self.query_patterns[context.query_type][-500:]
        ...
        logger.debug(f"Query {context.query_id} ({context.query_type}) completed in {duration:.3f}s")
...
class CacheTracker:
    ...
    def __init__(self, metrics_collector: Optional[MetricsCollector] = None):
        ...
        self.cache_patterns: Dict[str, Dict[str, Any]] = defaultdict(lambda: {
            "hits": 0, "misses": 0, "total_time": 0.0, "operations": 0
        })
        self.lock = threading.RLock()
    
    @contextmanager
    def track_cache_operation(self, operation_id: str, operation_type: str, key: str, **metadata):
    ...
    def _finish_cache_operation(self, operation: CacheOperation):
        ...
        # Record metrics
        self.metrics_collector.record_timer(f"cache_{operation.operation_type}_time", duration)
        ...
        logger.debug(f"Cache {operation.operation_type} for {operation.key} completed in {duration:.3f}s")
...
class SystemResourceMonitor:
    ...
    def _collect_system_metrics(self):
        ...
        self.metrics_collector.record_gauge("process_memory_mb", process.memory_info().rss / 1024 / 1024)
        ...
        if gc_stats:
            self.metrics_collector.record_gauge("gc_collections", sum(stat['collections'] for stat in gc_stats))
            self.metrics_collector.record_gauge("gc_collected", sum(stat['collected'] for stat in gc_stats))
...
class PerformanceMonitor:
    ...
    def _setup_alerting(self):
        ...
        def check_response_time(metric_name: str, value: Any):
            if metric_name == "response_time_avg" and isinstance(value, (int, float)):
                if value > self.thresholds.response_time_critical:
                    self._trigger_alert("critical", "High response time", {
                        "metric": metric_name,
                        "value": value,
                        "threshold": self.thresholds.response_time_critical
                    })
                elif value > self.thresholds.response_time_warning:
                    self._trigger_alert("warning", "Elevated response time", {
                        "metric": metric_name,
                        "value": value,
        ...
        def check_error_rate(metric_name: str, value: Any):
            if metric_name == "error_rate" and isinstance(value, (int, float)):
                if value > self.thresholds.error_rate_critical:
                    self._trigger_alert("critical", "High error rate", {
                        "metric": metric_name,
                        "value": value,
                        "threshold": self.thresholds.error_rate_critical
                    })
                elif value > self.thresholds.error_rate_warning:
                    self._trigger_alert("warning", "Elevated error rate", {
                        "metric": metric_name,
                        "value": value,
        ...
        def check_memory_usage(metric_name: str, value: Any):
            if metric_name == "memory_usage_percent" and isinstance(value, (int, float)):
                if value > self.thresholds.memory_usage_critical:
                    self._trigger_alert("critical", "High memory usage", {
                        "metric": metric_name,
                        "value": value,
                        "threshold": self.thresholds.memory_usage_critical
                    })
                elif value > self.thresholds.memory_usage_warning:
                    self._trigger_alert("warning", "Elevated memory usage", {
                        "metric": metric_name,
                        "value": value,
        ...
        # Register callbacks
        self.metrics_collector.add_metric_callback("response_time_avg", check_response_time)
        self.metrics_collector.add_metric_callback("error_rate", check_error_rate)
        self.metrics_collector.add_metric_callback("memory_usage_percent", check_memory_usage)
