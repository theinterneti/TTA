...
class BenchmarkResult:
    ...
    def meets_success_criteria(self) -> Tuple[bool, List[str]]:
        ...
        if self.average_response_time > criteria.get("max_response_time", float('inf')):
            failures.append(f"Average response time {self.average_response_time:.2f}s exceeds limit {criteria['max_response_time']}s")
        
        if self.error_rate > criteria.get("max_error_rate", float('inf')):
            failures.append(f"Error rate {self.error_rate:.2f}% exceeds limit {criteria['max_error_rate']}%")
        
        if self.throughput < criteria.get("min_throughput", 0):
            failures.append(f"Throughput {self.throughput:.2f} RPS below minimum {criteria['min_throughput']} RPS")
        
        # Check system metrics
        if "cpu_usage" in self.system_metrics:
            max_cpu = max(self.system_metrics["cpu_usage"]) if self.system_metrics["cpu_usage"] else 0
            if max_cpu > criteria.get("max_cpu_usage", float('inf')):
                failures.append(f"CPU usage {max_cpu:.2f}% exceeds limit {criteria['max_cpu_usage']}%")
        
        if "memory_usage" in self.system_metrics:
            max_memory = max(self.system_metrics["memory_usage"]) if self.system_metrics["memory_usage"] else 0
            if max_memory > criteria.get("max_memory_usage", float('inf')):
                failures.append(f"Memory usage {max_memory:.2f}% exceeds limit {criteria['max_memory_usage']}%")
...
class HTTPLoadGenerator(LoadGenerator):
    ...
    async def _constant_load(self, config: BenchmarkConfig, result: BenchmarkResult):
        ...
        request_interval = 1.0 / config.requests_per_second if config.requests_per_second else 0
    ...
    async def _ramp_up_load(self, config: BenchmarkConfig, result: BenchmarkResult):
        ...
        steady_duration = config.duration_seconds - config.ramp_up_time - config.ramp_down_time
        ramp_down_duration = config.ramp_down_time
        
        # Ramp up phase
        await self._ramp_phase(config, result, 0, config.concurrent_users, ramp_up_duration)
        ...
        # Ramp down phase
        await self._ramp_phase(config, result, config.concurrent_users, 0, ramp_down_duration)
    ...
    async def _spike_load(self, config: BenchmarkConfig, result: BenchmarkResult):
        ...
        # Back to normal
        await self._constant_load_phase(config, result, config.duration_seconds - 2 * spike_duration, normal_users)
    
    async def _ramp_phase(self, config: BenchmarkConfig, result: BenchmarkResult,
                         start_users: int, end_users: int, duration: int):
        ...
        for step in range(steps):
            ...
            await self._constant_load_phase(config, result, step_duration, current_users)
    
    async def _constant_load_phase(self, config: BenchmarkConfig, result: BenchmarkResult,
                                  duration: float, concurrent_users: int = None):
    ...
    async def _make_request(self, semaphore: asyncio.Semaphore,
                           config: BenchmarkConfig, result: BenchmarkResult):
    ...
    async def run_all(self) -> List[BenchmarkResult]:
        ...
        for config in self._configs:
            ...
            try:
                ...
                if not success:
                    logger.warning(f"Benchmark {config.name} failed criteria: {failures}")
...
class LoadTester:
    ...
    @staticmethod
    def create_load_test(base_url: str, concurrent_users: int = 10, duration: int = 300) -> BenchmarkConfig:
    ...
    @staticmethod
    def create_stress_test(base_url: str, max_users: int = 100, duration: int = 600) -> BenchmarkConfig:
    ...
    @staticmethod
    def create_spike_test(base_url: str, spike_users: int = 50, duration: int = 180) -> BenchmarkConfig:
        """Create a spike test."""
        return BenchmarkConfig(
            name="spike_test",
...
class PerformanceBenchmark:
    ...
    def _analyze_results(self, results: List[BenchmarkResult]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if result.average_response_time > 2.0:
                analysis["bottlenecks"].append(f"High response time in {test_name}: {result.average_response_time:.2f}s")
            
            if result.error_rate > 5.0:
                analysis["bottlenecks"].append(f"High error rate in {test_name}: {result.error_rate:.2f}%")
            
            max_cpu = max(result.system_metrics.get("cpu_usage", [0]))
            if max_cpu > 80.0:
                analysis["bottlenecks"].append(f"High CPU usage in {test_name}: {max_cpu:.2f}%")
        ...
        elif analysis["summary"]["failed_tests"] <= analysis["summary"]["total_tests"] * 0.2:
            analysis["summary"]["overall_health"] = "good"
        elif analysis["summary"]["failed_tests"] <= analysis["summary"]["total_tests"] * 0.5:
    ...
    def _generate_recommendations(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        high_response_times = [
            test for test, metrics in analysis["performance_metrics"].items()
        ...
        if not recommendations:
            recommendations.append("System performance looks good! Continue monitoring and consider capacity planning for growth.")
