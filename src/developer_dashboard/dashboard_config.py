...
class DashboardConfig:
    ...
    def from_env(cls) -> 'DashboardConfig':
        ...
        return cls(
            ...
            enable_test_battery=os.getenv('DASHBOARD_TEST_BATTERY', 'true').lower() == 'true',
            test_results_dir=os.getenv('TEST_RESULTS_DIR', './test-results'),
            websocket_enabled=os.getenv('DASHBOARD_WEBSOCKET', 'true').lower() == 'true',
            
            enable_real_time_updates=os.getenv('DASHBOARD_REAL_TIME', 'true').lower() == 'true',
            enable_historical_data=os.getenv('DASHBOARD_HISTORY', 'true').lower() == 'true',
            max_history_records=int(os.getenv('DASHBOARD_MAX_HISTORY', '1000')),
            
            cors_origins=os.getenv('DASHBOARD_CORS_ORIGINS', '').split(',') if os.getenv('DASHBOARD_CORS_ORIGINS') else None,
            api_key_required=os.getenv('DASHBOARD_API_KEY_REQUIRED', 'false').lower() == 'true',
...
async def initialize_dashboard(config: Optional[DashboardConfig] = None) -> tuple[FastAPI, Any]:
...
async def setup_metrics_monitoring(app: FastAPI, config: DashboardConfig):
    ...
    async def get_test_performance_metrics():
        ...
        try:
            ...
            avg_duration = sum(r.get('duration_seconds', 0) for r in recent_results) / total_executions
            avg_success_rate = sum(r.get('success_rate', 0) for r in recent_results) / total_executions
            
            return {
                "total_executions": total_executions,
                "average_duration": round(avg_duration, 2),
                "average_success_rate": round(avg_success_rate, 2),
                "last_execution": max(r.get('end_time', '') for r in recent_results) if recent_results else None,
                "timestamp": "2025-09-15T12:00:00Z"
            }
